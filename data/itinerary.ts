...
export type DayEvent = {
  /**
   * \u{1F534} **id ของแถว `trip_stops` ที่แถวนี้มาจาก — และมันคือตัวชี้ว่า *แก้ได้หรือไม่*** (P2 · 4 ก.ย. 2026)
   *
   * `undefined` = แถวมาจาก `data/itinerary.ts` (ไฟล์โค้ด) \u21d2 **ไม่มีอะไรให้ `PATCH` ตามนิยาม**
   * มีค่า = แถวมาจากฐาน \u21d2 แก้/ลบผ่านหน้าเว็บได้
   *
   * \u{1F3AF} **จงใจไม่ทำเป็นธง `editable` แยกใบ** — ธงแยกจะโกหกได้ (ตั้ง `true` บนแถวที่ไม่มี id
   * แล้วปุ่มจะเปิดโมดัลที่บันทึกไม่ได้) · ที่นี่ *ของที่ใช้แก้* กับ *สัญญาณว่าแก้ได้* เป็นตัวเดียวกัน
   * \u26a0\ufe0f **คนละเรื่องกับ `editable` ข้างล่าง** ซึ่งแปลว่า *เวลานี้เลื่อนได้ในโลกจริงไหม* (ตั๋วจองแล้ว = เลื่อนไม่ได้)
   *    แถวที่ \u{1F512} อยู่ **ยังแก้ในเว็บได้** (พิมพ์เลขไฟลต์ผิดต้องแก้ได้) \u2014 อย่ายุบสองอย่างนี้เข้าหากัน
   */
  stopId?: string;
