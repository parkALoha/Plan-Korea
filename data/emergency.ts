...
/**
 * 🔴 **`countryOfCity(city: City)` ถูกถอดออกถาวร 29 ส.ค. 2026 — อย่าเขียนกลับมา**
 * มันคือ `return COUNTRY_OF_CITY[city]` เปล่า ๆ = index ตรงบน `Record` ที่คีย์เป็น union
 * → **`undefined` เงียบ ๆ ทันทีที่คีย์มาจากฐาน** (`cast` เข้า `City` ได้โดย `tsc` ไม่ทัก)
 * · 🎯 **ซ่อนการ index ไว้หลังชื่อฟังก์ชัน ทำให้ตัวสแกนที่จับรูป `X[...]` มองไม่เห็นด้วย**
 * · ✅ ใช้ `countryOfCitySlug()` แทนเสมอ — คืน `null` เมื่อไม่รู้จัก และผู้เรียกทุกแบบรับ `null` ได้อยู่แล้ว
 *
 * ## ประวัติที่เก็บไว้เพราะมันเปลี่ยนวิธีวัด ไม่ใช่เพราะเป็นเกร็ด
 * ถอนรอบแรก (`597237e`) วัดว่า *"ผู้เรียกโปรดักชัน = 0"* **จากทรีร่วมที่มี WIP ของคนอื่นบนดิสก์**
 * → `tsc` เขียวเพราะของค้างของเขา · ที่ `HEAD` จริงยังมีผู้เรียก → **หัว branch แดง**
 * 🎯 **ของค้างของคนอื่นทำให้ *เขียวหลอก* ได้ ไม่ใช่แค่แดงหลอก** (`waiting-on-user.md §3.10`)
 * · ✅ ท่าที่ใช้รอบนี้: `git grep -n "countryOfCity(" HEAD -- components/ app/ hooks/`
 *   **ถามที่ `commit` ไม่ใช่ที่ดิสก์** → เหลือ 6 บรรทัด **คอมเมนต์ล้วน โค้ดจริง 0**
 * · ⚠️ และต้อง **แยกโค้ดจริงออกจากคอมเมนต์ก่อนอ่านตัวเลข** — *"เจอ 6 ที่"* ไม่ใช่ *"มีผู้เรียก 6 ราย"*
 */
