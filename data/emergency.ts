...
/**
 * ⚠️ **คืนชั่วคราว 29 ส.ค. 2026 — หัว branch `tsc` แดงเพราะการถอดของผมเอง (P3)**
 * ถอดไปใน `597237e` โดยวัดว่า "ผู้เรียกโปรดักชัน = 0" **ซึ่งวัดจากทรีร่วมที่มี WIP ของ P2 อยู่บนดิสก์**
 * → ของจริงที่ `HEAD` `components/EmergencyCard.tsx:5,29` ยังเรียกอยู่ · ยืนยันแล้วใน worktree ที่ปักหมุด
 *   `54995ae`: **แดง 3 error ทั้งหมดในไฟล์นั้นไฟล์เดียว**
 * 🎯 **ของค้างของคนอื่นทำให้ *เขียวหลอก* ได้ ไม่ใช่แค่แดงหลอก** — `TEAM.md` เขียนเคสแดงหลอกไว้แล้ว
 *    เคสนี้คือทิศกลับ และผมเป็นคนเดินเข้าไปเอง
 * 🔴 **ถอดอีกครั้งทันทีที่ P2 commit ตัวแก้ `EmergencyCard` (ใช้ `countryOfCitySlug`) — อย่าปล่อยค้าง**
 *
 * 🔴 **เหตุผลเดิมที่ต้องถอดยังจริงทุกตัวอักษร — อย่าเขียนผู้เรียกใหม่มาใช้มัน**
 ...
 * · ใช้ `countryOfCitySlug()` แทนเสมอ — มันคืน `null` เมื่อไม่รู้จัก และผู้เรียกทุกแบบรับ `null` ได้อยู่แล้ว
 */
