...
begin
  ...
  -- 🔴 ใช้ `to_regprocedure` ไม่ใช่เทียบสตริงจาก `pg_get_function_identity_arguments`
  --    ฉบับแรกเทียบสตริงกับ `'text, date, date, text'` **แล้วล้มตอนรันจริง 27 ส.ค. 2026**
  --    เพราะฟังก์ชันนั้นคืน *ชื่อพารามิเตอร์ติดมาด้วย* (`p_title text, p_start_date date, …`)
  --    — ผมเดารูปแบบของมันเอาเอง แทนที่จะใช้ตัวที่ถามคำถามตรง ๆ
  -- 🎯 `to_regprocedure` ตอบคำถามที่เราถามจริง ๆ: **"มีฟังก์ชันลายเซ็นนี้อยู่ไหม"**
  --    ไม่ผ่านการจัดรูปสตริงเลยสักขั้น → ไม่มีรูปแบบให้เดาผิด
  if to_regprocedure('public.create_trip(text, date, date, text)') is null then
    raise exception 'ไม่พบ public.create_trip(text, date, date, text) — ผู้เรียกทั้งหมดจะพัง · ที่มีอยู่: %',
      coalesce((select string_agg(p.oid::regprocedure::text, ' | ')
                  from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace
                 where ns.nspname = 'public' and p.proname = 'create_trip'), '(ไม่มีเลย)');
  end if;
