...
--
--     🔴 **แก้ 27 ส.ค. 2026 — เหตุผลที่ผมเขียนไว้ตอนแรกตรงนี้ *ผิดข้อเท็จจริง* (P4 จับได้):**
--     ฉบับแรกเขียนว่า *"`_helpers.ts` จด `zz zy zx zw xq zv` แต่ในฐานมี `xt zs zt`
--     ที่ไม่อยู่ในทะเบียนเลย → denylist ตามของจริงไม่ทัน"*
--     **ทั้งสามอยู่ในทะเบียนครบ** — `tripContent:"zt"` · `tripHotels:"zs"` · `outsiderSweep:"xt"`
--     ทะเบียนมี **18 รหัส ไม่ใช่ 6** · ผมอ่านแค่ 13 บรรทัดแรกแล้วสรุปทั้งลิสต์
--     ⚠️ **และผมเอาข้อเท็จจริงที่ยังไม่ได้ตรวจ ไปใช้เป็นเหตุผลหลักของการตัดสินใจเชิงโครงสร้าง**
--
--     ✅ **เหตุผลจริงที่ยังทำให้ปฏิเสธทางนี้ และแข็งกว่าเดิม (P4 เป็นคนให้):**
--     กรองการค้นของ **prod** ด้วย *"ไม่ใช่รหัสทดสอบ"* แปลว่าโค้ดที่เสิร์ฟผู้ใช้
--     **ต้องรู้จักรหัสของชุดทดสอบ** — เป็น denylist ที่ผูกกับ artifact ของเทสต์
--     ทะเบียนนั้นมีไว้กัน *บล็อกเทสต์ชนกัน* ไม่ได้มีไว้ตัดสินว่า *ผู้ใช้เห็นอะไร*
--     🎯 `supported` เป็น **allowlist ที่ระบบประกาศเอง** — fail-safe โดยโครงสร้าง
--        ไม่ใช่โดยความขยันจด · ซึ่งเป็นคุณสมบัติที่ไม่ขึ้นกับว่าทะเบียนครบหรือไม่ครบเลย
