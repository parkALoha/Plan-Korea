...
-- ── 📌 ประกาศ: trigger นี้เปลี่ยนอะไรโดย **ไม่** stamp `updated_at` (P7 ชี้ · 27 ส.ค. 2026) ──────
-- `update … set is_active = true` ข้างล่างรัน **ในกิ่ง trigger** → `pg_trigger_depth() >= 1`
-- → `app.touch_updated_at()` (`20260825220608`) เจอ `depth > 1` แล้ว `return new` โดยไม่ stamp
-- **ผลจริง: `is_active` พลิก แต่ `updated_at` ไม่ขยับ**
--
-- 🔴 **จงใจไม่แก้ และเหตุผลไม่ใช่ "ไม่สำคัญ":**
-- ① ด่าน `depth > 1` นั้นมีไว้กันของที่หนักกว่ามาก — `on delete set null` บน `updated_by_user`
--    ทำให้ *การลบบัญชีหนึ่งคน* ไปประทับชื่อคนลบลงทุกแถวที่คนที่ถูกลบเคยสร้าง (`§11.18`)
--    **มันกัน "การเปลี่ยนที่ไม่มีคนทำ ไม่ให้ถูกนับว่ามีคนทำ"** — ไม่ใช่แค่กันประทับซ้ำ
-- ② **ไม่มีใครอ่าน `trip_plans.updated_at` เลยสักที่** — ตรวจแล้วทั้งรีโป
--    (`plansOfTrip` เลือก `id, name, is_active, created_at` เท่านั้น) → แก้ตอนนี้คือเขียนโค้ดให้ของสมมติ
-- ③ P7 วัดแล้วว่าโมเดล sync ไม่กระทบ — `§12.1` ใช้ **full refetch ไม่ใช่ delta**
--    โมเดลจึงไม่เคยถามว่า "มีใครแก้ไหม" **การเปลี่ยนที่ไม่มี actor จึงแยกไม่ออกจากที่มี และไม่ต้องแยก**
--
-- ⚠️ **แต่นี่เป็นสมาชิกตัวที่ 3 ของคลาสที่โตขึ้นเรื่อย ๆ และไม่มีใครนับมัน** (P7 · `E6-AC9`):
--    ① FK `on delete set null` เขียน `added_by_user` · ② `preserve_authorship` เขียน `legacy_*_by`
--    ③ **ตัวนี้** · 🎯 **trigger ตัวถัดไปจะเพิ่มข้อที่ 4 โดยไม่มีอะไรส่งเสียง**
--    → บล็อกนี้มีไว้เพื่อให้ตัวที่ 3 **ส่งเสียงด้วยตัวเอง** · trigger ใหม่ทุกตัวควรมีบล็อกแบบนี้
--    **ถ้าวันหนึ่งมีคนอ่าน `updated_at` ของตารางนี้ ให้กลับมาที่ย่อหน้านี้ก่อนตัดสินว่ามันเชื่อได้**
