...
begin
  ...
  --   · ฐานที่ `public` ว่าง → ผ่าน · **ซึ่งไม่ได้แปลว่าฐานนั้นไม่มีอะไร** (`P-34` · P4)
  --     🔴 โปรเจกต์ที่วางแอปไว้ในสคีมาอื่นจน `public` ว่าง **จะผ่านด่านนี้** แล้วโดน
  --        `alter default privileges … revoke all from anon, authenticated` ซึ่งพังเงียบและย้อนไม่ถึง
  --     ยอมรับข้อแลกนี้เพราะทางเลือกคือปฏิเสธฐานใหม่จริงด้วย — **แต่ห้ามเขียนว่ามันกัน "ทุกฐานที่ไม่ใช่ของเรา"**
  --     ทางกันจริงของเคสนั้นคือ `db-push.sh` ที่บังคับ ref ก่อนยิง (`D50` ชั้น ①)
