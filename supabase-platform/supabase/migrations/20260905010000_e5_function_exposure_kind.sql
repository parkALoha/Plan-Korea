...
--
-- ## 🔴 **ค่าคำใน `kind` เป็นสัญญา — มีด่านพึ่งมันอยู่ · เปลี่ยนคำ = ทำให้ด่านนั้นเขียวสนิทโดยไม่มีใครรู้**
--   ด่านทะเบียนสิทธิ์ของ P4 นิยาม *"เรียกได้จริง"* ว่า **`EXECUTE` + `USAGE` + `kind = 'normal'`**
--   ⇒ ถ้าใครเปลี่ยน `'normal'` เป็นคำอื่น · หรือทำให้คอลัมน์นี้คืน `null`
--     ตัวกรองของเขาจะ **กวาดเซตให้ว่าง** แล้ว ***ทุก role เขียวหมดโดยไม่ได้ตรวจอะไรเลย***
--   🎯 ***ตัววัดที่พังเงียบ ให้คำตอบที่ปลอดภัยที่สุดเสมอ — และนั่นคือสิ่งที่ทำให้ไม่มีใครสงสัย***
--   · ✅ P4 เขียนเคสควบคุมกันข้อนี้ไว้แล้ว (*ต้องเห็นแถว `kind='normal'` และแถว `USAGE` อย่างละหนึ่ง*)
--     ⇒ **เปลี่ยนคำเมื่อไหร่ เคสนั้นแดง** · บรรทัดนี้มีไว้ให้คนที่กำลังจะเปลี่ยน **รู้ก่อนว่าจะไปโดนอะไร**
--   · ⚠️ **และ `procedure` ถูกกันออกจากนิยาม "เรียกได้" ของด่านนั้น** — ปลอดภัยวันนี้เพราะ
--     ฐานไม่มี procedure สักตัว (วัดแล้ว 5 ก.ย. 2026: `normal 59 · trigger 8 · event_trigger 1 · procedure 0`)
--     🔴 **ปลอดภัยด้วยข้อเท็จจริงที่ไม่เกี่ยวกับตัวด่านเลย** — วันที่มีคนเพิ่ม procedure ตัวแรก ไม่มีอะไรส่งเสียง
--
