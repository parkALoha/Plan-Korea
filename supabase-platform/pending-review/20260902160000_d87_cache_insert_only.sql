...
--
-- ## ความคืบหน้า 2 ก.ย. 2026 (เย็น) — ผู้ใช้เลือกทาง (ข) · ครึ่งแรกลงแล้ว
--   ✅ `lib/engine/cacheKey.ts` — hash คีย์ด้วย salt ฝั่งเซิร์ฟเวอร์ (`TRAVEL_CACHE_KEY_SALT`)
--      ไม่มี salt = **ไม่แคชเลย** ไม่ถอยไปเขียนคีย์ดิบ · `lib/__tests__/cacheKey.test.ts` 7 เคส
--      · ยิงทิศแดงแล้ว: ถอด salt ออก → **แดงเคสเดียว** อีก 6 เคสยังเขียว
--        🎯 แปลว่าถ้าไม่มีเคสนั้น implementation ที่ลืมใส่ salt จะผ่านทั้งชุด
--   ✅ `app/api/travel-time/route.ts` — อ่าน/เขียนด้วยคีย์ hash ทั้งสองทาง
--
...
--   ② **`place_details_cache` ยังไม่ถูกแก้ และ hash คีย์ไม่พอ**
--      ตัวแถวถือ `name_local`/`address_local` ที่ Google ตอบกลับ → **บอกได้เองว่าเป็นที่ไหน**
--      · `placeQueryKey()` (lib/placeQuery.ts:19) คืน `mapsQuery` ตรง ๆ เมื่อไม่มี Google id
--        → สำหรับ custom place นั่นคือ **ข้อความที่ผู้ใช้พิมพ์เอง** (ชื่อ/ที่อยู่)
--      🔴 ต้องตัดสินแยก — ทางที่เห็น: ไม่แคชคิวรีของ custom place ลงตารางกลาง
--
