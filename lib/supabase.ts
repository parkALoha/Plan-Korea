...
export type CustomPlace = {
  /**
   * 🔴 รหัสประเทศของ **เมือง** ไม่ใช่ของทริป (`E4-AC3` · 27 ส.ค. 2026)
   * `trips` ไม่มีคอลัมน์ประเทศเลย และนั่นถูก — **ทริปเดียวข้ามประเทศได้**
   * (ทริปปัจจุบันข้ามเวียดนาม→เกาหลี) · ปุ่มแผนที่จึงต้องเลือกจากเมืองของสถานที่นั้น
   * · `null` = ยังไม่รู้ → ส่งเข้า `capabilitiesOf()` ได้ตรง ๆ มันคืนค่าปลอดภัยให้เอง
   * · **optional** เพราะแถวที่มาจากทางอื่น (เช่น cache เก่า) ยังไม่มีฟิลด์นี้
   */
  country?: string | null;
  id: string;
