 ...
 *
 * ## 🔴 แก้ 28 ส.ค. 2026 — **ย่อหน้าข้างบนกับข้างล่างพูดสิ่งที่เป็นเท็จ** (P4 วัด · P1 เขียนผิดเอง)
 * ประโยค *"ทริปที่สร้างใหม่มี `trip_days` ที่ไม่มีคู่ใน `ITINERARY` เลย"* และ *"ทริปที่เกิดบนแพลตฟอร์ม
 * ทุกใบจะมีสะพานว่างเปล่าตลอดไป"* **จริงเฉพาะทริปที่ช่วงวันไม่ทับ 11–21 ต.ค. 2026**
 * ```
 * buildDayBridge จับคู่ด้วย `dbByDate.set(d.date, d.id)` → ไม่มีที่ไหนถาม `trip_id` เลย
 * ทริปคนละใบที่วันที่ตรงกัน (uuid คนละชุด):  matched 11/11 · unmatchedDb 0
 *   toDbId("d0") = uuid ของ *ทริปอื่น*
 * ```
 * 🎯 **และช่วงวันที่ที่ทำให้เกิดคือช่วงของทริปจริง** — คนที่วางแผนช่วงเดียวกัน (หรือสร้างทริปทดสอบเลียนแบบ
 * ทริปจริง ซึ่งเป็นสิ่งที่ทำกันเป็นธรรมชาติที่สุด) **คือกรณีที่น่าจะเกิดที่สุด ไม่ใช่เคสสุดโต่ง**
 * · วัดจริงบนฐาน dev 28 ส.ค. 2026: มีทริปของ**บัญชีผู้ใช้จริง** 1 ใบที่ตกเคสนี้อยู่แล้ว
 * · ⚠️ **ไม่ใช่การรั่วของข้อมูลผู้ใช้คนอื่น** — `ITINERARY` เป็นไฟล์นิ่งใน bundle ไม่ได้มาจากฐาน ไม่ผ่าน RLS
 *   **แต่เป็นบั๊กที่ผู้ใช้เห็นจริง**: ทริปของเขาแสดงเที่ยวบิน/ที่พักที่เขาไม่ได้ใส่
 * 🔴 **ทางแก้ไม่ใช่ทำให้ `buildDayBridge` รู้จัก `tripId`** — หัว `dayBridge.ts` ตั้งใจให้มันไม่ผูกกับทริปใด
 *   ผู้เรียกต่างหากที่ต้องส่ง `ITINERARY` เฉพาะทริปที่แผนอยู่ในไฟล์จริง ๆ (P4 เสนอ · P1 รับ)
 *   · **สัญญาณ "ทริปไหนคือทริปนั้น" ยังไม่มีในฐาน** → เป็นของ `E7` ซึ่งเป็นเฟสที่ตัดสินว่าแผนของทริป
 *     เกาหลีจะอยู่ที่ไหน · **จนกว่าจะถึงตอนนั้น `/today` + `/summary` ปฏิเสธทุกทริป** (ตัดสิน 28 ส.ค.)
 *
