 ...
 * (ยิ่งสั้นยิ่งดีเพราะยิ่งลดหน้าต่างที่ bearer credential นี้มีชีวิตอยู่) **ลดจาก 90s เหลือ 30s**
 * 🔴 **เหตุผลของเลข 30 ยืนได้ด้วยตัวเอง:** URL นี้ไม่เคยออกจากฟังก์ชัน จึงไม่ต้องมีอายุยาวพอให้ผู้ใช้กด
 *    ต้องยาวพอแค่ให้ `fetch()` หนึ่งครั้งจบเท่านั้น
 * · เดิมเลขนี้อ้างอิงกับ `INTERNAL_SIGN_SECONDS` ของ `/api/booking-file/[...path]/route.ts`
 *   **ไฟล์นั้นถูกลบแล้ว 26 ส.ค. 2026 (ไม่มีใครเรียกเลยตั้งแต่วันแรก)** — เลขไม่เปลี่ยน แต่เหตุผลเปลี่ยนเจ้าของ
 */
