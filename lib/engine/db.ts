...
export function searchCatalogCities(
  ...
  /**
   * 🔴 **เรียงตามความนิยม *เฉพาะตอนไล่ดูประเทศเดียว*** (ผู้ใช้สั่ง 5 ก.ย. 2026)
   * > *"เมืองของแต่ละประเทศ ควรเรียงจากเมืองยอดฮิต"* — เขาเห็น `/explore/kr` ขึ้น
   * > *คยองจู · คังนึง · คาพย็อง …* แล้ว **โซลอยู่อันดับ 9 · ปูซานอันดับ 11**
   *
   * ⚠️ **ไม่แตะกิ่งค้นข้ามประเทศ และนั่นไม่ใช่ความขี้เกียจ** — `popularity` เป็นอันดับ
   * *ภายในประเทศ* ⇒ เอาไปเรียงผลค้นข้ามประเทศ จะได้ "เมืองอันดับ 1 ของทุกประเทศ" มากองหัวลิสต์
   * โดยไม่เกี่ยวกับว่าคำที่พิมพ์ใกล้เคียงใบไหน ⇒ **มันตอบคนละคำถามกับที่ผู้ใช้พิมพ์มา**
   * 🎯 ***เกณฑ์เรียงที่ถูกสำหรับ "ไล่ดู" ไม่ใช่เกณฑ์ที่ถูกสำหรับ "ค้นหา" — คนละคำถาม คนละคำตอบ***
   *
   * 🔴 `nullsFirst: false` จำเป็น — เมืองที่ยังไม่จัดอันดับต้องไปท้าย ไม่ใช่ขึ้นหัว
   *    (PostgREST เรียง `asc` ให้ `null` มาก่อนโดยปริยาย ⇒ ลืมแล้วได้ผลกลับด้านพอดี)
   * · ตรงกับ `list_public_cities()` ที่ใช้ `order by ci.popularity nulls last, ci.name_th`
   *   ⚠️ **สองที่ต้องตรงกัน** เพราะคนล็อกอินกับคนไม่ล็อกอินเปิดหน้าเดียวกัน — เห็นลำดับต่างกันไม่ได้
   */
  if (opts.countryId && safe === "") {
    return query
      .order("popularity", { nullsFirst: false })
      .order("name_th")
      .limit(opts.limit ?? 20);
  }
...
/**
 * เนื้อของทริปแนะนำใบหนึ่ง — **หนึ่งแถวต่อหนึ่งจุดแวะ**
 *
 * 🔴 **รูปแบนโดยตั้งใจ ไม่ใช่เพราะขี้เกียจซ้อน** (`20260905150000`)
 * `schemaPins` ปักหมุด *ชุดคอลัมน์ที่ไหลออกไปหา `anon`* จาก `returns table (…)`
 * และเขียนขอบเขตตัวเองไว้ว่าฟังก์ชันที่คืน `jsonb` **จะรั่วโดยชุดคอลัมน์ไม่ขยับเลย**
 * ⇒ คืน `jsonb` = เขียนฟังก์ชันที่เดินผ่านด่านที่สร้างมาเพื่อเรื่องนี้พอดี · **การซ้อนเป็นงานของ route**
 *
 * ⚠️ หัวทริป (`title`/`day_count`/`night_count`) **ซ้ำทุกแถว** — อ่านจากแถวแรกพอ
 * ⚠️ **วันที่ไม่มีจุดแวะ ยังคืนหนึ่งแถว** โดย `place_*` เป็น null — ห้ามกรองทิ้ง
 *    ไม่งั้นจำนวนวันบนหน้าพรีวิวจะไม่ตรงกับ `day_count` ที่การ์ดบอก
 * 🔴 ด่านทั้งหมดอยู่ใน RPC (`published_template_at is not null`) — **ห้ามเขียน wrapper ที่รับ `tripId`
 *    แล้วอ่านตารางตรง** · เหตุผลเดียวกับ `copyTripTemplate` ข้างล่าง
 */
export function getTripTemplate(db: Db, templateId: string) {
  return db.rpc("get_trip_template", { p_template_id: templateId });
}
