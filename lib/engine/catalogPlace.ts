...
export function cardToPlace(card: CatalogNameCard): Place | null {
  ...
  return {
     ...
     *
     * ## 🔴 สถานะของ `maps_query` บนแถว `transfer` — **ป้ายสองท่อน ไม่ใช่ท่อนเดียว** (P4 ไล่ครบ · 2 ก.ย. 2026)
     * ```
     * ✅ ไม่มีเส้นทางไหนในรีโปเติมให้      ← ตรวจได้จากไฟล์ ใครก็รันซ้ำได้
     *    ① คอลัมน์ไม่มี default · ไม่ not null
     *    ② migration ที่เขียน maps_query มีใบเดียวทั้งรีโป และตัด transfer ออกทุกจุด **รวม assert ของมันเอง**
     *    ③ seed ทั้งสองใบ (เกาหลี · ญี่ปุ่น · transfer รวม 8 จุด) ไม่เอ่ย maps_query เลย → insert เป็น null
     *    ④ โค้ดแอปไม่มี write path ไป catalog_places.maps_query (ที่เจอใน place-details เป็นการ *อ่าน* คนละตาราง)
     * ⚠️ ยังไม่ได้ยืนยันกับฐาน                 ← คนรัน SQL ด้วยมือใน dashboard มองไม่เห็นจากไฟล์
     * ```
     * 🎯 **ท่อนบนเขียนว่า "เป็น null" ไม่ได้ · ท่อนล่างคือเหตุผลว่าทำไม** — และรูนั้นไม่ใช่ทฤษฎี
     * (เกิดจริงเมื่อ 1 ก.ย.: ผู้ใช้รัน migration ผ่าน SQL editor แล้ว ledger ไม่รู้)
     * · 📌 **ป้ายเดิมที่ผมเขียน (*"migration ไม่ได้เติมให้"*) ต่ำกว่าหลักฐานที่มี** — P4 อัปเกรดท่อนบนให้
     *   จาก *"ใบนั้นไม่เติม"* เป็น *"ไม่มีใบไหนเติม"* ซึ่งแรงกว่า **และยังตรวจได้จากไฟล์เหมือนเดิม**
