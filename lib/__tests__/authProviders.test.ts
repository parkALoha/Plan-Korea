 ...
 *
 * ## 🔴 severity — ด่านนี้แบกอะไรอยู่ (วัด auto-link เมื่อ 26 ส.ค. 2026)
 * เราวัดจริงว่า **auto-link เปิดอยู่**: บัญชีที่สมัคร magic-link ก่อนแล้วเอา Google มาชน
 * ถูก **ผูกเป็นบัญชีเดียว ไม่แยก** — และปลอดภัย **เพราะทั้งสองช่องทาง verify อีเมลจริง**
 * 🎯 ผลข้างเคียงของข้อเท็จจริงนั้น: ถ้าวันหนึ่งมี provider ที่คืน email โดย **ไม่** verify
 *   auto-link จะ merge มันเข้าบัญชีเดิม **อัตโนมัติตอนล็อกอิน ไม่ต้องมี user action** →
 *   ผลไม่ใช่ *"ผู้ใช้ได้สองบัญชี"* อีกต่อไป แต่เป็น ***"คนอื่นเข้าบัญชีผู้ใช้ได้"***
 * → **ความรุนแรงของด่านนี้เปลี่ยนโดยที่ตัวด่านไม่ได้ขยับเลย** · auto-link คือตัวเปลี่ยน
 *
 * 🔴 **ขอบเขตที่ต้องพูดให้ตรง (อย่าอ่านเกินนี้):**
 *   auto-link เปิด · link บน verified email ปลอดภัย · **takeover ถูกกันโดย `AC8` pin (เคสข้างล่าง)
 *   ไม่ใช่โดยการทดสอบ auto-link** — เราไม่เคยเห็นกิ่ง provider-ไม่-verify เลย (Gmail คืน verified เสมอ)
 *   ⚠️ **ห้ามอ่านว่า "ทดสอบ auto-link แล้วปลอดภัย"** · สิ่งที่วัดคือ *"เมื่อทั้งคู่ verify ระบบผูก ไม่แยก"*
 *   ไม่ใช่ *"ถ้าช่องทางไม่ verify จะเกิดอะไร"* — กิ่งนั้นยังมืด และเคสข้างล่างคือฝาที่ปิดมันไว้
 */
  ...
  it("🔴 provider ที่เปิดอยู่ ต้องเป็นตัวที่ยืนยันอีเมลทั้งหมด — เปิดตัวใหม่ต้องมาอ่าน AC8 ก่อน", async () => {
    ...
    expect(
      ...
      "มี provider ที่ไม่ได้อยู่ในลิสต์ที่พิสูจน์แล้วว่ายืนยันอีเมล\n" +
        "  🔴 auto-link เปิดอยู่ (วัดแล้ว 26 ส.ค. 2026) → provider ที่คืน email โดยไม่ verify\n" +
        "     จะถูก merge เข้าบัญชีเดิม *อัตโนมัติตอนล็อกอิน ไม่ต้องมี user action* = **ยึดบัญชีได้ ไม่ใช่แค่บัญชีซ้ำ**\n" +
