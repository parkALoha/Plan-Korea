...
describe("DeviceOwnerStamp", () => {
   ...
   * 🔴 **และเหตุผล *fail closed* ที่เคย ④ อ้างไว้ อ้างถึงผู้อ่านที่ไม่มีอยู่จริง *ตอนนั้น***
   *
   * 🔴 **แก้ 3 ก.ย. 2026 — ประโยคเดิมตรงนี้เขียนว่า *"วัดแล้ว: `readDeviceOwner()` ไม่ถูกเรียกจาก
   *    ที่ไหนเลยนอกไฟล์ตัวเอง"* และมันเป็นเท็จไปแล้ว** (P4 เจอ · `hooks/useActiveTripId.ts` เรียกมันตั้งแต่ `9580f8d`)
   *    · ⚠️ **ประโยคนั้นคือสิ่งที่ค้ำการกลับด้าน ③④ ทั้งใบ** — ปล่อยไว้ คนถัดไปจะอ่านเหตุผลที่ไม่จริงแล้วสรุปต่อ
   *    🎯 **และมันหมดอายุโดยที่ไม่มีอะไรแตะไฟล์นี้เลย** — คนที่ทำให้มันเท็จอยู่คนละไฟล์ ไม่มีทางรู้ว่าประโยคนี้มีอยู่
   *    ✅ **ถ้อยคำใหม่ชี้ไปที่ของที่บังคับได้ แทนที่จะเล่าสภาพ ณ วินาทีหนึ่ง** (P4 เสนอถ้อยคำ):
   *
   *    ⇒ **ผู้อ่าน `readDeviceOwner()` ทุกตัวถูกทบทวนแล้วและขึ้นทะเบียนใน `deviceOwnerReaderScope.test.ts`**
   *      · ผู้อ่านรายใหม่ทำให้ด่านนั้น **แดง** → บังคับให้กลับมาทบทวน ③④ ก่อนขึ้นทะเบียน
   *      · 🔴 **ห้ามแทนที่บรรทัดนี้ด้วยการนับผู้อ่านอีก** — จำนวนหมดอายุเงียบ · ทะเบียนที่แดงเป็นไม่หมด
   *
