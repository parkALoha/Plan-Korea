 ...
 *
 * ## 🔴 ขอบที่สอง — **จักรวาลที่สแกนคือ `hooks/` ชั้นเดียว · แคบกว่าพื้นผิวที่ `AC4` อ้าง** (P3 เจอ · วัด 2 ก.ย. 2026)
 * ```
 * ยิง `/api/engine/` ทั้งโปรเจกต์ 26 ไฟล์  ·  ใน hooks/ 18  ·  **นอก hooks/ 8**
 *   app/summary/page.tsx · components/{TripHeader,TripPlanScreen,CreateTripForm,TripDestinationPicker}.tsx
 *   lib/engine/dayEvents.ts · lib/supabase.ts · components/HomeScreen.tsx
 * ```
 * → **เคส ④ ไม่มีวันแดงใส่ไฟล์นอก `hooks/` ไม่ว่ามันจะไม่แคชแค่ไหน** · `app/summary/page.tsx` ที่ทำให้
 *   ชื่อทริปกลายเป็น `"ทริปนี้"` ตอนออฟไลน์ **อยู่นอกจักรวาลนี้พอดี**
 * 🔴 **ทำไมยังไม่ขยาย (ตั้งใจ ไม่ใช่ยังไม่ได้ทำ):** นอก `hooks/` มี**ทางเขียนปนอยู่** — `CreateTripForm`
 *   ยิง POST · `lib/supabase.ts` ไม่ใช่พื้นผิวอ่าน · `TripDestinationPicker` ค้นสด
 *   → ขยายดื้อ ๆ = **แดงใส่ของที่ไม่ผิด ซึ่งจบด้วยมีคนลบด่านทั้งใบ** · ใส่ suppression list = ทะเบียนใบที่สอง
 * 🎯 **หน่วยจำแนกต้องเป็น *call site* ไม่ใช่ *ไฟล์* ทันทีที่ออกนอก `hooks/`** (แยก GET ออกจาก POST)
 *   — ยังไม่มีใครเขียน · **จนกว่าจะมี ขอบของด่านนี้คือ `hooks/` เท่านั้น**
 * · 📌 ข้อนี้เองคือ **ทิศ ⑨ ที่ยิงใส่ตัวด่าน**: ก่อนบรรทัดนี้ถูกเขียน ไฟล์นี้*อ่านเหมือนครอบ `AC4` ทั้งข้อ*
 *
...
/**
 * 🔴 **`readOwnedCache` ถูกเพิ่มเข้ารายการ 3 ก.ย. 2026 หลังด่านนี้แดงใส่ `useActiveTripId` (P1 เจอ)**
 *
 * `43c2f7c` เปลี่ยน `readCache(...)` → `readOwnedCache(...)` (`E6-AC14`) แล้วด่านนี้อ่านว่า
 * *"hook ยิง `/api/engine/` แล้วไม่อ่านแคชเลย"* — **ทั้งที่มันอ่าน แค่เปลี่ยนชื่อฟังก์ชัน**
 * · ⚠️ `readOwnedCache` **ไม่มีสตริง `readCache` อยู่ข้างใน** (`Owned` แทรกกลาง) → `\breadCache\b` จับไม่ได้ตามนิยาม
 *
 * ## ทำไมมันนับว่า "อ่านแคชได้"
 * มันเรียก `readCache` ข้างใน แล้วคืน `null` เมื่อเจ้าของไม่ตรง — **เจ้าของตัวจริงยังได้ข้อมูลออฟไลน์ครบ**
 * ⇒ ตรงกับคำถามของ `AC4` (*ออฟไลน์แล้วมีอะไรให้อ่านไหม*) · การที่คนอื่นไม่ได้ **คือความถูกต้อง ไม่ใช่แคชพัง**
 *
 * ## 🎯 บทเรียนที่แพงกว่าตัวชื่อ — และเป็นเหตุผลที่มีเคส ⑩ ข้างล่าง
 * **รายชื่อในตัวสแกนคือทะเบียน · การ *เปลี่ยนชื่อฟังก์ชัน* ทำให้ของหลุดจากทะเบียนโดยไม่มีสัญญาณ**
 * · ผู้เปลี่ยนชื่อไม่มีทางรู้ว่ามีตัวสแกนคนละไฟล์ถือรายชื่อนี้อยู่
 * · 🔴 **และอาการคือ *แดงใส่โค้ดที่ถูกต้อง*** — ซึ่งจบที่คนลบด่าน (`TEAM.md §3.4`)
 */
function cacheReaderNamesUsedIn(code: string): string[] {
  return [...new Set(code.match(/\bread[A-Za-z]*Cache\b|\bstoreGet\b/g) ?? [])];
}
...
describe("🔴 E6-AC4 — พื้นผิวที่ดึงข้อมูลทริป ต้องอ่านผ่านแคช (ตารางต้องตรงกับโค้ด)", () => {
  ...
  it("⑩ 🔴 ทะเบียนชื่อตัวอ่านแคช ต้องไม่ล้าหลังโค้ด — ตัวที่ถูกเปลี่ยนชื่อจะหลุดเงียบ", () => {
    /**
     * เคสนี้เกิดขึ้นเพราะ `43c2f7c` เปลี่ยน `readCache` → `readOwnedCache` แล้ว **ด่านแดงใส่โค้ดที่ถูกต้อง**
     * 🎯 **สิ่งที่หลุดไม่ใช่ *ไฟล์* แต่เป็น *ชื่อ*** — ⑥/⑧ ถามว่าไฟล์ยังอยู่/ยังถูกเรียกไหม **ไม่มีข้อไหนถามว่า
     * ตัวสแกนยังรู้จักชื่อที่โค้ดใช้อยู่หรือเปล่า**
     * · ⚠️ ไม่ได้บังคับว่าทุกชื่อต้อง *นับเป็น* การอ่านแคช — บังคับแค่ว่า **ต้องมีคนตัดสินใจเรื่องมันแล้ว**
     */
    const used = new Set(hooks.flatMap((h) => cacheReaderNamesUsedIn(h.code)));
    expect(used.size, "ไม่เจอชื่อตัวอ่านแคชสักตัวใน hooks/ — regex ค้นชื่อน่าจะเพี้ยน").toBeGreaterThan(1);

    const unknown = [...used].filter((n) => !CACHE_READERS.includes(n as (typeof CACHE_READERS)[number]));
    expect(
      unknown.sort(),
      "ชื่อพวกนี้หน้าตาเป็นตัวอ่านแคช แต่ `CACHE_READERS` ไม่รู้จัก\n" +
        "  🔴 ถ้ามันคือการอ่านแคชจริง → เติมเข้า `CACHE_READERS` **ไม่งั้นเคส ④ จะแดงใส่ hook ที่ทำถูก**\n" +
        "  · ถ้าไม่ใช่ (เช่นตัวเขียน/ตัวล้างที่ชื่อคล้าย) → เปลี่ยนชื่อมันให้ไม่ขึ้นต้นด้วย `read…Cache`\n" +
        "  🎯 ต้นเรื่อง: เปลี่ยนชื่อฟังก์ชันทำให้ของหลุดจากทะเบียนของตัวสแกน **โดยผู้เปลี่ยนไม่มีทางรู้ว่าทะเบียนนี้มีอยู่**"
    ).toEqual([]);
  });

  it("⑨ 🔴 *เหตุผล* ยังจริงไหม — ไม่ใช่แค่ *รายการ* ยังถูก", () => {
    /**
     * ⑥ ถามว่า *ยังมีอยู่ไหม* · ⑧ ถามว่า *ยังถูกใช้ไหม* · **⑨ ถามว่า *เหตุผลยังจริงไหม***
     * 🔴 ใบนี้อันตรายที่สุดเพราะ **รายการยังถูก** — ⑥/⑦/⑧ เขียวหมด ขณะที่คำตัดสินควรกลับด้านไปแล้ว
     * · เคสสด: `useTripDaysGate` จดเหตุผลว่า *"fail-open เป็น `ready` เมื่อเช็คไม่สำเร็จ"*
     *   → ถ้ามีคนแก้เป็น fail-closed **คำตัดสิน "ห้ามแคช" กลับด้านทันที** แต่ไฟล์ยังอยู่ · ยังมีคนเรียก · ยังไม่แคช
     */
    const code = stripTsComments(readFileSync(join(HOOKS, "useTripDaysGate.ts"), "utf8"));

    // เคสควบคุม **ก่อน** ข้อสรุป — regex ที่ไม่แมตช์อะไรเลย ให้ผลเหมือน "โค้ดถูกเปลี่ยน" เป๊ะ
    expect(/setState\s*\(\s*"(loading|empty|ready)"\s*\)/.test(code), "อ่านไฟล์ไม่ติด/regex พัง").toBe(true);

    expect(
      /if\s*\(\s*!res\.ok\s*\)\s*\{\s*setState\s*\(\s*"ready"\s*\)/.test(code),
      "กิ่ง `!res.ok` ไม่ได้ตั้ง `ready` แล้ว — **เหตุผลที่จดไว้ในทะเบียนตายแล้ว**\n" +
        "  เหตุผลปัจจุบันคือ *fail-open จึงออฟไลน์ไม่ถูกบล็อก* → ถ้าเปลี่ยนเป็น fail-closed\n" +
        "  คำตัดสิน 'ห้ามแคช' ต้องถูกทบทวน ไม่ใช่ปล่อยให้ทะเบียนอ่านเหมือนยังผ่านการตรวจ"
    ).toBe(true);

    expect(
      /catch\s*\{[^}]*setState\s*\(\s*"ready"\s*\)/.test(code),
      "กิ่ง `catch` (เน็ตหลุด/parse ล้ม) ไม่ได้ตั้ง `ready` แล้ว — ดูเหตุผลเดียวกับข้างบน"
    ).toBe(true);

    /**
     * 🔴 **และนี่คือส่วนที่ ⑨ ปิดไม่ได้ — เขียนไว้เพราะทะเบียนที่มี ⑥⑧⑨ ครบ
     * *อ่านเหมือนกันเป๊ะ* กับทะเบียนที่กันความล้าได้ทุกชนิด**
     * · `useSystemMode` — เหตุผลเป็นข้อเท็จจริงของโค้ด (ธงค้างเก่าแย่กว่าไม่มีธง) แต่เป็นเรื่อง *ความหมาย*
     *   ไม่ใช่กิ่งที่ probe ได้
     * · `useTripMembers` — เหตุผล **เชิงคุณค่า** (*ถือชื่อคนอื่น เก็บลงเครื่องโดยไม่มีคนขอไม่คุ้ม*)
     *   **ไม่มีข้อเท็จจริงให้ probe เลย** → เหลือทางเดียวคือมีคนทบทวน
     * 🎯 **สองใบนี้ตั้งใจไม่มี probe · ไม่ใช่ลืม** — ใครเพิ่มชื่อใหม่เข้าทะเบียน ให้ถามก่อนว่า
     * *เหตุผลนี้อ้างข้อเท็จจริงของโค้ดหรือเปล่า* · ถ้าใช่ **ต้องมี probe** · ถ้าไม่ใช่ ให้เขียนกำกับว่าทำไมไม่มี
     */
    expect(Object.keys(NOT_CACHED_BY_DECISION).sort()).toEqual(
      ["useSystemMode.tsx", "useTripDaysGate.ts", "useTripMembers.ts"].sort()
    );
  });
});
