...
describe("buildDayBridge", () => {
   ...
   * 🟢 **เกิดขึ้นแล้ว (`1ed5a3b` · P2 แก้ · 28 ส.ค. 2026): 4 hook ส่ง `buildDayBridge([], dbDays)`**
   * → `legacyDays` ว่าง → `unmatchedLegacy` ว่าง → **สถานะครึ่ง ๆ เข้าถึงไม่ได้จากแอปแล้ว**
   * · เคสนี้จึงเป็น **บันทึกว่ามันเคยมี** ไม่ใช่คำอธิบายของโค้ดวันนี้ — และยังปัก *พฤติกรรมของสะพาน*
   *   ซึ่งไม่ได้เปลี่ยนเลยสักบรรทัด (ผู้เรียกเปลี่ยน ไม่ใช่สะพาน)
   ...
   *   ตรงกับ `ITINERARY` → **เคย**เห็นแผนเกาหลี (VN610 · ปูซาน) ทับทริปตัวเอง · แก้แล้วที่ `1ed5a3b`
   */
   ...
   * ตรงกับ `ITINERARY` พอดี → ได้สะพานเต็ม → **เคย** render แผนเกาหลี (VN610 · ปูซาน) ทับทริปเขา (แก้แล้ว `1ed5a3b`)
   *
...
describe("ทะเบียนผู้เรียก buildDayBridge", () => {
  ...
  /**
   * 🔴 **ปักตัวแก้ของ `1ed5a3b` — ไม่มีผู้เรียกรายไหนส่ง `ITINERARY` เข้าสะพานอีกแล้ว** (P4 · 28 ส.ค. 2026)
   *
   * เดิม 4 hook ส่ง `buildDayBridge(ITINERARY, dbDays)` **ทุกทริปโดยไม่มีเงื่อนไข** → ทริปของผู้ใช้ที่วันที่
   * ทับ 11–21 ต.ค. 2026 ได้สะพานเต็ม แล้วเห็นแผนเกาหลีทับทริปตัวเอง (ดูเคส characterization ข้างบน)
   * · P2 แก้เป็น `buildDayBridge([], dbDays)` — **สะพานไม่ได้เปลี่ยน ผู้เรียกเปลี่ยน**
   *
   * 🎯 **ทะเบียนข้างบนตอบว่า *ใครเรียก* · เคสนี้ตอบว่า *เรียกด้วยอะไร*** — คนละคำถาม และคำถามที่สอง
   * คือตัวที่ถือตัวแก้ไว้ · **ถ้าใครเติม `ITINERARY` กลับเข้าไป ทะเบียนจะยังเขียว เพราะผู้เรียกไม่ได้เพิ่ม**
   * ⚠️ อ่าน *ข้อความ* ในไฟล์ ไม่ได้รันโค้ด — ถ้าส่งผ่านตัวแปร (`const d = ITINERARY; buildDayBridge(d, …)`)
   *    ด่านนี้มองไม่เห็น · **จับรูปที่เกิดจริงและเกิดง่ายที่สุด ไม่ใช่ทุกรูป**
   */
  it("🔴 ไม่มีผู้เรียกรายไหนส่ง `ITINERARY` เข้า `buildDayBridge` — ปักตัวแก้ `1ed5a3b`", () => {
    const offenders: string[] = [];
    for (const f of callers()) {
      const src = stripTsComments(readFileSync(f, "utf8"));
      const m = src.match(/buildDayBridge\s*\(\s*([^,]+),/);
      if (m && /ITINERARY/.test(m[1])) offenders.push(`${f} → buildDayBridge(${m[1].trim()}, …)`);
    }
    expect(
      offenders.sort(),
      "ผู้เรียกส่ง `ITINERARY` เข้าสะพานอีกแล้ว — ทริปที่วันที่ทับ 11–21 ต.ค. 2026 จะเห็นแผนเกาหลีทับ\n" +
        "  → ส่ง `[]` เว้นแต่ทริปนั้นมีแผนอยู่ในไฟล์จริง (วันนี้ไม่มีทริปไหนเข้าเงื่อนไขนั้นบน platform)",
    ).toEqual([]);
  });
