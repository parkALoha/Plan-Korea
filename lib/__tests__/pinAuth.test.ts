...
describe("expectedPinToken()", () => {
  ...
  it("คืน null เมื่อตั้งมาแค่ TRIP_PIN — ขาด secret ห้ามเดาว่าไม่ต้องใช้", () => {
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: undefined });
    expect(expectedPinToken()).toBeNull();
  });

  it("คืน null เมื่อตั้งมาแค่ TRIP_PIN_SECRET", () => {
    setEnv({ TRIP_PIN: undefined, TRIP_PIN_SECRET: "s3cr3t" });
    expect(expectedPinToken()).toBeNull();
  });

  it("คืน HMAC-SHA256 เป็น hex 64 ตัวเมื่อ env ครบ", () => {
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: "s3cr3t" });
    const token = expectedPinToken();
    expect(token).toBe(hmac("s3cr3t", "2539"));
  ...
  it("🔴 secret เปลี่ยน → token เปลี่ยน ทั้งที่ PIN เดิม", () => {
    ...
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: "secret-a" });
    const a = expectedPinToken();
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: "secret-b" });
  ...
  it("token ต้องไม่ใช่ตัว PIN เอง และไม่มี PIN ฝังอยู่ข้างใน", () => {
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: "s3cr3t" });
    const token = expectedPinToken();
    expect(token).not.toBe("2539");
    expect(token).not.toContain("2539");
  });

  it("PIN ยาวเท่าไหร่ก็ได้ ไม่ได้บังคับ 4 หลัก", () => {
    ...
    setEnv({ TRIP_PIN: "9182736450", TRIP_PIN_SECRET: "s3cr3t" });
    expect(expectedPinToken()).toBe(hmac("s3cr3t", "9182736450"));
  });
});
...
describe("pinTokenMatches()", () => {
  ...
  it("true เมื่อ cookie ตรงกับ HMAC ที่คำนวณเองแยกจากโค้ดที่เทสต์", () => {
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: "s3cr3t" });
    expect(pinTokenMatches(hmac("s3cr3t", "2539"))).toBe(true);
  });
  ...
  it("false เมื่อ cookie มาจาก secret อื่น (เช่นหมุน secret แล้ว)", () => {
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: "secret-ใหม่" });
    expect(pinTokenMatches(hmac("secret-เก่า", "2539"))).toBe(false);
  });
...
describe("pinIsCorrect()", () => {
  it("false เมื่อไม่ได้ตั้ง TRIP_PIN — ห้ามปล่อยผ่านตอนไม่มีอะไรให้เทียบ", () => {
    setEnv({ TRIP_PIN: undefined, TRIP_PIN_SECRET: "s3cr3t" });
    expect(pinIsCorrect("2539")).toBe(false);
    expect(pinIsCorrect("")).toBe(false);
  });

  it("true เมื่อ PIN ตรงเป๊ะ", () => {
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: "s3cr3t" });
    expect(pinIsCorrect("2539")).toBe(true);
  });

  it("false เมื่อ PIN ผิดแต่ยาวเท่ากัน", () => {
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: "s3cr3t" });
    expect(pinIsCorrect("2538")).toBe(false);
    expect(pinIsCorrect("9999")).toBe(false);
  });

  it("🔴 PIN ยาวไม่เท่ากันต้องคืน false ไม่ใช่โยน", () => {
    setEnv({ TRIP_PIN: "2539", TRIP_PIN_SECRET: "s3cr3t" });
    for (const input of ["", "2", "25", "253", "25390", "2539".repeat(50)]) {
      expect(() => pinIsCorrect(input)).not.toThrow();
      expect(pinIsCorrect(input)).toBe(false);
    }
  });

  it("ไม่ตัดช่องว่างให้ — ' 2539' ไม่ใช่ '2539'", () => {
  ...
  it("รับ PIN ที่ไม่ใช่ตัวเลขได้ ถ้า env ตั้งไว้แบบนั้น", () => {
    setEnv({ TRIP_PIN: "korea-2026-oct", TRIP_PIN_SECRET: "s3cr3t" });
