...
describe.runIf(hasCreds)("assert_cache_lockdown — ต้องแดงเมื่อมีคีย์นอกคลัง", () => {
  ...
  it("⑤ เคสควบคุม — ฐานต้องมีแถว `travel_time_cache` ที่คีย์ถูกต้องอยู่จริง แล้วด่านยังผ่าน", async () => {
    ...
    expect(
      count ?? 0,
      // 🔴 **ข้อความนี้ต้องบอกว่าทำอะไรต่อ ไม่ใช่แค่ว่าอะไรผิด** (P4 ชี้ 4 ก.ย. 2026)
      //    เคสนี้พึ่งสภาพของฐาน ⇒ วันที่มีคนล้างฐาน dev หรือรันบนโปรเจกต์ใหม่ **มันจะแดงโดยไม่มีบั๊ก**
      //    🎯 ***ด่านที่แดงใส่คนทำถูกจะถูกลบทั้งใบ · ด่านที่แดงพร้อมบอกว่าต้องทำอะไร จะถูกทำตาม***
      //       — ต่างกันแค่ข้อความ ราคาเท่ากัน
      "🟡 **เงื่อนไขของสนาม ไม่ใช่ข้อบกพร่องของโค้ด** — ฐานไม่มีแถว `travel_time_cache` เลยสักแถว\n" +
        "   ⇒ เคส ① ยังผ่าน **แต่ไม่ได้พิสูจน์ว่าด่านเลือกถูก** (ด่านที่แดงใส่ทุกแถวก็ให้ผลเดียวกัน)\n" +
        "   ✅ **เติมข้อมูลลงฐาน (อุ่นแคชหนึ่งรอบ) ไม่ใช่แก้โค้ด และไม่ใช่ลบเคสนี้**",
    ).toBeGreaterThan(0);
