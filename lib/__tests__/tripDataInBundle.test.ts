...
describe.skipIf(!hasBuild)("E6-AC10 — ข้อมูลทริปเกาหลีในบันเดิลของ route แพลตฟอร์ม", () => {
  ...
  it("ratchet: มี route แพลตฟอร์มเปื้อนอยู่ 3 route เท่านั้น — เพิ่มขึ้น = ถอยหลัง", () => {
    ...
    /**
     * 🔴 **ป้าย ไม่ใช่ด่าน** (P3 เจอกับตัว · P4 ตัดสินรูปและที่วาง · 2 ก.ย. 2026)
     * ```
     * $ BUNDLE_GUARD_REQUIRED=1 npx vitest run lib/__tests__/tripDataInBundle.test.ts
     *   ✓ lib/__tests__/tripDataInBundle.test.ts (8 tests)      ← บรรทัดที่คนอ่านจริง
     * ```
     * **"8/8 ผ่าน" กับ "บันเดิลสะอาด" อ่านเหมือนกันเป๊ะ** — เคสของ route แพลตฟอร์มเป็น `it.fails`
     * ซึ่ง *ผ่าน* แปลว่า **บั๊กยังเปิดอยู่** · P3 เกือบรายงานว่าสะอาด และรอดเพราะเปิดไฟล์อ่านคอมเมนต์
     *
     * 🎯 **ทำไมไม่แก้ด้วยการเปลี่ยน *ชื่อเคส*** — ชื่อเคสมีคำว่า *"บั๊กเปิดอยู่"* อยู่แล้ววันนี้
     * **แต่ vitest ไม่พิมพ์ชื่อเคสเลยเมื่อไฟล์ผ่านทั้งไฟล์** → ถ้อยคำดังแค่ไหนก็ไม่โผล่ตรงที่คนอ่าน
     * · **ตัวที่กัดคือ *ระดับที่แสดง* ไม่ใช่ *ถ้อยคำ*** · `console.warn` โผล่เหนือบรรทัดสรุปเสมอ
     *
     * 🔴 **จำกัดด้วย `BUNDLE_GUARD_REQUIRED` โดยตั้งใจ** — รอบชุดเต็มปกติไฟล์นี้ขึ้น
     * `1 passed | 7 skipped` ซึ่ง**ดังอยู่แล้ว** · เตือนซ้ำที่นั่น = เสียงพื้นหลัง แล้วคนจะเลิกอ่าน
     * `console.warn` เรื่อง *skip* ไปด้วย — **ซึ่งเป็นตัวที่ช่วย P3 ไว้ในอีกเคสหนึ่งวันเดียวกัน**
     *
     * ## 🔴 ยิงสามทิศ ไม่ใช่สอง — **และทิศที่สามคือทิศเดียวที่พิสูจน์ข้อที่เราอ้าง**
     * ```
     * ① ธง=1 · ไม่มี build   →  warn ไม่โผล่   **แต่เพราะ `hasBuild` โยนก่อนถึงเคส ไม่ใช่เพราะเงื่อนไขธง**
     * ② ธง=1 · มี build      →  warn โผล่ พร้อมเลขจริงจาก `dirty`
     * ③ ไม่ตั้งธง · มี build  →  เคส **ถูกรันจริง** แล้ว warn เงียบ  ← หลักฐานว่าธงจำกัดได้จริง
     * ```
     * 🎯 **ทิศ ① ผ่านด้วยเหตุผลคนละข้อกับที่มันถูกออกแบบมาพิสูจน์** — และผลรันของ ① กับ ③
     * *อ่านเหมือนกันเป๊ะ* (ไม่มี warn ทั้งคู่) · **ห้ามตัดทิศ ③ ทิ้งเพราะดูซ้ำกับ ①**
     *
     * ⚠️ **นี่ไม่ใช่การปิดช่อง** — ด่านจริงคือ `it.fails` เอง: **วันที่บั๊กถูกปิด มันจะพลิกเป็นแดง**
     * ป้ายนี้ครอบแค่ *ช่วงที่บั๊กยังเปิด* ซึ่งเป็นช่วงที่ผลรันอ่านผิดได้ · และมัน **เงียบเองเมื่อ `dirty = 0`
     * โดยไม่มีใครต้องจำไปลบ**
     */
    if (process.env.BUNDLE_GUARD_REQUIRED === "1" && dirty.length > 0) {
      console.warn(
        `\n⚠️  E6-AC10 ยังไม่ปิด — route แพลตฟอร์มยังลากข้อมูลทริปเกาหลี ${dirty.length}/${PLATFORM_ROUTES.length} ใบ\n` +
          `    ${dirty.join(" · ")}\n` +
          "    🔴 เคส xfail ที่ 'ผ่าน' = บันทึกว่าบั๊กเปิดอยู่ — **ไม่ใช่ว่าบันเดิลสะอาด**\n"
      );
    }
