...
describe("cardToPlace — ② ลำดับชื่อสำรอง", () => {
   ...
   *
   * ## 🔴 อินพุตนี้ **สคีมาอนุญาตให้เกิด** — ไม่ใช่สมมติฐานลอย (P4 ไล่ migration · 2 ก.ย. 2026)
   * `catalog_place_names.name` เป็น `not null` + `check length(trim(name)) between 1 and 200`
   * → **แถวชื่อมีชื่อเสมอ** · แต่ **ไม่มี trigger/constraint ไหนบังคับว่า `catalog_places`
   * ต้องมีแถวชื่ออย่างน้อย 1 ใบ** — FK เป็นทางเดียว (`place_id not null` บนตารางชื่อ) ไม่บังคับย้อนกลับ
   * → การ์ดที่ `nameTh`/`nameEn`/`nameLocal` เป็น `null` ทั้งสาม **เป็นสิ่งที่ระบบยอมให้เกิด**
   *
   * ⚠️ **ป้ายของเคสนี้คือ *"สคีมาอนุญาต → ฟังก์ชันต้องตอบได้"* ไม่ใช่ *"เกิดจริงกี่แถว"***
   * สองอย่างนี้ต่างกัน · ข้อหลังยืนยันไม่ได้จากที่นี่ (ต้องยิงฐาน = สิทธิ์ของผู้ใช้)
   * 📌 และของที่ใหญ่กว่าเคสนี้: **ไม่มีด่านฝั่งฐานที่บังคับว่าสถานที่ในคลังต้องมีชื่อ**
   *    → `?? slug` จึงเป็นตัวรับน้ำหนักจริง ไม่ใช่ของประดับ
   */
