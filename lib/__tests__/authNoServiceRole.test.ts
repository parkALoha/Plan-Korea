...
const ALLOWED_ENV: Record<string, string> = {
  ...
  // 🔴 เพิ่ม 4 ก.ย. 2026 (P1) — `lib/offers/index.ts` เลือกผู้ให้บริการราคาโรงแรม/ตั๋ว
  //    **เก็บ *ชื่อ* ผู้ให้บริการ ไม่ใช่ *คีย์*** — ค่าที่เป็นไปได้คือสตริงอย่าง `"travelpayouts"`
  //    ⇒ หลุดไปฝั่งไคลเอนต์แล้วไม่มีใครทำอะไรได้ · ไม่มีสิทธิ์กับ Supabase หรือ Google เลย
  //    ⚠️ **วันที่มีคีย์จริง มันจะเป็น env *คนละตัว*** (`OFFERS_*_KEY`) — **ตัวนั้นห้ามมาอยู่ในทะเบียนนี้**
  //       `lib/offers/index.ts` มี `import "server-only"` บรรทัดแรกให้ `build` แดงถ้าถูก import จากไคลเอนต์
  //       · 🔴 **ด่านนั้นยังไม่มีใครลองพัง** — P8 ตั้งเป็น `E10-AC3` แล้ว **อย่าถือว่ามันพิสูจน์แล้ว**
  OFFERS_PROVIDER: "lib/offers — ชื่อผู้ให้บริการราคา (ไม่ใช่คีย์) · ไม่มีสิทธิ์กับ Supabase",
};
