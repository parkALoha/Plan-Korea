...
describe("hydrateThenFetch — ลำดับที่ IndexedDB ไม่แถมมาให้", () => {
  ...
  it("🔴 ดิสก์ไม่ตอบเลยตลอดกาล → ของสดต้องขึ้นจอ (กิ่งที่ P2 เจอโดยอุบัติเหตุ)", async () => {
     ...
     *
     * ⚠️ **เหตุผลเดิมที่เขียนตรงนี้ว่า *"ยอมรับได้เพราะผลต่อผู้ใช้เป็นศูนย์ (จอได้ของสดไปแล้ว)"*
     * ตายไปแล้ว — P7 ทำให้มันเป็นเท็จเมื่อ 4 ก.ย. 2026** และเขาเป็นคนมาบอกเอง
     * ```
     * ตอนเขียน   ไม่มีอะไรต่อท้าย `await hydrateThenFetch(...)` → ค้างแล้วไม่มีใครเดือดร้อน
     * E6-AC7     hook ย้ายมา IndexedDB → หลัง `await` มี `setLoaded(true)` · `subscribe()`
     *            ⇒ ดิสก์ค้าง = **จอค้างสถานะกำลังโหลด + ไม่มี realtime เลย** โดยไม่มีอะไรฟ้อง
     * ```
     * 🎯 ***`await` ที่เพิ่มเข้าไปในเส้นทางเดิม ส่งต่อ "การไม่จบ" ให้ทุกอย่างที่อยู่ข้างหลังมัน***
     * · P7 แก้ที่ผู้เรียก 6 จุดแล้ว (`a7759fb`) — **แต่นั่นคือวินัยที่ต้องจำ ไม่ใช่กลไก**
     *   hook ใหม่ที่ `await` แล้วต่อท้ายด้วยอะไรสักอย่าง **ไม่มีอะไรเตือนเลย**
     *
     * 🔴 **สัญญาที่แท้จริงของ `hydrateThenFetch` และไม่เคยมีใครเขียนไว้:**
     * ***มันรับประกัน "จอได้ของ" ไม่ได้รับประกัน "promise จบ"*** — กติกา ① (ยิงของสดทันที) ทำให้
     * `applyFresh` ไม่ขึ้นกับดิสก์เลย · แต่ค่าที่ *คืน* ต้องรอดิสก์เสมอ เพราะกติกา ③ ต้องรู้ว่ามีแคชไหม
     * ⇒ **ผู้เรียกที่วางงานไว้หลัง `await` กำลังพึ่งสิ่งที่ไม่ได้ถูกสัญญาไว้**
     * · ✅ เขียนลงหัว `lib/engine/hydrateThenFetch.ts` แล้ว — คนถัดไปจะเห็นตอนกำลังจะใช้
     * · 📌 ทางแก้ที่ *ตัวช่วย* (แยก "จบ" ออกจาก "แสดง") **เปลี่ยนความหมายของ `outcome` ให้ผู้เรียก 12 ตัว**
     *   ⇒ ต้องมี AC ของมันเอง · **ไม่แก้เงียบ ๆ ในคอมมิตนี้**
     *
