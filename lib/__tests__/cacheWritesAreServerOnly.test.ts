 ...
 *
 * ฉบับแรกบังคับรูป `.from("<ชื่อ>")` → **ตัวห่อรูปอื่นหลุดหมด** (`wrap(db,"…").delete()` · ยิงพิสูจน์แล้ว)
 *
 * 🔴 **แก้คำอ้าง (P4 ถอนเอง · P1 ยืนยันซ้ำ):** เหตุผลแรกที่ใช้หักด่านนี้คือ *"ท่าประจำบ้าน
 * `engineTable(db, "<ชื่อ>")` ใช้จริง 51 จุด และด่านมองไม่เห็น"* — **เกินจริง**
 * ```
 * engineTable      **ไม่ถูก `export`** (`db.ts:113` เขียนเหตุผลไว้เอง) · 51 จุดอยู่ใน db.ts ไฟล์เดียว
 * EngineTable      **ไม่มีตารางแคชสักใบ**
 * ⇒ `engineTable(db, "place_photo_cache")` ใน route **เขียนไม่ได้อยู่แล้ว** สองชั้น
 * ```
 * 🎯 **เลข `51` ถูกทุกตัวอักษร · สิ่งที่ไม่ได้วัดคือ *ใครเรียกมันได้*** — แล้วอ่าน *ปริมาณ* เป็น *การเข้าถึง*
 * · คำตอบอยู่ห่างจากบรรทัดที่นับไป **3 บรรทัด** (`db.ts:113`) · นับด้วย `grep -c` แล้วหยุด ไม่ได้เปิดดู
 *
 * ✅ **สิ่งที่ยังยืนหลังถอนคำอ้างนั้น — และเป็นเหตุผลที่การแก้ยังคุ้ม:**
 * ① ตัวห่อรูปอื่นหลุดจริง (พิสูจน์ด้วยการยิง ไม่ใช่ด้วยตัวเลข)
 * ② **ทิศแดงรอบแรกยิงด้วย `.from(` ซึ่งเป็นท่าที่ด่านเห็นอยู่แล้ว → พิสูจน์ความครอบคลุมไม่ได้**
 *    ข้อนี้ไม่ขึ้นกับ `engineTable` เลย
 * ⚠️ **แต่ *ความเร่ง* ต่ำกว่าที่เคยเขียนไว้** — ของที่ด่านเก่าปล่อยคือ **ท่าสมมติ ไม่ใช่ 51 จุดที่ใช้งานอยู่**
 *
