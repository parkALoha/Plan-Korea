...
describe("คีย์แคชที่ผูกกับทริป", () => {
  ...
  it("🔴 ไม่มี hook ไหนเรียก readCache/writeCache ด้วยชื่อที่เป็นข้อมูลของทริป", () => {
    ...
    expect(
      offenders,
      // 🔴 ข้อความตอนแดงต้องชี้ทาง *ปัจจุบัน* ไม่ใช่ทางที่เรากำลังย้ายออก (P3 เจอ 4 ก.ย. 2026)
      //    ฉบับก่อนเขียนว่า "ต้องใช้ readTripCache/writeTripCache" ⇒ **คนที่ชนด่านจะถูกบอกให้ย้ายกลับ**
      //    🎯 ด่านที่แดงถูกแต่สอนผิด แพงกว่าด่านที่ไม่แดง — เพราะคนทำตามแล้วเชื่อว่าตัวเองแก้ถูก
      `คีย์พวกนี้เป็นข้อมูลของทริป ต้องผูก \`tripId\` ลงคีย์ด้วย \`tripKey(tripId, name)\`\n` +
        `  (\`readTripCache\`/\`writeTripCache\` เป็นทางเก่าบน localStorage — \`E6-AC7\` ย้ายออกแล้ว)\n` +
        `  ${offenders.join("\n  ")}`
    ).toEqual([]);
  });
