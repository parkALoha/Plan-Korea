...
// 🔴 `testClient` (+ สตับ throw-on-socket) ย้ายไป `./_testClient` (ของกลาง) — 27 ส.ค. 2026 (P4 · ตาม P1)
//    คนถัดไปที่เขียนเทสต์สดต้อง `import { testClient }` จากที่นั่น ไม่ใช่ `createClient` ตรง
//    เหตุผล + คำเตือน F1 อยู่ในไฟล์นั้นทั้งดุ้น
...
describe.runIf(hasCreds)("RLS matrix (สด)", () => {
  ...
  describe("🔴 P-63 — column grant ต้องไม่เปลี่ยนเงียบ ๆ (`revoke` แล้ว re-grant ตกหล่น)", () => {
    ...
    const COLUMN_GRANTS: Record<string, readonly string[]> = {
      ...
      // 🔴 `E5` 27 ส.ค. — `cover_image_path` **ถูก drop แล้ว** (`20260827230000` · ฟีเจอร์อัปโหลดรูปปกถูกถอน
      //    ตามมติผู้ใช้ — รูปปกเป็นไฟล์สถิตย์ใน `public/covers/` แทน) · grant หายตามคอลัมน์
      //    ⚠️ เคส positive ที่เคยพิสูจน์ grant ของคอลัมน์นี้ถูกลบทิ้งด้วย — ไม่มีคอลัมน์ให้พิสูจน์แล้ว
      // 🔴 **`base_timezone` ออกจากฝั่ง `update` เมื่อ 4 ก.ย. 2026** (P4 ชี้ว่ามันเปิดอยู่ · P1 `revoke` ที่ `…120000:125`)
      //    มันถูก `grant update` มาตั้งแต่ `20260825122247:78` **โดยที่ไม่มีใครในทีมรู้** —
      //    คอมเมนต์ของ `…120000` ยกมันเป็นตัวอย่างของ *"คอลัมน์ที่ไคลเอนต์ห้ามแตะ"* ทั้งที่แตะได้อยู่ 10 วัน
      //    🎯 ***ที่ปิดไม่ใช่เพราะมันอันตรายวันนี้ — วันนี้ไม่มีใคร *อ่าน* มันไปคำนวณเลย (ตัวที่ใช้จริงคือ
      //       `trip_days.timezone`) ⇒ มันปลอดภัยด้วยข้อเท็จจริงที่ไม่เกี่ยวกับตัวมันเอง
      //       และวันที่ `D37` ถูกต่อจนมีตัวอ่าน **จะไม่มีอะไรส่งเสียง*** (`§3.4`)
      //    ⚠️ **`insert` ยังมีอยู่โดยตั้งใจ** — คนละคำถามกับ `update` · และ `create_trip()` เป็น
      //       `security definer` ⇒ เส้นทางสร้างทริปจริงไม่ได้เดินผ่าน grant ของ `authenticated` อยู่แล้ว
      //    🔴 **ทะเบียนนี้เทียบกับ ACL *จริงจากฐาน*** ⇒ บรรทัดนี้ต้องขยับ **ก่อน** `db:push`
      //       ไม่ใช่หลัง · *สิ่งที่ย้อนไม่ได้ (ฐาน) ไปทีหลัง · สิ่งที่ย้อนได้ (ทะเบียน) ไปก่อน* (`§3.3`)
      "trips.insert": ["base_timezone","created_by","end_date","id","start_date","status","title"],
      "trips.update": ["end_date","start_date","status","title"],
    };
  ...
  describe("🔴 E2-AC1 — คนนอกอ่านอะไรของทริป A ไม่ได้เลย **ทุกตารางที่ผูกกับทริป**", () => {
    ...
    /**
     * 🔴 **`trip_invites` ถูกยกเว้นจากบล็อกนี้ — และเหตุผลคือมันปิดแน่นกว่าที่บล็อกนี้ทดสอบได้**
     * (P1 · 5 ก.ย. 2026 · เจอตอนรันชุดสดครั้งแรกหลัง `20260904220000` ลงฐาน)
     *
     * วิธีของบล็อกนี้คือ **เจ้าของ insert → เจ้าของอ่านได้ → คนนอกต้องอ่านไม่ได้**
     * ⇒ ต้องมี grant ระดับตารางให้ `authenticated` อย่างน้อย `insert`+`select`
     * · `trip_invites` **ไม่ให้ `authenticated` เลยสักเมธอด** (ทุกทางผ่าน RPC definer)
     *   ⇒ ทั้ง seed และการอ่านยืนยันได้ `42501` ⇒ บล็อกนี้ **สรุปอะไรไม่ได้เลย ไม่ใช่ "ผ่าน"**
     *
     * 🎯 ***ไม่ใช่ช่องที่ไม่ได้ตรวจ — มันคือตารางที่ปิดแน่นเกินกว่าเครื่องมือนี้จะเอื้อมถึง***
     *    เกณฑ์ของบล็อกนี้คือ *"คนนอกอ่านไม่ได้"* · ของจริงคือ ***ไม่มีไคลเอนต์คนไหนอ่านได้เลย***
     * · ✅ **แต่ห้ามปล่อยให้ไม่มีอะไรตรวจ** — มีเคสเฉพาะของมันที่ `E2-AC11` ข้างล่าง
     *   (`trip_invites — ศูนย์บานสำหรับไคลเอนต์`) ซึ่งถามคำถามที่ *แรงกว่า* บล็อกนี้
     * · 🔴 **วันที่มีคน `grant` ระดับตารางให้ `authenticated` เคสนั้นจะแดงทันที** ⇒ การยกเว้นตรงนี้
     *   ไม่ได้ทำให้ตารางหลุดจากการตรวจ · มันย้ายไปอยู่กับเครื่องมือที่ถามถูกคำถาม
     */
    const CLIENT_UNREACHABLE = new Set(["trip_invites"]);
    const SCOPED = tripScopedTables().filter((t) => !CLIENT_UNREACHABLE.has(t));
    let tripS = "";
  ...
  describe("🔴 E2-AC11 — ถามฐานว่าประตูบานไหนเปิดอยู่ (`table_exposure`)", () => {
     ...
     */
    /**
     * 🔴 **`trip_invites` — ตารางที่ถือ *แฮชของความลับ* ⇒ ต้องไม่มีทางเข้าจากไคลเอนต์เลยสักบาน**
     * (P1 · 4 ก.ย. 2026 · `20260904220000`)
     *
     * ต่างจากแคชข้างบนตรงที่ **ไม่มีบานไหนได้รับอนุญาตเลย** — ไม่ใช่ "มีได้แค่ 4 บาน" แต่เป็น "ศูนย์บาน"
     * · ตารางนี้ไม่มี policy สักใบ (`D18`: ไม่มี policy = เข้าไม่ถึง ไม่ใช่แค่ซ่อน)
     * · ทุกเส้นทางผ่าน RPC `security definer` ซึ่งตรวจ `owner`/`auth.uid()` เอง
     * 🎯 ***ถ้ามีคน `grant` กลับวันหนึ่ง จะไม่มี RLS มากรองให้เลย — เคสนี้คือชั้นเดียวที่เฝ้าอยู่***
     *
     * 🔴 **และเคสนี้มีทางเขียวแบบผิด ๆ ที่ต้องปิด**: ถ้าตารางหายไป (ถูก drop · ยังไม่ push migration)
     *    `exposure()` จะคืน **ศูนย์แถวของ client role เหมือนกันเป๊ะ** ⇒ อ่านเป็น "ปิดสนิท"
     *    ⇒ จึงต้อง assert ว่า **ตัววัดเห็นตารางนี้จริง** ก่อน แล้วผลว่าง ๆ ถึงจะมีความหมาย
     *    (รูปเดียวกับ `§3.4` — *`grep` ได้ `0` แยกไม่ออกระหว่าง "กันไว้แล้ว" กับ "ยังไม่มีใครลอง"*)
     */
    it("🔴 trip_invites — ศูนย์บานสำหรับไคลเอนต์ · และตัววัดต้องเห็นตารางจริง", async () => {
      const rows = await exposure(["trip_invites"]);
      /**
       * ① **ทิศบวกของ *ตัววัด*** — ไม่มีข้อนี้ ตารางที่หายไปจะอ่านเป็น "ปิดสนิท"
       *
       * 🔴 **ฉบับแรกของผมเขียน `expect(rows.length).toBeGreaterThan(0)` ซึ่ง *ผิด* และแดงทันที
       *    ที่รันกับฐานจริง** — ผมเดาว่า `table_exposure` แยก *"ไม่มีตาราง"* ออกจาก
       *    *"มีตารางแต่ไม่มีบาน"* ไม่ได้ จึงสร้างตัวแยกขึ้นเอง **แล้วสร้างผิดทาง**
       *    ยิงจริงแล้วพบว่ามันแยกให้อยู่แล้ว:
       *    ```
       *    ตารางไม่มีจริง       → [{door:"MISSING", detail:"ไม่มีตารางชื่อนี้ใน public …"}]
       *    trip_invites        → []          ← **มีตาราง · ศูนย์บาน = คำตอบที่เราต้องการพอดี**
       *    trips               → หลายแถว
       *    ```
       * 🎯 ***ผมสร้างด่านกันเขียวหลอก แล้วด่านนั้นให้แดงหลอก เพราะผมใช้ตัววัดตัวเดียว
       *    ตอบสองคำถามโดยไม่ได้ถามมันก่อนว่ามันตอบแยกได้ไหม***
       * ⇒ ตัวแยกที่ถูกคือ **แถว `MISSING`** ซึ่งเป็นของที่เครื่องมือมีอยู่แล้ว
       */
      expect(
        rows.filter((r) => r.door === "MISSING").map((r) => r.table_name),
        "`table_exposure` รายงานว่าไม่รู้จัก `trip_invites` — ตารางถูก drop หรือ migration ยังไม่ลงฐาน " +
          "⇒ ผลว่างข้างล่างจะไม่มีความหมาย",
      ).toEqual([]);
      // ② ของจริง — ศูนย์บานสำหรับ anon/authenticated/PUBLIC
      expect(
        rows
          .filter((r) => CLIENT_ROLES.includes(r.grantee))
          .map((r) => `${r.table_name} · ${r.door} · ${r.grantee} · ${r.detail}`),
        "มีทางเข้า `trip_invites` จากฝั่งไคลเอนต์ — ตารางนี้ถือแฮชของโทเคนที่ให้สิทธิ์เขียนทริป\n" +
          "  🔴 ทางที่ถูกคือ RPC เท่านั้น · ถ้าตั้งใจเปิด ต้องยกเป็นข้อยกเว้นใหม่พร้อมเหตุผล",
      ).toEqual([]);
    });
  ...
  /**
   * 🔴 **`E5-fn` — พื้นผิว *ฟังก์ชัน* ที่เรียกได้จริง เทียบกับทะเบียน (P4 · 5 ก.ย. 2026)**
   *
   * ใบที่ ① คือ `function_exposure` เอง (P1 เขียน · `20260905000000` + `…010000` · **P4 ขอ**) —
   * มัน **แค่ทำให้วัดได้** · ใบนี้คือด่านที่ใช้ผลของมัน
   *
   * ## 🔴 ทิศ — **ทั้งพื้นผิว → ทะเบียน** ไม่ใช่ทะเบียน → พื้นผิว
   * 🎯 ***ทะเบียนครอบ "ของที่เราเพิ่ม" — ของที่เข้ามาโดยไม่มีใครเพิ่ม ไม่มีทะเบียนไหนเห็น***
   * · ทิศ *ทะเบียน→พื้นผิว* จับได้อย่างเดียวคือ **ของที่เราลืมถอด**
   * · ทิศนี้จับ **ของที่โผล่มาเอง** ด้วย: `proacl = null` (ไม่มีใคร `revoke`) · extension · dashboard
   * · 📌 `schemaPins` ถามคำถามคล้ายกันจาก **ไฟล์ในทรี** · ใบนี้ถามจาก **ฐาน**
   *   ⇒ **สองแหล่ง ไม่ใช่สำเนา** — และรอบแรกที่รัน สองแหล่งนี้ตอบไม่ตรงกันจริง (เรื่องเต็มอยู่กลางบล็อก)
   *
   * ## นิยาม "เรียกได้จริง" — ครบ **สามข้อ** ขาดข้อเดียวเรียกไม่ได้
   * ```
   * ① EXECUTE   ตรงตัว **หรือผ่าน `PUBLIC`**   ← `proacl = null` แปลว่า PUBLIC ได้ไปแล้วโดยไม่มีบรรทัดไหนพูดถึง
   * ② USAGE     บน schema นั้น                 ← ไม่มี ⇒ EXECUTE ไม่มีความหมาย (`§3.5` เขียนไว้เอง)
   * ③ kind      **ไม่ใช่** `trigger`/`event_trigger`  ← สองชนิดนี้ Postgres ปฏิเสธการเรียกตรงเอง
   * ```
   *
   * ### 🔴 ข้อ ③ เขียนเป็น *บัญชีดำ* ไม่ใช่ *บัญชีขาว* — และทิศนี้สำคัญกว่าตัวรายชื่อ
   * ฉบับแรกของผมเขียนว่า `kind === "normal"` · **P1 ทักว่ามันตัด `procedure` ทิ้ง ซึ่งเรียกได้จริงด้วย `CALL`**
   * · เขาถูก — และเมื่อไล่ต่อจากที่เขาชี้ ปัญหาไม่ได้อยู่ที่ *ลืม `procedure`* แต่อยู่ที่ **ทิศของเงื่อนไข**:
   * ```
   * บัญชีขาว (`kind === "normal"`)   ชนิดใหม่ / คำที่เปลี่ยน  → **หลุดออกจากพื้นผิว เงียบ ๆ**  → เขียวหลอก
   * บัญชีดำ (`kind` ไม่ใช่สองตัวนี้)  ชนิดใหม่ / คำที่เปลี่ยน  → **ตกเข้าพื้นผิว**              → แดง มีคนมาอ่าน
   * ```
   * 🎯 ***บัญชีขาวทำให้ "สิ่งที่เราไม่ได้คิดถึง" หายไปจากผล · บัญชีดำทำให้มันโผล่มาถาม***
   * ⇒ ครอบ `procedure` ที่ P1 ชี้ **และครอบชนิดที่ยังไม่มีใครนึกถึงด้วย** โดยไม่ต้องแก้บรรทัดนี้อีก
   * · ⚠️ ราคาที่จ่าย: `aggregate`/`window` จะถูกนับเป็น "เรียกได้" ทั้งที่ต้องมีบริบท query
   *   ⇒ **แดงเกินจริงได้หนึ่งครั้ง** วันที่มีคนสร้างตัวแรก · ผมเลือกทิศนี้เพราะ *แดงเกินมีคนมาอ่าน · เขียวขาดไม่มี*
   * · 📌 P1 วัดวันนี้ได้ `normal 59 · trigger 8 · event_trigger 1 · procedure/aggregate/window 0`
   *   (เลขของเขา ไม่ใช่ของผม) — **ตัวเลขนั้นแปลว่า "วันนี้ไม่มีของให้พลาด" ไม่ใช่ "เขียนแบบไหนก็ได้"**
   *
   * ## 🔴 ทางเขียวหลอกที่เหลืออยู่ทางเดียว — และมันเงียบสนิท ⇒ ต้องมีเคสควบคุม
   * ```
   * แถว USAGE หาย (query พัง) → "ไม่มีใครมี USAGE" → "ไม่มีใครเรียกอะไรได้" → **เขียวสนิททั้งแผง**
   * ```
   * 🎯 ***ตัววัดที่พังเงียบ ให้คำตอบที่ปลอดภัยที่สุดเสมอ — และนั่นคือสิ่งที่ทำให้ไม่มีใครสงสัย*** (ถ้อยคำ P1)
   * · 🔴 **ไม่ใช่เรื่องสมมติ — ยิงมัลแตนต์กับแถวจริงจากฐานแล้ว (5 ก.ย. 2026):**
   *   ```
   *   ตัดเงื่อนไข USAGE ทิ้ง   → `PUBLIC` = [app.like_literal, app.search_norm]   (ฐาน: [])
   *   ตัดบัญชีดำ kind ทิ้ง     → `PUBLIC` = [public.rls_auto_enable]              (ฐาน: [])
   *   ```
   *   🎯 ***เคส `PUBLIC` ข้างล่างไม่ได้เขียวเพราะ "ไม่มีใคร grant ให้ PUBLIC" — มีอยู่จริงทั้งสองที่***
   *   มันเขียวเพราะ ***ทั้งสองตัวถูกกันไว้คนละข้อ*** (ตัวแรกไม่มี `USAGE` · ตัวหลังเป็น `event_trigger`)
   *   ⇒ จักรวาลที่ทดสอบ **ไม่ว่าง** · ถอดข้อไหนออก เคสนั้นแดงทันทีพร้อมชื่อ
   * · 📌 `public.rls_auto_enable` = ฟังก์ชันที่ `PUBLIC | EXECUTE` และ **ไม่มีใน migration ของเราสักไฟล์**
   *   ผมไม่ยิงมันเพราะชื่อแปลว่า "เปิด RLS อัตโนมัติ" (เปิด RLS บนตารางที่ไม่มี policy = ไม่มีใครอ่านได้อีก)
   *   ⇒ **`kind` ตอบแทนการเรียก** · 🎯 *สำหรับฟังก์ชันที่เขียนฐาน "ทดสอบว่าเรียกได้ไหม" กับ "เรียกมัน" เป็นสิ่งเดียวกัน*
   */
  describe("🔴 E5-fn — ฟังก์ชันที่เรียกได้จริง (`function_exposure`) เทียบทะเบียน", () => {
    type FnRow = {
      schema_name: string;
    ...
    /** schema ที่โค้ดของเราเป็นเจ้าของ — `extensions`/`pg_catalog` ไม่ใช่ของเรา ไม่ถามที่นี่ */
    const OUR_SCHEMAS = ["public", "app"];

    /** 🔴 บัญชี**ดำ** ไม่ใช่บัญชีขาว — เหตุผลเต็มอยู่ในคอมเมนต์หัวบล็อก (P1 ทัก · ผมกลับทิศ) */
    const UNCALLABLE_KINDS = ["trigger", "event_trigger"];

    const fnExposure = async (schemas: string[]): Promise<FnRow[]> => {
      const { data, error } = await admin.rpc("function_exposure", { p_schemas: schemas });
      if (error) throw new Error(`เรียก function_exposure ไม่ได้: ${error.message}`);
      return data as FnRow[];
    };

    /** schema ที่ role นี้มี `USAGE` — ผ่านชื่อตัวเอง **หรือผ่าน `PUBLIC`** */
    const usableSchemas = (rows: FnRow[], role: string) =>
      new Set(
        rows
          .filter(
            (r) =>
              ...
              r.privilege === "USAGE" &&
              (r.grantee === role || r.grantee === "PUBLIC"),
          )
          .map((r) => r.schema_name),
      );

    /** ① ∧ ② ∧ ③ — คืนชื่อเต็ม `schema.fn` ที่ role นี้ **เรียกตรงได้จริง** */
    const reachable = (rows: FnRow[], role: string): string[] => {
      const usable = usableSchemas(rows, role);
      return [
        ...new Set(
          rows
            .filter(
              (r) =>
                r.function_name !== null &&
                r.privilege === "EXECUTE" &&
                !UNCALLABLE_KINDS.includes(r.kind ?? "") &&
                (r.grantee === role || r.grantee === "PUBLIC") &&
                usable.has(r.schema_name),
            )
            .map((r) => `${r.schema_name}.${r.function_name}`),
        ),
      ].sort();
    };

    /**
     * สิทธิ์ `EXECUTE` ที่ **ไฟล์ในทรี** สั่งไว้ — เดิน `grant`/`revoke`/`drop` **ตามลำดับที่รันจริง**
     *
     * 🔴 **ทำไมไม่ใช่ลิสต์ชื่อพิมพ์มือสำหรับ `authenticated`**: มัน 30+ ใบและโตทุกสัปดาห์
     * ⇒ ลิสต์มือจะ **แดงใส่คนที่เพิ่ม RPC ถูกต้องพร้อม migration** ทุกครั้ง
     * และ `§3.4` บอกว่ากลไกแบบนั้น **จะถูกลบทั้งใบ พร้อมของที่มันเคยกันไว้**
     * 🎯 ***เทียบกับ "สิ่งที่ทรีสั่ง" แทน ⇒ ด่านนี้ถามคำถามเดียวเป๊ะ:
     *    "ฐานมีสิทธิ์ที่ไม่มีบรรทัดไหนในทรีสั่งหรือเปล่า"***
     *
     * 🔴 **ต้องเดินตามลำดับ ไม่ใช่ `grep` รวมทั้งรีโป — P1 ทัก และเขาถูก**
     * *"ไฟล์บอกว่า **เคยเขียนว่าอะไร** ไม่ได้บอกว่า **ตอนนี้เป็นอะไร**"* (ถ้อยคำจาก `effectiveFunctions()`
     * ซึ่งเป็นรูปเดียวกันเป๊ะ · ตัวนี้ยืมรูปนั้นมา ไม่ได้คิดใหม่)
     * ```
     * grant …  to service_role      ← 20260905000000
     * drop function …               ← 20260905010000  **สิทธิ์หายไปทั้งชุดตรงนี้**
     * grant …  to service_role      ← 20260905010000  ตั้งใหม่
     * ```
     * ⇒ `grep` รวมเห็น 2 บรรทัด `grant` แล้วสรุปถูกโดยบังเอิญ · **ถ้าไฟล์หลังไม่ตั้งใหม่ มันจะสรุปผิดเงียบ ๆ**
     *
     * ## ⚠️ ข้อจำกัดที่ฆ่าตัวนี้ได้ — เขียนไว้เพราะผมเป็นคนเดียวที่รู้ว่าช่องอยู่ตรงไหน
     * · **สตริงไม่ใช่สัญลักษณ์** — ตัวนี้ยังเป็น regex บน SQL · `stripComments` ตัดคอมเมนต์ให้
     *   แต่ `grant execute on function …;` ที่อยู่ใน string literal ของ `raise exception` **จะถูกนับ**
     *   ⇒ ทิศที่พลาด = **นับเกิน** ⇒ ด่านเขียวให้ของที่ไม่ควรเขียว (ทิศเงียบ)
     * · 🔴 `grant execute on all functions in schema …` **จับไม่ได้โดยตั้งใจ** — วันที่มีคนเขียนแบบนั้น
     *   เซตจากทรีไม่โต แต่พื้นผิวจริงโต ⇒ **แดงพร้อมชื่อทุกใบ** ซึ่งเป็นทิศที่ถูก
     * · ⚠️ ทิศกลับ (ทรีสั่ง แต่ฐานไม่มี) **ไม่ assert ที่นี่** — เคส "ฐานกับทรีตรงกัน" ถามข้อนั้นอยู่แล้ว
     *   และมันจะแดงระหว่างช่วง `commit` → `db push` ตามปกติ ⇒ แดงใส่คนที่ทำถูก
     */
    const treeExecuteGrants = (): Map<string, Set<string>> => {
      const out = new Map<string, Set<string>>();
      const EVENT = new RegExp(
        String.raw`(grant|revoke)\s+([a-z, ]+?)\s+on\s+function\s+((?:app|public)\.\w+)\s*\([^)]*\)\s*(?:to|from)\s+([^;]+);` +
          String.raw`|drop\s+function\s+(?:if\s+exists\s+)?((?:app|public)\.\w+)\s*\(`,
        "gi",
      );
      for (const f of migrationFiles) {
        ...
        for (const m of sql.matchAll(EVENT)) {
          if (m[5]) {
            // `drop` = สิทธิ์ทั้งชุดหายไปกับตัวฟังก์ชัน — ไฟล์ที่สร้างใหม่ต้อง `grant` เองอีกรอบ
          ...
          for (const raw of m[4].split(",")) {
            const role = raw.trim().toLowerCase().replace(/\s+/g, " ");
            if (!role) continue;
    ...
    /**
     * 🔴 **ด่านนี้เจอของจริงในการวัดครั้งแรก — และเรื่องนี้คือเหตุผลที่มันมีอยู่ ไม่ใช่เชิงอรรถ**
     *
     * รอบแรกที่ยิงกับฐาน (5 ก.ย. 2026) พบสองใบที่ `authenticated` เรียกได้ **โดยไม่มีบรรทัดไหนในทรีสั่ง**:
     * ```
     * app.like_literal(t text)  kind=normal  PUBLIC  EXECUTE     20260826023153
     * app.search_norm(t text)   kind=normal  PUBLIC  EXECUTE     20260826015533
     * ```
     * 🎯 ***ไม่มีใคร `grant` ให้ — และไม่มีใคร `revoke` ⇒ Postgres แจก `EXECUTE` ให้ `PUBLIC` เป็นค่าเริ่มต้น***
     *    เพื่อนอีก 7 ใบใน `app` มี `revoke … from public` + `grant … to authenticated` ครบ
     *    (`app.can_read_trip` โผล่เป็น `authenticated` ล้วน ไม่มีแถว `PUBLIC`) ⇒ **สองใบนี้ข้ามทั้งสองขั้น**
     * 🔴 **บรรทัดที่ *หายไป* ไม่ปรากฏใน `git diff`** — รีวิวด้วยตามองไม่เห็นตามโครงสร้าง ไม่ใช่เพราะใครเผลอ
     *
     * · P4 รายงาน → **P1 ปิดด้วย `20260905020000` (`eccb682`)** ⇒ ทรีสั่งชัดเจนแล้ว ⇒ **ส่วนต่างกลับเป็นศูนย์**
     * · 🔴 **จึงไม่มีบัญชีขาวในบล็อกนี้เลยสักรายการ — และนั่นคือเจตนา**
     *   ทะเบียนที่ว่างพร้อมเคสฝั่งบวกที่วนของว่าง = *เคสที่ผ่านโดยไม่ได้ตรวจอะไร* (`§3.4`)
     *   ⇒ ใครต้องเพิ่มข้อยกเว้นวันหลัง **ต้องเพิ่มเคสฝั่งบวกของมันในคอมมิตเดียวกัน** ไม่ใช่ใบถัดไป
     *
     * ## ⚠️ จุดบอดที่รู้ตัว — ด่านนี้เทียบ *ชื่อ* ไม่ได้เทียบ *เส้นทางที่ทำให้เรียกได้*
     * ขณะเขียนนี้ ไฟล์ `20260905020000` **ยังไม่ลงฐาน** ⇒ ฐานยังให้สิทธิ์ผ่าน `PUBLIC` อยู่
     * แต่ทรีสั่ง `to authenticated` แล้ว ⇒ **ส่วนต่างเป็นศูนย์ทั้งก่อนและหลัง `db push`**
     * 🔴 ⇒ *"เรียกได้เพราะเราสั่ง"* กับ *"เรียกได้เพราะลืม revoke"* **ด่านนี้แยกไม่ออกเมื่อชื่อตรงกัน**
     * · ✅ **สิ่งที่ทำให้จุดบอดนี้กว้างศูนย์วันนี้ ไม่ได้อยู่ในตัวด่าน** — มันคือ *ใครมี `USAGE` บน `app`*
     *   ```
     *   USAGE บน app = [authenticated] เท่านั้น
     *   ⇒ ใน `app` เซตของคนที่เรียกได้ผ่าน `PUBLIC` **เท่ากับ** เซตที่เราตั้งใจให้เรียกอยู่แล้ว
     *   ```
     *   🔴 วันที่มีคนให้ `USAGE on schema app` แก่ `anon` ประโยคข้างบนกลายเป็นเท็จทันที
     *   ⇒ **จึงมีเคสตรึงข้อเท็จจริงนั้นไว้ข้างล่าง** — ไม่ใช่เขียนไว้เฉย ๆ ให้หมดอายุเงียบ
     *   (`§3.4`: *ปลอดภัยด้วยข้อเท็จจริงที่ไม่เกี่ยวกับตัวมันเอง → วันที่มันเปลี่ยน ต้องมีอะไรส่งเสียง*)
     *
     * ## 🔴 จุดบอดใบที่สอง — **"เรียกได้" ของด่านนี้ = เรียก *ตรง* เท่านั้น · การเรียกผ่าน *index* ไม่นับ**
     * วัดได้ในวันเดียวกับที่เขียนบล็อกนี้ และมันทำระบบพังจริง (5 ก.ย. 2026 · P1 เป็นเคส):
     * ```
     * eccb682  revoke app.search_norm from public · grant to authenticated
     * ⇒ catalog_place_names มี **GIN index บน `app.search_norm(name)`** (`20260826015533:72`)
     * ⇒ `service_role` ที่ insert ตารางนั้น (ทะเบียนข้อ 3) ต้องมี EXECUTE **ตอนคำนวณค่า index**
     * ⇒ 3 suite ของ `E2` ตายที่ `beforeAll`: `permission denied for function search_norm`
     * ```
     * 🎯 ***ด่านนี้ถาม "ใครเรียกได้" · สิ่งที่พังคือ "ใครเขียนตารางที่ index เรียกฟังก์ชันนั้นได้" — คนละคำถาม***
     * · 🔴 **และมันเงียบสองชั้น**: `service_role` **ไม่มี `USAGE` บน `app`** ⇒ `reachable()` ไม่นับมันตั้งแต่แรก
     *   ⇒ ทั้งก่อนและหลังเหตุการณ์ ด่านนี้รายงานเหมือนกันเป๊ะ **โดยที่ระบบพังอยู่ตรงกลาง**
     * · 📌 หลักฐานว่าเส้นทาง index **ไม่** ตรวจ `USAGE` ระดับ schema: error ที่ได้พูดถึง *ฟังก์ชัน*
     *   ไม่ใช่ *schema* ทั้งที่ role นั้นไม่มี `USAGE` เลย (ข้อสังเกตจากผลที่ P1 วัด ไม่ใช่ที่ผมวัด)
     * · ✅ **ตัวที่จับได้จริงคือเคส `E2` ที่ไป *เขียนตารางจริง*** — ของที่ *ใช้* ฟังก์ชัน ไม่ใช่ของที่ *ตรวจ* ฟังก์ชัน
     *   🔴 **ห้ามแก้ด้วยการเติมเคส "ใครเรียก index ได้" ที่นี่** — นั่นคือย้ายคำถามมาไว้ผิดที่
     *   ของที่เฝ้าเรื่องนี้ถูกต้องแล้วคือ fixture ของ `E2` ซึ่งพังเสียงดังภายในรอบเดียว
     */

    /**
     * 🔴 **`anon` ใช้ *ลิสต์ชื่อเต็ม* ไม่ใช่ส่วนต่างกับทรี — คนละเกณฑ์กับ `authenticated` โดยตั้งใจ**
     * `anon key` เป็น `NEXT_PUBLIC_` ⇒ อยู่ในบันเดิล ⇒ **ทุกคนบนอินเทอร์เน็ตเรียกได้
     * ไม่ผ่าน route เรา ไม่ผ่าน `rateLimitGuard` เรา** · การเพิ่มหนึ่งใบเป็นเหตุการณ์ที่ควรดัง
     * · 📌 ลิสต์เดียวกันนี้มีอยู่ที่ `schemaPins` ด้วย **แต่อ่านจากไฟล์** — ใบนี้อ่านจากฐาน
     *   ⇒ ใบไหนใบหนึ่งผิด อีกใบยังพูด · **ถ้าทั้งคู่เขียวแปลว่าไฟล์กับฐานตรงกันจริง**
     */
    const ANON_REGISTERED = [
      /**
       * 🔴 **เพิ่ม 5 ก.ย. 2026 — `get_trip_template(uuid)` · `20260905150000`** (P1 ตัดสิน · P7 เจอ drift · P4 วัด)
       *
       * ใบนี้เข้ามาทาง **ฐาน** ก่อนทะเบียนใบนี้ ⇒ `E5-fn` แดงจนกว่าจะเติม
       * 🎯 ***และมันคือใบเดียวใน 4 ทะเบียนที่จับได้*** — อีก 3 ใบใน `schemaPins` อ่านจาก *ทรี*
       *    ⇒ เขียวตั้งแต่ P1 เขียนไฟล์ · **ใบนี้เทียบกับ *ฐาน* จึงเงียบจนวินาทีที่ `db:push` ลง**
       *    (`§3.5` — ทะเบียนที่ไม่มีอะไรเทียบกับความจริง = แหล่งความจริงใบที่สอง)
       *
       * **ทำไมถึงเปิดให้ `anon` — หลักฐานที่ไม่ใช่ migration ของคนเขียน migration เอง:**
       * ผู้ใช้สั่งสองใบคนละวัน — *"ดูทริปแนะนำได้ แต่สร้างทริปไม่ได้"* (4 ก.ย.) แล้ว
       * *"เมื่อกดจะบอกรายละเอียดของทริปทั้งหมด … และมีปุ่มให้กดสร้างทริป"* (5 ก.ย.)
       * ⇒ การ์ดเปิดให้คนยังไม่ล็อกอิน **แต่กดแล้วเจอกำแพง = flow ตายในคลิกเดียว**
       *
       * ⚠️ **ราคาที่แลกมา จดไว้เพราะมันเปลี่ยนจริง:** ธง `published_template_at` ติดผิดใบ
       * เดิม = *มีคนก๊อปแผนไปได้* · ตอนนี้ = ***อ่านทั้งแผนได้ทันที และคนยังไม่ล็อกอินก็อ่านได้***
       * ✅ ตัวที่ยังกันอยู่: ติดธงได้เฉพาะ `service_role` (`§3.5` ข้อ 8 · ไม่มี column grant ให้
       *    `authenticated` · assert บังคับสองทิศ) ⇒ **ช่องนี้เปิดได้ก็ต่อเมื่อ *เรา* พลาดเอง ไม่ใช่ผู้ใช้ยิง**
       */
      "public.get_trip_template",
    ...
    it("🔴 เคสควบคุม — ตัววัดต้องเห็นแถว `USAGE` และแยกชนิดได้จริง ไม่งั้นทั้งแผงเขียวฟรี", async () => {
      const rows = await fnExposure(OUR_SCHEMAS);

      expect(
        rows.filter((r) => r.privilege.startsWith("ไม่มี schema")).map((r) => r.schema_name),
        "schema ที่เราถามไม่มีอยู่บนฐาน — ผลว่างข้างล่างจะไม่มีความหมาย",
      ).toEqual([]);

      // ① ไม่มี USAGE ⇒ `reachable()` คืนว่างเสมอ ⇒ เขียวสนิททั้งแผงด้วยเหตุผลผิด
      expect(
        [...usableSchemas(rows, "authenticated")].sort(),
        "แถว `USAGE` หายไป — `reachable()` จะคืนค่าว่างให้ทุก role **ซึ่งอ่านเหมือนปิดสนิท**",
      ).toEqual(["app", "public"]);

      // ② คำในบัญชีดำต้องยัง match ของจริง — วันที่ `kind` เปลี่ยนคำ trigger จะตกเข้าพื้นผิว (แดง ไม่ใช่เขียว)
      //   ⇒ เคสนี้ไม่ได้กันเขียวหลอก มันแค่ทำให้ *สาเหตุ* ของแดงวันนั้นอ่านออกทันที
      expect(
        [...new Set(rows.map((r) => r.kind).filter(Boolean))].sort(),
        "ชุดคำของ `kind` เปลี่ยนไปจากที่บัญชีดำรู้จัก — ถ้ามีคำใหม่ ให้ตัดสินว่ามัน *เรียกตรงได้ไหม* ก่อนเติม\n" +
          "  🔴 ห้ามเติมเข้าบัญชีดำเพื่อให้เคยข้างล่างเขียว — นั่นคือตัดคำถามออกเพื่อให้ผ่าน",
      ).toEqual(["event_trigger", "normal", "trigger"]);
    });

    it("🔴 ด้านบวกของสัญญาณเตือน — schema ที่ไม่มีจริงต้องดัง ไม่ใช่คืนว่าง", async () => {
      const rows = await fnExposure([`schema_ที่ไม่มีจริง_${stamp}`]);
      expect(rows).toHaveLength(1);
      expect(
        rows[0].privilege.startsWith("ไม่มี schema"),
        "ชื่อ schema ที่ resolve ไม่ได้กลับเงียบ ⇒ พิมพ์ผิดเมื่อไหร่จะได้ผลว่าง **ซึ่งอ่านเป็น 'ไม่มีอะไรเปิด'**",
      ).toBe(true);
    });

    it("🔴 ตัวแยก `grant`/`revoke` จากไฟล์ ต้องเดินสองอาร์มจริง ไม่ใช่แค่บวกอย่างเดียว", () => {
      /**
       * 🔴 **เคสควบคุมของ *เครื่องมือ* ไม่ใช่ของฐาน** — ถ้าตัวแยกพัง เคสสองใบข้างล่างจะแดงทั้งแผง
       * แล้วคนอ่านจะไปไล่หาสิทธิ์ที่ฐาน **ทั้งที่ปัญหาอยู่ในไฟล์นี้**
       * · `function_exposure` เป็นตัวอย่างที่เดินครบทั้งสามอาร์มในสองไฟล์: `revoke` → `grant` → `drop` → `grant`
       *   ⇒ ผลที่ถูกคือ **มี `service_role` · ไม่มี `anon`** · อาร์มไหนพังก็ได้คำตอบผิดคนละแบบ
       */
      const tree = treeExecuteGrants();
      expect(tree.size, "แยก `grant execute on function` จากไฟล์ไม่ได้เลย — regex พัง").toBeGreaterThan(20);
      expect(
        tree.get("public.create_trip")?.has("authenticated"),
        "อาร์ม `grant` ไม่ทำงาน — `create_trip` ถูก grant ให้ `authenticated` ในไฟล์แน่นอน",
      ).toBe(true);
      expect(
        tree.get("public.function_exposure")?.has("service_role"),
        "อาร์ม `grant` หลัง `drop` ไม่ทำงาน — `20260905010000` ตั้งสิทธิ์ใหม่หลัง drop",
      ).toBe(true);
      expect(
        tree.get("public.function_exposure")?.has("anon"),
        "อาร์ม `revoke` ไม่ทำงาน — `revoke all … from public, anon, authenticated` ต้องหักออกจริง",
      ).toBe(false);
    });

    it("🔴 `PUBLIC` ต้องเรียกอะไรไม่ได้เลยสักใบ", async () => {
      /**
       * `PUBLIC` มี `USAGE` บน `public` อยู่แล้ว (Supabase ตั้งให้) ⇒ ฟังก์ชันใหม่ใน `public`
       * ที่ **ลืม `revoke all … from public`** จะโผล่ที่นี่ทันทีในวันที่มันถูกสร้าง
       * 🎯 ***นี่คือเคสที่จับ "บรรทัดที่ไม่มีใครเขียน" ได้ — ซึ่ง `grep` มองไม่เห็นตามนิยาม***
       */
      const rows = await fnExposure(OUR_SCHEMAS);
      expect(
        reachable(rows, "PUBLIC"),
        "มีฟังก์ชันที่ **ใครก็ได้** เรียกได้\n" +
          "  🔴 ค่าเริ่มต้นของฟังก์ชันใน Postgres คือ `EXECUTE` ให้ `PUBLIC` — **ต่างจากตารางที่ปิดโดยปริยาย**\n" +
          "  ⇒ สาเหตุที่เป็นไปได้มากที่สุดคือ **ลืม `revoke all on function … from public`** ไม่ใช่มีคนตั้งใจเปิด\n" +
          "  · ถ้าตั้งใจ ต้องยกเป็นข้อยกเว้นใหม่ใน `TEAM.md §3.5` พร้อมเหตุผล",
      ).toEqual([]);
    });

    it("🔴 `anon` เรียกได้เท่าที่ขึ้นทะเบียน — วัดจากฐาน ไม่ใช่จากไฟล์", async () => {
      const rows = await fnExposure(OUR_SCHEMAS);
      expect(
        reachable(rows, "anon"),
        "พื้นผิวที่คนยังไม่ล็อกอินเรียกได้ **เปลี่ยนไปจากทะเบียน**\n" +
          "  🔴 `anon key` อยู่ในบันเดิล ⇒ ทุกคนบนอินเทอร์เน็ตเรียกได้ ไม่ผ่าน route · ไม่ผ่าน rate limit\n" +
          "  ⇒ ถามสองข้อก่อนเติมชื่อ (**`ไม่` ไม่ได้แปลว่าห้าม — แปลว่าต้องบอกว่าอะไรกันแทน**):\n" +
          "     ① ตอบเหมือนกันหมดทุกคนไหม  ไม่ → อะไรกันคนที่เดาค่าถูก\n" +
          "     ② ยอมให้ถูกดูดทั้งใบไหม     ไม่ → อะไรกันการแจงรายการ\n" +
          "  · **มีขาด** = มีคน `revoke` ⇒ หน้าแรกของคนยังไม่ล็อกอินพังเงียบ ต้องลบชื่อออกจากทะเบียนด้วย",
      ).toEqual(ANON_REGISTERED);
    });

    it("🔴 `authenticated` — ฐานต้องไม่มีสิทธิ์ที่ไม่มีบรรทัดไหนในทรีสั่ง", async () => {
      const rows = await fnExposure(OUR_SCHEMAS);
      const tree = treeExecuteGrants();
      const live = reachable(rows, "authenticated");
      expect(live.length, "พื้นผิวของ `authenticated` ว่างเปล่า — ตัววัดพัง ไม่ใช่ระบบปิดสนิท").toBeGreaterThan(20);

      expect(
        live.filter((n) => !tree.get(n)?.has("authenticated")),
        "ฐานเปิดให้ `authenticated` เรียกฟังก์ชันที่ **ไม่มีบรรทัดไหนในทรีสั่ง**\n" +
          "  🔴 สาเหตุที่เป็นไปได้ เรียงตามที่เจอจริง:\n" +
          "     ① ฟังก์ชันใหม่ที่ **ลืม `revoke all … from public`** ⇒ `PUBLIC` ได้ `EXECUTE` เป็นค่าเริ่มต้น\n" +
          "        และ `authenticated` มี `USAGE` ทั้ง `public` และ `app` ⇒ เรียกได้ทันที\n" +
          "     ② `grant` ที่กดจาก dashboard หรือมากับ extension — **ไม่มีไฟล์ไหนอธิบายมันได้**\n" +
          "     ③ `grant execute on all functions in schema …` ซึ่งตัวแยกจากไฟล์ไม่นับให้ (ตั้งใจ)\n" +
          "  ⇒ ถ้าตั้งใจให้เปิด: เขียน `grant` ลง migration **แล้วมันจะเขียวเอง ไม่ต้องมาแก้ด่านนี้**",
      ).toEqual([]);
    });

    it("🔴 `service_role` — ทะเบียน `§3.5` ครึ่งที่เป็นฟังก์ชัน ต้องเทียบกับฐานได้", async () => {
      /**
       * 🔴 `service_role` มี **BYPASSRLS** ⇒ `grant` คือด่านสุดท้ายที่เหลือ ไม่มี policy มาช่วยอีกชั้น
       * · `table_exposure` ตอบครึ่งที่เป็น *ตาราง* · ครึ่งที่เป็น `grant execute` (ข้อ 6 · 7 · 10)
       *   **ไม่เคยมีอะไรเทียบกับความจริงเลย** จนกระทั่งใบนี้
       * 🎯 ***ด่านที่ครอบครึ่งเดียวของทะเบียน อ่านเหมือนทะเบียนถูกเฝ้าทั้งใบ — แย่กว่าไม่มีด่าน*** (P1)
       */
      const rows = await fnExposure(OUR_SCHEMAS);
      const tree = treeExecuteGrants();
      const live = reachable(rows, "service_role");
      expect(live.length, "พื้นผิวของ `service_role` ว่างเปล่า — ตัววัดพัง").toBeGreaterThan(10);

      expect(
        live.filter((n) => !tree.get(n)?.has("service_role")),
        "`service_role` เรียกฟังก์ชันที่ไม่มีบรรทัดไหนในทรีสั่งได้ — และมันข้าม RLS ทุกใบ\n" +
          "  🔴 ขยายสิทธิ์ `service_role` ต้องผ่าน P1 และจดใน `TEAM.md §3.5` ทุกครั้ง",
      ).toEqual([]);
    });

    it("🔴 `USAGE` บน `app` ต้องมีแค่ `authenticated` — ข้อเท็จจริงที่ทำให้จุดบอดข้างบนกว้างศูนย์", async () => {
      /**
       * เคสนี้ **ไม่ได้ตรวจสิทธิ์ของฟังก์ชันเลย** — มันตรึง *เงื่อนไขที่ทำให้เหตุผลของบล็อกนี้ยังจริง*
       * · `app.*` หลายใบมี `PUBLIC | EXECUTE` ติดมาจากค่าเริ่มต้นได้ตลอดเวลา
       * · สิ่งเดียวที่กัน `anon` ออกจากมันคือ **ไม่มี `USAGE on schema app`**
       * 🎯 ***วันที่มีคนเพิ่ม `USAGE` ให้ `anon` จะไม่มี `grant` ใบไหนเปลี่ยน และไม่มี diff ไหนน่าสงสัย —
       *    แต่พื้นผิวของคนที่ยังไม่ล็อกอิน จะโตขึ้นทั้งชุดในบรรทัดเดียว***
       */
      const rows = await fnExposure(OUR_SCHEMAS);
      expect(
        rows
          .filter((r) => r.function_name === null && r.privilege === "USAGE" && r.schema_name === "app")
          .map((r) => r.grantee)
          .sort(),
        "ใครมี `USAGE on schema app` เปลี่ยนไป\n" +
          "  🔴 เพิ่ม `anon` = ฟังก์ชัน `app.*` ทุกใบที่ยังมี `PUBLIC | EXECUTE` ค้างอยู่ **เปิดให้ทั้งอินเทอร์เน็ตทันที**\n" +
          "     โดยไม่มี `grant execute` ใบไหนถูกแตะเลย ⇒ เคส `anon` ข้างบนจะแดงตามมา แต่ *ต้นเหตุอยู่ที่นี่*\n" +
          "  · ลด (เหลือ 0) = definer ที่เรียก `app.can_read_trip` ฯลฯ ในฐานะผู้ใช้จะพังทั้งชุด",
