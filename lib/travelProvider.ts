...
export async function fetchRealTravelTimeOutcome(
  ...
  try {
     ...
     *
     * 🔴 **`900` เป็นค่าที่ *เลือก* ไม่ใช่ค่าที่ *วัด* — ยังไม่มีใครวัดว่าจราจรเกาหลีทำให้ผลเพี้ยนที่กี่นาที**
     *    ที่วัดจริงเมื่อ 4 ก.ย. 2026 คือ `walk`/`transit` เท่านั้น (ลากการ์ดซ้ำ → ไม่มีคำขอออกนอกเพิ่ม)
     *    🎯 **จดไว้ตรงนี้เพราะถ้าอยู่แต่ในข้อความ วันหนึ่งจะมีคนยก `15 นาที` ไปเป็นฐานของอย่างอื่น
     *       แล้วมันจะกลายเป็น *ตัวเลขที่วัดแล้ว* ในสายตาคนอ่าน** (P1 ชี้ · P3 จด)
     *    · จะแก้ให้ถูกต้องต้องวัด: ยิงคู่เดิมโหมด `drive` ห่างกัน N นาที แล้วดูว่าผลต่างเกินที่ยอมรับเมื่อไหร่
     */
