...
export const E5_COPY = {
  ...
  home: {
    ...
    brand: "luitrip",
    /**
     * ชื่อไทยที่วางต่อท้าย `brand` **บนบรรทัดเดียวกัน** (ผู้ใช้สั่งเอง 4 ก.ย. 2026:
     * *"ลุยทริป ควรอยู่ บรรทัดเดียวกับ luitrip"* · P1 ยืนยันการสะกดกับผู้ใช้แล้ว)
     *
     * 🎯 ***ชื่ออยู่กับชื่อ ไม่ใช่อยู่กับคำอธิบาย*** — ฉบับก่อนหน้าเอาไปรวมกับคำโปรย
     * (`"ลุยทริป · วางแผนทริปร่วมกัน"`) ซึ่งประหยัดที่จริง **แต่จัดชื่อไทยไปอยู่หมวดคำอธิบาย**
     *
     * ## 🔴 **ตัวอักษรเหมือน `brand` ทุกประการ — ผู้ใช้สั่งเอง หลังเห็นฉบับที่ต่างกันแล้ว**
     * *"ควรเป็นตัวอักษรที่เหมือนกับ luitrip ทุกประการ"* (รอบที่สองของคำสั่งเดียวกัน)
     * ⚠️ **ฉบับก่อนหน้าทำให้เล็กและจางกว่า** ด้วยเหตุผลว่า *"สองชื่อขนาดเท่ากันจะอ่านเหมือนสองแบรนด์"*
     *    — **ข้อนั้นตายแล้ว ห้ามอ้างอิงและห้ามเอากลับมาโดยไม่ถามเขาก่อน**
     * 🎯 ***เขาไม่ได้อยากได้ "ชื่อกับคำอ่าน" — เขาอยากได้ชื่อเดียวที่เขียนได้สองภาษา***
     * ⇒ เรนเดอร์ในสแปนเดียวกับ `brand` **ไม่มี class ของตัวเอง** (ดูจุดเรียกใน `HomeScreen`)
     *   · **ห้ามแยกเป็นสแปนซ้อนแล้วตั้งค่าให้ตรงกัน** — วันที่มีคนแก้ข้างนอก ข้างในจะไม่ตาม แล้วเพี้ยนเงียบ
     *
     * 🔴 **ห้าม find-replace คำว่า "ทริป" ทั้งไฟล์เด็ดขาด** — `upcomingTrips` ข้างล่างเป็น *คำไทยธรรมดา*
     *    และคำแทนที่เป็นภาษาไทยด้วยกัน ⇒ ผลลัพธ์จะ **อ่านลื่นและผิด**
     *    (`"ลุยทริปที่จะมาถึง"` ไม่สะดุดตาเหมือน `"luitripที่จะมาถึง"` — **ตัวจับด้วยตาหายไปทั้งตัว**)
     */
    brandTh: "ลุยทริป",
    ...
    login: "เข้าสู่ระบบ",
    account: "บัญชี",
    /** เมนูบัญชีบนแถบหัว (จอคอม) — ผู้ใช้ระบุคำเองว่า *"ข้อมูลส่วนตัว/ออกจากระบบ"* */
    accountProfile: "ข้อมูลส่วนตัว",
    signOut: "ออกจากระบบ",
    ...
    myTrips: "ทริปของฉัน",
    /**
     * คำบนปุ่มกลับหน้าแรก — **ใช้คำเดียวทั้งเว็บ** (ผู้ใช้สั่ง 5 ก.ย. 2026)
     * 🔴 เขาเลือกคำนี้เองตอนแก้หน้า `/account` ⇒ ยึดคำของเขา ไม่ใช่คำที่เราคิดว่าสวยกว่า
     * ⚠️ **ไม่ใช่ "ทริปทั้งหมด"** — หน้าแรกไม่ได้มีแค่รายการทริปแล้ว
     */
    ...
    /**
     * แถวสมาชิกบนการ์ด — ผู้ใช้สั่งเอง 4 ก.ย. 2026:
     * > *"ควรโชว์โปรไฟล์มาสักอย่างน้อย 5 คนไหม แล้วที่เหลือก็เขียนว่า + … และกดเข้าไปเพื่อดูสมาชิกทั้งหมดได้"*
     * 🔴 `membersOpen` เป็น `aria-label` ของปุ่ม — **ต้องบอกจำนวน** เพราะคนที่ใช้โปรแกรมอ่านหน้าจอ
     *    ไม่เห็นวงกลมที่ซ้อนกันอยู่ ⇒ ได้ยินแค่ "ปุ่ม" จะไม่รู้ว่ากดแล้วจะเจออะไร
     */
    membersOpen: (n: number) => `สมาชิก ${n} คน — แตะเพื่อดูทั้งหมด`,
    membersTitle: "สมาชิกในทริป",
    /**
     * **ชวนเพื่อน** — ผู้ใช้สั่งเอง: *"เชิญได้ หรือส่งลิงก์เชิญร่วมทริปนี้ได้"*
     * 🔴 `inviteOnce` เป็นข้อความที่ **ห้ามอ่อนกว่านี้** — ฐานเก็บแค่แฮชของลิงก์
     *    ⇒ ปิดกล่องแล้ว **ไม่มีทางดูซ้ำได้จริง ๆ** ไม่ใช่แค่ "ไม่สะดวก"
     */
    inviteOpen: "＋ ชวนเพื่อนเข้าทริป",
    inviteRoleQuestion: "เพื่อนที่เข้ามาจะทำอะไรได้",
    inviteRoleEditor: "แก้ไขแผนได้",
    inviteRoleViewer: "ดูอย่างเดียว",
    inviteCreate: "สร้างลิงก์ชวน",
    inviteCreating: "กำลังสร้างลิงก์…",
    inviteOnce: "🔴 ลิงก์นี้แสดงครั้งเดียว — ปิดแล้วดูซ้ำไม่ได้ ต้องสร้างใหม่",
    inviteCopy: "คัดลอกลิงก์",
    inviteCopied: "คัดลอกแล้ว",
    inviteFailed: "สร้างลิงก์ไม่สำเร็จ — ลองใหม่อีกครั้ง",
    inviteExpiresAt: (d: string) => `ใช้ได้ถึง ${d}`,
    inviteActiveCount: (n: number) => `ลิงก์ที่ยังใช้ได้ ${n} ลิงก์`,
    inviteRevoke: "ยกเลิกลิงก์",
    inviteRevokeFailed: "ยกเลิกลิงก์ไม่สำเร็จ",
    /** 🔴 คนที่ไม่ใช่ owner สร้างลิงก์ไม่ได้ (ด่านอยู่ในฐาน) ⇒ ไม่ต้องแสดงปุ่มให้เขาเห็นตั้งแต่แรก */
    inviteOwnerOnly: "เฉพาะเจ้าของทริปเท่านั้นที่ชวนคนเพิ่มได้",
    /** หน้า `/invite/<token>` — **คนที่ยังไม่ล็อกอินก็ต้องอ่านรู้เรื่อง** */
    inviteLandingTitle: "คำชวนเข้าร่วมทริป",
    inviteLandingBy: (name: string) => `${name} ชวนคุณเข้าร่วม`,
    inviteLandingRole: (role: string) =>
      role === "editor" ? "คุณจะแก้ไขแผนได้" : "คุณจะดูแผนได้อย่างเดียว",
    inviteLandingAccept: "เข้าร่วมทริปนี้",
    inviteLandingJoining: "กำลังเข้าร่วม…",
    inviteLandingNeedLogin: "เข้าสู่ระบบเพื่อเข้าร่วม",
    inviteLandingChecking: "กำลังตรวจสอบคำชวน…",
    /** 🔴 ข้อความจากฝั่งฐานใช้ได้จริง (หมดอายุ/ถูกยกเลิก/ใช้ครบ) — แสดงตรง ๆ ดีกว่าแปลรวมเป็นคำเดียว */
    inviteLandingDead: "คำชวนนี้ใช้ไม่ได้แล้ว",
    inviteLandingHome: "ไปหน้าแรก",
    membersLoading: "กำลังโหลดรายชื่อ…",
    /** 🔴 อ่านไม่ได้ ≠ ไม่มีสมาชิก — ทริปทุกใบมีเจ้าของอย่างน้อยหนึ่งคนเสมอ */
    membersUnreadable: "อ่านรายชื่อสมาชิกไม่ได้ตอนนี้",
    /** ชื่อที่ยังอ่านไม่ได้ — **ไม่ใช้คำว่า "ไม่ระบุชื่อ"** ซึ่งแปลว่าเขาเลือกที่จะไม่บอก */
    memberNoName: "ยังไม่ได้ตั้งชื่อโปรไฟล์",
    memberRole: (role: string) =>
      role === "owner" ? "เจ้าของทริป" : role === "editor" ? "แก้ไขได้" : "ดูอย่างเดียว",
    /**
    ...
    /**
     * โมดัลค้นหาของ **จอโทรศัพท์** — บนจอเล็กแถบหัวมีที่ให้ *ไอคอน* เท่านั้น
     * 🔴 ปุ่มต้องบอกได้ว่า **กำลังกรองอยู่หรือเปล่า** — ไม่งั้นผู้ใช้เห็นรายการสั้นลงโดยไม่มีอะไรอธิบาย
     *    (โมดัลปิดไปแล้ว คำค้นมองไม่เห็น) ⇒ `searchActive` เป็น `aria-label` ตอนมีคำค้นค้างอยู่
     */
    searchOpen: "ค้นหาทริป",
    searchActive: (q: string) => `กำลังค้นหา "${q}" — แตะเพื่อแก้หรือล้าง`,
    /**
     * ปุ่มปิดโมดัล — **ข้อความขึ้นกับว่า *ค้นหรือยัง* ไม่ใช่แค่ *เจอกี่ใบ***
     * ```
     * ยังไม่พิมพ์   "ปิด"                 ← ยังไม่มีการค้น จึงยังไม่มี "ผลลัพธ์" ให้ดู
     * พิมพ์แล้ว 0   "ไม่พบทริปที่ตรง"
     * พิมพ์แล้ว n   "ดูผลลัพธ์ n ทริป"     ← รายการอยู่หลังโมดัล มองไม่เห็นตอนพิมพ์ ⇒ ตัวเลขคือคำตอบ
     * ```
     * 🔴 **ฉบับแรกบอกจำนวนตั้งแต่ยังไม่พิมพ์ (`ดูผลลัพธ์ 5 ทริป`) — ผู้ใช้ทักว่า "เป็นคำที่แปลกมาก" และถูก**
     * 🎯 ***`5` ไม่ได้ผิด มันคือจำนวนทริปทั้งหมด — แต่คำว่า "ผลลัพธ์" อ้างว่ามีการค้นหาเกิดขึ้นแล้ว
     *    ทั้งที่ยังไม่มี*** · เลขถูก แต่ประโยคที่ห่อมันไว้เป็นเท็จ
     * · และมันกลบปุ่มปิดธรรมดาไปด้วย — คนเปิดมาแล้วเปลี่ยนใจ ไม่มีคำว่า "ปิด" ให้กด มีแต่ "ดูผลลัพธ์"
     */
    searchShowResults: (n: number, hasQuery: boolean) =>
      !hasQuery ? "ปิด" : n === 0 ? "ไม่พบทริปที่ตรง" : `ดูผลลัพธ์ ${n} ทริป`,
    /**
     * หัวข้อที่ ② จากสามหัวข้อที่ผู้ใช้สั่ง — **ขึ้นเฉพาะตอนมีแผนจริง**
     * 🎯 ***หัวข้อเปล่าอ่านเหมือนเว็บพัง · ไม่มีหัวข้อเลยอ่านเหมือนยังไม่มีฟีเจอร์ — อย่างหลังจริงกว่า***
     */
    recommended: "ทริปแนะนำ",
    recommendedSubheading: "แผนที่จัดไว้แล้ว หยิบไปใช้ได้เลย",
    /**
     * 🔴 **"5 วัน 4 คืน" มาจากฐาน ไม่ได้คำนวณที่นี่** — ฝั่งฐานคืน `dayCount`/`nightCount` มาให้
     * เพราะถ้อยคำนี้เป็นสิ่งที่ผู้ใช้พูดเอง ⇒ ต้องมาจากที่เดียว **ไม่ใช่สองที่ที่อาจไม่ตรงกัน**
     * ⇒ ที่นี่มีหน้าที่ *ประกอบคำ* อย่างเดียว ห้ามคิดเลขเอง
     */
    tripLength: (days: number, nights: number) => `${days} วัน ${nights} คืน`,
  },
  /**
   * ฟอร์มสร้างทริป
