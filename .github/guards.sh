...
if [ -n "$scoped" ]; then
  ...
  echo "🔴 ref: scoped ว่าง — ไม่มี path ไหนใน $ROOT ให้ตรวจเลยสักอัน · ด่านนี้ไม่ได้ตรวจอะไรทั้งที่ควรตรวจ"
  fail=1
fi
...
# ── ภาพปกต้องมาเป็น "คู่" เสมอ — ใบใหญ่ ≤800px + ใบเล็ก ≤400px (P5 เสนอ · P6 6 ก.ย. 2026) ──
# 🔴 `CityPickerScreen.tsx` ใช้ `srcSet` (400w/800w) ⇒ ความถูกต้องย้ายจาก *คุณสมบัติของไฟล์เดี่ยว*
#    ไปเป็น *ความสัมพันธ์ระหว่างสองไฟล์* — ด่านที่ดูทีละใบมองไม่เห็นสิ่งนี้ตามนิยาม
# 🎯 ***ใบเล็กหาย = ไม่มีอะไรพัง*** — เบราว์เซอร์ถอยไปใช้ใบใหญ่เอง หน้าเว็บยังสวย
#    แต่มือถือกลับไปโหลด 800px หนักเหมือนเดิมเงียบ ๆ (230KB → 830KB ต่อภาพ) ไม่มีสัญญาณอะไรเลย
# ⚠️ ใช้ glob ตรงดิสก์ ไม่ใช่ `git ls-files` — ต่างจากด่านข้างบนที่ตั้งใจไม่มองไฟล์ untracked
#    (ที่นี่คือของที่ควรจับ *ก่อน* commit ด้วย ไม่ใช่แค่ตอน CI checkout)
#
# 🔴 แก้ 6 ก.ย. 2026 (P3 จับ) — ฉบับแรกตรวจแค่ "ใบเล็กมีไฟล์อยู่ไหม" ไม่ตรวจ *ขนาด*
#    ทั้งที่หัวคอมเมนต์เดิมเขียนว่ากันเรื่องขนาด — P3 ยิงมัลแตนต์ **กับด่านตัวนี้จริง** (ไม่ใช่รอบแรก
#    ที่เครดิตเพี้ยนไปว่าเป็นสคริปต์ของเขาเอง — ดู `dcacb7f`) ก๊อปใบใหญ่ 800px ไปเป็น `-sm.jpg`
#    ตรงๆ (ชื่อถูก ขนาด 800px เดิม 830KB) → **ผ่านด่านฉลุย** ตรงกับโหมดพังที่หัวเรื่องอ้างว่ากันไว้พอดี
#    ⇒ แยกการตรวจขนาดไปที่ check-cover-pairs.py (เหตุผลที่ไม่ใช้ `sips` ตรงๆ อยู่ในหัวไฟล์นั้น —
#    สั้นๆ คือ `sips` เป็นคำสั่ง macOS เท่านั้น ใส่ในบล็อก bash นี้จะพังทุกรอบบน `ubuntu-latest`)
for big in "$ROOT"/public/catalog/*/*.jpg; do
  case "$big" in *-sm.jpg) continue;; esac
  [ -e "$big" ] || continue
  sm="${big%.jpg}-sm.jpg"
  rel="${big#$ROOT/}"
  if [ ! -f "$sm" ]; then
    echo "🔴 ภาพปกไม่มีใบเล็กคู่: $rel → ต้องมี ${rel%.jpg}-sm.jpg (400px)"
    echo "   สร้างด้วย: sips -Z 400 \"$rel\" --out \"${rel%.jpg}-sm.jpg\""
    fail=1
  fi
done
for sm in "$ROOT"/public/catalog/*/*-sm.jpg; do
  [ -e "$sm" ] || continue
  big="${sm%-sm.jpg}.jpg"
  if [ ! -f "$big" ]; then
    echo "🔴 ใบเล็กกำพร้า (ไม่มีใบใหญ่): ${sm#$ROOT/}"
    fail=1
  fi
done
COVERPAIRS="$(cd "$(dirname "$0")" && pwd)/check-cover-pairs.py"
if [ -d "$ROOT/public/catalog" ] && [ -f "$COVERPAIRS" ]; then
  python3 "$COVERPAIRS" "$ROOT" || fail=1
fi
