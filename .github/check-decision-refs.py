...
def main(argv) -> int:
    ...
    for ident in sorted(defs, key=lambda x: (x[0], float(re.sub(r"[^0-9.]", "", x)))):
        ...
        if len(lines) > 1:
            ...
            print("   ทะเบียนที่เลขหนึ่งตัวชี้ได้สองที่ ไม่ใช่ทะเบียน · ทางออกมี 3 ทาง:")
            print("     ① รวมเป็นหัวข้อเดียว   ② ออกเลขใหม่ให้อันที่สอง")
            print("     ③ ถ้าอันที่สองเป็น **การอัปเดตสถานะ** ของ finding เดิม (ไม่ใช่นิยามใหม่)")
            print("        → เขียนโดย **ไม่ใส่ em-dash หลัง ID** ตามธรรมเนียมที่ใช้อยู่แล้วในไฟล์:")
            print("          `### 🔴 `P-33` ปิดฝั่ง client แล้ว · …`   (README.md:2525)")
            print("        🔴 ชนิดที่สามนี้ P1 ชี้ไว้ 26 ส.ค. 2026 — em-dash คือสิ่งเดียวที่แยกมันออกจากนิยาม")
            dupbad += 1
