...
def main(argv) -> int:
    ...
    # 🔴 **ประกอบทีละชิ้น ห้ามเขียนติดกัน** (P6 · 27 ส.ค. 2026 · P1 รายงาน)
    #    ด่านนี้สแกน `*.py` ด้วย → **มันเจอ canary ของตัวเองแล้วแดงใส่ตัวเอง**
    #    canary เป็น *string literal* ไม่ใช่คอมเมนต์ → `strip_py_comments` ไม่แตะ (ตามที่ควร)
    # 🎯 ทีมนี้แก้ปัญหารูปนี้ไปแล้วครั้งหนึ่งที่ `guards-selftest.sh:11`
    #    `TRIP_REF="$(printf 'ejzibhgqhxdz%s' 'kovsnpds')"` — **เหตุผลเดียวกันเป๊ะ**
    #    (ไฟล์ที่ต้องถือ "ของที่ห้าม" ไว้เพื่อทดสอบ ต้องไม่ถือมันในรูปที่ค้นเจอ)
    # ✅ เลือกทางนี้แทน "ข้ามไฟล์ตัวเอง" เพราะด่านนี้ **เป็นไฟล์ที่จัดการรูปแบบคอมเมนต์โดยตรง
    #    จึงเป็นที่ที่รูปพัง ๆ มีโอกาสโผล่จริงมากที่สุด** — ปิดตามันต่อตัวเองคือปิดตาผิดจุด
    _PY = "/" + "/"
    _TS = "\\/" + "\\/"
    canaries = [
        'src = re.sub(r"' + _PY + '.*$", "", line)',
        'const out = s.replace(/' + _TS + '.*/g, "");',
        'code = line.replace(/' + _TS + '[^\\n]*/, "")',
    ]
    for c in canaries:
