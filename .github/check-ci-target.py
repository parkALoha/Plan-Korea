...
def main() -> int:
    ...
    for name in ("NEXT_PUBLIC_SUPABASE_ANON_KEY", "SUPABASE_SERVICE_ROLE_KEY"):
        ...
        if ref is None:
            ...
            # 🔴 **ห้ามยิงโพรบถ้า host ยังไม่ผ่าน — บั๊กที่ผมสร้างเองแล้วจับได้ตอนยิงทิศแดง ③**
            #    ฉบับแรกของ patch นี้ยิงโพรบเสมอ ⇒ ตั้ง URL เป็นโปรเจกต์อื่นเมื่อไหร่
            #    **ด่านจะส่ง `SUPABASE_SERVICE_ROLE_KEY` ไปที่ host นั้นเอง**
            # 🎯 ***ด่านที่มีไว้กันการยิงผิดฐาน กลายเป็นตัวยิงผิดฐานเสียเอง***
            #    · และมันแย่กว่าฉบับก่อนมี patch ซึ่งแค่พิมพ์ 🔴 แล้วจบ
            #    · ⚠️ **ไม่มีอะไรในผลลัพธ์บอกเลย** — output อ่านเหมือนด่านทำงานถูก
            # ✅ กติกา: **ยืนยันปลายทางก่อน แล้วค่อยส่ง credential ไปที่นั่น** ไม่ใช่พร้อมกัน
            if not url_ok:
                print(f"⚠️ ci-target: {name} ไม่ใช่ JWT และ **ไม่ยิงยืนยัน เพราะ host ยังไม่ผ่าน** — "
                      f"ห้ามส่งคีย์ไปยังปลายทางที่ยังพิสูจน์ไม่ได้")
                continue
