    ...
    if (counted.failed > 0) {
      console.error(
        `\n🟠 มีเคสล้ม ${counted.failed} เคส · ก่อนสรุปว่าเป็น regression:\n` +
          ...
          // 🔴 R11 อีกหน้า: `TEST_COUNTRY_CODES` จองกัน*ข้ามบล็อก* ไม่กัน*ข้ามเซสชัน* (P1 · P4 · 27 ส.ค.)
          //    สองเซสชันรันบล็อกเดียวกันพร้อมกัน = seed รหัสประเทศเดียวกัน = ชน · และ *รันเดี่ยวก็ยังชน* ถ้าอีกฝั่งยังรันอยู่
          "   🔴 *ยกเว้น* `duplicate key`/`unique constraint` บน **ตาราง fixture** (เช่น `catalog_countries`) ตอน `beforeAll`:\n" +
          "      = อีกเซสชัน seed รหัส `TEST_COUNTRY_CODES` เดียวกัน **พร้อมกัน** · `beforeAll` ล้ม → เคสถูก skip → โผล่เป็น \"failed\" ที่ไม่ใช่ของจริง\n" +
          "      → **รันไฟล์เดี่ยว *ไม่* ช่วยถ้าอีกฝั่งยังรันอยู่** (ต่างจาก assertion ล้ม) · รอ ~45 วิ แล้วรันใหม่ · อาการ *`duplicate key` บนตาราง fixture* คือตัวแยก\n" +
          // 🔴 ด่านระดับชุด (สแกนทั้งโฟลเดอร์) พังกฎ "รันไฟล์เดี่ยว" — เคสล้มอยู่ *ในด่าน* แต่ชี้ชื่อไฟล์อื่น
          //    (P1 เจอจริง 27 ส.ค.: `mockShape S6` ชี้ `signIn.test.ts` · รันไฟล์นั้นเดี่ยว = เขียว = หลอก)
          "   🔴 ยกเว้น **ด่านระดับชุด** ที่อ่านทั้งโฟลเดอร์ (`mockShape` · `apiHostGuard` · ด่านสแกนไฟล์อื่น ๆ):\n" +
          "      เคสล้มอยู่ *ในตัวด่าน* แต่มันชี้ชื่อ*ไฟล์ที่ผิด* (เช่น `S6 … signIn.test.ts`)\n" +
          "      → รัน *ไฟล์ที่ถูกชี้* เดี่ยว ๆ แล้วเขียว **ไม่ได้แปลว่าไม่ใช่ของจริง** — ไฟล์เดี่ยวไม่มีด่านสแกนมัน\n" +
          "      ต้องรัน *ไฟล์ด่านนั้นเอง* หรือแก้ไฟล์ที่ถูกชี้ตามที่ด่านบอก\n" +
