"use client";

import { useEffect, useState } from "react";
import type { CountryOption } from "@/components/TripDestinationPicker";
import { CoverCard } from "@/components/CoverCard";
import { E5_COPY } from "@/lib/i18n";

const COPY = E5_COPY.explorer;

/**
 * **เมนู "เลือกปลายทาง"** — กดธงประเทศ → เลือกเมือง → เปิดฟอร์มสร้างทริปโดยเติมปลายทางให้แล้ว
 * เจ้าของ: P2-UI/UX · 4 ก.ย. 2026 · ผู้ใช้สั่งเอง
 *
 * ## 🔴 เมนูนี้ **ไม่ตั้งจำนวนวันให้ผู้ใช้** — และนั่นคือสิ่งที่แยกมันออกจาก "ทริปแนะนำ"
 * ผู้ใช้แก้ข้อเสนอเดิมของ P1 ตรงจุดนี้เอง:
 * > *"เขาเลือก ประเทศ หรือเมือง **แต่ไม่ได้ระบุวันที่เพราะเขาจะกรอกเอง**"*
 * ⇒ กดเมืองแล้ว **ยังไม่สร้างทริป** · เปิดฟอร์มที่เติมปลายทางไว้ เหลือให้เขาใส่ชื่อ+วันที่
 * 🎯 ***ทริปที่วันที่มั่ว คือทริปที่เขาต้องลบทิ้ง — และวันนี้ลบไม่ได้ด้วย***
 *
 * ## ทำไมธงอีโมจิ ไม่ใช่ไฟล์รูปประเทศ
 * `public/covers/` มี `country-*.svg` แค่ **4 จาก 9 ประเทศ** (jp · kr · th · vn)
 * ⇒ ใช้ไฟล์รูป = ห้าประเทศได้กล่องเปล่า **ทั้งที่ผู้ใช้ขอ "กดที่ icon นั้น ๆ"**
 * 🔴 ธงคำนวณจาก `country_id` (ISO-3166-1 alpha-2) ได้ครบทุกประเทศ **โดยไม่ต้องรออาร์ตเวิร์ก**
 * · ⚠️ ข้อแลก: ธงเรนเดอร์ต่างกันตามระบบปฏิบัติการ (บาง Windows ได้ตัวอักษร `JP` แทนธง)
 *   **ยอมรับได้** เพราะชื่อประเทศอยู่ข้าง ๆ เสมอ — ไอคอนเป็นของเสริม ไม่ใช่ตัวบอกความหมายตัวเดียว
 */
function flagOf(countryId: string): string {
  const code = countryId.trim().toUpperCase();
  if (!/^[A-Z]{2}$/.test(code)) return "🏳️";
  return String.fromCodePoint(...[...code].map((c) => 0x1f1e6 + c.charCodeAt(0) - 65));
}

type ListState<T> = { status: "loading" } | { status: "ready"; items: T[] } | { status: "error" };

/**
 * รูปปกประเทศ — **สามชั้น: ภาพจริง → ภาพวาด SVG → พื้นไล่สี** (5 ก.ย. 2026 · P5 ยกธง)
 *
 * ## 🔴 ชั้นแรกเพิ่งมี และมันเคยเป็นของที่ *มีอยู่แต่ไม่มีใครเห็น*
 * ทีมเจนภาพจริงไว้ 63 ใบ **แต่ `CountryThumb` ไม่รู้จัก `public/catalog/` เลย** ⇒ ไล่แค่ `svg → ไล่สี`
 * ⇒ การ์ดเมืองข้าง ๆ เป็นรูปถ่ายจริง · การ์ดประเทศเป็นภาพวาดแบน **สองหน้านี้อยู่ห่างกันคลิกเดียว**
 * 🎯 ***ของที่มีอยู่แล้วแต่ไม่มีใครเห็น ถูกกว่าของที่ยังต้องสร้าง — และมันไม่ส่งเสียงว่ามันมีอยู่***
 *
 * ## ✅ ทำไมชั้นแรกไม่ต้องมี "ตารางแปลง id → ชื่อไฟล์"
 * ฉบับแรกที่ผมคิดไว้ต้องมีตาราง (`jp→japan` · `hk→hong-kong`) ⇒ **เป็น *ข้อเท็จจริงที่เก็บคนละที่กับสิ่งที่ทำให้มันจริง* เป๊ะ**
 * (วันที่มีคนตั้งชื่อไฟล์ต่างออกไป มันเงียบ) · 🔴 **P5 ปิดช่องนั้นที่ *ต้นทาง* แทน — ตั้งกติกาชื่อ `<countryId>.jpg`
 * แล้วเปลี่ยนชื่อ `japan.jpg → jp.jpg` ให้ตรง** ⇒ พาธ derive จาก `countryId` ตัวเดียว **รูปเดียวกับ `CityThumb` เป๊ะ**
 * 🎯 ***แก้ที่กติกาการตั้งชื่อ ถูกกว่าแก้ที่ตัวแปลชื่อ — ตัวแปลต้องมีคนซิงก์ กติกาไม่ต้อง***
 *
 * ## 🔴 ทั้งสามชั้นมีของจริงให้เห็นในหน้าเดียวกัน — ทิศบวกกับทิศลบอยู่ข้างกัน
 * ```
 * jp · kr   ภาพจริง (`/catalog/<id>/<id>.jpg`)
 * th · vn   ตกไปชั้น SVG (ไม่มี .jpg)
 * cn tw hk mo sg   ตกถึงพื้นไล่สี (ไม่มีทั้งคู่)
 * ```
 * ⇒ **ทดสอบชั้นตกทอดได้โดยไม่ต้องแกล้งลบไฟล์** · และเมื่อ P5 เจอครบ 9 ใบ ชั้นล่างจะเงียบไปเอง **ไม่ต้องมีใครมาแก้โค้ด**
 * · ⚠️ `hong-kong.jpg` · `macao.jpg` · `singapore.jpg` ใน `catalog/` **เป็นภาพ *เมือง* ไม่ใช่ภาพประเทศ**
 *   (นครรัฐมีเมืองเดียว ชื่อไฟล์เลยอ่านเหมือนชื่อประเทศ) — **ผมเคยอ่านผิดและรายงานผิดไปแล้วหนึ่งรอบ**
 *   🎯 ชื่อไฟล์บอกว่า *ตั้งใจเรียกมันว่าอะไร* ไม่ได้บอกว่า *มันเป็นอะไร*
 *
 * ## 📌 ธงไม่ซ้ำในพื้นไล่สี — ธงอยู่ในบรรทัดชื่อแล้ว **ทุกใบ**
 * ยิงจริงตอนใส่ทั้งสองที่: ประเทศที่ไม่มีไฟล์รูปได้ `🇨🇳 🇨🇳จีน` **ธงซ้ำสองรอบ** ⇒ การ์ดเก้าใบมีสองแบบโดยไม่ได้ตั้งใจ
 * 🎯 ***ไอคอนต้องอยู่ที่เดียว "ที่มีเสมอ" ไม่ใช่สองที่ "ที่มีบ้างไม่มีบ้าง"***
 */
function CountryThumb({ countryId }: { countryId: string }) {
  /**
   * 🔴 **`useState` สองตัว ไม่ใช่ตัวเดียวที่นับเลข** — แต่ละชั้นต้องจำ *ของตัวเอง* ว่าเจ๊งไปแล้ว
   * ถ้าใช้ตัวนับรวม จะแยกไม่ออกว่า "ภาพจริงไม่มี" กับ "SVG ไม่มี" ⇒ เดาลำดับผิดเมื่อชั้นใดชั้นหนึ่งหายทีหลัง
   * · 🔴 **`key` บน `<img>` จำเป็น** — ไม่มีมัน React จะ reuse โหนดเดิมตอนสลับ `src`
   *   แล้ว `onError` ของภาพเก่าอาจยิงทับสถานะของภาพใหม่ (เจอรูปนี้มาแล้วกับ `CoverCard`)
   */
  const [photoBroken, setPhotoBroken] = useState(false);
  const [svgBroken, setSvgBroken] = useState(false);

  if (!photoBroken) {
    return (
      // eslint-disable-next-line @next/next/no-img-element -- ไฟล์ static ใน public/catalog/ ที่ทีมวางเอง
      <img
        key={`photo-${countryId}`}
        src={`/catalog/${countryId}/${countryId}.jpg`}
        /**
         * 🔴 **`sizes` วัดจากกริดของหน้านี้เอง — P5 ส่งค่าของหน้า `/explore/<cc>` มาให้ และเขากำกับเองว่าอย่ายกไปใช้**
         * ```
         * 375px   การ์ด 164px (43.6vw)   ← สองคอลัมน์
         * 768px   การ์ด **360px**         ← ยังสองคอลัมน์ (minmax 17rem ใส่ได้แค่ 2 ใน 736)
         * 1024px  การ์ด 278px            ← สามคอลัมน์
         * 1440px  การ์ด 278px            ← เท่าเดิม `max-w-4xl` ตรึงไว้แล้ว
         * ```
         * 🎯 ***ช่วง 640–1023 การ์ด *ใหญ่กว่า* ตอนจอกว้าง — เพราะจำนวนคอลัมน์เปลี่ยนก่อนความกว้างสูงสุดจะตัน***
         *    ⇒ ค่าที่ P5 ใช้ (`280px` สำหรับทุกจอที่ไม่ใช่มือถือ) **ขอรูปเล็กเกินไปที่ 768** สำหรับหน้านี้
         * · ⚠️ ที่ DPR2 ทั้งสองค่าจบที่ใบ 800 เหมือนกัน — **ต่างกันจริงเฉพาะจอ DPR1/1.5 ช่วงแท็บเล็ต**
         *   จดไว้เพราะ *ถ้าไม่เขียน คนถัดไปจะคิดว่าตัวเลขนี้ลอกกันได้*
         * · ✅ **และเวลาเลือกไม่ลง ให้เลือกทางที่ *ขอเกิน*** (ถ้อยคำ P5 · เขาเจอจุดพลิกเดียวกันที่หน้าเขา):
         *   🎯 ***ขอเกิน = เปลืองไบต์ · ขอขาด = ภาพนุ่ม ⇒ พลาดไปทางที่แก้ได้ด้วยเงิน ไม่ใช่ทางที่ผู้ใช้เห็น***
         */
        srcSet={`/catalog/${countryId}/${countryId}-sm.jpg 400w, /catalog/${countryId}/${countryId}.jpg 800w`}
        sizes="(max-width: 639px) 45vw, (max-width: 1023px) 360px, 280px"
        alt=""
        /**
         * 🔴 **ตั้งใจ *ไม่* ใส่ `loading="lazy"` ที่นี่ — ผมใส่ไปแล้ว วัดแล้ว และถอนออก** (5 ก.ย. 2026)
         * ```
         * ใส่ eager 4 ใบแรก · lazy ที่เหลือ  →  ไบต์ตอนเปิดหน้า 1.53MB → 782KB (−49%)  ดูเหมือนได้กำไร
         * แล้ววัดตำแหน่งจริงที่ 375px       →  การ์ดใบที่ 4-7 อยู่ที่ top 496 · 670 = **อยู่ในจอ**
         *                                     `naturalWidth === 0` ทั้งสี่ใบ ณ วินาทีที่วัด
         * ```
         * 🎯 ***49% ที่ประหยัดได้ ไม่ใช่ไบต์ที่ผู้ใช้ไม่ต้องการ — มันคือไบต์ของรูปที่เขากำลังมองอยู่***
         * ⇒ สิ่งที่ได้จริงคือ **กล่องว่างสี่ใบตอนเปิดหน้า** แลกกับตัวเลขที่ดูดีขึ้นในเครื่องมือวัด
         * · 🔴 การ์ดสูง ~174px · จอ 812px ⇒ **8 จาก 9 ใบอยู่เหนือ fold** ⇒ `lazy` มีของให้เลื่อนแค่ใบเดียว
         *   ⇒ ***หน้าที่เนื้อหาสั้นพอจะเห็นเกือบหมดในจอเดียว ไม่ใช่หน้าที่ `lazy` ช่วย***
         * ⚠️ **ค่าที่ผมเดาไว้ก่อนวัดคือ "เห็นราว 4 ใบแรก" — ผิด และมันผิดในทิศที่ทำให้ตัวเลขดูดีขึ้น**
         * ✅ ตัวที่แก้รากคือ **ย่อไฟล์ที่ต้นทาง** (1024×572 → เรนเดอร์จริง 166px ที่ 375px · จ่ายค่าขนส่งเกิน 6 เท่า)
         *    โซน P5 · **อย่าเอา `lazy` กลับมาที่นี่โดยไม่วัดตำแหน่งการ์ดก่อน**
         */
        className="aspect-video w-full object-cover"
        onError={() => setPhotoBroken(true)}
      />
    );
  }

  const broken = svgBroken;
  if (broken) {
    /**
     * 🔴 **พื้นไล่สีเปล่า ไม่ใส่ธงซ้ำ** — ธงอยู่ในบรรทัดชื่อแล้ว **ทุกใบ**
     * ยิงจริงตอนใส่ทั้งสองที่: ประเทศที่ไม่มีไฟล์รูปได้ `🇨🇳 🇨🇳จีน` **ธงซ้ำสองรอบ**
     * ส่วนสี่ประเทศที่มีไฟล์รูปได้ธงรอบเดียว ⇒ **การ์ดเก้าใบมีสองแบบโดยไม่ได้ตั้งใจ**
     * 🎯 ***ไอคอนต้องอยู่ที่เดียว "ที่มีเสมอ" ไม่ใช่สองที่ "ที่มีบ้างไม่มีบ้าง"***
     */
    return <div className="aspect-video w-full bg-gradient-to-br from-pine to-maple" />;
  }
  return (
    // eslint-disable-next-line @next/next/no-img-element -- ไฟล์ static ใน public/covers/ ที่ทีมวางเอง
    <img
      key={`svg-${countryId}`}
      src={`/covers/country-${countryId}.svg`}
      alt=""
      className="aspect-video w-full object-cover"
      onError={() => setSvgBroken(true)}
    />
  );
}

/**
 * 🔴 **ไม่มี prop แล้ว และกิ่ง "กางเมืองในหน้าเดิม" ถูกถอดออกทั้งก้อน** (5 ก.ย. 2026)
 * ตั้งแต่การ์ดประเทศกลายเป็น `<Link href="/explore/<id>">` (`636e9ab`) `setCountryId` เหลือที่เรียกที่เดียว
 * คือ *รีเซ็ตกลับว่าง* ⇒ **กิ่งกริดเมืองเข้าไม่ถึงตามนิยาม · `onPickCity` ยิงไม่ได้อีกเลย** (P3 วัดเจอ)
 * ⚠️ **คอมเมนต์เดิมของผมในไฟล์นี้เขียนว่า *"ยังถูกใช้ที่กริดเมืองข้างล่าง — ยังไม่ถอด"***
 *    ⇒ *จริงตอนพิมพ์ และเป็นเท็จตั้งแต่คอมมิตเดียวกันนั้นเอง* — ระยะห่างระหว่างข้อเท็จจริง
 *      กับสิ่งที่ทำให้มันจริง = **ศูนย์ commit** และไม่มีอะไรส่งเสียงเลยตลอดทาง
 *
 * ✅ **เทียบความสามารถก่อนถอด ตามที่ P5 ขอ** — ของเดิมทำได้: *เลือก **หนึ่ง** เมือง → เปิดฟอร์มพร้อมเมืองนั้น*
 *    ของใหม่ (`/explore/<id>` ของ P5): *เลือก **หลาย** เมือง **ข้ามประเทศ** → ฟอร์มเดียวกัน*
 *    ⇒ **ครอบของเดิมทั้งหมด ไม่มีอะไรหาย** (P1 กับ P5 ยิงเส้นทางจริงจนถึงฐานแล้วทั้งคู่)
 */
export function DestinationExplorer() {
  const [countries, setCountries] = useState<ListState<CountryOption>>({ status: "loading" });

  useEffect(() => {
    let cancelled = false;
    fetch("/api/engine/countries")
      .then((r) => {
        if (!r.ok) throw new Error(`countries ${r.status}`);
        return r.json() as Promise<CountryOption[]>;
      })
      .then((rows) => {
        if (!cancelled) setCountries({ status: "ready", items: rows });
      })
      // 🔴 ล้มเหลว = "อ่านไม่ได้" ไม่ใช่ "ไม่มีประเทศ" — โชว์ลิสต์ว่างเงียบ ๆ คือการโกหก
      .catch(() => {
        if (!cancelled) setCountries({ status: "error" });
      });
    return () => {
      cancelled = true;
    };
  }, []);



  if (countries.status === "error") {
    return <p className="py-6 text-center text-sm text-content-soft">{COPY.countriesError}</p>;
  }

  return (
    <div>
      {/**
       * ── การ์ดประเทศ ──────────────────────────────────────────────
       * 🔴 **ประเทศเป็น *การ์ด* ไม่ใช่ชิป/ปุ่มธง** — ผู้ใช้สั่งเอง (รอบสอง):
       * > *"จะโชว์ component เหมือนกับรูป trip ของตัวเอง **เป็นชื่อประเทศ กดเข้าไป ค่อยมีเมืองเด้งมาให้เลือกต่อ**"*
       * ⇒ ใช้ `CoverCard` ใบเดียวกับการ์ดทริป · ธงยังอยู่ **แต่ย้ายจาก *ตัวการ์ด* มาเป็น *ของในการ์ด***
       *
       * 🔴 **การ์ดประเทศมีแค่ *ชื่อประเทศ* — ผู้ใช้สั่งเอง 4 ก.ย. 2026**
       * > *"ไม่ต้องบอกว่ามีกี่เมือง เมืองอะไรบ้าง โชว์แค่ชื่อประเทศพอ"*
       * เดิมมี "N เมือง" + ชื่อเมืองตัวอย่าง (`📍 ฮวาเหลียน · เกาสง · ไถหนาน`) — ถอดทั้งสองบรรทัด
       *
       * 🎯 ***ทั้งสองบรรทัดตอบคำถามที่ยังไม่มีใครถามในหน้านี้*** — ขั้นนี้ผู้ใช้เลือก *ประเทศ*
       *    จำนวนเมืองไม่ช่วยตัดสิน (มากกว่า ≠ ดีกว่า) และเมืองตัวอย่างสามชื่อจากทั้งหมด 7–23
       *    **บอกไม่ได้ว่าประเทศนั้นมีอะไร แต่ดูเหมือนบอก** ⇒ หน้าจอที่ยาวขึ้นโดยไม่ได้ให้ข้อมูลเพิ่ม
       *    · เมืองทั้งหมดอยู่หน้าถัดไปอยู่แล้ว ซึ่งเป็นที่ที่คำถาม *"เมืองอะไรบ้าง"* เกิดขึ้นจริง
       *
       * ✅ **ไม่ส่ง `children` และไม่ override `titleClassName`** — เปลือกออกแบบให้ทุกช่องเป็นทางเลือก
       *    (*"ช่องที่มีที่ว่างแต่ว่างได้ คือคำตอบ"*) ⇒ ได้ขนาดชื่อค่าเริ่มต้นซึ่งใหญ่กว่าเดิม
       *    🔴 **ตั้งใจใช้ค่าเริ่มต้น ไม่ใช่ตั้งค่าใหม่** — ชื่อประเทศเป็นเนื้อหาเดียวที่เหลือ มันควรเป็นตัวเด่น
       *       และการ *ถอด override* ทิ้ง ทำให้การ์ดนี้ห่างจากเปลือกน้อยลง ไม่ใช่มากขึ้น
       *    ⚠️ `cityCount`/`sampleCities` ยังมาจาก API อยู่ **ไม่ได้ถอดฝั่งข้อมูล** — ถ้าวันหนึ่งอยากได้กลับ
       *       ของยังอยู่ครบ · ถ้าแน่ใจว่าไม่ใช้แล้ว ค่อยถอดฝั่ง API เป็นอีกใบ (โซน P1)
       */}
      {/**
       * 🔴 **มือถือได้ *สองคอลัมน์* — การ์ดประเทศมีแค่ธงกับชื่อ มันไม่ต้องการความกว้าง 17rem**
       * (ผู้ใช้สั่ง *"ปรับปรุงหน้านั้นให้ดี"* หลังสั่งถอดแถบหัวออก · 5 ก.ย. 2026)
       * ```
       * ก่อน  minmax(17rem)  ที่ 375px = **1 คอลัมน์** · การ์ดสูง ~215px ⇒ เห็นทีละ 1.5 ใบ · 9 ประเทศ = เลื่อน 3 จอ
       * หลัง  minmax(10rem)  ที่ 375px = **2 คอลัมน์** ⇒ เห็น 4 ใบต่อจอ · เลื่อนจอเดียวจบ
       * ```
       * 🎯 ***หน้านี้เป็น "เลือกจากทั้งหมด" — ของที่ต้องเทียบกัน ต้องเห็นพร้อมกัน · การ์ดใหญ่ทำให้เทียบไม่ได้***
       * ⚠️ **ไม่ได้เปลี่ยนตัวการ์ด** — ยังเป็น `CoverCard` ใบเดียวกับการ์ดทริปทุกประการ (ผู้ใช้สั่งไว้ว่าทั้งเว็บต้องใบเดียว)
       *    ที่เปลี่ยนคือ **ความกว้างของช่องในกริด** ซึ่งเป็นของหน้า ไม่ใช่ของการ์ด
       * · `sm:` ขึ้นไปกลับไปใช้ 17rem เท่าหน้าแรก ⇒ **จอคอมไม่เปลี่ยนอะไรเลย**
       */}
      <div className="grid gap-3 [grid-template-columns:repeat(auto-fill,minmax(10rem,1fr))] sm:[grid-template-columns:repeat(auto-fill,minmax(17rem,1fr))]">
        {countries.status === "loading"
          ? Array.from({ length: 9 }, (_, i) => (
              <div key={i} className="h-32 animate-pulse rounded-2xl bg-surface-soft sm:h-36" />
            ))
          : countries.items.map((c) => (
              <CoverCard
                key={c.id}
                /**
                 * 🔴 **พาไปหน้า `/explore/<countryId>` — ไม่ใช่กางเมืองในหน้าเดิมอีกแล้ว**
                 * ผู้ใช้สั่งเอง 5 ก.ย. 2026:
                 * > *"ถ้ากดเลือกประเทศใน ไปไหนดี … ควรจะพาไป /อันใหม่ เพื่อเลือกเมืองในประเทศนั้น ๆ
                 * >  โดยเลือกได้มากกว่า 1 เมือง … มีแถบให้เปลี่ยนประเทศ"*
                 *
                 * ⚠️ **คำสั่งนี้ทับของเดิมที่เขาสั่งเองเมื่อวาน** (*"กดเข้าไป ค่อยมีเมืองเด้งมาให้เลือกต่อ"*)
                 *    ⇒ ย่อหน้าข้างบนที่อธิบายการกางในหน้าเดิม **หมดอายุแล้ว เก็บไว้เป็นรอย ไม่ใช่เป็นเหตุผล**
                 * 🎯 ***สิ่งที่เปลี่ยนไม่ใช่รสนิยม — เมื่อวานเลือกได้เมืองเดียว วันนี้ต้องเลือกได้หลายเมือง
                 *    และ "เลือกหลายอย่างแล้วกดยืนยัน" ต้องการหน้าเป็นของตัวเอง ไม่ใช่แถบที่กางในหน้าอื่น***
                 *
                 * · ใช้ `href` ไม่ใช่ `onClick` ⇒ **เปิดแท็บใหม่ · คลิกกลาง · คัดลอกลิงก์ได้** (P1 ทักไว้)
                 * 🔴 **`setCountryId` ยังอยู่ในไฟล์นี้และยังถูกใช้ที่กริดเมืองข้างล่าง** — ยังไม่ถอด
                 *    เพราะหน้า `/explore` ยังเลือกได้ทีละเมือง · **ถอดเมื่อหน้านั้นเลือกหลายเมืองได้แล้ว**
                 */
                href={`/explore/${c.id}`}
                /* ชื่อการ์ดเป็น JSX (ธง + ชื่อ) ⇒ ลิงก์ดึงชื่อเองไม่ได้ · ส่งชื่อประเทศล้วนไปให้ */
                linkLabel={c.name_th}
                cover={<CountryThumb countryId={c.id} />}
                /**
                 * 🔴 **คงความสูงการ์ดเท่าเดิม แล้วจัด *ตัวอักษร* ในพื้นที่นั้นแทน** (ผู้ใช้สั่ง 4 ก.ย. 2026)
                 * > *"คุณปรับขนาดของ component ทำไม ควรจะเท่าเดิม และไปจัดที่ตัวอักษรแทน ว่าจะวางยังไง"*
                 *
                 * ⚠️ **ฉบับก่อนหน้าของผมผิดตรงนี้** — ถอดสองบรรทัดออกแล้ว *ปล่อยให้การ์ดเตี้ยลงเอง*
                 *    ⇒ เปลี่ยนขนาดของ component ทั้งที่ผู้ใช้ขอแค่ให้เอาข้อความออก
                 *    🎯 ***"เอาเนื้อหาออก" กับ "ย่อกล่อง" เป็นคนละคำสั่ง — และผมทำอย่างที่สองโดยไม่ได้ตั้งใจ***
                 *
                 * ✅ `min-h-[4.125rem]` = ความสูงเดิมของบล็อกข้อความ (ชื่อ + 2 บรรทัดที่ถอดออก)
                 *    `items-center` ⇒ ชื่อประเทศลอยอยู่ **กึ่งกลางแนวตั้ง** ของพื้นที่เดิม
                 *    🎯 หนึ่งบรรทัดในพื้นที่สามบรรทัด ถ้าชิดบน = ดูเหมือนของหาย · ถ้ากึ่งกลาง = ดูเหมือนตั้งใจ
                 * · ขนาดตัวอักษรใหญ่ขึ้น (`text-base sm:text-lg`) เพราะเป็นเนื้อหาเดียวที่เหลือ
                 *   **ไม่ได้ทำให้การ์ดสูงขึ้น** เพราะความสูงถูกตรึงด้วย `min-h` แล้ว
                 * · `truncate` ย้ายไปอยู่ span ชั้นใน — h3 กลายเป็น flex ⇒ ถ้าปล่อยไว้ที่ h3 มันจะไม่ตัดคำ
                 */
                titleClassName="flex min-h-[2.75rem] items-center text-sm font-bold sm:min-h-[4.125rem] sm:text-lg"
                title={
                  <>
                    <span aria-hidden className="mr-2 shrink-0">
                      {flagOf(c.id)}
                    </span>
                    <span className="truncate">{c.name_th}</span>
                  </>
                }
              />
            ))}
      </div>
    </div>
  );
}
