 ...
 * ```
 *   components/DayEventsPanel.tsx    ตารางบิน/เวลาตายตัวของวัน
 *   components/SortableStopRow.tsx   จุดแวะในหน้าแผน
 *   app/(app)/today/page.tsx         ลิสต์ "ถัดจากนี้" / "ผ่านมาแล้ว"
 *   app/(app)/summary/page.tsx       สรุปรายวัน
 * ```
 ...
 * · `corner` อยู่ขวาบนเสมอ — ไม่ว่าจะเป็น 🔒 (ล็อก) · ✏️ (ปรับได้) หรือที่จับลาก ⠿
 *
...
export function RowIconBox({ children }: { children: ReactNode }) {
  return (
    <span className="flex h-24 w-24 shrink-0 items-center justify-center rounded-lg bg-pine-soft/50 text-3xl">
      {children}
    </span>
  );
}
