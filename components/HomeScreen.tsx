      ...
      <div className="grid gap-3 [grid-template-columns:repeat(auto-fill,minmax(17rem,1fr))]">
        {templates.map((t) => (
          <CoverCard
            key={t.id}
            /**
             * 🔴 **ปลายทางมีแล้ว — ผูกได้** (`/templates/<id>` · 5 ก.ย. 2026)
             * ฉบับก่อนตั้งใจ *ไม่* ส่ง `href` เพราะ *"ไม่ผูกปลายทางที่ยังไม่มี"* — **เหตุผลนั้นหมดอายุแล้ว**
             * ⇒ การ์ดกลับไปเป็นลิงก์จริง: **เปิดแท็บใหม่ · คลิกกลาง · คัดลอกลิงก์ได้**
             *
             * 🎯 ***ปลายทางคือ *หน้าพรีวิว* ไม่ใช่ *ปุ่มก๊อปทันที* — ผู้ใช้สั่งเอง:***
             * > *"เมื่อกดจะบอกรายละเอียดของทริปทั้งหมด แต่ละวันไปไหนบ้าง **และมีปุ่มให้กดสร้างทริป**"*
             * ⇒ การ์ดไม่ควรสร้างอะไรเลย มันควรพาไปดูก่อน · **ก๊อปเกิดที่หน้าโน้น หลังเลือกวันเริ่มทริป**
             */
                  href={`/templates/${t.id}`}
            linkLabel={t.title}
...
export function HomeScreen() {
  ...
  return (
    <main className="min-h-full bg-surface pb-36 text-content lg:pb-24">
      {/* แถบหัวใบกลาง — เหตุผลของรูปทรงและประวัติการรื้ออยู่ที่ `components/SiteHeader.tsx` */}
      <SiteHeader>
