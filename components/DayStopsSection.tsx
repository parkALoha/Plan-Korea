  ...
  return (
    ...
    >
      ...
      >
        ...
        {day.noHotel ? (
          ...
          <div className="mt-1 text-xs opacity-90">
            {/* 🔴 เดิมขึ้น "⚠️ ยังไม่มีมื้ออาหารวันนี้" — **สามเหลี่ยมเตือนสำหรับของที่แค่ยังไม่ได้กรอก**
                ทริปที่เพิ่งสร้างจะขึ้นเตือนทุกวันพร้อมกัน ⇒ คำเตือนที่ขึ้นตลอดเวลาเลิกเป็นคำเตือน
                และมันกินความหมายของ ⚠️ ที่ใช้กับของที่ *ผิดจริง* ในการ์ดเดียวกัน */}
            {mealCount > 0 ? `🍽️ วางมื้อไว้ ${mealCount} มื้อ` : "🍽️ ยังไม่ได้วางมื้ออาหาร"}
          </div>
        )}
        ...
        {locked ? (
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs opacity-90">
            {/* ไม่ใส่ 🕐 นำหน้า — ช่อง `type="time"` มีไอคอนนาฬิกาของตัวเองอยู่แล้ว ใส่อีกจะได้นาฬิกาสองอันติดกัน */}
      ...
      <div className="lg:flex lg:items-start lg:gap-3 lg:px-3 lg:py-3">
        ...
        >
          {stops.length === 0 && (
            /* 🔴 เดิมเป็นประโยคเดียวยาว ๆ ที่ **อ้างชื่อปุ่มที่อยู่ห่างไป 2 บรรทัดและมองเห็นอยู่แล้ว**
               (*"กดปุ่ม “+ เพิ่มสถานที่ให้วันนี้” ด้านล่าง"*) — คำสั่งบอกทางจำเป็นเมื่อ
               ปุ่มหาไม่เจอ ไม่ใช่เมื่อมันอยู่ในสายตา · และมันดันคำใบ้ที่ *มีประโยชน์จริง*
               (ลากจากคลัง) ไปอยู่ท้ายประโยคที่คนอ่านข้ามไปแล้ว
               📌 คำใบ้เรื่องลาก ขึ้นเฉพาะ `lg` เพราะคลังด้านข้างมีเฉพาะที่นั่น — บอกเรื่องที่ทำไม่ได้
               บนจอที่กำลังใช้อยู่ คือการสอนสิ่งที่ผู้ใช้เอาไปใช้ไม่ได้ */
            <div className="px-4 py-5 text-center text-sm text-content-soft">
              {locked ? (
                "🔒 วันนี้ล็อกไว้ — ปลดล็อกที่หัวการ์ดถ้าจะเพิ่มจุดแวะ"
              ) : (
                <>
                  ยังไม่มีจุดแวะในวันนี้
                  <span className="hidden lg:inline"> · ลากจากคลังด้านขวามาวางก็ได้</span>
                </>
              )}
            </div>
          )}
