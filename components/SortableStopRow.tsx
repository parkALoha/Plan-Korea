          ...
          stop.kind === "intercity"
            ...
            : stop.kind === "hotel"
              ...
              : stop.kind === "transfer"
                ...
                : /* แถวสถานที่: ชื่อเรื่องคือชื่อสถานที่อยู่แล้ว บรรทัดรองจึงเป็น *คำใบ้ว่ากดได้*
                     แบบเดียวกับแถวเวลาตายตัว (ที่นั่นชื่อเรื่องคือชื่อเหตุการณ์ บรรทัดรองคือสถานที่)
                     🔴 ก่อนหน้านี้แถวนี้ไม่มีบรรทัดรองเลยเมื่อไม่มี `added_by` ⇒ สูงไม่เท่าแถวอื่น
                        และ **ไม่มีอะไรบอกว่าแตะแล้วเปิดรายละเอียดได้** ทั้งที่แตะได้ (ผู้ใช้สั่งให้เหมือนกันทั้งหมด 4 ก.ย. 2026) */
                  sched.place
                  ? `📍 แตะดูรูป/แผนที่/นำทาง${stop.added_by ? ` · เลือกโดย ${stop.added_by}` : ""}`
                  : undefined
        }
        /* 🔴 ใช้ช่อง `detail` เหมือนอีกสองลิสต์แล้ว (เดิมเป็น div พี่น้องที่ *ลอกคลาส* มาให้ตรงกัน)
             ⇒ ระยะขอบ/ขนาดตัวอักษร/สีของย่อหน้านี้มาจาก `TripListRow` ที่เดียว
             ของเดิมเยื้อง `pl-10 sm:pl-14` ซึ่งจัดให้ตรงกับคอลัมน์ [ที่จับ][เวลา][รูป] แบบเก่า
             **ที่ไม่มีอยู่แล้วหลังยุบเข้า TripListRow** ⇒ ดูไม่เหมือนแถวเวลาตายตัวทั้งที่ใช้ตัวเดียวกัน
             (ผู้ใช้ทัก 4 ก.ย. 2026) · โน้ต + ปุ่มปรับเวลาต้องอยู่แถวเดียวกัน จึงห่อ flex ข้างในเอง
             ไม่แยกไป `actions` ซึ่งเป็นแถวของมันเอง */
        detail={
