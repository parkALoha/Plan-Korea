...
function CityThumb({ slug, countryId }: { slug: string | null | undefined; countryId: string }) {
  ...
  return (
    ...
    <img
      ...
      /**
       * 🔴 **กริดนี้ไม่ได้โตตามจอแบบเส้นตรง — วัดสี่จุดแล้ว ไม่ได้อนุมานจาก `minmax`** (P2 เจอที่หน้าเขาก่อน · ผมวัดหน้านี้ซ้ำ)
       * ```
       *  375px → กล่อง 164   2 คอลัมน์
       *  640px → กล่อง 296   2 คอลัมน์
       *  768px → กล่อง 360   2 คอลัมน์   ← **ใหญ่สุด** ทั้งที่จอยังไม่กว้างสุด
       * 1023px → กล่อง 278   3 คอลัมน์
       * 1440px → กล่อง 278   3 คอลัมน์   (`max-w-4xl` ตรึงไว้)
       * ```
       * 🎯 ***จำนวนคอลัมน์เปลี่ยน *ก่อน* ความกว้างจะตัน ⇒ ช่วงแท็บเล็ตการ์ดใหญ่กว่าตอนจอกว้าง***
       * · 🔴 ฉบับแรกของผมเขียน `280px` สำหรับทุกจอที่ไม่ใช่มือถือ ⇒ **ขอรูปเล็กเกินไปที่ 768**
       *   (ที่ DPR1 ยังจบที่ใบเดียวกัน แต่ DPR1.5 จะได้ภาพนุ่ม — ผิดในทิศที่มองไม่เห็นจากจอที่ผมวัด)
       * · ⚠️ ค่านี้ **ขอเกินเล็กน้อยในช่วง ~900–1023** (บอก 360 แต่จริง 278) — ตั้งใจ:
       *   ***ขอเกิน = เปลืองไบต์ · ขอขาด = ภาพนุ่ม*** ⇒ เลือกพลาดไปทางที่แก้ได้ด้วยเงิน ไม่ใช่ทางที่ผู้ใช้เห็น
       */
      sizes="(max-width: 639px) 45vw, (max-width: 1023px) 360px, 280px"
      alt=""
      /**
       ...
       *
       * 🔴 **ตัวที่ประหยัดจริงคือ *ขนาดไฟล์* ไม่ใช่ *จังหวะโหลด*** — **ทำไปแล้วทั้งสองขั้น ดู `srcSet`/`sizes` ข้างบน**
       * ```
       * ① ย่อต้นทาง 1200 → 800   คลัง 16.7MB → 9.2MB   (3514f41)
       * ② srcset 400/800         /explore/jp มือถือ **2774KB → 815KB (−71%)**  (วัดด้วย fetch cache:'reload')
       * ```
    ...
    <main className="mx-auto w-full max-w-4xl px-4 py-6 pb-24 sm:py-10 lg:pb-10">
      ...
      {selected.length > 0 && (
        ...
        >
          ...
          {/**
           * 🔴 **เป้าแตะ 44px และระยะห่าง 8px — ไม่ใช่ค่าที่เลือกให้สวย** (P4 วัด · P2 เคาะ · 6 ก.ย. 2026)
           * ```
           * เดิม  ชิป 74×30 · ห่างกัน 6px · พื้นผิว **ทั้งใบ** คือปุ่มลบ · กดแล้วหายทันที ไม่มี undo
           * ```
           * 🎯 ***ตัวที่ทบราคาคือ ชิปที่เหลือ **เลื่อนมาอยู่ตำแหน่งเดิม*** ⇒ กดพลาด → รีบกดซ้ำเพื่อแก้
           *    → **ลบใบถัดไปที่เพิ่งเลื่อนเข้ามาแทน** (ถ้อยคำ P4)
           * · `min-h-11` = 44px · ตัวอักษรเท่าเดิม (ขยายเป้า ไม่ขยายตัวหนังสือ)
           *
           * 🔴 **ตั้งใจ *ไม่* ทำ undo / ไม่ถามยืนยัน — และนี่คือเหตุผล ไม่ใช่การละเลย** (P2 ตัดสิน)
           *    ***การลบนี้ย้อนเองได้ในสองแตะ เพราะกริดเมืองอยู่บนจอเดียวกัน***
           *    ⇒ undo จะเพิ่ม toast + state ให้กับการกระทำที่ผู้ใช้แก้เองได้อยู่แล้ว
           * · ⚠️ **เงื่อนไขที่ทำให้คำตัดสินนี้พลิก: วันที่ลิสต์ยาวจนกริดเมืองเลื่อนพ้นจอ**
           *   ตอนนั้น "แก้เอง" ไม่ใช่สองแตะอีกต่อไป — **กลับมาคิด undo ใหม่ อย่าอ่านบรรทัดนี้ว่าปิดประตูถาวร**
           */}
            ...
            {selected.map((c, i) => (
              <li key={c.id}>
                ...
                <button
                  type="button"
                  onClick={() => toggleCity(c)}
                  className="flex min-h-11 items-center gap-1 rounded-full border border-line px-2.5 py-1 text-xs"
