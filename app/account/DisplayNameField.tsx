  ...
  return (
    <div>
      ...
      {/* 🔴 **บอกเพดานตั้งแต่ยังไม่ชน** (ผู้ใช้ทัก 4 ก.ย. 2026 — พิมพ์ยาวแล้วไม่มีอะไรเตือนเลย)
          เดิมกฎ 60 ตัวมีอยู่จริงและบังคับจริงทั้งสองฝั่ง **แต่ผู้ใช้ไม่มีทางรู้จนกว่าจะพิมพ์ถึง 46 ตัว**
          (ตัวนับโผล่ที่ `NAME_MAX - 15`) ⇒ คนที่พิมพ์ 31 ตัวเห็นหน้าจอที่บอกว่า *ไม่มีขีดจำกัด*
          ...
          · ตัวเลขมาจาก `NAME_MAX` ตัวเดียวกับที่ใช้ตรวจ **ไม่ใช่เลขที่พิมพ์มือ** — ไม่งั้นวันที่เพดานเปลี่ยน
            ข้อความนี้จะกลายเป็นเท็จเงียบ ๆ · เพดานฝั่งเซิร์ฟเวอร์อยู่ที่ `api/engine/profile/route.ts:22`
            **สองที่ต้องเท่ากันเสมอ** (ทั้งคู่นับด้วย `Array.from` แล้ว) */}
