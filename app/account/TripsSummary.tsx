 ...
 *
 * ## 📌 ไฟล์นี้เข้าประวัติมาทางไหน — จดไว้เพราะ `git log` ตอบเองไม่ได้
 * commit แรกของไฟล์นี้คือ **`ea88f77`** ซึ่งชื่อเรื่องพูดถึง *หน้าแรก* ไม่ใช่หน้าบัญชี
 * · ผม `git add` ไฟล์นี้ไว้ **แล้วไปเขียนข้อความ commit ต่ออีกพักหนึ่ง** · ระหว่างนั้นเจ้าของ `ea88f77`
 *   รัน `git commit` ที่เก็บทั้ง index ⇒ ไฟล์นี้ติดไปด้วย 109 บรรทัด · **ตรวจแล้วเนื้อครบ ไม่เสียหาย**
 * · 🔴 **ไม่ใช่ความผิดของคนกด** — เขาไม่มีทางรู้ว่ามีของใครค้างอยู่ใน index กลาง และข้อความ commit
 *   ของเขาไม่ได้พูดถึงหน้านี้เลย ⇒ **ช่วงที่ของค้างอยู่ ผมเป็นคนเปิดเอง**
 * 🎯 ***กติกาที่ต้องใช้มีอยู่แล้ว ผมแค่ไม่ได้ทำตาม: "แก้แล้ว commit ทันทีในคำสั่งเดียว
 * อย่าค้างไว้ระหว่างขั้นตอนตรวจ"*** (P2 เขียนไว้ที่ `TEAM.md §3.2` — `db71f18`)
 * · วันเดียวกันเกิดกลับด้านด้วย: ผมกวาดกติกาของ P2 เข้าใบผม (`eb76bfd` · เขาลงรอยไว้ที่ `5e99e08`)
 *   ⇒ **สองครั้ง คนละทิศ กลไกเดียวกัน** — รูปของเครื่องมือ ไม่ใช่ความเลินเล่อของใครคนหนึ่ง
 */
