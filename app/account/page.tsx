 ...
 *
 * 🔴 **ตั้งแค่ชื่อหน้า ไม่ต่อชื่อเว็บเข้าไปเอง** — root คือ `title: "luitrip"` เฉย ๆ ไม่มี `template`
 * ⇒ ถ้าผมเขียน `"บัญชีของฉัน · luitrip"` ตรงนี้ ก็คือ **วางสำเนาของชื่อแบรนด์ไว้อีกที่**
 * ซึ่งชื่อนั้น **เพิ่งเปลี่ยนวันนี้เอง** (`แพลนเที่ยวเกาหลี` → `luitrip` · 4 ก.ย. 2026)
 * 🎯 ***สำเนาที่ต้องมีคนซิงก์ จะล้าเสมอ*** — และคนที่เปลี่ยนชื่อแบรนด์รอบหน้าจะไม่เปิดไฟล์นี้
 *
 * ✅ ทางที่ถูกจริงคือ `title: { template: "%s · luitrip", default: "luitrip" }` ที่ **root layout**
 * แล้วทุกหน้าตั้งแค่ชื่อตัวเอง · **`app/layout.tsx` เป็นโซน P3 — ส่งให้เขาตัดสิน ไม่ใช่แก้เอง**
 */
