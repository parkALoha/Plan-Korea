 ...
 *
 * ## 🔴 `409` เมื่อวันที่จะหายมีจุดแวะอยู่ — **เป็นการปฏิเสธถาวร ไม่มี `force`**
 * ย่อทริปจาก 7 วันเหลือ 5 = จุดแวะของสองวันนั้นจะหายไปด้วย ⇒ ตอบ `409` พร้อมจำนวนจุดและวันที่
 * · ⚠️ นับ **ทุกแผน** ไม่ใช่แค่แผนที่ active — ของในแผน B ก็หายเหมือนกัน และผู้ใช้มองไม่เห็นมันตอนกด
 *
 * ### 🔴 ฉบับแรกมี `force: true` ให้ข้ามด่านนี้ — **มันเดินไม่ถึงปลาย และผมเขียนคำอธิบายที่ผิดไว้ด้วย**
 * ผมเขียนว่า *"ฐาน **ไม่กันให้และไม่ควรกัน** เพราะบางครั้งผู้ใช้ตั้งใจจริง"* — **ครึ่งแรกเป็นเท็จ**
 * ```
 * 20260825142639:94-105   trigger `app.assert_day_has_no_stops()` before delete on trip_days
 *   'ลบวันที่ยังมีจุดแวะอยู่ไม่ได้ — ย้ายหรือลบจุดแวะก่อน (cascade จะลบทิ้งเงียบ ๆ)'
 * ```
 * ⇒ `force: true` ⇒ ข้าม `409` ⇒ แก้ช่วงวันสำเร็จ ⇒ **ลบวันล้มด้วย `P0001`** ⇒ `502` + `partial: true`
 * 🎯 ***ผู้ใช้ยืนยันว่า "ทิ้งได้" แล้วได้ error กับทริปที่อยู่ครึ่งทาง — แย่กว่าไม่ให้ยืนยันตั้งแต่แรก***
 * · P4 ยิงเจอ (⑪ · 4 ก.ย. 2026) · trigger มีมาก่อนใบนี้ 10 วัน **ไม่ใช่ regression ของ migration ผม**
 *   `grant delete on trip_days` ที่ผมเปิดถูกต้อง — มันแค่ไม่พอ เพราะมี trigger ทับอยู่อีกชั้น
 * · 🎯 ***และ `D73` เขียนถ้อยคำนี้ไว้แล้วตั้งแต่ต้น*** (*"ให้ตัวปรับช่วงวัน **ปฏิเสธ** วันที่ยังมีจุดแวะ"*)
 *   — P4 ทักตอนรีวิวว่าผม **ผ่อนจาก "ปฏิเสธ" เป็น "เตือนแล้วผ่านได้"** · ฐานบังคับถ้อยคำเดิมอยู่แล้ว
 *   ⇒ **เราไปเขียนทางที่มันไม่เคยเปิด**
 *
 * ### ทำไมเลือกถอด `force` แทนที่จะทำให้มันเดินได้
 * ทางที่จะทำให้ `force` มีความหมายจริงคือ **soft-delete จุดแวะของวันนั้นก่อนแล้วค่อยลบวัน**
 * ⇒ เป็นการลบข้อมูลผู้ใช้จริง (กู้คืนได้ที่ชั้นฐาน **แต่ไม่มี UI ให้กู้**) ⇒ **เป็นฟีเจอร์ ไม่ใช่การแก้บั๊ก**
 * 🔴 **ถอด `force` ปิดสภาพครึ่งทางได้ทั้งหมด** — `409` เกิด *ก่อน* เขียนอะไรเลย (ดูลำดับข้างล่าง)
 * · ผู้ใช้ยังย่อทริปได้ **แค่ต้องเอาจุดแวะออกก่อน** และ `409` บอกครบว่ากี่จุด วันไหนบ้าง
 * · 📌 ถ้าจะทำทางแรกวันหลัง **ให้เป็นใบของตัวเอง** พร้อม UI ที่บอกว่าจะลบอะไรบ้าง
 */
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ tripId: string }> }) {
  ...
  // ── ด่านข้อมูลหาย — **ปฏิเสธ ไม่ใช่เตือน** · เกิดก่อนเขียนอะไรทั้งสิ้น ⇒ ไม่มีสภาพครึ่งทาง
  if (toRemove.length > 0) {
    ...
    if (losing > 0) {
      return NextResponse.json(
        {
          // 🔴 ข้อความต้องบอก *ทางออก* ไม่ใช่แค่ *ข้อห้าม* — ผู้ใช้ทำต่อเองได้ทั้งหมด
          error:
            `ย่อช่วงวันนี้ไม่ได้ เพราะมีจุดแวะ ${losing} จุดอยู่ใน ${toRemove.length} วันที่จะถูกถอน` +
            " — ย้ายหรือลบจุดแวะพวกนั้นก่อน แล้วลองใหม่",
          // ⚠️ ยังไม่มีอะไรถูกเขียนลงฐาน ณ จุดนี้ · ทริปยังเหมือนเดิมทุกอย่าง
          nothingWritten: true,
  ...
  if (toRemove.length > 0) {
    ...
    if (error) {
      return NextResponse.json(
        {
          // 🔴 ถึงตรงนี้ได้ = มีของมาถึงระหว่างที่เราตรวจ (`P0001` จาก trigger) หรือสิทธิ์เปลี่ยน
          //    **ไม่ใช่สภาพปกติอีกต่อไปหลังถอด `force`** — ด่าน `409` ข้างบนควรกันไว้หมดแล้ว
          //    ⚠️ "กดอีกครั้ง" ยังถูก: รอบถัดไปจะเห็นจุดแวะนั้นแล้วตอบ `409` แทนที่จะพยายามลบซ้ำ
          error: "แก้ช่วงวันแล้ว แต่ถอนวันส่วนเกินไม่สำเร็จ — กดบันทึกอีกครั้งเพื่อดูว่าติดอะไร",
          code: error.code,
          partial: true,
        },
        { status: 502 },
