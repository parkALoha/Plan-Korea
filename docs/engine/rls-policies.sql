...
--    (โปรเจกต์ Supabase ของทริปจริง) ซึ่งจะ **ล็อกคน 2 คนออกจากเว็บทันที** เพราะทุก policy ข้างล่างนี้
--    เป็น `to authenticated` แต่เว็บทริปวันนี้ยังคุยกับ Supabase ด้วย anon key ล้วนๆ
--    → ปลายทางที่ถูกต้องคือ **โปรเจกต์ Supabase คลาวด์ใน organization ใหม่** ของระยะ 2 เท่านั้น
--      (D14 เปลี่ยนจากมติ Docker เดิม) · 🔴 ห้าม `supabase link` โฟลเดอร์นี้เข้าโปรเจกต์ใด
--
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- ส่วนที่ 12 — รีวิวใหม่หลัง D6/D7 · **แก้ข้อสรุปของไฟล์นี้ 1 ข้อ**
-- ═══════════════════════════════════════════════════════════════════════════════
--
-- ผมเขียนไว้เองใน security-review.md §9 ว่าถ้า schema ต่างจากบรีฟ **ต้องรีวิวใหม่ทั้งชุด
-- ไม่ใช่แก้ทีละชื่อคอลัมน์** · D6 (rank key) กับ D7 (updated_at จาก trigger + soft delete
-- ทุกตารางผู้ใช้) เข้าข่ายนั้น นี่คือผลการรีวิวใหม่

-- ── 12.1 🔴 soft delete เปลี่ยน DELETE ให้เป็น UPDATE — DELETE policy 13 ตัวเลิกเป็นด่านของการลบ
--
-- นี่เป็นผลที่ต้องพูดก่อนอย่างอื่น เพราะกระทบทุกตารางในไฟล์นี้พร้อมกัน:
-- พอมี soft delete client ไม่ยิง `DELETE` อีกแล้ว มันยิง `UPDATE ... set deleted_at = now()`
-- → **UPDATE policy กลายเป็นด่านของการลบ ส่วน DELETE policy ไม่ได้กันการลบอีกต่อไป**
--
-- ตรวจแล้วว่าโมเดลสิทธิ์ **ไม่พัง**: ทั้ง UPDATE และ DELETE ของทุกตารางเนื้อหาใช้
...
--    (ข้อจำกัดเดิมของ RLS ที่ส่วนที่ 0 ระบุไว้ กลับมาเป็นเรื่องจริงอีกครั้งที่นี่)

-- ── 12.2 อ่านแถวที่ถูก soft delete ได้ไหม — ต้องตัดสิน ไม่ใช่ปล่อยให้เป็นผลข้างเคียง
--
-- ถ้ากรอง `deleted_at is null` แค่ในคำสั่งของแอป **client ยิง REST ขอแถวที่ลบแล้วได้ตรงๆ**
-- (`?deleted_at=not.is.null`) · ไม่ใช่ cross-tenant (ข้อมูลของทริปตัวเอง) แต่สำหรับ
-- `bookings`/`bookings_secret` ที่ถือชื่อตามพาสปอร์ต "ลบแล้วยังอ่านได้" คือเรื่องที่ผู้ใช้
-- จะไม่คาดคิด · เสนอ RESTRICTIVE policy ให้เห็นเฉพาะ owner:
--
--   create policy "bookings: hide soft-deleted from non-owners"
--     on public.bookings as restrictive for select to authenticated
--     using (deleted_at is null or app.is_trip_owner(trip_id));
--
-- ปิดช่อง: สมาชิกทั่วไปขุดของที่ถูกลบขึ้นมาอ่าน · ยังเปิดทางให้ owner ทำ UI "กู้คืน" ได้
-- (ถ้าไม่มีเงื่อนไข owner จะกู้คืนไม่ได้เลยเพราะมองไม่เห็นว่ามีอะไรถูกลบ)

-- ── 12.3 🔴 partial unique index — ถ้าไม่ทำ ฟีเจอร์พังทันทีในการใช้งานปกติ
--
-- 4 ตารางมี PK ที่เป็นคีย์ธรรมชาติ ไม่ใช่ surrogate id:
--   trip_settings (trip_id) · place_notes (plan_id, place_id)
--   · trip_hidden_places (trip_id, place_id) · travel_time_cache (from, to, mode)
-- soft delete แถวหนึ่งแล้วสร้างคีย์เดิมซ้ำ = **ชน PK ทันที**
-- ลำดับที่ผู้ใช้ทำได้ในไม่กี่วินาที: **ซ่อนสถานที่ → เลิกซ่อน → ซ่อนอีกครั้ง = พัง**
-- → ต้องเป็น partial unique index แทน PK บนคีย์ธรรมชาติ:
--   create unique index trip_hidden_places_live_key
--     on public.trip_hidden_places (trip_id, place_id) where deleted_at is null;
-- (`travel_time_cache` ย้ายไป schema `cache` แล้วและไม่ควรมี soft delete เลย — ส่วนที่ 7)

-- ── 12.4 D7 ช่วยแก้ F2 ให้ครึ่งหนึ่ง — ข่าวดีที่ควรบันทึก
--
-- F2 (ส่วนที่ 10) บอกว่า DELETE event จะหายเงียบเมื่อเปิด RLS เพราะ payload มีแค่ PK
-- ให้ policy ประเมินไม่ได้ · **soft delete ทำให้การลบที่ผู้ใช้กดกลายเป็น UPDATE**
-- ซึ่ง WAL ส่งแถวใหม่มาเต็มเสมอ → policy ประเมินได้ → **event ไม่หาย**
-- → F2 เหลือขอบเขตแค่ **hard delete ของ purge job** ซึ่งไม่มี client ไหน subscribe รออยู่
-- 🟢 ยัง**ควรตั้ง `replica identity full` อยู่** (ถูกกว่าการค้นพบทีหลัง) แต่ความเร่งด่วนลดลงมาก
--    · เคส T-14 ยังต้องรัน แต่ย้ายจาก "ข้อแรกของ E1" เป็นของ E2 ได้

-- ── 12.5 D6 rank key — 2 อย่างที่ RLS ทำไม่ได้ ต้องเป็น CHECK constraint
--
-- `rank` เป็น `text` ที่ **client เป็นคนคำนวณและส่งมา** (ต่างจาก `order_index` ที่เป็น int
-- ซึ่งจำกัดตัวเองอยู่แล้ว) → เปิด 2 ช่องที่ policy กันไม่ได้เลย:
--
-- ① **ความยาวไม่จำกัด** — เขียน rank ยาวเป็นเมกะไบต์ได้ · D6 ระบุเองว่าแทรกซ้ำที่เดิมทำให้
--    คีย์ยาวขึ้นเรื่อยๆ จนต้อง rebalance → ค่าที่ยาวผิดปกติเป็นได้ทั้งการโจมตีและบั๊กของ client
--   check (length(rank) between 1 and 64)
--
-- ② 🔴 **ชุดอักขระ** — `COLLATE "C"` เรียงตามไบต์ ส่วน JS เรียงตาม UTF-16 code unit
--    **ตรงกันเฉพาะเมื่อ rank เป็น ASCII** · ถ้ามีอักขระนอก ASCII หลุดเข้าไปแม้ตัวเดียว
--    ลำดับใน DB กับลำดับที่ client คำนวณจะ**ต่างกัน** → จอ 2 เครื่องเห็นลำดับไม่เหมือนกัน
--    โดยไม่มีอะไร error · อาการจะเหมือนบั๊ก 8.1 ที่ D6 ตั้งใจปิด แต่หาสาเหตุยากกว่ามาก
--   check (rank ~ '^[0-9A-Za-z]+$')
--
-- ทั้งสองข้อเป็น CHECK ไม่ใช่ policy เพราะเป็นเรื่อง "ค่าที่ยอมรับได้" ไม่ใช่ "ใครแตะได้"
-- · index `(trip_day_id, rank)` ของ D6 ควรเป็น partial `where deleted_at is null` เมื่อมี soft delete
-- · ยืนยันว่า index นี้ **ต้องไม่ unique** — D6 ให้เรียงด้วย `(rank, id)` ซึ่งแปลว่ายอมให้ rank ชนกันได้

-- ── 12.6 soft delete กับไฟล์ใน Storage — ต้องมีคนเป็นเจ้าของคำตอบ
--
-- security-review.md §6.3 ข้อ 5 เสนอว่า "ลบ booking แล้วต้องลบไฟล์จริง"
-- **ขัดกับ soft delete ตรงๆ**: ถ้าแถวยังอยู่รอ purge ไฟล์ก็ต้องอยู่ ไม่งั้นกู้คืนมาแล้วไฟล์หาย
-- → ผลคือ **"ผู้ใช้กดลบตั๋วแล้ว แต่รูปที่มีชื่อตามพาสปอร์ตยังอยู่ในระบบ"** จนถึงวัน purge
-- นี่ไม่ใช่บั๊ก แต่เป็น **นโยบายเก็บรักษาข้อมูลที่ต้องมีคนตัดสินและบอกผู้ใช้**
-- (จะเก็บกี่วัน · กู้คืนได้จริงไหม · purge แล้วลบไฟล์ด้วยไหม) → ฝาก P8 เขียนลง backlog
-- ⚠️ ห้ามปล่อยให้เป็นค่าที่ติดมาจากการ implement — เป็นคำถามที่ผู้ใช้ควรได้ตอบ
