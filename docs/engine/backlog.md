...
- [ ] **E1-AC1** A ล็อกอินสำเร็จ · `auth.uid()` ใน DB ไม่ใช่ `null` · session อยู่ข้ามการรีเฟรชหน้า
- [ ] **E1-AC2** มี 3 ตาราง `profiles` · `trips` · `trip_members` และมี RLS จริงที่อ้าง `auth.uid()` — วัดด้วย `grep -c 'auth.uid' <migration ใหม่>` ต้อง **> 0** (วันนี้ทั้ง 31 migrations ได้ **0** — นับเองยืนยันแล้ว)
- [ ] **E1-AC3** **C ยิง `GET /rest/v1/trips` ด้วย JWT ของตัวเอง ได้ `[]`** ไม่ใช่ทริปของ A
- [ ] **E1-AC4** **D (ไม่มี JWT / anon key เปล่า) ยิงตารางทั้ง 3 ได้ 401 หรือ `[]` ทุกกรณี** — ปิดช่อง B3 สำหรับ 3 ตารางนี้
...
**3.1 🔴 งานใหม่ที่ `D42` สร้างให้ผู้ใช้ และไม่มีในแผนเดิม — ตั้ง Google OAuth consent screen**
