...
- [ ] **E2-AC6** 📋 **เกณฑ์ที่ยกความเสี่ยงของ E7 มาไว้ที่นี่:** มีตารางแมป **ทุกคอลัมน์ของ 14 ตารางเดิม → ปลายทางในสคีมาใหม่** และคอลัมน์ไหนที่ตั้งใจทิ้งต้องเขียนเหตุผลกำกับ · **ห้ามมีคอลัมน์ไหนที่ไม่มีคำตอบ** (ถ้าไม่ทำข้อนี้ที่ E2 จะไปรู้ตอน E7 ว่าสคีมาใหม่เก็บของเดิมไม่ได้ ซึ่งแปลว่าต้องรื้อ E2–E5 ใหม่ทั้งหมด)
      ...
      🔴 **`E2-AC6` เหลือบล็อกเดียวคือ `Q7`** — `bookings` เก็บ `DayEvent` ได้แค่ 3 จาก 15 ฟิลด์ (`anchor` ขอบของ timeline ทั้งวัน · `dayOffset` วันบินเข้าประเทศคลาดไป 1 วัน · `flight` หน้า ตม.) ไม่มีที่ไป — DB ครบ 111/111 · `places`/`transferPoints`/`emergency`/`airportAccess`/`itinerary` (ยกเว้น `day.events`) ครบหมด · P7/P5 กำลังตอบอยู่ (`9447101`/`1165c3c`)
