...
- [ ] **E4-AC1** 🔴 **เกณฑ์ผ่านที่ P1 กำหนดเอง:** สร้างทริปญี่ปุ่นได้โดยไม่แก้โค้ดสักบรรทัด · **วิธีวัดที่ผมเสนอ:** เพิ่มประเทศใหม่ = แก้ข้อมูลใน DB/config 1 ที่ แล้ว `git diff --stat` ของโฟลเดอร์โค้ดต้องได้ **0 ไฟล์**
      🔴 **แก้ 26 ส.ค. 2026 (P1 ชี้ — บันทึกความสัมพันธ์ที่ไม่มีเอกสารไหนเขียนไว้):** ข้อนี้ **บล็อกโดย `E5`** — เกณฑ์ถามว่า "สร้างทริปที่สองได้โดยไม่แก้โค้ดไหม" แต่ถ้า **เปิดทริปที่สองไม่ได้เลย** (ยังไม่มี `/trip/[tripId]` — ดู `E5-AC1`) **ก็ทดสอบไม่ได้ว่าเกณฑ์นี้ถูกหรือผิด** ไม่ใช่แค่ทำก่อนไม่ได้ แต่**วัดผลก่อนไม่ได้เลย** — เรียง `E5` ก่อน `E4-AC1` ในทางปฏิบัติแม้ตำแหน่งเฟสจะสลับกัน
- [ ] **E4-AC2** 🔴 **ยังไม่ปิด — เกือบถูกปิดผิด 27 ส.ค. 2026 (P1 ตรวจซ้ำแล้วเจอเอง):** `countryOfCity(c) { return c === "hanoi" ? "vn" : "kr" }` ไม่มีอยู่ในโค้ดแล้ว (B6) · `grep -rn 'hanoi' lib/ app/` ไม่เจอตรรกะตัดสินใจ เจอได้แค่ข้อมูล
      ...
      🔴 **แก้ 27 ส.ค. 2026 — ของจริงยังอยู่ครบทีละตัวอักษรที่ `data/emergency.ts:58` จนถึงเมื่อชั่วโมงก่อน และไม่ใช่โค้ดตาย:** ผู้เรียก 3 จุดคือชั้นหลายประเทศทั้งชั้น — การ์ดฉุกเฉิน · `mapActionsFor()` · `shouldSkipTravelApi()` **ทะเบียนที่สร้างมาเลิกเดาประเทศ ถูกป้อนด้วยการเดาประเทศที่มันมาแทนอยู่ดี** แก้แล้วเป็นตาราง `Record<City, CountryCode>` + `countryOfCitySlug()` ที่คืน `null` เมื่อไม่รู้จัก · **เหลือค้างบรรทัดเดียว:** `app/page.tsx:152` `ITINERARY.find(d => d.city !== "hanoi")` (แจ้ง P3 แล้ว)
      🔴 **`P-61` ครั้งใหม่ (P1 เป็นคนก่อเอง 27 ส.ค. 2026):** เกณฑ์บอกให้วัดด้วย `grep -rn 'hanoi' lib/ app/` — **`data/` ไม่อยู่ในขอบเขตที่พิมพ์ไว้** ถ้าใครทำตามเกณฑ์ทุกตัวอักษรจะติ๊กผ่านโดยที่ของอยู่ตรงหน้า P1 เองก็ทำแบบนั้นจริง (รัน `grep lib/ app/ components/` แล้วรายงานกับตัวเองว่า "ไม่มีนอกทะเบียน") 🎯 **เกณฑ์ที่พก `grep` ของตัวเองมาด้วย ให้ความมั่นใจเท่ากับที่มันปิดตา** — ขอบเขตที่พิมพ์มือไม่มีใครตรวจ · **ข้อเสนอของ P1 (ยังไม่ตัดสิน — ส่งต่อทีมพิจารณา):** เกณฑ์ที่วัดด้วย `grep` ทุกข้อ ควรใช้ `git ls-files` เป็นขอบเขต ไม่ใช่รายชื่อโฟลเดอร์พิมพ์มือ
- [ ] **E4-AC3** registry บอกความสามารถรายประเทศได้ — เกาหลี: `TRANSIT` ✅ / `DRIVE`,`WALK` ❌ (ข้อจำกัดกฎหมายเกาหลี ยืนยันด้วยการทดสอบจริงแล้ว `PLAN.md §2`) · เวียดนาม: ครบ · UI แสดงป้าย "(ประมาณการ)" จาก registry ไม่ใช่จาก `if (country === "kr")`
      🔴 **แก้ 27 ส.ค. 2026 — วัดจริงแล้ว (P1 · ครบทั้ง 173 ไฟล์ที่ `git` ติดตาม) ไม่มี `"kr"` นอกทะเบียน+ตารางข้อมูลเลย แต่ห้ามติ๊ก ✅ ตอนนี้:** ⚠️ **ก่อนแก้ `E4-AC2` ให้ครบ นี่คือการปิดหลอก** เพราะ input ของหน้าจอคือเทอร์นารีที่ยังไม่หายไปทั้งหมด — จดคู่กันไว้ ปิดพร้อมกับ `AC2` เท่านั้น
