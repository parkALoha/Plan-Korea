...
🎯 **ปัญหาจริงคือช่องติ๊กมีสองค่าแต่เรามีสามสถานะ** — คืนนี้พิสูจน์ให้เห็นชัด:
| สถานะ | จำนวน | อาการ |
|---|---|---|
...
ทั้งสามกลุ่มหน้าตาเหมือนกันเป๊ะในไฟล์เดิม (`[x]` ล้วน) — **แก้ด้วยป้าย `🏷️ ด่าน: มี` / `ด่าน: ไม่มี (หลักฐาน ณ <วันที่>)` / `ด่าน: ยังไม่ตรวจ (ต้องใช้ฐาน)` ที่ตัว AC เอง** (แปะไว้แล้วทั้ง 11 จุดข้างบน + `E2-AC8`) — ป้ายนี้ตอบคำถามที่ % ตอบไม่ได้: **"ถ้าพรุ่งนี้มีคนละเมิด เราจะรู้ไหม"**
