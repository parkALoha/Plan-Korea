...
- [ ] **E7-AC8** 🔴 **ด่านแรกสุด — ไม่ผ่านข้อนี้ ห้ามแตะ schema ของ DB ทริปแม้แต่คำสั่งเดียว (`D21`)**
      ...
      **③ แยกคำถามที่ถูกมัดไว้ด้วยกัน** — "dump นี้ restore ได้จริงไหม" (ไม่ต้องใช้โควตา org) ≠ "มีโปรเจกต์ที่ 2 ไว้ซ้อม migrate ไหม" (ต้องใช้ — `AC1`/`AC6` พึ่งมัน) เกณฑ์เดิมบังคับตอบข้อหลังก่อนถึงจะตอบข้อแรกได้ **รูปเดียวกับ "เห็นครบ 11 วัน" ที่แก้ไปรอบ 84 แค่ครึ่งเดียว** — บรรยายสภาพปลายทาง (มีโปรเจกต์พร้อมแล้ว) แล้วใช้เป็นด่านแรกสุด
      ...
      · [ ] **② โปรเจกต์ที่ 2 พร้อมซ้อม migrate** — เงื่อนไขแยกต่างหาก ใช้ตอนเริ่ม `AC1`/`AC6` ไม่ใช่ส่วนหนึ่งของ "เป็น backup หรือไม่" **ยังเปิด ไม่เปลี่ยน** — บล็อกด้วยโควตา org เต็ม (2 โปรเจกต์เต็มแล้ว: `engine-dev` ACTIVE + ของค้างชื่อ `muphoneseller` INACTIVE)
...
| `E7` | 0/9 | ด่านแรก `AC8` (ห้ามแตะ schema DB ทริปจริง) — งานเรฮาร์เซิลบน `engine-dev` มีจริง (868 แถว/9 ก้อน) **แต่ยังไม่ติ๊กสักข้อ** ตามที่ตัดสินไว้ก่อนหน้า (ข้อมูลย้ายบน dev ≠ เกณฑ์ของ AC ที่ต้องการ "โปรเจกต์ซ้อมจาก snapshot" + verify เต็มรูป) |
| `E9` | 0/8 | `AC1` ระบุชัดว่า `blocked-on-E7` (สูตร freeze window ต้องมีเวลาจริงของ `E7-AC7` มาคูณ — ยังไม่มีเพราะ `E7` ยังไม่เคยรันแบบนับเวลาได้) ที่เหลือยังไม่เริ่มเลย |

**M1 รวม: 49/74 ≈ 66%** (ไม่รวม `E4-AC1`/`E4-full` ในตัวส่วนตามนิยาม M1)
