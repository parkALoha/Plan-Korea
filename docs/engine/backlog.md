...
> **ระยะออกแบบเท่านั้น** · เอกสารนี้ไม่มีโค้ดที่รันได้ · อ่าน `docs/engine/README.md` (กติกาเหล็ก 5 ข้อ) ก่อน
...
ทุกเฟสต้องผ่าน **6 ข้อนี้** ก่อนถือว่าจบ นอกเหนือจาก AC เฉพาะเฟส

- [ ] **DoD-1** `npm run lint` ผ่านสะอาด ไม่มี warning ใหม่
- [ ] **DoD-2** `npm test` ผ่าน — **13 ไฟล์เทสต์เดิมห้ามแดงสักไฟล์** (`lib/__tests__/`)
...
> 📌 **หมายเหตุการตั้งชื่อ:** DoD ในไฟล์นี้ใช้ `DoD-1`…`DoD-7` · **ไม่ใช่ `D1`…`D7`** ซึ่งเป็นเลขข้อตัดสินของ P1 ใน `README.md`
...
> 🔴 ทุกเฟสในหัวข้อนี้ **เริ่มได้หลัง 21 ต.ค. 2026 และหลังผู้ใช้สั่งเท่านั้น** (`README.md` กติกาข้อ 5)
> ยกเว้น E0 ที่ผมเสนอให้พิจารณาเริ่มก่อน — ดูเหตุผลใน E0
...
**DoD:** DoD-1–DoD-8 · ไม่มี migration
...
| ❌ **ส่วนที่แตะ repo / สร้าง branch ที่ 2** | **รอหลัง 21 ต.ค.** | `git checkout` ผิดครั้งเดียวคือ 8 เซสชันพังพร้อมกัน (R2) · **และขัดกับ "โหมดหน้างาน" ที่เพิ่งรับกันไป** (R1) |

**ผมเห็นด้วยกับการปฏิเสธครึ่งหลัง** — ตอนเสนอผมชั่งน้ำหนักเฉพาะ "เวลาที่ประหยัดได้" แต่ P1 ชี้ว่ามันขัดกับข้อ A
ที่รับไปในข้อความเดียวกัน ซึ่งถูก · **E0-AC1 (branch `platform`) จึงเริ่มนับหลัง 21 ต.ค. เท่านั้น**
...
**DoD:** DoD-1–DoD-8 + มี migration → ผู้ใช้รันเอง + ยืนยันด้วย curl
...
**DoD:** DoD-1–DoD-8 + migration (ผู้ใช้รันบน local ก่อน · **ห้ามแตะ production**)
...
**DoD:** DoD-1–DoD-8 · ไม่ควรมี migration (ถ้ามีแปลว่า E2 ออกแบบไม่ครบ — ให้ตีกลับ E2)
...
**DoD:** DoD-1–DoD-8 + P2 ยืนยัน responsive/a11y + P4 รัน e2e
...
**DoD:** DoD-1–DoD-8 + ตัวเลขวัดได้ก่อน/หลังทุกข้อ
...
**DoD:** DoD-1–DoD-8 · ⚠️ **ห้ามแตะ `ejzibhgqhxdzkovsnpds` ทุกกรณี** — snapshot ต้องมาจาก export ที่ผู้ใช้เป็นคนดึงเอง
...
**DoD:** DoD-1–DoD-8 + ผู้ใช้ต้องมี LLM API key ก่อน (หัวข้อ 8 ข้อ 6)
...
- [ ] **E9-AC3** A และ B ล็อกอินเข้าของใหม่ได้จริงทั้งคู่ **บนเครื่องของตัวเอง** และเห็นทริปเกาหลีครบ
...
### M1 — MVP: แพลตฟอร์มใช้แทนของเดิมได้ 🎯
...
🔴 **2 ข้อที่ต้องเขียนกำกับไว้ ไม่งั้นทะเบียนนี้จะถูกใช้ผิด**
...
### 🔴 R2 — 8 เซสชันแชร์ working tree เดียว
...
P1 ตั้งข้อสังเกตว่า *"ของที่พังโดยไม่มีสัญญาณ คือสิ่งที่ทีมนี้เจอซ้ำที่สุดในวันนี้"* — ผมไล่นับแล้ว **ไม่ใช่ความรู้สึก มี 14 ตัวจริง** (โตจาก 10 → 14 ภายในวันเดียว · ตัวที่ 13-14 เจอตอนกำลังจะ commit)
...
· **11 ใน 14 ตัวจะถูกรู้ตอนใช้งานจริงหน้างาน หรือไม่ถูกรู้เลย** — ไม่มีตัวไหนที่ CI/lint/เทสต์จับได้ตามธรรมชาติ
...
**3. ล็อกอินด้วยอะไร**
ตัวเลือก: magic link ทางอีเมล (Supabase Auth ฟรี ไม่ต้องตั้งค่าอะไรเพิ่ม) · Google OAuth (ต้องตั้ง consent screen)
...
### ⏳ ของค้างจากทริปที่ยังไม่เกี่ยวกับแพลตฟอร์ม แต่ค้างอยู่จริง
