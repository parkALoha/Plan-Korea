...
- [x] **E5-AC8** 🔴 **ใหม่ 27 ส.ค. 2026 (สเปกผู้ใช้ผ่าน P1 · รอบแก้ที่ 65) — หน้า Home:** route ใหม่ที่เห็น "ทริปทั้งหมดของฉัน" — header (โปรไฟล์ซ้าย/ตั้งค่าขวา) · การ์ดทริปเรียงตามวัน (รูปปก · จุดหมาย · จำนวนสมาชิก) · FAB "+ สร้างทริปใหม่"
      ...
      · **UI:** `TripCoverUpload` ใน `TripSettingsModal` (`cd31e70`) — P2 live-verify ด้วยไฟล์จริงครบวงจร: อัปโหลด → การ์ด Home เปลี่ยนจาก gradient เป็นรูปจริง → ทับ → ลบ → กลับ fallback · ข้อความผ่าน `E5_COPY` (`AC7` ไม่ถอย)
      · **จุดวาง — ต่างจาก mockup โดยตั้งใจ:** อยู่ใน `TripSettingsModal` ไม่ใช่บนการ์ด เพราะการ์ดทั้งใบเป็น `<Link>` ไปหน้าเทริปอยู่แล้ว ปุ่มซ้อนจะชนพื้นที่คลิก — จดไว้เผื่อผู้ใช้ถามว่าทำไมต่างจากภาพที่ส่งมา
      🔴 **ช่องที่ยังไม่วัดด้วยตา — ปิดได้ แต่ต้องจดไว้ ไม่ใช่เงียบ:** "editor ไม่เห็นปุ่ม" ยังไม่มีใครดูด้วยตา (P2 ไม่มีบัญชีที่สองใน browser session ของตัวเอง) — **แต่ด่านจริงพิสูจน์แล้วโดย probe ของ P4** (editor ยิง PUT ตรงๆ ได้ 403 ไม่มีไฟล์ค้าง) → การซ่อนปุ่มในหน้าเป็นแค่ชั้นสื่อสาร ไม่ใช่ชั้นบังคับ (หลักเดียวกับ read-only §9) ถ้า UI พลาดโชว์ปุ่มให้ editor เห็น ก็ยังกดไม่ผ่านอยู่ดี
