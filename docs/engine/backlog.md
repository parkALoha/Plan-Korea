...
- [x] **E1-AC8** ✅ **ผ่าน 26 ส.ค. 2026 — `authProviders.test.ts` เขียวสด (provider เปิด = `{google, email}` เท่านั้น · `mailer_autoconfirm = false`) แต่ความรุนแรงเปลี่ยนไปแล้ว อ่านหัวข้อถัดจากนี้ก่อนถือว่า "แค่ปิดของค้าง"** เดิม 🔴 **เงื่อนไขความปลอดภัยของการ link ที่ E1-AC7 ไม่ได้ครอบ — ต้องเขียนแยก เพราะมันดันไปคนละทาง**
...
- [x] **E1-AC6** ✅ **ตอบแล้วโดย `D1` — ไม่ต้องตัดสินอีก:** `/unlock` คงเป็น global ระหว่างเปลี่ยนผ่าน แล้ว **หายไปทั้งหน้าหลัง E1** แทนที่ด้วย session auth จริง · สิทธิ์ต่อทริปมาจาก `trip_members` ไม่ใช่ PIN
...
- [x] **E2-AC8** ✅ **ปิดแล้ว 25 ส.ค. 2026 (P1 ยืนยัน)** เดิม 🔴 **`order_index` → rank key (`D6`)** — ลากจุดแวะ 1 จุดต้องเขียน DB **แถวเดียว** ไม่ใช่ n แถว · `sortStops` ต้องมี tie-break (วันนี้ `useStops.ts:13-17` ไม่มี → เลขซ้ำ = ลำดับไม่นิ่ง = บั๊ก 8.1 ที่ยังเปิดอยู่) · **ต้องมีกลไก rebalance** เพราะแทรกซ้ำที่เดิมทำให้คีย์ยาวขึ้นเรื่อย ๆ
      ...
      🔴 **แก้ 29 ส.ค. 2026 (P4 พบว่าปิดตอนนั้นเป็นทิศ ② · P1 แก้ทันที):** ปิด 25 ส.ค. ด้วยการยืนยันด้วยตา ไม่มีด่านคุ้ม — ยัด `.order("id")` ออกจาก `db.ts` แล้วชุดเทสต์เขียวทั้ง 803 เคส (ไม่มีอะไรจับ) **แก้แล้ว:** `lib/__tests__/stopOrderingContract.test.ts` (P8 ตรวจไฟล์มีจริง) ตอนนี้เป็นด่านจริง
...
- [x] **E4-AC2** ✅ **ปิดแล้ว 27 ส.ค. 2026 (P3 ปิดบรรทัดสุดท้ายให้)** เดิม `countryOfCity(c) { return c === "hanoi" ? "vn" : "kr" }` ไม่มีอยู่ในโค้ดแล้ว (B6) · `grep -rn 'hanoi' lib/ app/` ไม่เจอตรรกะตัดสินใจ เจอได้แค่ข้อมูล
...
- [x] **E4-AC3** ✅ **ปิดแล้ว 27 ส.ค. 2026 (ปิดคู่กับ `AC2` ตามที่กำหนดไว้ — ไม่ใช่ปิดหลอกอีกต่อไป)** เดิม registry บอกความสามารถรายประเทศได้ — เกาหลี: `TRANSIT` ✅ / `DRIVE`,`WALK` ❌ (ข้อจำกัดกฎหมายเกาหลี ยืนยันด้วยการทดสอบจริงแล้ว `PLAN.md §2`) · เวียดนาม: ครบ · UI แสดงป้าย "(ประมาณการ)" จาก registry ไม่ใช่จาก `if (country === "kr")`
...
- [x] **E5-AC8** 🔴 **ใหม่ 27 ส.ค. 2026 (สเปกผู้ใช้ผ่าน P1 · รอบแก้ที่ 65) — หน้า Home:** route ใหม่ที่เห็น "ทริปทั้งหมดของฉัน" — header (โปรไฟล์ซ้าย/ตั้งค่าขวา) · การ์ดทริปเรียงตามวัน (รูปปก · จุดหมาย · จำนวนสมาชิก) · FAB "+ สร้างทริปใหม่"
...
· ✅ **`E2-AC8` ปิดจริงแล้วตอนนี้ (P1 · `stopOrderingContract.test.ts` — P8 ตรวจไฟล์มีจริง):** เจอเป็นทิศ ② แล้วแก้ทันที ไม่ใช่แค่บันทึกว่าเป็นช่องโหว่
· `E1-AC6`/`E4-AC2`/`E4-AC3` ยังไม่มีด่านคุ้ม — จดไว้เป็นหนี้ ยังไม่ได้ตัดสินว่าจะสร้างด่านหรือยอมรับความเสี่ยง (ส่งให้ P1 ตัดสิน)
