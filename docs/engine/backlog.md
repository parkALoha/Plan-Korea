...
> · 🟡 **`E6-AC4` ยังไม่ปิด และมีจุดที่ต้องระวังก่อนปิด (ยังไม่ได้ยิงตรวจ ส่งเป็นข้อสังเกตให้ P1/P7):** แถวตาราง "วัน (itinerary) → `usePlatformItinerary` → ✅" ถูกติ๊กผ่านไว้ **ก่อน** `E6-AC11` แก้บั๊ก `buildDayBridge([], rows)` (ซึ่งทำให้ `writeTripCache` ไม่เคยถูกเรียกเลยตั้งแต่ 28 ส.ค.) — แถวนั้นอาจเป็น "ผ่านเพราะมีโค้ดเรียก `hydrateThenFetch`" ไม่ใช่ "ผ่านเพราะแคชมีข้อมูลจริง" ต้องยิงตรวจ airplane-mode ซ้ำเฉพาะแถวนี้ก่อนเชื่อว่า `AC11` ปลดล็อกให้ปิด `AC4` ได้ฟรี — ยังเหลือ 2 แถว 🟡 รอ P7 ตัดสินอยู่แล้วด้วย ยังปิดไม่ได้จนกว่าจะครบทั้งสองเงื่อนไข
...
- [ ] **E6-AC4** offline อ่านอย่างเดียวยังใช้ได้จริงบน airplane mode ~~(ผู้ใช้ยืนยันเองแล้วครั้งหนึ่ง 13 ส.ค. 2026)~~
      ...
      |---|---|---|
      | วัน (itinerary) | `usePlatformItinerary` | ✅ แคชผ่าน `hydrateThenFetch`+`offlineStore` |
