...
- [ ] **E8-AC1** 🔴 **บล็อกด้วย LLM API key (28 ส.ค. — P5 พบ · P1 ยืนยัน · P8 ตรวจ `.env.local` เอง: มี 5 คีย์ Supabase/Google Maps ไม่มีของ LLM สักตัว)** ให้วันที่ยัดเกินจริง Copilot ตอบด้วยข้อเสนอที่**อ้างเวลาเดินทางจริงจาก Routes API** ไม่ใช่ตัวเลขที่แต่งขึ้น — วัดไม่ได้จนกว่าผู้ใช้จะใส่คีย์ (หัวข้อ 8 ข้อ 6) เข้าเกณฑ์เดียวกับ `E7`/`E9`: บล็อกด้วยการลงมือของผู้ใช้ ไม่ใช่งานค้างของทีม
- [ ] **E8-AC2** ✏️ **ผ่าเป็น `AC2a`/`AC2b` 28 ส.ค. 2026 — ครึ่งแรกวัดได้วันนี้ ครึ่งหลังบล็อกด้วย LLM API key:**
      · [ ] **AC2a** (วัดได้วันนี้) ห้ามมีคำว่างบประมาณ/ราคา/ค่าใช้จ่ายใน **prompt · tool schema** — `D2` ยืนยันแล้วว่าทั้ง repo ไม่มี `₩`/`KRW`/`THB`/`currency`/`Intl.NumberFormat` สักจุด และไม่มีคอลัมน์ราคาในตารางไหนเลยรวมทั้ง `bookings` · ผู้ใช้ยืนยันซ้ำ 17 ส.ค. 2026 · **ห้ามออกแบบเผื่อ** — วัดด้วย static grep บนไฟล์ prompt/schema ตรงๆ
      · [ ] **AC2b** 🔴 บล็อกด้วย LLM API key — **Copilot ต้องปฏิเสธคำถามเรื่องราคา ไม่ใช่เดาราคามาตอบ** วัดด้วยเทสต์ที่ถามราคาตรง ๆ กับโมเดลจริงแล้วดูว่าปฏิเสธ ต้องมีคีย์ถึงจะยิงได้
- [ ] **E8-AC6** ✏️ **ผ่าเป็น `AC6a`/`AC6b` 28 ส.ค. 2026 — ครึ่งแรกวัดได้วันนี้ ครึ่งหลังบล็อกด้วย LLM API key:**
      · [ ] **AC6a** (วัดได้วันนี้) ตั้งค่าโมเดลตาม `D9` ให้ถูกตั้งแต่แรก — `claude-opus-5` · **ห้ามปิด thinking** ในโค้ด (Opus 5 เปิด thinking เป็นค่าเริ่มต้นอยู่แล้ว) — วัดด้วยอ่าน config ตรงๆ ไม่ต้องเรียกโมเดล
      · [ ] **AC6b** 🔴 บล็อกด้วย LLM API key — **พิสูจน์ว่าปิด thinking แล้วโมเดลไม่เขียน tool call เป็นข้อความธรรมดา** (เทิร์นสำเร็จ แต่ tool ไม่ถูกเรียกจริงและไม่มี error = ความพังที่มองไม่เห็น) · `max_tokens` คุม thinking + คำตอบ **รวมกัน** → ต้องเผื่อตอนประเมินค่าใช้จ่าย — ต้องเรียกโมเดลจริงถึงจะพิสูจน์ได้
- [ ] **E8-AC3** 🔴 **บล็อกด้วย LLM API key** เคารพเวลาเปิด-ปิดจริง — เสนอเวลาที่สถานที่ปิดอยู่ = ไม่ผ่าน (มีเคสจริงให้ใช้แล้ว: กระเช้าซอรัคซานเปิด 09:00 · ตลาดจุงอังคังนึงปิด 19:00) — ต้องมี Copilot ตอบจริงถึงจะวัด
- [ ] **E8-AC4** ✏️ **ผ่าเป็น `AC4a`/`AC4b` 2 ก.ย. 2026 (P5 พบ 3 ข้อวัดไม่ได้ + 1 ข้อถ้อยคำสวนทางดีไซน์ — ส่งตรง P8 ตาม P1 สั่ง · P8 ตรวจไฟล์/เทสต์จริงก่อนรับ):** ถ้อยคำเดิม ("เรียก DAL ด้วย identity ของ Copilot ตรง ๆ ให้พยายามแก้ทริปที่ไม่ได้เป็นสมาชิก แล้วต้องถูกปฏิเสธ") วัดไม่ได้ ① ยังไม่มี wrapper ฝั่งเขียนสักตัว (`lib/copilot/` มีแค่ `getBookings.ts`/`toolSchemas.ts`) "พยายามแก้" จึงไม่มีเส้นทางให้เดิน ② RLS ไม่ throw สำหรับ SELECT คืน 200+`[]` (`rlsMatrix.test.ts:36-37`) เส้นทางอ่านแทนจะแยกไม่ออกจาก "ทริปนี้ไม่มีการจอง" ③ ครึ่งที่วัดได้จริงวันนี้เขียวอยู่แล้วโดยไม่ต้องทำอะไร (`authNoServiceRole.test.ts` คลุม `lib/copilot/` อยู่แล้ว — พิสูจน์แดงจริงเมื่อแทรกการละเมิด) ④ 🔴 **"identity ของ Copilot" สวนทางดีไซน์เอง** — `getBookings(db, …)` รับ client ของ**ผู้ใช้**เข้ามา ไม่สร้าง identity ของตัวเอง (`getBookings.ts:18-19`) ทางที่ตรงที่สุดในการทำให้เกณฑ์เดิมรันได้ตามตัวอักษรคือสร้าง identity ให้ Copilot ซึ่งเป็นสิ่งที่ข้อนี้ห้าม — เข้าตระกูล "เกณฑ์รับที่เขียนจากสภาพปลายทาง" ที่เพิ่งเข้า `TEAM.md §3.4`
      · [ ] **AC4a** (วัดได้วันนี้ — มีด่านแล้ว) ไม่มีไฟล์ใดใน `lib/copilot/` แตะ service role key หรือสร้าง Supabase client ของตัวเอง — ทุก tool ต้องรับ `Db` ของผู้ใช้เป็นพารามิเตอร์
        🔴 **แก้ 2 ก.ย. 2026 (P5 พบเองว่าถ้อยคำที่ตัวเองส่งมากว้างกว่าด่านที่อ้าง — ปิดช่องแล้ว `1bb95d5` · P8 ตรวจซ้ำอิสระ: `grep -c` ยืนยัน `authNoServiceRole.test.ts` ไม่มี pattern constructor เลยสักตัว + รัน `copilotToolResponseFields.test.ts` เอง 7/7 ผ่าน):** ด่านเดิมที่จดไว้ (`authNoServiceRole.test.ts`) ตอบแค่ *"ไม่มีใครถือคีย์ที่มีสิทธิ์เกิน"* ไม่ได้ตอบ *"tool สร้าง client เองหรือเปล่า"* — tool ที่สร้าง client ด้วย **anon key** เองจะเขียวผ่าน `authNoServiceRole` สบาย ๆ ทั้งที่ RLS จะตัดสินด้วย session อื่น (หรือไม่มี session เลย) ซึ่งเป็นสิ่งที่ `AC4` ทั้งข้อมีไว้กัน — **ด่านจึงเป็นสองใบ ครอบคนละวรรค**
        · **ด่าน:** `authNoServiceRole.test.ts` (ครอบวรรค "ไม่แตะ service role key") **+ `copilotToolResponseFields.test.ts`** (ครอบวรรค "ไม่สร้าง Supabase client เอง" — พิสูจน์แดงจริงเมื่อแทรก `createClient(...)` เข้า `getBookings.ts`)
        🔴 **วรรค "ทุก tool ต้องรับ `Db` ของผู้ใช้เป็นพารามิเตอร์" ยังไม่มีด่านตรง** — วันนี้ถูกยึดไว้ทางอ้อมเท่านั้น (เคสใน `copilotToolResponseFields.test.ts` เรียก wrapper ทุกตัวด้วย `Db` ปลอม ถ้า wrapper ไม่รับ `Db` เคสจะคอมไพล์ไม่ผ่าน) ตอนนี้มี wrapper ตัวเดียว (`getBookings.ts`) ด่านจึงตรวจไฟล์เดียวกับที่เขียนกฎ (`P-30`) — เขียนไว้เป็นเจตนาก่อน รอ wrapper ตัวที่สองแล้วค่อยเขียนด่านตรง
      · [ ] **AC4b** 🔴 **บล็อกด้วยไม่มี wrapper ฝั่งเขียนตัวแรก ไม่ใช่บล็อกด้วยคีย์ LLM** — เรียก wrapper ฝั่งเขียนตัวแรกด้วย client ของผู้ใช้ที่ไม่ได้เป็นสมาชิกทริปนั้น ต้องได้ **error** ไม่ใช่ผลว่าง (คำว่า "error ไม่ใช่ผลว่าง" กันกับดัก ② ข้างบน ห้ามตัดออก) · การห้ามต้องบังคับที่ฐาน (RLS/trigger) ไม่ใช่ที่โค้ด tool — `D87` พิสูจน์แล้วว่าการตรวจในโค้ด route เป็นแค่มารยาท ผู้ใช้ยิง PostgREST ตรงได้
- [ ] **E8-AC5** ✏️ **ผ่าเป็น `AC5a`/`AC5b` 2 ก.ย. 2026 (P5 พบ — ส่งตรง P8 ตาม P1 สั่ง · P8 ตรวจไฟล์จริงก่อนรับ):** ถ้อยคำเดิม ("วัดด้วยการนับ call ได้โดยไม่ต้องเรียกโมเดลจริง จึงยังทำได้วันนี้") วัดไม่ได้ — ยังไม่มี route ของ Copilot สักตัว (`grep -rl copilot app/` = 0 ไฟล์) ไม่มี call ให้นับ · และต่อให้มี การนับจำนวน call ก็ไม่ผูกกับเงิน — `E8-AC6` เขียนเองว่า `max_tokens` คุม thinking+คำตอบ**รวมกัน** หนึ่งเทิร์นราคาต่างกันหลายเท่า
      · [ ] **AC5a** 🔴 **บล็อกด้วยไม่มี route ของ Copilot ไม่ใช่บล็อกด้วยคีย์ LLM** เพดานจำนวนเทิร์นต่อผู้ใช้ — ทำได้เมื่อมี route ของ Copilot
      · [ ] **AC5b** เพดานยอด token สะสมต่อผู้ใช้ — ต้องมีที่เก็บนอก process (ผูกกับ `E2`/`E3`) **เป็นงานสร้าง ไม่ใช่งานต่อสาย**
      🔴 **แก้ 29 ส.ค. 2026 (P5 พิสูจน์แล้ว):** ถ้อยคำเดิมไม่ระบุทิศแดง/เคสควบคุม (แค่บอกว่า "มีเพดาน" ไม่บอกว่าเกินเพดานแล้วต้องเกิดอะไร) — **เกณฑ์ที่ใช้จริง: ยิงเกินเพดานแล้ว call ต่อไปต้องถูกปฏิเสธก่อนถึงโมเดล (ไม่ใช่แค่ log)** พร้อมเคสควบคุมฝั่งบวก (ยิงไม่เกินเพดานต้องผ่านปกติ) — ใช้กับ `AC5a`/`AC5b` ทั้งคู่
      · **และนี่คืองานสร้าง ไม่ใช่งานต่อสาย** — `lib/rateLimit.ts` ที่มีอยู่แล้ว (ใช้กับ Google API proxy) **ใช้ซ้ำกับ `AC5` ตรงๆ ไม่ได้**: คีย์ด้วย IP ไม่ใช่ผู้ใช้ (LLM ต้องคุมต่อบัญชี ไม่ใช่ต่อ IP) นับจำนวน call ไม่ใช่ต้นทุน (LLM คิดเงินตาม token ไม่ใช่ต่อ request) เก็บใน memory ต่อ instance (รีเซ็ตเองเมื่อ serverless recycle — รับได้กับ rate limit ปกติ แต่ไม่รับได้กับเพดานค่าใช้จ่ายที่ต้องคงอยู่ข้ามการรีไซเคิล) ไม่มีแนวคิดเรื่อง "ค่าใช้จ่ายสะสม" เลย มีแค่ "จำนวนครั้งในหน้าต่างเวลา" — ต้องออกแบบกลไกใหม่ ไม่ใช่เรียก `rateLimitGuard()` ซ้ำ
- [ ] **E8-AC7** 🔴 **บล็อกด้วย LLM API key** ข้อห้ามทั้ง 9 ข้อ (`copilot-spec.md §5`) ต้องมีเคสคู่ครบทุกข้อ — ปฏิเสธถูกอย่างเดียวไม่นับว่าผ่าน — ต้องมีโมเดลจริงให้ยิงเคสคู่ทั้งหมด
