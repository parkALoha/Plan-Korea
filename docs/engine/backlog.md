...
- [ ] **E6-AC11** 🔴 **มินท์ใหม่ 30 ส.ค. 2026 (P1 เสนอ · เจ้าของงาน P3 · ส่งตรงถึง P8 ตามที่ P1 บอก) — รวมแหล่งข้อมูล "วัน" ให้เหลือ provider เดียว ไม่ใช่ต่างคนต่างยิง `/days` แล้วสร้าง `buildDayBridge()` เอง:**
      ...
      **เกณฑ์รับ — สองข้อ ข้อ ② สำคัญกว่า ①:**
      - [ ] **① จำนวนคำขอ:** เปิดหน้าใดหน้าหนึ่งใน `/trip/<id>/*` แล้ว `GET .../days` ถูกเรียกไม่เกิน 1 ครั้งต่อการโหลดหน้า — วัดจาก network log ของเบราว์เซอร์จริง
      - [ ] **② โครงสร้าง:** มี provider เดียวที่แปลง `legacyDays → dbId` และ **ไม่มีผู้เรียก `buildDayBridge()` ตัวอื่นเหลือในรีโปนอกจาก provider ตัวนั้น** — บังคับด้วยเทสต์ที่ไล่ `grep -rn 'buildDayBridge(' hooks lib --include='*.ts' --include='*.tsx' | grep -v __tests__` ต้องเจอแค่ 1 บรรทัด
      ⚠️ **ห้ามปิดด้วย ① เดี่ยวๆ:** ถ้าตั้งเกณฑ์ไว้ที่จำนวนคำขออย่างเดียว งานปิดได้ด้วยการใส่ dedupe/แคชคำขอทับไว้ (เช่น SWR dedupe) ซึ่งไม่แตะราก — 4 จุดที่เรียก `buildDayBridge()` แยกกันยังอยู่ครบ บั๊กแบบ `edd17e2` ยังเกิดซ้ำได้ทุกจุดที่เหลือ · ① วัดอาการที่มองเห็นได้ ② วัดสิ่งที่กัดจริง (จำนวนจุดที่ความหมายของค่าเพี้ยนได้)
