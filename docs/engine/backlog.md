...
- [ ] **E6-AC4** offline อ่านอย่างเดียวยังใช้ได้จริงบน airplane mode ~~(ผู้ใช้ยืนยันเองแล้วครั้งหนึ่ง 13 ส.ค. 2026)~~
      ...
      |---|---|---|
      | วัน (itinerary) | `usePlatformItinerary` | 🟡 **ยังไม่ยืนยัน — แก้ 30 ส.ค. 2026 (P1 ทัก · ถอด ✅ ทันทีไม่รอผลยิง):** ✅ เดิมถูกติ๊กก่อน `E6-AC11` แก้บั๊ก `buildDayBridge([], rows)` ที่ทำให้ `writeTripCache` ไม่เคยถูกเรียกเลยตั้งแต่ 28 ส.ค. — **ข้อเท็จจริงเรื่องลำดับเวลา ไม่ใช่ "อาจจะ":** ณ เวลาที่ติ๊ก แคชไม่มีข้อมูลอยู่จริง ไม่ว่าเกณฑ์จะเขียนว่าอะไร สิ่งที่วัดได้ไม่ใช่ "อ่านออฟไลน์ได้" P1 ลองยิงเองแล้วยิงไม่ผ่าน (เซสชัน Google หมดอายุ) + คีย์ `trip-cache:v2:*` เก่าใน `localStorage` ใช้เป็นหลักฐานไม่ได้แล้วเพราะ P7 ย้ายไป IndexedDB (`plan-korea:v1`) — **มอบให้ P3 ยิง airplane-mode จริง** (เจ้าของ `hydrateThenFetch` ใน `AC11` + มีท่ายิงเบราว์เซอร์อยู่แล้ว) จนกว่าจะมีผล ถือเป็น 🟡 ไม่ใช่ ✅ และไม่ใช่ ❌ |
