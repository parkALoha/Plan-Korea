...
- [ ] **E0-AC1** `git branch --show-current` = `platform` (ไม่ใช่ `main`) และ `main` ยังชี้ commit เดิมของทริป
- [ ] **E0-AC2** ✏️ **เปลี่ยนตาม `D14` — dev DB คือ Supabase คลาวด์ใน org ใหม่ ไม่ใช่ Docker**
...
- [ ] **E0-AC3** 🔴 **ด่านกันหายนะ — เข้มขึ้นกว่าเดิมเพราะตอนนี้ dev DB อยู่บนคลาวด์แล้ว**
...
- [ ] **E0-AC4** โปรเจกต์ dev โหลด `supabase/migrations/0001`–`0031` ครบแล้ว ได้ 14 ตารางเท่า production
- [ ] **E0-AC9** 🔴 **`gitleaks` ใน CI เป็นข้อบังคับ ไม่ใช่ nice-to-have — และต้องเห็นมันจับของจริงได้ก่อน 1 ครั้ง**
...
- [ ] **E0-AC10** 🟡 **ด่านรอง — CI ตรวจว่าไม่มี `.sql` ไฟล์ไหนใน `docs/engine/` อยู่นอก `schema/`**
...
- [ ] **E0-AC7** ⚠️ **staging ต้องมี keep-alive** — free tier หลับเมื่อไม่มีทราฟฟิก 7 วัน เหมือนที่ทริปเจอ (เฟส 25 แก้ด้วย cron `0 3 * * *`) · ถ้าไม่ทำ จะเจอ "DB หาย" กลางการพัฒนาแล้วเสียเวลาไล่หาสาเหตุที่ไม่ใช่บั๊ก
- [ ] **E0-AC5** CI บน GitHub Actions รัน `lint` + `test` + `build` อัตโนมัติทุก push · **เห็นมันแดงมาก่อน 1 ครั้ง** ด้วยการ push โค้ดที่ผิดเจตนา แล้วเขียวหลัง revert
- [ ] **E0-AC6** CI **บล็อก merge** ถ้าไฟล์ที่เปลี่ยนมี path ขึ้นต้นด้วย `supabase/migrations/` บน branch `platform` (ด่านกันหายนะข้อที่ 2 — `README.md` กติกาข้อ 3 เขียนไว้เป็นข้อตกลง แต่ข้อตกลงกันคนลืมไม่ได้ เครื่องกันได้)
...
> 🔴 **คำเตือนของ P4 ที่ต้องอ่านคู่กับเครื่องหมาย ✅ ทั้ง 4 ข้อบน — "เขียว" ไม่เท่ากับ "พิสูจน์แล้วว่าจับได้"**
> เมทริกซ์นี้ **ยังไม่เคยเห็นตัวเองแดงเพราะ RLS ผิดจริงสักครั้ง** — ทุกครั้งที่แดงวันนี้ แดงเพราะเครื่องมือทดสอบเอง (คีย์ผิดรูปแบบ · `owner_id` · `P-26`) ไม่ใช่เพราะ policy หละหลวม
> ตามกฎ `E0` ของทีม **ด่านที่ไม่เคยเห็นแดง พิสูจน์อะไรไม่ได้** — P4 เสนอทำ **mutation test ตอนเช้า** (ผ่อน policy ชั่วคราว → ต้องเห็นเมทริกซ์แดง → คืนค่า) **ต่อหน้า P1 ไม่ใช่ทำเงียบๆ ตอนกลางคืน**
> **จนกว่าจะทำข้อนั้น ให้อ่านเครื่องหมาย ✅ ข้างบนว่า "เขียวแล้ว แต่ยังไม่ได้พิสูจน์ว่าด่านจับได้จริง" ไม่ใช่ "AC ผ่านสมบูรณ์"**
- [ ] **E1-AC5** `added_by` ที่วันนี้เป็นข้อความอิสระ มี field ใหม่ที่ผูกกับ `profiles.id` แล้ว **และค่าข้อความเดิมยังอยู่ครบ** ในคอลัมน์ `legacy_added_by` (ห้ามทิ้งสตริงเดิม — เป็นข้อมูลเดียวที่บอกได้ว่าใครเพิ่มอะไรในทริปจริง)
...
· 🔴 **เช็คลิสต์ปิด E1 — รายการที่ต้องถามผู้ใช้ (P8 ใส่เอง 24 ส.ค. 2026 · P1 มอบให้ใส่)**
  - [ ] **ถามข้อ 10 ของหัวข้อ 8: คำเชิญส่งถึงคนที่ถูกเชิญยังไง** (ลิงก์ส่งเอง vs ระบบส่งอีเมล)
...
- [ ] **E2-AC1** 🔴 **เกณฑ์หลักที่ P1 ต้องการ:** มี test matrix ที่รัน **13 ตาราง × 4 verb (SELECT/INSERT/UPDATE/DELETE) × 2 persona (B ที่เป็น member · C ที่ไม่ใช่) = 104 เคส** · C ต้องถูกปฏิเสธ **104/104** · B ต้องผ่านตามสิทธิ์ที่ออกแบบไว้ · **ต้องเห็นเมทริกซ์นี้แดงบน schema วันนี้ก่อน** ถึงจะเชื่อว่ามันวัดอะไรจริง
- [ ] **E2-AC2** `grep -c 'using (true)' <migrations ใหม่ทั้งหมด>` = **0** (วันนี้: 53 policy · `using (true)` 35 ตัว + `with check (true)` 14 ตัว — นับเองยืนยันแล้ว 17 ส.ค.)
- [ ] **E2-AC3** ทุกตารางเนื้อหามีคอลัมน์ `trip_id` และมี FK จริง — ปิด B4/B5
- [ ] **E2-AC4** `day_id` ไม่ใช่สตริง `d0`–`d10` อีกต่อไป · มีตาราง `trip_days` ที่ผูกวันที่จริง · **สร้างทริป 3 วันที่ไม่มีอะไรเกี่ยวกับเกาหลีได้โดยไม่แก้โค้ด**
- [ ] **E2-AC5** 🔴 **ปิดรูรั่ว Storage (`D12` — ยืนยันแล้ว และหนักกว่าที่ผมเขียนตอนแรก):** `0019_bookings_file.sql:1,7` ตั้ง bucket เป็น public โดยตั้งใจ · policy ทั้ง 4 gate แค่ `bucket_id` · **ต่อกับ B2/B3 เป็นลูกโซ่ได้: anon key ในบันเดิล → อ่าน `bookings.file_url` ทั้งตาราง → ดึงไฟล์ทุกใบ ไม่ต้องเดา URL ด้วยซ้ำ**
...
- [ ] **E2-AC10** 🔴 **`D42` โดนเมทริกซ์ 104 เคสด้วย — persona ชุดเดิมไม่ครอบคนที่มี 2 ทางเข้า**
...
- [ ] **E2-AC8** 🔴 **`order_index` → rank key (`D6`)** — ลากจุดแวะ 1 จุดต้องเขียน DB **แถวเดียว** ไม่ใช่ n แถว · `sortStops` ต้องมี tie-break (วันนี้ `useStops.ts:13-17` ไม่มี → เลขซ้ำ = ลำดับไม่นิ่ง = บั๊ก 8.1 ที่ยังเปิดอยู่) · **ต้องมีกลไก rebalance** เพราะแทรกซ้ำที่เดิมทำให้คีย์ยาวขึ้นเรื่อย ๆ
- [ ] **E2-AC9** 🔴 **`updated_at` เขียนโดยเซิร์ฟเวอร์เท่านั้น (`D7`)** — วันนี้ client เขียนเอง ทับ `default now()` ที่ `0006_trip_stops.sql:10` ตั้งไว้ · เครื่องที่ตั้งนาฬิกาผิดชนะ LWW ตลอดกาลอย่างเงียบ ๆ และเกิดจริงได้ (`lib/localDate.ts` บันทึกเองว่าเจอมือถือค้างเวลาไทยระหว่างอยู่เกาหลี)
...
- [ ] **E2-AC6** 📋 **เกณฑ์ที่ยกความเสี่ยงของ E7 มาไว้ที่นี่:** มีตารางแมป **ทุกคอลัมน์ของ 14 ตารางเดิม → ปลายทางในสคีมาใหม่** และคอลัมน์ไหนที่ตั้งใจทิ้งต้องเขียนเหตุผลกำกับ · **ห้ามมีคอลัมน์ไหนที่ไม่มีคำตอบ** (ถ้าไม่ทำข้อนี้ที่ E2 จะไปรู้ตอน E7 ว่าสคีมาใหม่เก็บของเดิมไม่ได้ ซึ่งแปลว่าต้องรื้อ E2–E5 ใหม่ทั้งหมด)
- [ ] **E2-AC7** `trip_selections` ถูกลบทิ้งอย่างเป็นทางการพร้อมบันทึกเหตุผล (ตารางตายตั้งแต่ `0006`)
- [ ] **E2-AC11** 🔴 **ใหม่ 24 ส.ค. 2026 — ตัวหารของเมทริกซ์ต้องนับจากสคีมาใหม่ ไม่ใช่จากสคีมาวันนี้** (พบจาก [`D45` จังหวะที่ 1 กับ E2](#-d45-จังหวะที่-1-กับ-e2--เดินตามรอยต่อระหว่างเอกสารคนละคน))
...
- [ ] **E2-AC12** 🔴 **ใหม่ 24 ส.ค. 2026 — soft delete เปลี่ยน verb ที่ทำลายข้อมูล แต่เมทริกซ์ยังตรวจ verb เดิม**
...
- [ ] **E2-AC13** 🔴 **ใหม่ 24 ส.ค. 2026 — `bookings.file_url` ต้องเลิกเป็น URL ก่อนปิด bucket ไม่ใช่หลัง**
      ...
      · **เกณฑ์เพิ่ม: หลังปิด bucket ต้องเปิดไฟล์ตั๋วของแถวที่มีอยู่เดิมได้จริงจากในเว็บ** ไม่ใช่แค่เปิดจากข้างนอกไม่ได้
      · 🔴 **ส่งต่อ P3 ก่อนเริ่ม E2:** `public/sw.js:113,118` ใช้ `cacheFirst` กับ ASSET_CACHE · **signed URL เปลี่ยนลายเซ็นทุกครั้งที่เซ็น = URL ไม่ซ้ำเดิม = cache ไม่มีวัน match**
...
- [ ] **E3-AC1** `grep -rn '\.from("' app components hooks lib --include='*.ts*' | grep -v 'storage\.from('` ได้ **0 บรรทัด** ที่รันฝั่ง client · วันนี้ได้ **67 บรรทัดใน 13 ไฟล์**
- [ ] **E3-AC2** 10 hooks ที่ถือ 60/67 จุด แปลงครบทุกตัว — เช็คทีละไฟล์ตามตารางในหัวข้อ 1
- [ ] **E3-AC3** `NEXT_PUBLIC_SUPABASE_ANON_KEY` ไม่ปรากฏใน client bundle อีก **ยกเว้น** เส้นทาง Realtime ที่ยังต้องใช้ (ถ้ายังต้องใช้ ต้องเขียนเหตุผลกำกับ ไม่ใช่ปล่อยผ่านเงียบ ๆ)
- [ ] **E3-AC4** ✏️ **แก้ตาม `D15`:** `lib/writeGuard.ts` เป็น choke point ของ**ตาราง** (10/10) แต่ **ไม่ใช่ของ Storage (0/9)**
      → เกณฑ์ผ่านคือ **หลัง E3 ต้องเป็น 19/19** — ทั้ง `BookingEditModal.tsx` (5 จุด · โซน P2) และ `lib/stopPhoto.ts` (4 จุด) ต้องเข้ามาอยู่ใต้ choke point เดียวกัน
...
- [ ] **E3-AC7** 🔴 **ออกแบบทางเข้าสู่โหมด read-only ตั้งแต่ตอนนี้ ไม่ใช่ไปคิดตอน E9 (`D18`)** — E9-AC2 ต้องการ read-only ที่บังคับได้ที่ชั้น DB/API · ถ้าไม่วางสวิตช์ไว้ใน DAL ตั้งแต่ E3 จะไม่มีที่ให้ใส่ตอน E9 นอกจากไล่แก้ทุกเส้นทางใหม่ (P6 ชี้ข้อเดียวกันจากฝั่ง infra)
...
|---|---|---|
| **INV-1** | ตาราง `trip_invitations` (อีเมล · trip · role · ผู้เชิญ · token · สถานะ · หมดอายุ · ใครรับ) | ตรงไปตรงมาที่สุดในก้อนนี้ |
...
| **INV-7** | 🔴 **ช่องทางส่ง** | **ไม่ใช่งานโค้ด และไม่เคยอยู่ในแผนเลย** — ดู "การตัดสินใจที่ต้องขึ้นถึงผู้ใช้" ด้านล่าง |
...
**🔴 การตัดสินใจที่ต้องขึ้นถึงผู้ใช้ก่อนเริ่ม `M2-INV` — `INV-7` ช่องทางส่งคำเชิญ**

| ทาง | ได้ | เสีย |
|---|---|---|
| 🟢 **ก. ลิงก์ที่ A ส่งเอง (ผมแนะนำ)** | **ตัด `INV-7` ออกทั้งก้อน** — ไม่ต้องมีผู้ให้บริการอีเมล ไม่ต้องมีโดเมน ไม่ต้องดู deliverability | A ต้องก็อปลิงก์ไปส่งเองทาง LINE — **สำหรับทริป 3–5 คนนี่ไม่ใช่ปัญหา** |
...
> 🔴 **แต่ทางที่ผมแนะนำมีกับดัก และต้องเขียนกำกับไว้ตั้งแต่วันนี้:**
> **ลิงก์ต้องยังผูกกับอีเมลอยู่** — รับได้เฉพาะบัญชีที่อีเมล **verified และตรงกับที่จ่าหน้าไว้**
> **ลิงก์เป็นแค่ช่องทางส่ง ไม่ใช่ตัวให้สิทธิ์** · ถ้าตัดการผูกอีเมลออกเพื่อความง่าย **ใครได้ลิงก์ก็เข้าทริปได้ = `B3` กลับมาในรูปแบบใหม่ที่ RLS มองไม่เห็น**
> → **ทางเลือก ก. ประหยัดที่ `INV-7` ห้ามประหยัดที่ `INV-2`**
...
> · **ตอนนี้มี 1 ขั้น** · ถ้า `M2-INV` มาถึง (คำเชิญ = คนอื่นต้องกดรับ) **ทั้งก้อนจะเป็นขั้นชนิดนี้**
...
**10. 🔴 คำเชิญส่งไปถึงคนที่ถูกเชิญยังไง — ใหม่ 24 ส.ค. 2026 (`D44` · `INV-7`) · ตอบก่อนเริ่ม `M2` ไม่ใช่ก่อน M3**
> **คำถามที่ต้องถามผู้ใช้:** *"ตอนเชิญเพื่อนเข้าทริป อยากให้ระบบส่งอีเมลให้เอง หรือให้คุณก็อปลิงก์ไปส่งเองทาง LINE"*

· 🟢 **ผมแนะนำ "ก็อปลิงก์ไปส่งเอง"** — ตัดงานที่ไม่ใช่โค้ดออกทั้งก้อน (ผู้ให้บริการอีเมล · โดเมน · SPF/DKIM) และสำหรับทริป 3–5 คนแทบไม่ต่างกันในการใช้งานจริง
· ⚠️ **ไม่ว่าตอบทางไหน ลิงก์ต้องผูกกับอีเมลอยู่ดี** — นี่เป็นเรื่องความปลอดภัย ไม่ใช่เรื่องความสะดวก (เหตุผลอยู่ที่ [`M2-INV`](#-m2-inv--ตารางคำเชิญ--flow-ยอมรับคำเชิญ))
· 📌 **ข้อนี้เป็นชนิดเดียวกับ "ตั้ง OAuth consent screen" ที่โผล่มาใน `D42`** — **งานของผู้ใช้ที่งอกจากมติทางเทคนิค** และไม่มีใครเห็นตอนตัดสินมตินั้น
  → **ยังไม่ต้องรีบตอบ แต่ต้องอยู่ในรายการตั้งแต่วันนี้** เพราะถ้าไปถามตอนเริ่ม M2 แล้วคำตอบคือ "ส่งอีเมลเอง" **งานที่ไม่ใช่โค้ดจะโผล่กลางเฟส ซึ่งเป็นรูปแบบที่ `R4` อธิบายไว้แล้ว**
...
| ของค้าง | เดดไลน์จริง |
|---|---|
| จอง Sky Capsule (14 ต.ค.) | ควรจองก่อน 2–4 สัปดาห์ → **ราว 16–30 ก.ย.** |
| จอง Express Bus ปูซาน→ซกโช (15 ต.ค.) | 1–2 สัปดาห์ก่อนบิน → **ราว 27 ก.ย.–4 ต.ค.** |
| จอง KTX คังนึง→โซล (17 ต.ค.) | เปิดจองล่วงหน้า 30 วัน = **~17 ก.ย. 2026** |
