...
- [ ] **E2-AC13** 🔴 **ใหม่ 24 ส.ค. 2026 — `bookings.file_url` ต้องเลิกเป็น URL ก่อนปิด bucket ไม่ใช่หลัง**
      ...
      **แก้ 25 ส.ค. 2026 — แยกสถานะ 3 ข้อของ AC นี้ให้ชัด (P1):**
      ...
      🔴 **แก้ 26 ส.ค. 2026 — ส่งให้ P3 แล้วพร้อมข้อสังเกตที่อาจเปลี่ยน*เนื้อ*ของ ③ ไม่ใช่แค่ให้ติ๊ก:** ③ เขียนตอน 24 ส.ค. ว่า `sw.js` `cacheFirst` ใช้กับ signed URL ตรงไม่ได้ **แต่ ② (`/api/booking-file/[...path]` proxy same-origin) เปลี่ยนพื้นใต้ข้อนั้นไปแล้ว** — ถ้าทุกจุดอ่านผ่าน proxy จริง URL เสถียรและแคชได้ตามปกติ งานที่เหลืออาจเหลือแค่ยืนยันว่าไม่มีเส้นทางหลงเหลือที่ยังอ้าง signed URL ตรง **ไม่ใช่แก้ `sw.js` ตามที่ ③ เขียนไว้เดิม** — รอ P3 ตอบว่าเป็นกรณีไหน ถ้าใช่ ต้องแก้ถ้อยคำของ ③ เอง (รูป `D71`: เนื้อขยับ ป้ายไม่ตาม)
