...
- [ ] **E4-AC1** 🔴 **เกณฑ์ผ่านที่ P1 กำหนดเอง:** สร้างทริปญี่ปุ่นได้โดยไม่แก้โค้ดสักบรรทัด · **วิธีวัดที่ผมเสนอ:** เพิ่มประเทศใหม่ = แก้ข้อมูลใน DB/config 1 ที่ แล้ว `git diff --stat` ของโฟลเดอร์โค้ดต้องได้ **0 ไฟล์**
      ...
      | ② หน้าเว็บ fallback ไปเรนเดอร์ `ITINERARY` ทั้งก้อน — ทริปวันเดียวโชว์ 11 วันของทริปเกาหลี | ❌ งาน `E5` |
      | ③ เพิ่มจุดแวะล้มเงียบต่อผู้ใช้ (`useStops.ts:154` — `console.error` ไม่มี toast) | ❌ P3 แก้แล้วบางส่วน (`aefd350`) |
