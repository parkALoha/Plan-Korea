...
- [x] **E2-AC8** ✅ **ปิดแล้ว 25 ส.ค. 2026 (P1 ยืนยัน)** เดิม 🔴 **`order_index` → rank key (`D6`)** — ลากจุดแวะ 1 จุดต้องเขียน DB **แถวเดียว** ไม่ใช่ n แถว · `sortStops` ต้องมี tie-break (วันนี้ `useStops.ts:13-17` ไม่มี → เลขซ้ำ = ลำดับไม่นิ่ง = บั๊ก 8.1 ที่ยังเปิดอยู่) · **ต้องมีกลไก rebalance** เพราะแทรกซ้ำที่เดิมทำให้คีย์ยาวขึ้นเรื่อย ๆ
      ...
      **รอบ 2 (`b6cd6bb`) — `B5` เป็นชนิดใหม่ ไม่ใช่พวกเดียวกับ 3 ทางแรก:** arrow function ที่ไม่มี `;` ปิด chain (ASI) ทำให้ regex หน้าต่าง 200 ตัวอักษรวิ่งข้ามขอบฟังก์ชันไปยืม `.order("id")` ของก้อน**ถัดไป**มาเป็นหลักฐานว่าตัวละเมิดผ่าน — **รอดครั้งแรกเพราะระยะ 232 > 200 เท่านั้น ไม่ใช่เพราะออกแบบถูก** ร้ายกว่า `B2` ตรงที่ `B2` ยังพิมพ์ชื่อฟังก์ชันที่ตรวจออกมา ส่วนตัวนี้รายงานว่า site *นี้* ผ่านโดยยืมหลักฐานจาก site *อื่น* — **และแก้รอบแรกของ P1 เอง (หยุดที่ `;`/`}` ตัวแรก) ไม่ได้ปิด `B5`** ยิงแล้วได้ 11 passed ไม่แดงเลย เพราะ arrow ไม่มี `;` ทำให้ `;` ตัวแรกไปอยู่คนละฟังก์ชัน — รู้เพราะยิง ไม่ใช่เพราะคิด แก้จริงด้วยหยุดที่ขอบ statement จริง (`;` / `\n\s*}` / `\n\s*\n` / `\nexport`) ไม่ใช่นับตัวอักษร
      🔴 **`B4` (arrow ที่ลืม `deleted_at`) — ยังเปิด และ P4 ยิงยืนยันแล้วว่าเปิดจริง ไม่ใช่แค่คาดไว้:** ครึ่ง `deleted_at` ยังไล่ทีละ `export function` เท่านั้น **`E2-AC8` จึงปิดครึ่ง "ลำดับ" เท่านั้น (ครบทั้ง `B1`/`B2`/`B3`/`B5`) ครึ่ง `deleted_at` (`B4`) ยังเปิด**
...
· ✅ **`E2-AC8` แก้แล้ว — แต่ปิดครึ่งเดียว ไม่ใช่ปิดครบ (แก้คำเดิมที่จดไว้ผิด 2 รอบซ้อน — ครั้งนี้เช็คกับ `HEAD` ไม่ใช่ sha ที่ได้รับมา):** เจอเป็นทิศ ② แล้วแก้ทันที ด่านฉบับแรกของ P1 เองหักได้ทั้งหมด **5 ทาง ใน 2 รอบ** (`B1`/`B2`/`B3` รอบแรก `dc0bd36` · `B5` รอบสอง `b6cd6bb` — ชนิดใหม่ "รอดด้วยจำนวนตัวอักษร ไม่ใช่ด้วยการออกแบบ") ปิดครบทั้ง 4 ทางนั้นแล้ว — **แต่ครอบแค่ครึ่ง "ลำดับ" ครึ่ง "`deleted_at`" (`B4`) ยังไล่ทีละฟังก์ชัน ยังหลุดได้ถ้าเป็น arrow function — P4 ยิงยืนยันแล้วว่าเปิดจริง** (รายละเอียดเต็มที่ตัว `E2-AC8`)
