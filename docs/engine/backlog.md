...
- [x] **E6-AC11** ✅ **ปิดแล้ว 30 ส.ค. 2026 (P3 ลง lint rule จริง `ca425c6` · P1 ตรวจของจริง · P8 ตรวจซ้ำอีกชั้นก่อนติ๊ก)** เดิม 🔴 **มินท์ใหม่ 30 ส.ค. 2026 (P1 เสนอ · เจ้าของงาน P3 · ส่งตรงถึง P8 ตามที่ P1 บอก) — รวมแหล่งข้อมูล "วัน" ให้เหลือ provider เดียว ไม่ใช่ต่างคนต่างยิง `/days` แล้วสร้าง `buildDayBridge()` เอง:**
      ...
      **เกณฑ์รับ — สองข้อ ข้อ ② สำคัญกว่า ①:**
      ...
      🔴 **เพิ่ม 30 ส.ค. 2026 (P1 พบระหว่างไล่ `E6-AC4` — P8 ตรวจไฟล์จริงยืนยัน):** เกณฑ์ ① ("`/days` ≤ 1 ครั้งต่อการโหลดหน้า" — เก็บไว้เป็นเบาะแส ไม่ใช่เกณฑ์ปิด) **ยังไม่จริง และตอนนี้รู้สาเหตุแล้ว:** `hooks/useLegacyDayPlanGate.ts:52` (ผ่าน `fetchReadJson`) และ `hooks/useTripDaysGate.ts:42` (ผ่าน `fetch` ตรง) **ยิง `/api/engine/trips/<id>/days` เองคนละครั้ง ไม่ผ่านสะพาน `buildDayBridge` เลย** — ไม่ขัดกับเกณฑ์ ② ของ `AC11` ที่ปิดไปแล้ว (เกณฑ์คือ "ผู้เรียก `buildDayBridge`" สองตัวนี้ไม่ใช่ผู้เรียก) แต่ทำให้จำนวนคำขอ `/days` จริงยังมากกว่า 1
      🔴 **สมมติฐาน (P1 ตั้ง — ยังไม่ยิงพิสูจน์ ห้ามอ่านเป็นข้อเท็จจริง):** อาจเป็นคำอธิบายของ `E6-AC4` — offline แล้ว `useLegacyDayPlanGate` ยิง `/days` ล้ม → `fetchReadJson` คืน `null` → `classifyLegacyDayPlan(null)` = `"unreadable"` (ยืนยันตรง `lib/engine/legacyDayPlan.ts:71`) → `app/today/page.tsx:524`/`app/summary/page.tsx:840,861` เรนเดอร์ `<DayPlanUnavailableNotice reason="unreadable" />` **แทนที่จะอ่านจาก IndexedDB ที่มีข้อมูลอยู่ครบ** — โค้ดสนับสนุนสมมติฐานนี้จริง (คอมเมนต์ในไฟล์เองก็เขียนว่า `unreadable` ควรแปลว่า "ต่อเน็ตแล้วลองใหม่ได้เอง" แต่ path ปัจจุบันยังบล็อกการแสดงผลอยู่ ไม่ได้ fallback ไปอ่านแคช) — **แต่ยังไม่ได้ยิง airplane-mode พิสูจน์จริง** ต้องรอผลจาก P3
