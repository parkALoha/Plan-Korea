...
- [x] **E2-AC8** ✅ **ปิดแล้ว 25 ส.ค. 2026 (P1 ยืนยัน)** เดิม 🔴 **`order_index` → rank key (`D6`)** — ลากจุดแวะ 1 จุดต้องเขียน DB **แถวเดียว** ไม่ใช่ n แถว · `sortStops` ต้องมี tie-break (วันนี้ `useStops.ts:13-17` ไม่มี → เลขซ้ำ = ลำดับไม่นิ่ง = บั๊ก 8.1 ที่ยังเปิดอยู่) · **ต้องมีกลไก rebalance** เพราะแทรกซ้ำที่เดิมทำให้คีย์ยาวขึ้นเรื่อย ๆ
      ...
      🔴 **ด่านฉบับแรกของ P1 เอง หักได้ 3 ทาง — แก้โครง ไม่ใช่เติมเงื่อนไข (`dc0bd36` · P8 ตรวจ commit เอง):** ① `.order("rank", { ascending: false })` ผ่านฉลุย — ฉบับแรกเทียบแค่*ตำแหน่ง* `.order` ไม่ดูทิศทาง คืนแถวย้อนลำดับทั้งวันแต่ `it.each` รายงานว่าผ่าน ② `export const … =>` ③ `engineTable(db, STOPS_TABLE)` ผ่านตัวแปร — สองทางนี้ไม่เข้าจักรวาลเลย ไม่ถูกแจงออกมาด้วยซ้ำ (หายเงียบกว่า B2 อีก) → **แก้ด้วยเปลี่ยนจักรวาลเป็นไล่ทุก `.order("rank")` ในไฟล์ + ตรวจทิศทาง** ปิดทั้ง 3 ทางพร้อมกัน ยิงพิสูจน์ใน worktree แยก (รอบสะอาด 10 passed ก่อน แล้ว B1/B2/B3 ยิง 1 failed ทุกใบ)
      🔴 **ขอบที่ยังเปิด (เขียนไว้ในไฟล์เอง — ยืนยันตรง commit):** ครึ่ง `deleted_at` ยังไล่ทีละ `export function` — arrow function ที่ลืม `deleted_at` ยังหลุด ยังไม่มีใครหักแต่ช่องอยู่ตรงนั้น **`E2-AC8` จึงปิดครึ่ง "ลำดับ" เท่านั้น ครึ่ง "deleted_at" ยังเปิด**
...
· ✅ **`E2-AC8` แก้แล้ว — แต่ปิดครึ่งเดียว ไม่ใช่ปิดครบ (แก้คำเดิมที่จดไว้ผิด):** เจอเป็นทิศ ② แล้วแก้ทันที ด่านฉบับแรกของ P1 เองยังหักได้อีก 3 ทาง (`dc0bd36`) แก้โครงจนปิดครบทั้ง 3 ทาง — **แต่ครอบแค่ครึ่ง "ลำดับ" ครึ่ง "`deleted_at`" ยังไล่ทีละฟังก์ชัน ยังหลุดได้ถ้าเป็น arrow function** (รายละเอียดเต็มที่ตัว `E2-AC8`)
