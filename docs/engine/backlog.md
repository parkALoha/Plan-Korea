...
- [ ] **E5-AC8** 🔴 **ใหม่ 27 ส.ค. 2026 (สเปกผู้ใช้ผ่าน P1 · รอบแก้ที่ 65) — หน้า Home:** route ใหม่ที่เห็น "ทริปทั้งหมดของฉัน" — header (โปรไฟล์ซ้าย/ตั้งค่าขวา) · การ์ดทริปเรียงตามวัน (รูปปก · จุดหมาย · จำนวนสมาชิก) · FAB "+ สร้างทริปใหม่"
      ...
      🔴 **แก้ 27 ส.ค. 2026 (P8 ตรวจโค้ดจริง — ยังไม่ครบ อย่าติ๊ก แม้ P1 รายงานว่า "ลงครบแล้ว"):** `components/HomeScreen.tsx` มีจริง ดึงจาก `GET /api/engine/trips` จริง และ **จุดหมาย (`destinations`) เป็นข้อมูลจริงแล้ว** (คอลัมน์ผ่าน `trip_destinations`) — **แต่รูปปกยังเป็น placeholder ไล่สีตายตัว** (คอมเมนต์ในไฟล์เอง: *"fallback ของรูปปก — ยังไม่มีใครอัปโหลด (ยังไม่มีตัวอัปโหลดด้วยตามแผน E5)"*) ครึ่งหลังของเกณฑ์ "ไม่ใช่ placeholder ตายตัว" ยังไม่ผ่าน — ยังไม่มีตัวอัปโหลดรูปปกเลยในแผนด้วยซ้ำ ไม่ใช่แค่ยังไม่ได้ทำ
      📌 **เหตุผลของช่องว่างนี้ (P1 ชี้แจงเอง 27 ส.ค. 2026 — ต่างจาก `AC9` ตรงที่ข้อนี้ P1 ตัดสินคนเดียว ไม่ใช่ผู้ใช้):** ไม่เพิ่ม Storage bucket ใหม่ตอนนี้เพราะ `D12` (bucket เดิมเป็น public) ยังเปิดอยู่ฝั่งโค้ด — เพิ่ม bucket ใหม่ตอนนี้คือถอยหลัง P1 **ส่งเรื่องนี้ให้ผู้ใช้ตัดสิน ไม่ตัดสินเอง** (เพราะเป็นคนละกรณีกับ `AC9` ที่ผู้ใช้ตัดสินไปแล้ว) — เปิด `[ ]` ไว้ต่อจนกว่าจะมีคำตอบ
