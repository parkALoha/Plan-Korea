...
> · 🔴 **ของที่ P1 ขอให้ผมถือไว้ — ลำดับเฟสชนกันเอง:** `catalog_places` **ไม่มี migration ไหน seed เลยสักแถว** → เพิ่มจุดแวะจากคลังสถิตย์ (`data/places.ts`) ทำไม่ได้เลยทุกทริปบนแพลตฟอร์ม **`E5` (UI นี้) verify ไม่ครบจนกว่า `E7` (seed คลัง) จะลง แต่ลำดับเฟสวางให้ `E7` อยู่หลัง `E5`/`E6`** และ `E7` คือเงื่อนไขเดียวที่อนุญาต merge เข้า `main`
>   · **ทางเลี่ยงที่มีอยู่แล้ว:** เส้น custom-place (จาก Google, สร้าง uuid จริง) ไม่เดินกิ่งที่พัง — ถ้า `E5-AC1`–`AC7` วัดผ่านทางนี้ครบ **ไม่ต้องรอ `E7` เพื่อปิด `E5`** แค่ต้องเขียนกำกับชัดว่าเส้นทางคลังสถิตย์ยังพิสูจน์ไม่ได้จนกว่า `E7` seed จริง (ห้ามให้ `E5` ติ๊กผ่านแบบไม่มีเงื่อนไขนี้กำกับ — ตระกูลเดียวกับ `§11`: เขียนแยก "พิสูจน์แล้ว" กับ "ยังเป็นคำเชื่อ")
>
...
- [ ] **E5-AC10** 📌 **ใหม่ 27 ส.ค. 2026 — เกณฑ์คู่ `E7`:** เส้นทางเพิ่มจุดแวะจาก **คลังสถิตย์** (`data/places.ts` ผ่าน `catalog_places`) ยังพิสูจน์ไม่ได้จนกว่า `E7` seed คลังลงจริง (ตารางยังไม่มี migration ไหน insert สักแถว) — **ปิด `E5` ได้ก่อน `E7` เฉพาะทาง custom-place (Google, uuid จริง)** ต้องเขียนกำกับชัดในผลวัดว่าเส้นทางไหนพิสูจน์แล้ว/เส้นทางไหนยังเป็นคำเชื่อ ห้ามติ๊กผ่านทั้งข้อโดยวัดแค่ทางเดียว
