...
- [ ] **E6-AC11** 🔴 **มินท์ใหม่ 30 ส.ค. 2026 (P1 เสนอ · เจ้าของงาน P3 · ส่งตรงถึง P8 ตามที่ P1 บอก) — รวมแหล่งข้อมูล "วัน" ให้เหลือ provider เดียว ไม่ใช่ต่างคนต่างยิง `/days` แล้วสร้าง `buildDayBridge()` เอง:**
      ...
      **เกณฑ์รับ — สองข้อ ข้อ ② สำคัญกว่า ①:**
      ...
      - [ ] **② โครงสร้าง — แก้ 30 ส.ค. 2026 (P1 grep จริงแล้วพบว่าเกณฑ์เดิมวัดไม่ได้ตามที่เขียน — ตรวจซ้ำเองแล้วตรงกัน):** เดิมเขียนว่า "grep `buildDayBridge(` เจอแค่ 1 บรรทัด" — grep จริงจับ `import`/ตำแหน่งชนิด (`ReturnType<typeof buildDayBridge>`)/คอมเมนต์ที่มีคำว่า `buildDayBridge(` อยู่ในนั้นปนกับผู้เรียกจริง (ยืนยันตรง: `dayBridgeIncomplete.ts:11`/`:108` มี "`buildDayBridge(...)`" อยู่ในคอมเมนต์ ก็ยัง match) **นับข้อความ ไม่ได้นับผู้เรียก** — ปิดงานได้ด้วยการลบคอมเมนต์โดยไม่แตะราก และถ้าเปลี่ยนชื่อฟังก์ชัน grep จะได้ 0 → อ่านเป็น "ผ่าน" ทั้งที่ไม่มีใครแก้อะไรเลย
        → **เปลี่ยนเป็น: `buildDayBridge` เลิก `export` จาก `lib/engine/dayBridge.ts`** ให้เหลือแค่ provider ตัวเดียว (อยู่ไฟล์เดียวกัน หรือเป็นที่เดียวที่ `import` ได้) เรียกตรง — **ระบบโมดูล/TypeScript บังคับให้เอง ไม่มีข้อความให้ grep ไม่มีชื่อให้ผูก คนเรียกใหม่คอมไพล์ไม่ผ่านทันที** (ผิดไม่ได้ ดีกว่าผิดแล้วจับได้ — รูปเดียวกับ `push` จาก worktree ที่ `HEAD` ขยับไม่ได้)
        · **ถ้าทำไม่ได้ด้วยเหตุผลทางโครงสร้าง (เช่น ต้องใช้ข้ามโมดูลจริงๆ):** เกณฑ์สำรองต้องนับ*นิพจน์การเรียก*ไม่ใช่ข้อความ — กรอง `import`/ประเภท/คอมเมนต์ออกก่อน (เช่น อ่าน AST ไม่ใช่ grep ข้อความดิบ) **และต้องมีเคสควบคุมฝั่งลบ**: ลบ/เปลี่ยนชื่อ `buildDayBridge` แล้วด่านต้องแดงเพราะ "หาไม่เจอ" ไม่ใช่เงียบผ่านเป็น 0
      ...
      📌 **หลักฐานทิศแดง ณ วันเปิด AC (30 ส.ค. 2026 — P8 ตรวจตรง):** `grep -rn "buildDayBridge(" hooks lib --include='*.ts' --include='*.tsx' | grep -v __tests__` วันนี้เจอผู้เรียกจริง **4 จุด** (`useStops.ts:113` · `useDaySettings.ts:84` · `useBookings.tsx:101` · `useOvernightOverrides.ts:85`) + ผู้ยิง `/days` ตรงอีก 1 (`usePlatformItinerary.ts:115`) — เกณฑ์ ② **แดงอยู่จริงวันนี้** จดไว้ให้วันปิดพิสูจน์ได้ว่ามันเคยแดงจริง ไม่ใช่เขียวมาตั้งแต่ต้น
