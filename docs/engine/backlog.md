...
- [x] **E6-AC12** ✅ **ปิดแล้ว (P3 · `b4a28a5` · P8 ตรวจไฟล์จริง + รัน `tsc --noEmit` เอง ก่อนติ๊ก — ไม่เชื่อแค่รายงาน)** เดิม 🔴 **มินท์ใหม่ 30 ส.ค. 2026 (P1 ตัดสิน — เต็มที่ [`resolve-place-design.md §7`](./resolve-place-design.md) · P8 อ่านไฟล์จริงก่อนจด ไม่ยกจากข้อความ) — มาแทนครึ่งหลังของ `AC1` เดิม (`PLACES`): `Place["city"]`/`["category"]` เลิกเป็นยูเนียนปิดที่ cast มาจากฐาน:**
      ...
      🔴 **ต้องทำก่อน `E6-AC13` เสมอ** (ลำดับผูกกัน — มาจากการอ่านโค้ด ไม่ใช่การรัน แต่ P3 ตรวจซ้ำอิสระแล้วยืนยันว่าประกอบ `Place` โดยไม่ cast ไม่ได้จริง ตราบใดที่ยูเนียนยังอยู่ — ถ้ามีใครพบภายหลังว่าประกอบได้โดยไม่ต้อง cast ลำดับนี้ผิดและต้องแก้กลับมาที่นี่)
...
- [ ] **E6-AC13** 🔴 **มินท์ใหม่ 30 ส.ค. 2026 (P1 ตัดสิน — เต็มที่ [`resolve-place-design.md §7`](./resolve-place-design.md) · P8 อ่านไฟล์จริงก่อนจด) — มาแทนครึ่งหลังของ `AC1` เดิม ส่วนที่สอง: `/stops` ส่ง side-map `places: Record<place_id, Place>` มาด้วย · `resolvePlace(id, { customPlaces, catalog })`:**
      ✅ **ปลดล็อกแล้ว — `E6-AC12` ปิดจริง (`b4a28a5`)** เดิม 🔴 **ต้องรอ `E6-AC12` ปิดก่อนเริ่ม** (เหตุผลอยู่ใน `AC12` ข้างบน — แก้ 30 ส.ค. ครั้งที่ 2: ไม่ใช่แค่ "เพิ่มจุด cast" แต่เป็นการสร้าง**เส้นทางประกอบ `Place` ใบที่สอง**ที่ cast แบบเดียวกับ `toPlace()` ใน `useCatalogPlaces.ts` — สองแหล่งความจริงของการแปลงเดียวกัน ถ้าชนิดยังไม่แก้ก่อน) — มอบ P3 ตามที่ P1 แจกงาน
      ...
      ✅ **ปิดคำถามเปิด 30 ส.ค. 2026 (P1 ตรวจเองแล้วยืนยันตรงกับที่ P8 พบ — `resolve-place-design.md §5 ข้อ 2` แก้แล้ว):** `app/api/place-name/route.ts` **ถอดออกจากขอบเขต `AC13`** — ไม่ใช่จุดเรียก `resolvePlace` ที่มีอยู่จริง (ยืนยันซ้ำอิสระ 2 รอบแล้วว่ามีแค่ในคอมเมนต์) และความสัมพันธ์จริง**กลับด้าน**จากที่เอกสารเคยเขียน: route นี้คือ**ตัวแก้ที่มีอยู่แล้ว**สำหรับจุดอ่อนของ `resolvePlace` เอง — `resolvePlace` fallback `name_en ?? name_th` ทำให้เอกสาร ตม./K-ETA มีชื่อไทยปน (เกิดจริง: "ตลาดปลาจากัลชิ") จึงมีการสร้าง `/api/place-name` ขึ้นมาดึงชื่ออังกฤษจาก Google แทนโดยเฉพาะ
      🔴 **ข้อควรรู้ที่ต้องถือไว้ตลอดงานนี้ (ไม่ใช่แค่ตอนเริ่ม):** ตอนรื้อ `resolvePlace` **ห้ามพา fallback `name_en ?? name_th` กลับเข้าเส้นทางหน้า ตม./K-ETA** — ไม่งั้นรื้อของเก่าทิ้งแล้วดึงบั๊กที่ `/api/place-name` แก้ไปแล้วกลับมาโดยไม่ตั้งใจ
      🔴 **แก้ 2 ก.ย. 2026 — ตัดสิน: เกณฑ์เดิม `= 0` ถอนแล้ว ด่าน ratchet เป็นฉบับผูกพันแทน (P1 ส่งมาให้ตัดสิน ไม่ตัดสินเอง เพราะเขาเป็นคนได้ประโยชน์ถ้าเลือกด่าน · P3 เป็นคนยกขึ้นมาเองทั้งที่มันขวางไม่ให้เขาปิดงานตัวเอง · P8 ตรวจไฟล์จริงทั้งสองฝั่งก่อนตัดสิน):**
      **เหตุผล:** `resolvePlace.ts:1,89-99` ยังคง `import { PLACES }` เป็น**ตาข่ายรับสุดท้ายโดยตั้งใจ** — ย้ายมาไว้ท้ายลำดับค้น (`catalog → transfer → hotel → custom → PLACES`) เมื่อ **2 ก.ย. 2026 (P1 ตัดสิน · P3 ทำ · ยืนยัน 3 ชั้นจาก P1/P2/P3)** **หลัง** `AC13` มินท์ (30 ส.ค.) โดยเจตนาเดิมคือ "ถ้าคิวรีคลังล้มแล้ว `/stops` คืน `places: {}` (คลังล่ม) ผู้ใช้ต้องไม่เห็นแย่ลงกว่าพฤติกรรมเดิมเลย แม้คลังล่มทั้งใบ" — ถอด `import` นี้ทิ้งเพื่อให้ grep ได้ 0 จริง **จะทำให้คลังล่มหนึ่งครั้ง = ทริปเกาหลีเสียชื่อสถานที่ทั้ง 72 แห่งทันที** เกณฑ์ `= 0` เดิมเขียนก่อนการตัดสินใจนี้เกิด จึงไม่ได้คิดถึงตาข่ายนี้ตอนเขียน — **การบังคับ `= 0` ตอนนี้เท่ากับบังคับให้ถอดตาข่ายความปลอดภัยที่เพิ่งตัดสินใจแยกกันไว้โดยเจตนา** ซึ่งเป็นการเปลี่ยนเกณฑ์รับหลังงานเสร็จไปในทิศที่สร้างความเสี่ยงใหม่ ไม่ใช่แค่ปรับถ้อยคำให้ตรงกับของจริง
      **เกณฑ์รับที่ถูกต้อง (แทนที่ `= 0`):** ด่าน `lib/__tests__/tripDataInBundle.test.ts` ("ratchet: มี route แพลตฟอร์มเปื้อนอยู่ 3 route เท่านั้น") ต้องผ่าน — `dirty` (route ที่ยังลาก chunk ข้อมูลเกาหลี) ต้องตรงกับ `PLATFORM_ROUTES` ที่ปักไว้ (`trip/[tripId]` · `trip/[tripId]/today` · `trip/[tripId]/summary`) เป๊ะ **ไม่มากกว่า ไม่น้อยกว่า** — เพิ่มขึ้น = ถอยหลัง (แดงทันที) ลดลงโดยไม่แก้ pin = ด่านก็แดงเช่นกัน (บังคับให้คนที่ทำให้ route ใดสะอาดขึ้นต้องหด `PLATFORM_ROUTES` เอง ไม่ใช่ปล่อยให้ pin ค้าง) **การถอดตาข่าย `PLACES` (ถ้าจะทำ) ต้องเป็น AC ของตัวเองแยกต่างหาก** ไม่ใช่ผลพลอยได้อัตโนมัติของ `AC13`
      🔴 **ช่องที่ด่าน ratchet เองมีอยู่ — ต้องปิดก่อน `AC13` จะปิดได้ ไม่ใช่แค่เปลี่ยนไปเชื่อมัน (P1 ชี้เอง):** `expect(dirty.sort()).toEqual([...PLATFORM_ROUTES].sort())` **เทียบลิสต์กับตัวมันเอง** — ถ้ามีคนเติม route ใหม่เข้า `PLATFORM_ROUTES` ตอนที่มันเปื้อนอยู่แล้ว ด่านจะเขียวทันทีโดยไม่มีอะไรฟ้อง (ตระกูลเดียวกับทะเบียนที่ไม่มีอะไรตรวจมันที่ P4 เจอมาแล้วหลายรอบ) **เกณฑ์ปิด `AC13` จึงต้องมีด่านที่สองด้วย:** ทุก route ภายใต้ `app/trip/[tripId]/**` ที่ *ไม่อยู่ใน* `PLATFORM_ROUTES` ต้องสะอาด (`taintedChunksOfRoute` ว่างเสมอ) ให้ `PLATFORM_ROUTES` ทำหน้าที่ได้แค่ "รายชื่อข้อยกเว้นที่ปิดไม่ได้ตอนนี้" ไม่ใช่ "รายชื่อที่ยกเว้นตัวเองจากการตรวจได้"
      📌 **ยังไม่วัด — คำทำนายของ P1 (การสลับลำดับใน `resolvePlace` ไม่แตะ `import` สักบรรทัด → ratchet ไม่ควรขยับ) ต้องรอ `npm run build` จริงก่อนปิด `AC13`**
