...
- [ ] **E7-AC8** 🔴 **ด่านแรกสุด — ไม่ผ่านข้อนี้ ห้ามแตะ schema ของ DB ทริปแม้แต่คำสั่งเดียว (`D21`)**
      ...
      🔴 **แก้ 28 ส.ค. 2026 (P1 พบตอนผู้ใช้ทำสำเนาแช่แข็งจริง — เกณฑ์เดิมเขียนจากสมมติฐานที่เป็นเท็จ):** "เห็นครบ 11 วัน" ตรวจไม่ได้เลย — **ไม่มีตาราง `days` ในฐานทริปจริง** (ยืนยันด้วย `column-map.md` เอง: 14 ตารางเดิมไม่มีตารางวันเลยสักใบ โครงวันอยู่ใน `data/itinerary.ts` ฐานเก็บแค่ `day_id` เป็นสตริง `'d0'`–`'d10'` ปรากฏใน `trip_stops`/`trip_day_settings`/`trip_selections`/`bookings`) — เกณฑ์เดิมบรรยายสภาพ*หลัง* `E7` ย้ายเสร็จแล้ว เอามาใช้เป็นด่าน*ก่อน*ย้าย
      · **เกณฑ์ที่ใช้จริง:** จำนวนแถวทุกตารางเท่าต้นฉบับ (14 ตาราง · 690 แถว ณ 28 ส.ค. 2026) **และ `day_id` ที่ปรากฏครบ `d0`–`d10`** — ห้ามเขียนเป็น "เห็นครบ 11 วัน" อีก
      🔴 **ไฟล์ dump ที่ยังไม่เคยถูก restore ไม่นับเป็น backup** — ข้อนี้คือทั้งหมดของแผน rollback จึงพิสูจน์ด้วยการเปิดดูเท่านั้น
      📊 **สถานะจริง 28 ส.ค. 2026:** ✅ ผู้ใช้ทำ `pg_dump` แล้ว (schema 29K + data 1.3M) นับแถวสองฝั่งตรงกัน 14 ตาราง/690 แถว/ต่างกัน 0 · ❌ **ยังไม่ได้ restore ขึ้นโปรเจกต์ที่ 2 — ข้อนี้คือทั้งหมดของเกณฑ์ ยังไม่ผ่าน** 🔴 บล็อกด้วยโควตา org (2 โปรเจกต์เต็มแล้ว: `engine-dev` ACTIVE + ของค้างชื่อ `muphoneseller` INACTIVE)
