...
- [x] **E2-AC8** ✅ **ปิดแล้ว 25 ส.ค. 2026 (P1 ยืนยัน)** เดิม 🔴 **`order_index` → rank key (`D6`)** — ลากจุดแวะ 1 จุดต้องเขียน DB **แถวเดียว** ไม่ใช่ n แถว · `sortStops` ต้องมี tie-break (วันนี้ `useStops.ts:13-17` ไม่มี → เลขซ้ำ = ลำดับไม่นิ่ง = บั๊ก 8.1 ที่ยังเปิดอยู่) · **ต้องมีกลไก rebalance** เพราะแทรกซ้ำที่เดิมทำให้คีย์ยาวขึ้นเรื่อย ๆ
      ...
      🔴 **แก้ 29 ส.ค. 2026 (P1 พบว่าบันทึกก่อนหน้านี้ล้าไปหนึ่งรอบ — ผมยึด sha ที่ได้รับมาแทนที่จะเช็ค `HEAD` ตอนกำลังจดสถานะปัจจุบัน — คนละกรณีกับ `§3.3` ที่บอกให้ยึด sha ของคนอื่นตอน *ยืนยันผลของเขา*, ตอนจด*สถานะไฟล์*ต้องอ่านจาก `HEAD`):** ของจริงคือ **หัก 2 รอบ 5 ทาง ไม่ใช่ 1 รอบ 3 ทาง**
      **รอบ 1 (`dc0bd36`) — 3 ทาง:** `B1` `export const … =>` และ `B3` `engineTable(db, STOPS_TABLE)` ผ่านตัวแปร ไม่เข้าจักรวาลเลย ไม่ถูกแจงออกมาด้วยซ้ำ · `B2` `.order("rank", { ascending: false })` ผ่านฉลุย — ฉบับแรกเทียบแค่*ตำแหน่ง* `.order` ไม่ดูทิศทาง คืนแถวย้อนลำดับทั้งวันแต่ `it.each` พิมพ์ `✓` เป็นหลักฐานเท็จ → แก้ด้วยเปลี่ยนจักรวาลเป็นไล่ทุก `.order("rank")` ในไฟล์ + ตรวจทิศทาง ยิงพิสูจน์ใน worktree (เขียว 10 ก่อน แล้ว B1/B2/B3 แดงทุกใบ)
      **รอบ 2 (`b6cd6bb`) — `B5`:** arrow function ที่ไม่มี `;` ปิด chain (ASI) ทำให้ regex หน้าต่าง 200 ตัวอักษรวิ่งข้ามขอบฟังก์ชันไปยืม `.order("id")` ของก้อน**ถัดไป**มาเป็นหลักฐานว่าตัวละเมิดผ่าน — รอดครั้งแรกเพราะระยะ 232 > 200 เท่านั้น แก้ด้วยหยุดที่ขอบ statement จริง (`;` / `\n\s*}` / `\n\s*\n` / `\nexport`) แทนการนับตัวอักษร
      🔴 **แก้ 29 ส.ค. 2026 (P1 พบรอบ 3 อีกที — อ่านจาก `HEAD` `5f97631` ตามที่บอก ไม่ใช่จาก sha ในข้อความ):** **`B7`** หักรอบ 2 ได้อีก — เพื่อนบ้านเป็น `const` ที่ไม่ `export` ไม่ตรงกับรายชื่อขอบ 4 โทเคน (`;`/`\n}`/`\n\n`/`\nexport`) ที่รอบ 2 เพิ่งตั้งไว้ site แรกจึงยืม `.order("id")` ของ `const` ตัวถัดไปมาเป็นหลักฐานของตัวเองอีกครั้ง
      🎯 **บทเรียนที่ P1 ขอให้จดแทนรายชื่อเคส — วินิจฉัยของ P4:** แก้ 2 รอบที่ผ่านมาเป็นการแก้*อาการ*โดยไม่รู้ตัว — รอบแรกย้ายขอบจาก *จำนวนตัวอักษร* (`B5` รอด 232>200) ไปเป็น *รายการโทเคน* (`B7` รอดเพราะ `const` ไม่อยู่ในรายการ) **คนละเลข ตรรกะเดียวกัน — ทั้งสองครั้งยังเป็นการแจงนับ** ต้นเหตุคือคำถาม *"statement นี้จบตรงไหน"* ต้อง parse ถึงจะตอบถูก **แจงนับกี่รอบก็มีเคสที่ห้าเสมอ** (P4 เจอเคสที่ห้าในการลองครั้งแรก ไม่ได้ไล่หลายรอบ — ความหนาแน่นของเคสที่เหลือยังสูงพอที่การอุดทีละใบจะไม่มีวันจบ)
      ✅ **ท่าที่ใช้แทน (P4 เสนอ · ยิงต้นแบบมาก่อนเสนอ) — เลิกถามเรื่องขอบ ถามว่าข้อความที่ติดกันถูกไหม:** `.order("rank",{ascending:true})` ต้องตามด้วย `.order("id",{ascending:true})` **ทันที** (regex `PAIR` ใน `stopOrderingContract.test.ts` ปัจจุบัน) — ไม่มีคำว่า "ขอบ" ในเงื่อนไข จึงไม่มีอะไรให้แจงนับผิด ยิงพิสูจน์ใน worktree ที่ `b6cd6bb`: สะอาด 10 passed ก่อน แล้ว `B1`/`B2`/`B3`/`B5`/`B7` แดงทุกใบ
      ⚠️ **ราคาของท่าใหม่ — สองข้อที่ P4 บังคับให้จดคู่กัน ไม่ให้คนมาเจอเอง:**
      ...
      🔴 **`B4` (arrow ที่ลืม `deleted_at`) — ยังเปิด และ P4 ยิงยืนยันแล้วว่าเปิดจริง ไม่ใช่แค่คาดไว้:** ท่าคู่ติดกันใช้กับครึ่งนี้ไม่ได้ตรงๆ เพราะ `deleted_at` ไม่ได้อยู่ติดกับอะไรที่ระบุตารางได้ — **ต้องออกแบบจักรวาลใหม่ ไม่ใช่ยืดท่าเดิมให้พอดี** (ยืดแล้วจะเป็นการแจงนับใบที่สาม ซึ่งเป็นบทเรียนข้างบนพอดี) **`E2-AC8` จึงปิดครึ่ง "ลำดับ" เท่านั้น (ครบทั้ง `B1`/`B2`/`B3`/`B5`/`B7`) ครึ่ง "`deleted_at`" (`B4`) ยังเปิด**
...
· ✅ **`E2-AC8` แก้แล้ว — แต่ปิดครึ่งเดียว ไม่ใช่ปิดครบ (ตัวเลขนี้แก้มาแล้ว 2 รอบซ้อน — เช็คกับ `HEAD` `5f97631` ทุกครั้ง ไม่ใช่ sha ในข้อความ):** เจอเป็นทิศ ② แล้วแก้ทันที ด่านฉบับแรกของ P1 เองหักได้ทั้งหมด **6 ทาง ใน 3 รอบ** (`B1`/`B2`/`B3` รอบแรก · `B5` รอบสอง · `B7` รอบสาม — ปิดครบทั้ง 5 ทางนั้นแล้ว) **บทเรียนสำคัญกว่ารายชื่อเคส (P4 วินิจฉัย):** รอบ 2/3 ยังเป็นการแจงนับขอบ (จำนวนตัวอักษร → รายการโทเคน) คนละเลขตรรกะเดียวกัน — **แจงนับกี่รอบก็มีเคสที่ห้าเสมอ** ท่าสุดท้ายเลิกถามเรื่องขอบ ถามว่าคู่ `.order("rank")`+`.order("id")` ติดกันไหมแทน — **แต่ครอบแค่ครึ่ง "ลำดับ" ครึ่ง "`deleted_at`" (`B4`) ยังไล่ทีละฟังก์ชัน ยังหลุดได้ถ้าเป็น arrow function — P4 ยิงยืนยันแล้วว่าเปิดจริง** (รายละเอียดเต็มที่ตัว `E2-AC8`)
