...
| C ถูกปฏิเสธครบทุกเคสของเมทริกซ์ **และ** เมทริกซ์ครอบทุกตารางที่มี RLS | 🔴 **แก้ 2 ก.ย. 2026 ครั้งที่ 2 — ถอนคำว่า "ผ่าน" ที่จดไว้รอบก่อน ผ่าเป็นสองท่อนแล้ว (P4 หักทัน · P1 รับ · P8 ตรวจไฟล์ก่อนแยก):** ประโยคนี้เป็นคำถามสองข้อ ไม่ใช่ข้อเดียวขยายความ — จำนวนเคสที่ผ่านตอบได้แค่ท่อนแรก | ดูราย ละเอียดที่ `4a`/`4b` ด้านล่าง |
...
| ↳ **4b: เมทริกซ์ครอบทุกตารางที่มี RLS** | 🔴 **ยังไม่มีอะไรวัด — ไม่ใช่ "ยังไม่ผ่าน" แต่ "ยังไม่มีเครื่องวัด"** | **ตรวจแล้ว ไม่มีด่านไหนตอบคำถามนี้ในทรีวันนี้:** `readOnlyCoverage` (`schemaPins.test.ts`) วัดความครอบคลุมของด่าน *read-only mode* คนละเรื่อง ไม่ใช่ความครอบคลุมของเมทริกซ์ RLS · หัวไฟล์ `rlsMatrix.test.ts:22-29` **ประกาศเองว่าขอบเขตที่เขียนไว้ล้า** ("ครอบ 3 ตารางของ `E1`" — "ขอบเขตที่เขียนไว้แคบกว่าของจริง อ่านแล้วเลิกตรวจ" ยืนยันตรงไฟล์) → อ่านจากคอมเมนต์ก็ตอบไม่ได้ **ต้องสร้างด่านที่เทียบเซตตารางที่เมทริกซ์ยิงจริง กับเซตตารางที่ `enable row level security` จริงในสคีมา** — ตัวเลข "14 ตาราง" ที่ลอยอยู่ (P4 ไล่เอง) เป็นเบาะแสไม่ใช่จำนวน (ผมลองไล่เองด้วยแพทเทิร์นคนละแบบได้ 17 — คนละเรื่องกับ "313" ที่มีคนวัดตรงกันสองคนแล้ว)
📌 **ด้ายที่ยังไม่ขาด — รอ P4 ตอบ ก่อนถือว่า `4b` เป็นของใหม่ทั้งใบ (P1 ตั้งคำถามไว้ 2 ก.ย. 2026 ยังไม่ปิด):** `E2-AC11` (ปิดแล้ว 25 ส.ค. — 51/51 กิ่ง · 4/4 ตาราง 0-policy) เขียนเกณฑ์ไว้ว่า "ทุกตารางที่มี RLS ในสคีมาใหม่ต้องอยู่ในเมทริกซ์ — วัดด้วยการนับตาราง ณ วันปิด `E2` แล้วเทียบกับจำนวนแถวในเมทริกซ์" ซึ่ง**อ่านเหมือนเป็นบ้านของคำถาม `4b` อยู่แล้ว** ตัวเศษ (จำนวนแถวเมทริกซ์) ยืนยันแล้วว่าดึงจากซอร์สเทสต์เอง ไม่ใช่ทะเบียนที่คนกรอก (`_runIntegrity.ts:22`: *"ตัวนับ E2-AC11 อ่านจากซอร์สว่ามีเคสอยู่"*) — **แต่ยังไม่ยืนยันว่าตัวส่วน (จำนวนตารางที่มี RLS) มาจากสคีมาจริงหรือมาจากรายการที่เขียนไว้ในซอร์สเทสต์เองเหมือนกัน** ถ้าเป็นอย่างหลัง `E2-AC11` จะเป็นทะเบียนที่ตรวจตัวเอง และ `4b` ก็ยังว่างเปล่าเหมือนเดิม ถ้าเป็นอย่างแรก `4b` ไม่ใช่งานสร้างใหม่ทั้งใบ แต่เป็นแค่ "เอาตัวหารที่ถูกอยู่แล้วมาต่อเป็นด่านแยก" — คนละขนาดงาน ยังไม่ตัดสิน รอ P4 |
