...
- [ ] **E6-AC13b** 🔴 **มินท์ใหม่ 2 ก.ย. 2026 (P3 เสนอแยก · P1 เห็นด้วย · P8 ตรวจไฟล์จริงก่อนจด) — ครึ่งที่สองของ `AC13` เดิม: `data/places.ts` ออกจากบันเดิลของ route แพลตฟอร์มจริง (`trip/[tripId]`/`today`/`summary`)**
      ...
      ✅ **ด่าน ratchet ของ `AC13a`/`AC13b` ยังต้องปิดช่องตัวเองก่อนใช้วัด `AC13b` ได้จริง (มอบ P4):** `expect(dirty.sort()).toEqual([...PLATFORM_ROUTES].sort())` **เทียบลิสต์กับตัวมันเอง** — ถ้ามีคนเติม route ใหม่เข้า `PLATFORM_ROUTES` ตอนที่มันเปื้อนอยู่แล้ว ด่านจะเขียวทันทีโดยไม่มีอะไรฟ้อง (ตระกูลเดียวกับทะเบียนที่ไม่มีอะไรตรวจมันที่ P4 เจอมาแล้วหลายรอบ) — **เกณฑ์ปิด `AC13b` ต้องมีด่านที่สองด้วย:** ทุก route ภายใต้ `app/trip/[tripId]/**` ที่ *ไม่อยู่ใน* `PLATFORM_ROUTES` ต้องสะอาด (`taintedChunksOfRoute` ว่างเสมอ) ให้ `PLATFORM_ROUTES` ทำหน้าที่ได้แค่ "รายชื่อข้อยกเว้นที่ปิดไม่ได้ตอนนี้" ไม่ใช่ "รายชื่อที่ยกเว้นตัวเองจากการตรวจได้" — 📌 **ผูกกับ `AC13b` ไม่ใช่ `AC13a`** งานนี้จึงไม่ด่วนเท่าที่ P4 อาจเข้าใจตอนได้รับมอบหมายครั้งแรก (ตอนนั้นยังผูกกับ `AC13` เดี่ยว)
