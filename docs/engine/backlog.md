...
| เห็นทริปเกาหลีครบทุกแถว | 🟡 **ใช้ได้บน `engine-dev` เท่านั้น ยังไม่ใช่บน DB ทริปจริง** | รีเฮิร์สอลบน `engine-dev` ยืนยันแล้ว 868 แถว/11 วัน/62 จุดแวะ/18 event และ P2 เทียบบนจอจริงแล้ว (การ์ด 11 ใบ · 62+18=80 คานกัน — อ้างอิงตามที่ P1/P2 รายงาน ยังไม่ได้ยิงยืนยันเองอิสระรอบนี้) — **แต่ `E7`/`E9` (ย้ายเข้า DB ทริปจริง + cutover) ยังไม่เริ่มเลย** ดังนั้นบน DB ทริปจริงวันนี้ A/B ยังเห็นของเดิม ไม่ใช่ของแพลตฟอร์ม |
| C ถูกปฏิเสธครบทุกเคสของเมทริกซ์ | 🔴 **ไม่มีหลักฐานของวันนี้** | หัว `bd17c9a` (ค้างส่ง 30 คอมมิต) — ชุดที่ไม่แตะฐานเขียว 0 แดง **แต่เมทริกซ์ RLS ที่แตะฐานไม่ได้ถูกรันวันนี้** (รอผู้ใช้) เลข "313 เคส" เป็นของ P1 รายงาน ไม่ได้ยิงยืนยันเองอิสระ — **ห้ามอ้างผลรันเก่าว่าผ่านข้อนี้** |
