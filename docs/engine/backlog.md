...
- [ ] **E7-AC8** 🔴 **ด่านแรกสุด — ไม่ผ่านข้อนี้ ห้ามแตะ schema ของ DB ทริปแม้แต่คำสั่งเดียว (`D21`)**
      ...
      **③ แยกคำถามที่ถูกมัดไว้ด้วยกัน** — "dump นี้ restore ได้จริงไหม" (ไม่ต้องใช้โควตา org) ≠ "มีโปรเจกต์ที่ 2 ไว้ซ้อม migrate ไหม" (ต้องใช้ — `AC1`/`AC6` พึ่งมัน) เกณฑ์เดิมบังคับตอบข้อหลังก่อนถึงจะตอบข้อแรกได้ **รูปเดียวกับ "เห็นครบ 11 วัน" ที่แก้ไปรอบ 84 แค่ครึ่งเดียว** — บรรยายสภาพปลายทาง (มีโปรเจกต์พร้อมแล้ว) แล้วใช้เป็นด่านแรกสุด
      · **เกณฑ์ที่ใช้จริง (แทนของเดิมทั้งหมด):**
        - **restore ได้จริง** — restore `trip-schema.sql` + `trip-data.sql` ขึ้นที่ไหนก็ได้ที่มี Postgres เปล่า (**ไม่ต้องใช้โควตา org**) แล้วนับแถวตรงกับไฟล์ dump เอง (670 ทั้งก้อน) — ผ่านข้อนี้แล้วถึงนับเป็น backup จริง: **"ไฟล์ dump ที่ยังไม่เคยถูก restore ไม่นับเป็น backup"**
        - **เนื้อทริป 203 แถวต้องตรงเป๊ะ** — ห้ามขาดแม้แถวเดียว (11 ตารางที่ไม่ใช่แคช)
        - **แคช 467 แถวขาดได้ไม่ตก AC** — สร้างใหม่ได้เองจาก Google API
        - **`day_id` ปรากฏครบ `d0`–`d10`**
        - **โปรเจกต์ที่ 2 พร้อมซ้อม migrate** — เงื่อนไขแยกต่างหาก ใช้ตอนเริ่ม `AC1`/`AC6` ไม่ใช่ส่วนหนึ่งของ "เป็น backup หรือไม่"
      📊 **สถานะจริง 28 ส.ค. 2026:** ✅ ผู้ใช้ทำ `pg_dump` แล้ว (schema 29K + data 1.3M) ✅ นับแถวจากไฟล์เอง 670/670 ตรงกันสองวิธีอิสระ (P1 + P8) ❌ **ยังไม่ได้ restore ที่ไหนเลยสักครั้ง — ข้อนี้คือทั้งหมดของเกณฑ์ ยังไม่ผ่าน** 📌 restore เพื่อพิสูจน์ backup ไม่ต้องรอโควตา org (ดูเกณฑ์ข้างบน) — โควตาบล็อกแค่ขั้นซ้อม migrate (`AC1`/`AC6`) คนละขั้นกัน (2 โปรเจกต์เต็มแล้ว: `engine-dev` ACTIVE + ของค้างชื่อ `muphoneseller` INACTIVE)
      ...
      📖 ตารางแถวต่อตาราง + ท่า restore ทีละขั้น: [`e7-ac8-restore-runbook.md`](./e7-ac8-restore-runbook.md) (P1 เขียน 28 ส.ค. 2026 — ยังไม่ commit ตอนที่จดบรรทัดนี้)
