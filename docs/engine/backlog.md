...
- [ ] **E7-AC1** สคริปต์ migrate รันจบโดยไม่มี error บนโปรเจกต์ซ้อม ที่ **restore มาจากสำเนาแช่แข็ง** (ไม่ใช่ข้อมูลสังเคราะห์ — ของจริงมีเคสที่แต่งเองไม่ถูก เช่น `added_by` ที่สะกดต่างกัน `D19`)
      ...
      📌 **`E7` ก้อน 01–08 ครบทุกตารางที่มาจาก legacy แล้ว** (ตรวจตรงดิสก์: `supabase-platform/e7/{01..08}_*.sql` มีจริงครบ 8 ไฟล์ + `09_completeness.sql` — P1 กำลังทำก้อน 09 เป็นด่านความครบถ้วน ยังไม่เสร็จ) `trip_selections` (ตารางที่ 14) ไม่ต้องมีก้อนเพราะ `E2-AC7` สั่งลบทั้งตารางไปแล้ว (ยืนยันย้อนกับข้อ 1218 ในไฟล์นี้)
