...
- [ ] **E2-AC2** ✏️ **แก้ 25 ส.ค. 2026 (P1 ชนเข้าจริงตอนเขียน `catalog_countries`/`catalog_cities`):** `grep -c 'using (true)' <migrations ใหม่ทั้งหมด>` = **0 นอกตารางคลัง** · **ตารางคลังที่ยกเว้นต้องถูกระบุชื่อครบในเกณฑ์ ห้ามเขียนว่า "ยกเว้น catalog" เฉยๆ**
      ...
      **เกณฑ์: ทุกตารางที่มี RLS ในสคีมาใหม่ ต้องอยู่ในเมทริกซ์ ไม่มีข้อยกเว้น** — วัดด้วยการนับตาราง ณ วันปิด E2 แล้วเทียบกับจำนวนแถวในเมทริกซ์ **ต้องเท่ากัน**
      ...
      · 🔴 **แก้ 25 ส.ค. 2026 (`D69`/`P-51`) — ตัวหารขยับอีกครั้งก่อน `E2` จะเริ่มด้วยซ้ำ:** `column-map.md` สั่งยุบ `trip_day_settings` เข้า `trip_days` แต่ `plan_id` ("คงเดิม") ยืนอยู่ไม่ได้ถ้ายุบจริง (`usePlans.ts:104` ก๊อปตั้งค่ารายวัน**ต่อแผน**อยู่จริงวันนี้)
        ...
        · 🔴 **แก้ 25 ส.ค. 2026 (`D77`) — ตัวหารขยับเป็นเหตุผลที่ 4:** `place_details_cache` แยกเป็น `place_details_cache` + `place_details_local_cache` (ดูเหตุผลที่ `E2-AC6`) — ตารางใหม่นับรวมในเมทริกซ์ด้วย
          📊 **นับจากฐานจริง (P1 · 25 ส.ค. 2026):** **22 ตารางใน `public`** ไม่ใช่ 18 ตามที่ `column-map.md` ออกแบบไว้ · ส่วนต่าง 4 ใบตรงกับ 4 เหตุผลด้านล่างเป๊ะ: `trip_day_plan_settings` (`D69`) · `custom_place_names` (`D75`) · `catalog_countries` (`D74`) · `place_details_local_cache` (`D77`) · P1 เติมบล็อก "นับจากฐานจริง" พร้อมคำสั่งรันซ้ำได้ไว้ท้าย `column-map.md` แล้ว — **เป็นข้อมูลเข้า เจ้าของเกณฑ์ยังเป็น P4**
          📌 **ตอนนี้ตัวหารขยับมาแล้ว 4 เหตุผลที่ไม่เกี่ยวกันเลย** (เพิ่มตาราง `E1` · แยก `trip_day_plan_settings` แทนยุบ · คลังนับรวมด้วย · แยก `place_details_cache`) — **หลักฐานสะสมว่าทำไมห้ามเขียนเกณฑ์เป็นตัวเลขคงที่แม้แต่ครั้งเดียว ไม่ใช่แค่ครั้งแรกที่เจอ**
...
- [ ] **E2-AC13** 🔴 **ใหม่ 24 ส.ค. 2026 — `bookings.file_url` ต้องเลิกเป็น URL ก่อนปิด bucket ไม่ใช่หลัง**
      ...
      **แก้ 25 ส.ค. 2026 — แยกสถานะ 3 ข้อของ AC นี้ให้ชัด (P1):**
      ...
      · **② เปิดไฟล์ตั๋วของแถวเดิมได้จริงจากในเว็บ หลังปิด bucket** — 🔴 **ยังไม่ปิด** ไม่ใช่แค่เปิดจากข้างนอกไม่ได้ (ฝั่งโค้ด: ทุกจุดอ่านต้องเซ็นตอนใช้ + `E7`: ย้ายค่าเดิมจริง) — สิ่งที่ปิดแล้ววันนี้คือ**กลไก**ใช้ได้ ยังไม่ใช่ว่าเว็บใช้มันแล้ว
      · **③ `sw.js` `cacheFirst` กับ signed URL** — 🔴 **ยังไม่ปิด ส่งให้ P3 แล้ว:** `public/sw.js:113,118` ใช้ `cacheFirst` กับ ASSET_CACHE · **signed URL เปลี่ยนลายเซ็นทุกครั้งที่เซ็น = URL ไม่ซ้ำเดิม = cache ไม่มีวัน match**
...
- [ ] **E2-AC14** 🔴 **ใหม่ 24 ส.ค. 2026 — ครึ่งสคีมาของ `~~E1-AC5~~` ที่ย้ายมาจาก `E1` (`D58`)**
      ...
      🔴 **`P-55` — ของที่ใหญ่กว่าเกณฑ์ที่กำลังไล่ ยิงจริงแล้ว 25 ส.ค. 2026:** B (editor) เพิ่มแถว `checklist_items` (`added_by_user`=B, `legacy_added_by`=null) → ลบบัญชี B → อ่านกลับ: **`added_by_user`=null · `legacy_added_by`=null พร้อมกัน ไม่มีอะไรบอกได้อีกเลยว่าใครเพิ่ม ไม่มี error ไม่มีร่องรอย** ทั้งที่ `column-map.md` เขียนเหตุผลของคอลัมน์นี้เองว่า "ห้ามทิ้ง" — **ออกแบบให้ประวัติรอดสำหรับของที่ย้ายมา แต่ปล่อยให้มันตายสำหรับของที่เกิดใหม่ สลับข้างกับสัญชาตญาณพอดี**
        → **`Q4` เปิดแล้วใน `README.md` — ข้อแรกในทะเบียนที่ระบุว่ารอผู้ใช้ ไม่ใช่รอทีม:** คนที่ลบบัญชีตัวเอง ควรมีชื่อค้างในทุกแถวที่เคยแตะไหม (`display_name` เก็บสำเนา / ข้อความไม่ระบุตัวตน "ผู้ใช้ที่ลบบัญชีแล้ว" / ยอมให้ประวัติหาย)
        · 🔴 **บล็อกการเขียน trigger:** ทั้งสามทางใช้ trigger ตัวเดียวกัน ต่างกันแค่ค่าที่เขียน **แต่เขียนไปแล้วถอนกลับไม่ได้**สำหรับคนที่ลบบัญชีไประหว่างนั้น — เป็นการตัดสินใจทิศทางเดียว **ต้องถามก่อนทำ ไม่ใช่ทำก่อนแล้วปรับ** ห้ามเริ่ม trigger นี้ก่อน `Q4` ตอบ
