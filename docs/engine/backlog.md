...
- [ ] **E4-AC8** 🔴 **มินท์ใหม่ 28 ส.ค. 2026 (P1 พบระหว่างวัดเรื่องอื่น ส่งให้ P8) — คลังสถานที่ไม่มี `maps_query`/`google_place_id` เลยสักแถว** (มี `lat`/`lng` ครบ):** ผลคือ **รูป · เวลาเปิด-ปิด · เรตติ้ง ของสถานที่ในคลังโหลดไม่ได้ตามนิยาม** — หน้าเว็บยิง `?queries=` เปล่าไปที่ Google ได้ `400` ทุกครั้งที่เปิด
      ...
      🔴 **ยังไม่มีตัวเลขจำนวนแถว — ห้ามเดา:** วัดตอนที่มีชุดเทสต์สดรันขนานอยู่ ตัวเลขขยับ (950→948 ระหว่างวัด) เท่ากับ**วัดของที่เคลื่อนที่** ไม่ใช่ค่านิ่ง — รอวัดใหม่แล้วส่งตัวเลขจริงมา ก่อนหน้านั้นถือว่ายังไม่มีข้อมูลจำนวน
      · **ผูกกับ `E4` ไม่ใช่ `E5`** (P1 เสนอ — คลังต้องใช้ได้จริงคือเกณฑ์ของ `E4` เหมือน `AC7`) รับไว้ตามนั้น
