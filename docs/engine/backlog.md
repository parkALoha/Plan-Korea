...
> · ✅ **`E1-AC8` ปิด** — `authProviders.test.ts` เขียวสด แต่ **ความรุนแรงเปลี่ยน**: ยืนยันแล้วว่า auto-link เปิดจริง (วัดจากพฤติกรรม ไม่ใช่ config) → `AC8` ไม่ใช่แค่ระเบียบ แต่คือด่านเดียวที่กัน account takeover ตอนมี provider ที่ 3 ที่ไม่ verify — ถ้อยคำบังคับ (ห้ามย่อ): *"auto-link เปิด · link บน verified email ปลอดภัย · takeover ถูกกันโดย `AC8` pin ไม่ใช่โดยการทดสอบนี้"*
> · 🔴 **`P-77` มินท์ใหม่ (P1+P4):** ทั้งคู่สรุปว่า "ทิศ auto-link เดินไม่ได้ ต้องมีบัญชี Google ที่สอง" — ผู้ใช้คิดออกเอง (สมัคร magic link ก่อน ไม่ต้องมีบัญชี Google ที่สองจนขั้นสุดท้าย) **ไม่ใช่ `P-76`** (ไม่มีเงื่อนไขนำให้ตรวจ ไม่มีอะไรเชื่อผิด) เป็นคนละกลไก: เอาขอบเขตของ*ทางที่นึกออก*มาอ่านเป็นขอบเขตของ*ปัญหาจริง*
...
- [x] **E2-AC6** ✅ **ปิดตามถ้อยคำ 26 ส.ค. 2026 (P8 ตัดสิน — ดูเหตุผลท้ายบล็อก) — `data/*.ts` แยกเป็น `E2-AC15`** เดิม 📋 **เกณฑ์ที่ยกความเสี่ยงของ E7 มาไว้ที่นี่:** มีตารางแมป **ทุกคอลัมน์ของ 14 ตารางเดิม → ปลายทางในสคีมาใหม่** และคอลัมน์ไหนที่ตั้งใจทิ้งต้องเขียนเหตุผลกำกับ · **ห้ามมีคอลัมน์ไหนที่ไม่มีคำตอบ** (ถ้าไม่ทำข้อนี้ที่ E2 จะไปรู้ตอน E7 ว่าสคีมาใหม่เก็บของเดิมไม่ได้ ซึ่งแปลว่าต้องรื้อ E2–E5 ใหม่ทั้งหมด)
      ...
      **เกิดจริง 3 ครั้ง:** P4 — id ปลอมยิง `soft_delete_*` ได้ `P0001` เกือบรายงานว่า "ถูกบล็อกแล้ว" ทั้งที่ล้มที่ขั้น lookup **ก่อน**ถึงจุดที่ trigger ยิง · P1 (`D83`) — เห็นว่าเลขซ้ำ*เป็นไปได้* แต่ไม่ตรวจว่าซ้ำ*จริง*ก่อนมินท์ · P1 (`linked-project.json`) — เห็นไฟล์ link → สรุปว่า `db push` จะรัน migration ทริปใส่ `engine-dev` **P6 ยิงจริงพบว่า CLI หยุดเองที่ `Cannot find project ref`** เพราะไฟล์ที่ CLI อ่านคือ `project-ref` คนละไฟล์กับที่ P1 ดู — P1 รายงานผู้ใช้ไปด้วยความรุนแรงเกินจริง แก้กับผู้ใช้แล้ว
      ...
      🔴 **`P-77` ใหม่ 26 ส.ค. 2026 (P1+P4 · `E1-AC7`) — รูปที่ไม่ใช่ `P-76`: เอา "ทางที่เรานึกออก" ไปอ่านเป็น "ข้อจำกัดของปัญหาเอง":**
      P1 กับ P4 สรุปตรงกันไว้ก่อนหน้านี้ว่า *"ทิศ auto-link (magic link→Google) เดินไม่ได้ ต้องมีบัญชี Google ที่สอง ผู้ใช้ไม่มี เลื่อนไปก่อน"* — **ผู้ใช้คิดออกเอง: สมัครฝั่ง magic link ก่อน ซึ่งไม่ต้องใช้บัญชี Google ที่สองจนถึงขั้นสุดท้าย** ทั้งสองคนคิดไม่ออก
      🎯 **ต่างจาก `P-76` ตรงจุดนี้เป๊ะ:** `P-76` คือเชื่อคำอธิบายที่ไม่เคยถูกยิง (มีเงื่อนไขนำที่ตรวจได้แต่ไม่ตรวจ) — เคสนี้**ไม่มีเงื่อนไขนำให้ตรวจเลย ไม่มีอะไรผิด ไม่มีอะไรให้เชื่อผิด** สิ่งที่เกิดคือ **เอาขอบเขตของทางที่นึกออก (ต้องมีบัญชี Google 2 ใบ) มาอ่านเป็นขอบเขตของปัญหาจริง (ทิศนี้เดินไม่ได้)** — คนละกลไก แม้ผลลัพธ์หน้าตาคล้ายกัน (สรุปผิดแล้วแก้ทีหลัง)
      📌 **สังเกต:** ยิ่งคนคิดเก่งด้วยกัน ยิ่งเสี่ยงแบบนี้มากขึ้น ไม่ใช่น้อยลง — เพราะทางที่ "คนเก่งสองคนนึกไม่ออก" ให้ความมั่นใจปลอมว่าไม่มีทางอื่นจริงๆ ทั้งที่แปลได้แค่ว่า**ไม่มีทางอื่นในเซตที่สองคนนี้ค้นมา**
