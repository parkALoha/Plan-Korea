...
- [ ] **E8-AC4** เขียนลง DB ได้ต้องผ่าน DAL ของ E3 และ RLS ของ E2 เหมือนผู้ใช้ทั่วไป — **Copilot ไม่มีสิทธิ์พิเศษ** · วัดด้วย: เรียก DAL ด้วย identity ของ Copilot ตรงๆ ให้พยายามแก้ทริปที่ไม่ได้เป็นสมาชิก แล้วต้องถูกปฏิเสธ — วัดที่ชั้น DAL/RLS ได้โดยไม่ต้องเรียกโมเดลจริง จึงยังทำได้วันนี้
- [ ] **E8-AC5** มีเพดานค่าใช้จ่าย/rate limit ต่อผู้ใช้ ป้องกันบิล LLM วิ่ง — งานตั้งค่า infra/rate-limiter วัดด้วยการนับ call ได้โดยไม่ต้องเรียกโมเดลจริง จึงยังทำได้วันนี้
      🔴 **แก้ 29 ส.ค. 2026 (P5 พิสูจน์แล้ว):** ถ้อยคำเดิมไม่ระบุทิศแดง/เคสควบคุม (แค่บอกว่า "มีเพดาน" ไม่บอกว่าเกินเพดานแล้วต้องเกิดอะไร) — **เกณฑ์ที่ใช้จริง: ยิงเกินเพดานแล้ว call ต่อไปต้องถูกปฏิเสธก่อนถึงโมเดล (ไม่ใช่แค่ log)** พร้อมเคสควบคุมฝั่งบวก (ยิงไม่เกินเพดานต้องผ่านปกติ)
