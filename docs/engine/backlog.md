...
> · ✅ **สถานะหัว `platform` อัปเดต 27 ส.ค. 2026 (P1 วัดเอง) — เขียวหมดแล้ว:** ทั้ง 2 จุดที่เคยแดงปิดแล้ว — P4 แก้ `signIn.test.ts` (`24efe59`) · P6 แก้ `check-naive-strip` จับตัวเอง (`4e796da`) · ตอนนี้เทสต์ **797/56 ไฟล์** · lint สะอาด · `guards.sh` **exit=0 · 17/17** · self-test **111** · **64+ commit ยังไม่ push** — ทุกคนตรงกันว่าสิทธิ์อนุมัติเป็นของผู้ใช้แต่ละเซสชัน ผู้ใช้ P1 ยังหลับ ไม่ push
>
...
- [x] **E2-AC6** ✅ **ปิดตามถ้อยคำ 26 ส.ค. 2026 (P8 ตัดสิน — ดูเหตุผลท้ายบล็อก) — `data/*.ts` แยกเป็น `E2-AC15`** เดิม 📋 **เกณฑ์ที่ยกความเสี่ยงของ E7 มาไว้ที่นี่:** มีตารางแมป **ทุกคอลัมน์ของ 14 ตารางเดิม → ปลายทางในสคีมาใหม่** และคอลัมน์ไหนที่ตั้งใจทิ้งต้องเขียนเหตุผลกำกับ · **ห้ามมีคอลัมน์ไหนที่ไม่มีคำตอบ** (ถ้าไม่ทำข้อนี้ที่ E2 จะไปรู้ตอน E7 ว่าสคีมาใหม่เก็บของเดิมไม่ได้ ซึ่งแปลว่าต้องรื้อ E2–E5 ใหม่ทั้งหมด)
      ...
      **เคสของ P1:**
      ...
      📌 **ยังไม่มีตัวอย่างที่สองยืนยัน** — P1 อ้างว่ามี แต่ในข้อความที่ส่งมาไม่ชัดว่าชี้ไปที่ไหน ถามคืนแล้ว รอยืนยันก่อนถือว่าเป็นรูปแบบที่เกิดซ้ำ ตอนนี้มีแค่ 1 เหตุการณ์
