...
- [x] **E2-AC2** ✅ **ผ่านแล้ว 25 ส.ค. 2026 พร้อมหลักฐาน:** `using (true)` ใน `E2` = 4 จุดเป๊ะ ระบุชื่อครบ (`catalog_countries`/`catalog_cities` ที่ `20260825132854_e2_catalog_geo.sql:123,127` · `catalog_places`/`catalog_place_names` ที่ `20260825134043_e2_catalog_places.sql:171,175`) **นอกคลัง = 0** ตรงตามเกณฑ์เป๊ะ เดิม ✏️ **แก้ 25 ส.ค. 2026 (P1 ชนเข้าจริงตอนเขียน `catalog_countries`/`catalog_cities`):** `grep -c 'using (true)' <migrations ใหม่ทั้งหมด>` = **0 นอกตารางคลัง** · **ตารางคลังที่ยกเว้นต้องถูกระบุชื่อครบในเกณฑ์ ห้ามเขียนว่า "ยกเว้น catalog" เฉยๆ**
...
- [ ] **E2-AC6** 📋 **เกณฑ์ที่ยกความเสี่ยงของ E7 มาไว้ที่นี่:** มีตารางแมป **ทุกคอลัมน์ของ 14 ตารางเดิม → ปลายทางในสคีมาใหม่** และคอลัมน์ไหนที่ตั้งใจทิ้งต้องเขียนเหตุผลกำกับ · **ห้ามมีคอลัมน์ไหนที่ไม่มีคำตอบ** (ถ้าไม่ทำข้อนี้ที่ E2 จะไปรู้ตอน E7 ว่าสคีมาใหม่เก็บของเดิมไม่ได้ ซึ่งแปลว่าต้องรื้อ E2–E5 ใหม่ทั้งหมด)
      ...
      🔴 **`E2-AC6` ยังติ๊กไม่ได้จนกว่า `Q5` ปิด** — ครึ่ง DB (14 ตารางเดิม → 22 ตารางใหม่) ครบแล้ว **ครึ่งไฟล์ (`data/itinerary.ts` → ปลายทางในสคีมาใหม่) ยังไม่มีคำตอบ**
...
- [x] **E2-AC11** ✅ **ปิดแล้ว 25 ส.ค. 2026 โดย P4 — 49/49 กิ่ง · 4/4 ตาราง 0-policy** เดิม 🔴 **ใหม่ 24 ส.ค. 2026 — ตัวหารของเมทริกซ์ต้องนับจากสคีมาใหม่ ไม่ใช่จากสคีมาวันนี้** (พบจาก [`D45` จังหวะที่ 1 กับ E2](#-d45-จังหวะที่-1-กับ-e2--เดินตามรอยต่อระหว่างเอกสารคนละคน))
