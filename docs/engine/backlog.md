...
- [ ] **E6-AC4** offline อ่านอย่างเดียวยังใช้ได้จริงบน airplane mode ~~(ผู้ใช้ยืนยันเองแล้วครั้งหนึ่ง 13 ส.ค. 2026)~~
      ...
      | checklist | `useChecklist` | 🔴 **ไม่แคชเลย** (`git log -S readCache -- hooks/useChecklist.ts` = ว่าง — ไม่เคยมีแม้แต่คอมมิตเดียว) |
      | สถานที่ที่ซ่อนไว้ | `useHiddenPlaces` | 🔴 **ไม่แคชเลย** |
      ...
      🔴 **ยังไม่ผ่าน — 🔴 สองแถวยังแดง** (`useChecklist`/`useHiddenPlaces`) จนกว่าจะแคช · แถว 🟡 สองแถวรอ P7 ตัดสินก่อนว่านับหรือไม่ (`mobile-arch.md §13.1`)
...
- [ ] **E6-AC7** 🔴 **ย้าย `localStorage` → IndexedDB (`D17` — ของที่ `D14` สร้างขึ้นใหม่ ไม่ใช่ของเก่า)**
      ...
      | hook ที่ย้ายแล้ว: `usePlatformItinerary` · `useCatalogPlaces` · `useTripCatalogCities` | ✅ 3 ตัว |
      ...
      | ผู้เรียกที่เหลือ (~5 hook โซน P3 · `personalLocalValue` โซน P2) | ⏳ ยังไม่ย้าย ไม่ใช่ไฟล์ P7 |
