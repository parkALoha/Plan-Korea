...
- [ ] **E7-AC9** 🔴 **มินท์ใหม่ 28 ส.ค. 2026 (P7 เสนอ · P1 รับ — P1 เคยบอก P7 ว่าจดแล้วแต่ไม่ได้จด ตรวจแล้วยืนยันว่าไม่เคยมีในไฟล์นี้ก่อนวันนี้) — ทริปที่แผนรายวันอยู่ใน `data/itinerary.ts` ต้องมี *ตัวระบุ* ในฐาน ไม่ใช่ให้โค้ดเดาจากรูปข้อมูล:**
      ...
      🔴 **เพิ่ม 28 ส.ค. 2026 (P7 เสนอผ่าน P1 — เงื่อนไขจังหวะ ไม่ใช่แค่รูปแบบ):** **ตัวระบุต้องถูกตั้งโดย migration ที่สร้างแถว `trip_days` ในขั้นตอนเดียวกัน — ห้ามเป็น migration แยกที่ตามมาทีหลัง** เหตุ: ฐานทริปวันนี้ไม่มี `trip_days` เลยสักแถว · `E7` คือจังหวะเดียวที่ **สร้าง** แถวเหล่านั้นขึ้นมา จึงเป็นจังหวะเดียวที่ตั้งตัวระบุได้โดยไม่ต้องเดา — หลังแถวถูกสร้างแล้ว ไม่มีทางรู้ว่าแถวไหนมาจาก `E7` โดยไม่เดาย้อนหลัง และการเดาคือสิ่งที่ AC นี้มีไว้ห้ามตั้งแต่ต้น (ดู "สิ่งที่พิสูจน์แล้วว่าใช้ไม่ได้" ด้านบน — heuristic เชิงรูปร่างทุกแบบตกหมด)
