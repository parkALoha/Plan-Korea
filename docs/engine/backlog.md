...
- [ ] **E3-AC4** ✏️ **แก้ตาม `D15`:** `lib/writeGuard.ts` เป็น choke point ของ**ตาราง** (10/10) แต่ **ไม่ใช่ของ Storage (0/9)**
      ...
      🔴 **แก้ 26 ส.ค. 2026 — ยังไม่ครบตามถ้อยคำ อย่าเพิ่งติ๊ก:** เกณฑ์ขอ "เทสต์ที่พิสูจน์ว่าการเขียนที่ไม่ผ่าน guard ถูกปฏิเสธ" — ด่านที่ลงแล้ว (`bookingFileStorageGate.test.ts`) บังคับ**"เขียนได้ที่ไหน"** ไม่ได้บังคับ**"ห่อ guard หรือไม่"** (regex match ไม่สนว่าอยู่ใน `writeGuard` จริงหรือเปล่า) — P1 เสนอรวมเป็นโมดูล storage เดียวที่ห่อ guard เสมอ (รูปเดียวกับ `lib/engine/db.ts` ที่เป็นไฟล์เดียวที่พิมพ์ชื่อตารางได้) ให้ "อยู่ที่ไหน" กับ "ห่อหรือไม่" เป็นคำถามเดียวกัน — **รอ P2 ตอบว่าเห็นด้วยไหมก่อนปิด `AC4`**
