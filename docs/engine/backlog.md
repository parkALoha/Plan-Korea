...
- [x] **E7-AC8** ✅ **ปิดแล้ว 2 ก.ย. 2026 — ทั้ง ①(restore ได้จริง)② (โปรเจกต์ที่ 2 พร้อม — โควตาเปิดแล้ว) ปิดครบ** เดิม 🔴 **ด่านแรกสุด — ไม่ผ่านข้อนี้ ห้ามแตะ schema ของ DB ทริปแม้แต่คำสั่งเดียว (`D21`)**
      ...
      **③ แยกคำถามที่ถูกมัดไว้ด้วยกัน** — "dump นี้ restore ได้จริงไหม" (ไม่ต้องใช้โควตา org) ≠ "มีโปรเจกต์ที่ 2 ไว้ซ้อม migrate ไหม" (ต้องใช้ — `AC1`/`AC6` พึ่งมัน) เกณฑ์เดิมบังคับตอบข้อหลังก่อนถึงจะตอบข้อแรกได้ **รูปเดียวกับ "เห็นครบ 11 วัน" ที่แก้ไปรอบ 84 แค่ครึ่งเดียว** — บรรยายสภาพปลายทาง (มีโปรเจกต์พร้อมแล้ว) แล้วใช้เป็นด่านแรกสุด
      ...
      📊 **สถานะจริง 28 ส.ค. 2026:** ✅ `pg_dump` แล้ว (schema 29K + data 1.3M) ✅ นับแถวจากไฟล์เอง 670/670 ✅ **① restore + เปิดดูจริงแล้ว — ปิด** ❌ **② ยังไม่มีโปรเจกต์ที่ 2 — ยังเปิด บล็อกด้วยโควตา**
