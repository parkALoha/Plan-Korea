...
> | **P4** | `E3-AC9` เมทริกซ์ข้ามผู้ใช้ระดับ route (9 เส้น trip-scoped · 2/9 ยิงแล้ว) · `E3-AC8` static guard | สเปคส่งแล้ว |
...
- [ ] **E3-AC9** 🔴 **`D38` — Server Action ไม่ใช่สิทธิ์พิเศษ · ต้องมีเทสต์ที่พิสูจน์ ไม่ใช่ย่อหน้าที่เตือน**
      ...
      **แล้วถูกยืมไปใช้กับอีกเรื่องหนึ่งที่มันไม่จริงเลย** (สิทธิ์เข้าถึงข้อมูล) — รูปแบบเดียวกับที่เราเจอมาทั้งวัน
      ...
      🎯 **ขอบเขต `②` คือ 9 route ที่ trip-scoped จริง** (`engineAttackSurface.test.ts` จำแนก 12 ไฟล์ engine ทั้งหมด — 3 ไฟล์ `plans`/`system-mode`/`trips` ไม่ใช่เป้า cross-user เพราะไม่มี `tripId` เป็น input หรือเป็น own-list) **2/9 ยิงแล้ว (bookings · checklist) เหลือ 7** (custom-places · day-settings · days · hidden-places · hotels · place-notes · stops) — ตัวเลขนี้แก้แล้ว 27 ส.ค. (ก่อนหน้าเขียนผิดเป็น "เหลือ 10" เพราะลืมกรอง 3 ไฟล์ที่ไม่ใช่เป้าออกก่อนลบ)
