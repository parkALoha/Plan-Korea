...
- [x] **E2-AC6** ✅ **ปิดตามถ้อยคำ 26 ส.ค. 2026 (P8 ตัดสิน — ดูเหตุผลท้ายบล็อก) — `data/*.ts` แยกเป็น `E2-AC15`** เดิม 📋 **เกณฑ์ที่ยกความเสี่ยงของ E7 มาไว้ที่นี่:** มีตารางแมป **ทุกคอลัมน์ของ 14 ตารางเดิม → ปลายทางในสคีมาใหม่** และคอลัมน์ไหนที่ตั้งใจทิ้งต้องเขียนเหตุผลกำกับ · **ห้ามมีคอลัมน์ไหนที่ไม่มีคำตอบ** (ถ้าไม่ทำข้อนี้ที่ E2 จะไปรู้ตอน E7 ว่าสคีมาใหม่เก็บของเดิมไม่ได้ ซึ่งแปลว่าต้องรื้อ E2–E5 ใหม่ทั้งหมด)
      ...
      **เกิดจริง 3 ครั้ง:** P4 — id ปลอมยิง `soft_delete_*` ได้ `P0001` เกือบรายงานว่า "ถูกบล็อกแล้ว" ทั้งที่ล้มที่ขั้น lookup **ก่อน**ถึงจุดที่ trigger ยิง · P1 (`D83`) — เห็นว่าเลขซ้ำ*เป็นไปได้* แต่ไม่ตรวจว่าซ้ำ*จริง*ก่อนมินท์ · P1 (`linked-project.json`) — เห็นไฟล์ link → สรุปว่า `db push` จะรัน migration ทริปใส่ `engine-dev` **P6 ยิงจริงพบว่า CLI หยุดเองที่ `Cannot find project ref`** เพราะไฟล์ที่ CLI อ่านคือ `project-ref` คนละไฟล์กับที่ P1 ดู — P1 รายงานผู้ใช้ไปด้วยความรุนแรงเกินจริง แก้กับผู้ใช้แล้ว
      ...
      🎯 **กลไกจริง:** ของกลางป้องกันได้ก็ต่อเมื่อคนถัดไป**เจอมันก่อนจะพิมพ์ของตัวเอง** — และ `grep` ช่วยไม่ได้ในสถานการณ์นี้ เพราะ**ต้องรู้ชื่อของกลางอยู่แล้วถึงจะหามันเจอ** ตอนกำลังจะเขียนโค้ดใหม่ เรายังไม่รู้ว่ากำลังหาอะไร จึงไม่มีคำที่จะ `grep`
      · **ต่างจากทุก `P` ก่อนหน้าวันนี้ตรงที่มันไม่ใช่การข้ามขั้นตรวจหรือขอบเขตวัดผิด — เป็นปัญหาของ *discoverability* ของของกลางเอง** ไม่มีเครื่องมือใดในทะเบียนวันนี้แก้ตรงนี้ได้ ต้องเป็นโครงสร้าง (เช่น lint rule ที่ห้ามนิยามซ้ำ) ไม่ใช่วินัยคน
      ...
      **เคสของ P1 เอง 2 อันวันเดียวกัน — รูปเดียวกันเป๊ะ ไม่เคยมองในรูปนี้มาก่อน:**
      ...
      ② สั่งทีมว่า *"อย่ารายงานสิ่งที่ยังไม่ได้วัด"* + บอก P7 ว่า *"โค้ด Realtime ไม่เคยถูกทดสอบ"* โดยไม่ได้เปิดไฟล์ดู → P7 เปลี่ยนข้อเสนอจาก "ทำให้เห็นได้" เป็น "ต้องเอาออก" เพราะเชื่อประโยคนั้น 🔴 **P7 รับข้อเท็จจริงที่ไม่ได้วัดโดยไม่ตรวจ ซึ่งผิด — แต่ P1 เป็นคนใส่ข้อเท็จจริงที่ไม่ได้วัดเข้าระบบตั้งแต่แรก** ราคาจริงของการพูดสิ่งที่ยังไม่วัด: ไม่ได้หยุดที่ตัวคนพูดเอง มันไหลต่อไปยังคนที่เชื่อ
