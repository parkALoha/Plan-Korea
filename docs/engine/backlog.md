...
- [ ] **E7-AC2** 🔴 **รายงาน diff เป็น 0 — เทียบกับสำเนาแช่แข็ง ไม่ใช่เทียบกับตัวเลขที่จดไว้**
      ...
      **กลไก:** `md5(string_agg(col1||'|'||col2||..., E'\n' order by <คีย์ตายตัว>))` ต่อตาราง — คืน **ลายนิ้วมือ** ไม่ใช่ข้อมูล ที่อยู่บ้านใน `custom_places` ไม่ออกจากฐานสักตัวอักษร (ตอบมติเดิมได้โดยไม่ขัดกัน)
      🔴 **กติกาที่ต้องเขียนกำกับเวลาใช้จริง:** ① **ห้ามใส่ `updated_at`/`id`** ในลายนิ้วมือ — ขยับเองโดยไม่มีอะไรเปลี่ยนเชิงความหมาย ลายนิ้วมือที่เปลี่ยนทุกครั้งไม่บอกอะไรเลย ② **`order by` ต้องมีและตายตัว** — ไม่มี = ลำดับไม่รับประกัน = ลายนิ้วมือต่างกันเองแม้ข้อมูลเดิม ③ **เทียบได้เฉพาะฐานเดียวกัน ก่อน–หลัง** ไม่ใช่ข้ามฐาน (prod กับ dev ไม่มีวันตรงเพราะข้อมูลคนละชุดโดยตั้งใจ) — ตรงกับรูปที่ `AC3` ใช้อยู่แล้วพอดี ("ฐานเดิม ก่อน–หลังแก้โค้ด")
