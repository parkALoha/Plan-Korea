...
> · 🔴 **แก้เกณฑ์:** ยืนยันกับ `column-map.md` เอง — 14 ตารางเดิมไม่มีตารางวันสักใบ โครงวันอยู่ใน `data/itinerary.ts` ฐานเก็บแค่ `day_id` สตริง `'d0'`–`'d10'` เกณฑ์เดิมบรรยายสภาพหลัง `E7` ย้ายเสร็จ เอามาใช้เป็นด่านก่อนย้าย → **เกณฑ์ใหม่: แถวเท่าต้นฉบับ (14 ตาราง/690 แถว) + `day_id` ครบ `d0`–`d10`**
> · ✅ **ผู้ใช้ทำสำเนาสำเร็จวันนี้:** `pg_dump` schema+data ครบ นับแถวสองฝั่งตรงกัน 690/690 · ❌ **ยังไม่ restore ขึ้นโปรเจกต์ที่ 2 — ยังไม่ผ่าน** บล็อกด้วยโควตา org เต็ม
...
- [ ] **E6-AC4** offline อ่านอย่างเดียวยังใช้ได้จริงบน airplane mode ~~(ผู้ใช้ยืนยันเองแล้วครั้งหนึ่ง 13 ส.ค. 2026)~~
      ...
      · **เกณฑ์ที่ใช้จริงแทนของเดิม:** ทดสอบบน**ทริปแพลตฟอร์ม** (ญี่ปุ่น/ไทย ฯลฯ ที่ดึงจาก DB จริง ไม่ใช่ทริปเกาหลี) — เปิดตอนมีเน็ตให้แคชอุ่น → ปิดเน็ต → ต้อง**เห็นข้อมูลจริง** (วัน/สถานที่) ไม่ใช่แค่ "เปิดแล้วไม่มี error" · **มีเคสควบคุม:** ล้างแคชแล้วเปิดตอนไม่มีเน็ตต้องเห็นว่าง — ไม่งั้นเคสบวกพิสูจน์ไม่ได้ว่าแคชคือตัวที่ทำให้เห็น (P1 กำหนดไว้ที่ `mobile-arch.md §13.3`)
      📌 **สภาพจริงวันนี้ (P7 วัด):** `usePlatformItinerary`/`useCatalogPlaces`/`useTripCatalogCities` แคชผ่าน `hydrateThenFetch`+`offlineStore` แล้วจริง (3 hook) แต่ `useChecklist`/`useHiddenPlaces` ยังไม่แคชเลย (ควรแคช) — **ยังไม่ผ่านเกณฑ์เต็มรูปที่แก้ใหม่**
      🔴 **ยังไม่สร้างด่านอัตโนมัติ ตามที่ P1 สั่ง** (`mobile-arch.md §15.6`) — รอถ้อยคำนิ่งก่อน (ข้อนี้คือถ้อยคำที่นิ่งแล้ว) ค่อยออกแบบด่าน · **ห้ามผูกด่านกับชื่อ `hydrateThenFetch` ตรงๆ** (นั่นคือเกณฑ์ของ `AC7` เรื่องที่เก็บ ไม่ใช่ของ `AC4` เรื่องอ่านออฟไลน์ได้จริงไหม) — เกณฑ์ที่ตรงกว่าคือ "อ่านผ่านแคชอะไรสักอย่างไหม" ซึ่งทางเก่า (`readCache`/`readTripCache`) ก็นับผ่านด้วย
      🔴 **สองแหล่งไม่ตรงกัน — จดไว้กันสับสน:** `mobile-arch.md §14` (handoff ของ P7) เขียนว่า `E6-AC4` "ผ่าน" แต่หมายถึงเฉพาะสิ่งที่วัด (วัน/สถานที่/เมือง) ซึ่งแคบกว่าถ้อยคำเต็มของ AC นี้ — **`backlog.md` คือฉบับที่ผูกพัน ยังไม่ติ๊กจนกว่าจะครบตามเกณฑ์ที่แก้ใหม่ข้างบน**
