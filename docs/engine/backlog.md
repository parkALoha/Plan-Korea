...
- [ ] **E2-AC13** 🔴 **ใหม่ 24 ส.ค. 2026 — `bookings.file_url` ต้องเลิกเป็น URL ก่อนปิด bucket ไม่ใช่หลัง**
      ...
      **แก้ 25 ส.ค. 2026 — แยกสถานะ 3 ข้อของ AC นี้ให้ชัด (P1):**
      ...
      · **② เปิดไฟล์ตั๋วของแถวเดิมได้จริงจากในเว็บ หลังปิด bucket** — ✅ **ปิดแล้ว 26 ส.ค. 2026:** `/api/booking-file/[...path]` ลงแล้ว (`3bcd9b8` · ดีไซน์ของ P3 — proxy same-origin แทนแคช signed URL ตรง · `app/api/` โซน P1) P2 ต่อสายครบทุกจุดที่ใช้จริง (`BookingsPanel` · `BookingEditModal` · `PlaceCard` · `DayStopsSection` · `today` · `summary`)
      · **③ `sw.js` `cacheFirst` กับ signed URL** — 🔴 **ยังไม่ปิด รอ P3 ต่อ `sw.js` ตอน `E3` ตามที่สัญญาไว้ใน `§11`** (ไม่ใช่ของค้างใหม่ ยังเป็นตัวเดิมที่ส่งให้ตั้งแต่ 24 ส.ค.) `public/sw.js:113,118` ใช้ `cacheFirst` กับ ASSET_CACHE · **signed URL เปลี่ยนลายเซ็นทุกครั้งที่เซ็น = URL ไม่ซ้ำเดิม = cache ไม่มีวัน match**
...
- [x] **E2-AC15** ✅ **ตัวตรวจลงแล้ว 26 ส.ค. 2026 (`941014f`) — ติ๊กตามเกณฑ์ที่เขียนไว้ (ดูขอบเขตที่ไม่ครอบท้ายบล็อก)** เดิม 🔴 **ใหม่ 26 ส.ค. 2026 — แยกออกจาก `E2-AC6` (P8 ตัดสิน · ดูเหตุผลที่ `AC6`):** `data/*.ts` (5 ไฟล์ · `P-57` เป็นจุดเริ่ม) ต้องมีตัวตรวจว่าทุกฟิลด์มีปลายทางในสคีมาใหม่ **แบบเดียวกับที่ `columnMapCoverage.test.ts` ทำให้ DB** แต่คนละเทคโนโลยี (อ่าน TS type ไม่ใช่ SQL) — P1 เสนอสร้างเอง (แพงกว่าตัวตรวจ SQL)
      ...
      🔴 **เคสตัวอย่างจริงของขอบเขตที่ไม่ครอบ — เกิดแล้ว 26 ส.ค. 2026 ระหว่างแปลง hook ที่สองใน `E3` (P1 ขอให้อยู่ในนี้เป็นหลักฐาน ไม่ใช่แค่บันทึกเหตุการณ์):** หัวข้อ `Day (13 ฟิลด์)` ของ `column-map.md` เขียนว่า *"ไล่ครบด้วยตัวคอมไพเลอร์"* — **ตารางข้างใต้มีแค่ 11 แถว** `id` กับ `date` หายไปทั้งคู่ (`date` คือคีย์ที่ทุก hook ที่คีย์ด้วยวันต้องใช้อ้าง) ไม่มีใครนับรวมทั้ง P1 ที่เขียนหัวข้อนั้นเอง — พบได้เพราะต้องการคำตอบจริงสำหรับงาน ไม่ใช่เพราะตัวตรวจแดง
        🎯 **ตระกูลเดียวกับ `111/111`/`D82` เคสที่ 5:** ตัวเลขในหัวข้อไม่ได้ถูกตรวจโดยอะไรเลย มันเป็น*คำอธิบาย* ไม่ใช่*ข้อความที่มีคนพิสูจน์* — **ตัวเลขที่ถูก** กับ **ตัวเลขที่บอกขอบเขตครบ** เป็นคนละคุณสมบัติ **นี่คือเหตุผลตรงตัวว่าทำไม `E2-AC15` ต้องมีตัวตรวจ ไม่ใช่แค่เอกสาร**
        · **คำตอบที่ลงไปแล้ว (`d9d0517`):** `Day.date` → `trip_days.date` เป็นคีย์ธรรมชาติ (`unique(trip_id, date)`) · `Day.id` (`"d0"`) → **ทิ้ง ไม่มีคอลัมน์ไหนเก็บ** จับคู่ด้วย `date` ตอน `E7` — ผลกระทบเต็มอยู่ที่ `E3-AC2` (สะพาน `"d0"→date→uuid` ต้องอยู่ที่เดียว)
        · ⚠️ **ยังไม่ยืนยันว่าทำไมตัวตรวจที่ลงแล้ว (`941014f`) ไม่จับ 2 ฟิลด์นี้** — ถ้า `id`/`date` เป็นฟิลด์จริงของ `Day` ที่คอมไพเลอร์มองเห็น ด่านควรแดงตั้งแต่ก่อนหน้านี้ ไม่ใช่ให้คนหาเจอเอง คำถามที่ต้องมีคนตอบ: ตารางหายไปตอนไหนเทียบกับตอนตัวตรวจ commit — ก่อนหรือหลัง · ถ้าหลัง แปลว่าตัวตรวจมีจุดบอดจริง ไม่ใช่แค่ปัญหาจังหวะเวลา
...
- [ ] **E3-AC1** `grep -rn '\.from("' app components hooks lib --include='*.ts*' | grep -v 'storage\.from('` ได้ **0 บรรทัด** ที่รันฝั่ง client · วันนี้ได้ **67 บรรทัดใน 13 ไฟล์**
      · 📊 **ความคืบหน้า 26 ส.ค. 2026:** 68 → **66** — `useCustomPlaces` แปลงครบทั้งอ่านและเขียน · เหลือ 9 hooks
...
- [ ] **E3-AC7** 🔴 **ทำก่อนใครใน `E3` — ออกแบบทางเข้าสู่โหมด read-only ตั้งแต่ตอนนี้ ไม่ใช่ไปคิดตอน E9 (`D18`)** — E9-AC2 ต้องการ read-only ที่บังคับได้ที่ชั้น DB/API · ถ้าไม่วางสวิตช์ไว้ใน DAL ตั้งแต่ E3 จะไม่มีที่ให้ใส่ตอน E9 นอกจากไล่แก้ทุกเส้นทางใหม่ (P6 ชี้ข้อเดียวกันจากฝั่ง infra)
      ...
      |---|---|---|
      | **DB** (`revoke` สิทธิ์เขียน หรือเงื่อนไขใน policy) | **บังคับจริง** | ไม่มีทางเดินอ้อม แม้ผ่าน RPC/Server Action |
...
· **คืนนี้เกิด 1 ครั้ง ยังไม่ถึงเส้น** — ยังไม่ต้องลงมือแก้ถาวร
