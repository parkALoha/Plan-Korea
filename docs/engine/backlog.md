...
- [ ] **E6-AC12** 🔴 **มินท์ใหม่ 30 ส.ค. 2026 (P1 ตัดสิน — เต็มที่ [`resolve-place-design.md §7`](./resolve-place-design.md) · P8 อ่านไฟล์จริงก่อนจด ไม่ยกจากข้อความ) — มาแทนครึ่งหลังของ `AC1` เดิม (`PLACES`): `Place["city"]`/`["category"]` เลิกเป็นยูเนียนปิดที่ cast มาจากฐาน:**
      ...
      🔴 **ต้องทำก่อน `E6-AC13` เสมอ (ลำดับผูกกัน — P1 เขียนกำกับเองว่ามาจากการอ่านโค้ด ไม่ใช่การรัน ถ้า P3 ลงมือแล้วพบว่าประกอบ `Place` ได้โดยไม่ต้อง cast ลำดับนี้ผิดและต้องแก้กลับมาที่นี่):** งานของ `AC13` (side-map) ต้องประกอบ `Place` จากแถว `catalog_places` ซึ่งเป็นขอบเดียวกับที่ cast เกิด — ทำ `AC13` ก่อนจะเพิ่มจุด cast อีก 1 จุดก่อนไปแก้ราก ทำ `AC12` ก่อนแปลว่าตอนทำ `AC13` ไม่มี cast ให้เพิ่มเลย
      **เกณฑ์รับ:** ป้อนเมือง/หมวดหมู่ที่ไม่รู้จัก (เช่น `"tokyo"`) เข้าไปทุก lookup ที่ใช้ `Place["city"]`/`["category"]` เป็นคีย์ แล้วยืนยันพฤติกรรมที่นิยามไว้ (เช่น fallback แบบ `cityMetaOf()`) — **ห้ามเขียนเป็น "ไม่มี `as Place[...]` เหลือในรีโป"** เพราะปิดได้ด้วยการย้าย cast ไปที่อื่นโดยไม่แตะราก
      ⚠️ **ต้องแก้ `resolvePlace.ts:19` (`"seoul"` hardcode) ไปพร้อมกันในเกณฑ์เดียวกัน** ไม่ใช่แยกเป็นข้อย่อยที่เลื่อนได้
...
- [ ] **E6-AC13** 🔴 **มินท์ใหม่ 30 ส.ค. 2026 (P1 ตัดสิน — เต็มที่ [`resolve-place-design.md §7`](./resolve-place-design.md) · P8 อ่านไฟล์จริงก่อนจด) — มาแทนครึ่งหลังของ `AC1` เดิม ส่วนที่สอง: `/stops` ส่ง side-map `places: Record<place_id, Place>` มาด้วย · `resolvePlace(id, { customPlaces, catalog })`:**
      🔴 **ต้องรอ `E6-AC12` ปิดก่อนเริ่ม** (เหตุผลอยู่ใน `AC12` ข้างบน — side-map เพิ่มจุด cast เป็นจุดที่ 14 ถ้าชนิดยังไม่แก้)
