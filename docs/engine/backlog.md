...
- [ ] **E6-AC12** 🔴 **มินท์ใหม่ 30 ส.ค. 2026 (P1 ตัดสิน — เต็มที่ [`resolve-place-design.md §7`](./resolve-place-design.md) · P8 อ่านไฟล์จริงก่อนจด ไม่ยกจากข้อความ) — มาแทนครึ่งหลังของ `AC1` เดิม (`PLACES`): `Place["city"]`/`["category"]` เลิกเป็นยูเนียนปิดที่ cast มาจากฐาน:**
      ...
      🎯 **พบของแถมที่ตรงกับสมมติฐาน — สิ่งนี้เคยเกิดมาแล้วครั้งหนึ่งกับ `Day["city"]` (คนละ type แต่ตระกูลเดียวกัน) และมีแบบแผนแก้อยู่แล้ว:** `components/cityMeta.ts:19-31` เขียนไว้เองว่า `CITY_META[x]` เป็น `undefined` → อ่าน `.icon` ต่อ → ทั้งหน้าไม่ขึ้น (เกิดจริง 28 ส.ค. 2026 ที่ `DayJumpBar`) แก้ด้วยฟังก์ชัน `cityMetaOf()` ที่ fallback เป็น `UNSET_CITY_META` เสมอเมื่อเมืองไม่รู้จัก — **`Place["city"]`/`["category"]` ควรเดินตามรูปเดียวกัน** (accessor ที่นิยามพฤติกรรมไม่รู้จักไว้ชัดเจน) ไม่ใช่คิดค้นทางใหม่
...
- [ ] **E6-AC13** 🔴 **มินท์ใหม่ 30 ส.ค. 2026 (P1 ตัดสิน — เต็มที่ [`resolve-place-design.md §7`](./resolve-place-design.md) · P8 อ่านไฟล์จริงก่อนจด) — มาแทนครึ่งหลังของ `AC1` เดิม ส่วนที่สอง: `/stops` ส่ง side-map `places: Record<place_id, Place>` มาด้วย · `resolvePlace(id, { customPlaces, catalog })`:**
      ...
      🔴 **แก้ 30 ส.ค. 2026 (P8 พบระหว่างตรวจ — ต่างจากที่ design doc เขียนไว้):** เอกสารอ้างว่า `app/api/place-name/route.ts` "เรียก `resolvePlace` ฝั่งเซิร์ฟเวอร์" — **เปิดไฟล์จริงแล้วไม่พบการเรียก `resolvePlace` เลยสักบรรทัด** (56 บรรทัดทั้งไฟล์ · `grep -rln "resolvePlace(" --include='*.ts' --include='*.tsx' .` ที่ `5dcfb2a` เจอ 10 ไฟล์ ไม่มี `place-name/route.ts` อยู่ในนั้น) มีแค่คอมเมนต์อธิบาย**บั๊กที่คล้ายกัน**ของ route นี้เอง (fallback `name_en ?? name_th` แบบเดียวกับที่ `resolvePlace` ทำ ไม่ใช่เรียกฟังก์ชันเดียวกัน) — **ส่งกลับให้ P1 ยืนยันเจตนา:** จะให้ route นี้เริ่มเรียก `resolvePlace` จริงเป็นส่วนหนึ่งของงานนี้ (แก้บั๊กที่คล้ายกันไปพร้อมกัน) หรือถอดออกจากขอบเขต `AC13` เพราะไม่ใช่จุดเรียกที่มีอยู่จริง — ยังไม่ตัดสินเอง
