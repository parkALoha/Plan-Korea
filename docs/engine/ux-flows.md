...
### 12.2 หลักการ: เซ็นตอน "โชว์รูปย่อ" กับเซ็นตอน "ผู้ใช้กดเปิด" ต้องคนละจังหวะกัน

โมเดลเดิมใช้ `fileUrl` ตัวเดียวกันทั้ง 3 ที่ (thumbnail `<img>`, ลิงก์ "เปิดไฟล์" แท็บใหม่, `PhotoLightbox`)
— ถ้าย้ายมาใช้ signed URL ตัวเดียวแบบเดิมทั้ง 3 จุด จะเจอปัญหาตรงตามที่ P1 อธิบาย: เปิดโมดัลค้างไว้ 10
นาทีแล้วกดเปิดไฟล์ → URL ที่เซ็นไว้ตอนเปิดโมดัลตายไปแล้ว ได้ 400 เปล่าๆ

**แยกเป็น 2 เส้นทางตามความเสี่ยง:**

| การใช้งาน | เซ็นตอนไหน | เหตุผล |
|---|---|---|
| **thumbnail `<img>` ในโมดัล** | ตอนโมดัลเปิด (เซ็นครั้งเดียว) | ต้อง**เห็นทันที**ไม่มีปฏิสัมพันธ์ก่อน — ไม่มีจังหวะ "ตอนคลิก" ให้เซ็นใหม่ได้ เพราะ `<img>` render แบบ passive |
| **ปุ่ม "เปิดไฟล์" (แท็บใหม่) + ปุ่มเปิด `PhotoLightbox`** | **เซ็นใหม่ทุกครั้งตอนคลิก** ไม่ใช้ตัวที่เซ็นไว้ตอนเปิดโมดัลซ้ำ | รับประกันความสดเสมอไม่ว่าโมดัลจะเปิดค้างมานานแค่ไหน — เปลี่ยนจาก `<a href={fileUrl}>` เป็น `<button onClick={async () => { const url = await sign(); window.open(url) }}>` |

**ผลคือ TTL ของสองเส้นทางไม่ต้องเท่ากัน**: เส้นทาง thumbnail อยากได้ TTL ยาวพอให้ครอบคลุมช่วงเวลาที่คน
เปิดโมดัลดูจริง (เสนอ **10 นาที** — ยาวกว่า 60 วินาทีที่ P1 ทดสอบเยอะพอสำหรับการดูจริง แต่ไม่ยาวจนเก็บ
ความเสี่ยงไว้นาน) เส้นทาง sign-on-click ใช้ TTL สั้นได้เลย (**60-120 วินาที** พอ) เพราะเซ็นแล้วใช้ทันที
ไม่มีช่วงรอ — สั้นกว่า = ปลอดภัยกว่า โดยไม่กระทบ UX เพราะผู้ใช้ไม่เคยเห็นความล่าช้านั้น

### 12.3 เมื่อ thumbnail หมดอายุระหว่างเปิดโมดัลค้างไว้ — ต้องมี fallback ที่กดแก้ได้ ไม่ใช่รูปพัง

`<img>` ที่ src หมดอายุจะขึ้นไอคอนรูปพักมาตรฐานของเบราว์เซอร์ — **อ่านไม่ออกว่าเกิดอะไรขึ้น** (เหมือนโมดัล
เดิมที่ P1 ไม่อยากให้เกิดกับปุ่ม "เปิดไฟล์") ต้องมี `onError` บน `<img>` ที่สลับไปโชว์สถานะที่กดแก้ได้:
