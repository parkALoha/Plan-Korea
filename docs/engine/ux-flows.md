...
## 12. `E2-AC5`/`D12` — ไฟล์ตั๋วเลิกเป็น public URL: flow ใหม่ + UX ของ signed URL ที่หมดอายุ

> ยังไม่แก้โค้ด — `main` freeze และนี่คืองาน `E5` บน `platform` ตามที่ P1 บอก บันทึกไว้เป็นการออกแบบก่อน

### 12.1 flow เปลี่ยนจากอะไรเป็นอะไร

```
เดิม:   อัปโหลด → getPublicUrl() → เก็บ URL ถาวรลง fileUrl → <img src={fileUrl}> ตรงๆ ตลอดไป
ใหม่:   อัปโหลด (path ขึ้นต้นด้วย tripId/) → เก็บ path ลง file_path → เซ็น signed URL ตอนต้องแสดงผล
        → URL หมดอายุตาม TTL → ต้องเซ็นใหม่ถ้าจะใช้ต่อ
```

**ผลที่กระทบ 3 จุดใน `BookingEditModal.tsx` ตรงกับที่ P1 ระบุ** (`:85` path ต้อง prefix `tripId/`, `:97`
เปลี่ยนจาก `getPublicUrl` เป็น `createSignedUrl`, `:366` `accept` ต้องแคบลงให้ตรงกับที่ DB บังคับจริง) —
สามจุดนี้เป็นเรื่อง implementation ตรงไปตรงมา ส่วนที่ต้องออกแบบเพิ่มคือ **สิ่งที่เกิดขึ้นระหว่าง URL ยังไม่
หมดอายุกับหมดอายุแล้ว** ซึ่งไม่เคยมีในโมเดลเดิมเลย (public URL ไม่มีวันหมดอายุ)

### 12.2 🔴 แก้ไข — `mobile-arch.md §11.15` (P7): signed URL เป็น bearer credential ไม่มี TTL ไหน "ยาวได้"

**ข้อเสนอเดิมของผมในหัวข้อนี้ผิด** — เคยเสนอให้ thumbnail เซ็นครั้งเดียวตอนเปิดโมดัลด้วย TTL 10 นาที
P1 ยืนยันแล้วว่า `createSignedUrl` เปิดได้โดยไม่ล็อกอิน ไม่ผ่าน policy เลย — **TTL ยาวคือการเก็บ credential
ที่ใครถือ URL นั้นก็เข้าถึงไฟล์ได้ทันที ไม่ใช่แค่ "ลิงก์สะดวก"** สิบนาทีคือหน้าต่างเปิดกว้างเกินไปสำหรับของ
ที่ยกเลิกงาน `E2-AC5` ทั้งหมดถ้าหลุด (URL โผล่ใน devtools/ประวัติเบราว์เซอร์/extension ระหว่างนั้น)

**หลักที่ต้องยึดใหม่: TTL สั้นเท่ากันทุกเส้นทาง (60-120 วินาที) ไม่มีข้อยกเว้นให้ thumbnail** — ของที่ต่างกัน
ระหว่าง thumbnail กับปุ่มกดไม่ใช่ TTL แต่คือ**ใครเป็นคนสั่งเซ็นใหม่**:

| การใช้งาน | เซ็นตอนไหน | ต่ออายุยังไง |
|---|---|---|
| **ปุ่ม "เปิดไฟล์" (แท็บใหม่) + ปุ่มเปิด `PhotoLightbox`** | **เซ็นใหม่ทุกครั้งตอนคลิก** ไม่ใช้ค่าที่เคยเซ็นไว้ก่อนหน้าเลย | ไม่ต้องต่ออายุ — ใช้ครั้งเดียวจบ (`<button onClick={async () => { const url = await sign(); window.open(url) }}>`) |
| **thumbnail `<img>` ในโมดัล** | เซ็นตอนโมดัลเปิด **ด้วย TTL สั้นเท่ากับเส้นทางคลิก** ไม่ใช่ TTL ยาวแยกต่างหาก | **ต่ออายุเงียบๆ อัตโนมัติ** ด้วย timer เซ็นใหม่ก่อนหมดอายุ (เช่นทุก 45 วินาทีถ้า TTL 60 วินาที) สลับ `src` ทับของเดิมโดยผู้ใช้ไม่รู้ตัว — เคลียร์ timer ทันทีตอนโมดัลปิด/unmount |

**ทำไมไม่ใช้ "ไม่โชว์ thumbnail จนกว่าจะกด" แทน (ตัวเลือกที่ปลอดภัยกว่าแต่ตัดออก):** เสียของดี (เห็นรูปตั๋ว
ทันทีที่เปิดโมดัล) ทั้งที่มีทางที่ได้ทั้งคู่ — สั้น + ต่ออายุอัตโนมัติ ให้ TTL ที่ปลอดภัยเท่าเส้นทางคลิกจริง
โดยผู้ใช้ยังเห็นรูปทันทีเหมือนเดิม **ไม่ใช่การประนีประนอมความปลอดภัยเพื่อ UX** — เป็นแค่ระดับความซับซ้อนของ
โค้ดที่สูงขึ้นนิดหน่อย (timer + cleanup) แลกกับได้ทั้งสองอย่าง

**ที่ต้องแก้ตาม (ไม่ใช้แค่ signed URL ธรรมดา ห้ามใช้ URL รูปแบบอื่นแทน):** ห้ามเก็บ signed URL ที่เซ็นแล้ว
ลง `state` ที่มีอายุยาวกว่าช่วงที่ใช้จริง, `localStorage`, หรือ cache ใดๆ ตามกฎของ P7 — `pendingUploadPathRef`
ที่มีอยู่แล้วเก็บแค่ **path** (ไม่ใช่ signed URL) อยู่แล้ว ไม่กระทบกฎนี้ แต่ `fileUrl` state เดิม (เก็บ URL
ตรงๆ) ต้องเปลี่ยนความหมายเป็น "signed URL ล่าสุดที่เซ็นไว้ใช้แสดงผล ณ ตอนนี้" ไม่ใช่ "URL ถาวรของไฟล์"

### 12.3 เมื่อเซ็นใหม่ล้มเหลวระหว่าง auto-renew — ต้องมี fallback ที่กดแก้ได้ ไม่ใช่รูปพัง

`<img>` ที่ src หมดอายุ (เช่น auto-renew พลาดจังหวะเพราะเน็ตหลุดตอนนั้นพอดี) จะขึ้นไอคอนรูปพักมาตรฐานของ
เบราว์เซอร์ — **อ่านไม่ออกว่าเกิดอะไรขึ้น** ต้องมี `onError` บน `<img>` ที่สลับไปโชว์สถานะที่กดแก้ได้:

```
🖼️ รูปหมดอายุการแสดงผล — [กดเพื่อโหลดใหม่]
```
กดแล้วเซ็น signed URL ใหม่อีกรอบ (เรียกฟังก์ชันเซ็นเดิมซ้ำ) แล้วเปลี่ยน `src` — ไม่ต้องปิดโมดัลหรือรีเฟรช
หน้า ผู้ใช้ไม่เสียบริบทที่กำลังกรอกอยู่ (ชื่อ/วันที่/เลขคอนเฟิร์ม ที่อาจกำลังแก้ในฟอร์มเดียวกัน)

### 12.4 ถ้า sign-on-click พังเอง (เช่นเน็ตหลุดตอนกด "เปิดไฟล์")

ต้องมี loading state สั้นๆ ระหว่างเซ็น (ไฟล์ตั๋วไม่ใช่ของที่ต้องเปิดเร็วจนรอ 1 request ไม่ได้) และถ้าเซ็นไม่
สำเร็จ toast แบบเดียวกับ `writeGuard`/`showToast` เดิม (`บันทึกไม่สำเร็จ` ใช้แพทเทิร์นเดียวกัน):
```
เปิดไฟล์ไม่สำเร็จ — ลองใหม่อีกครั้ง
```
**ไม่ปล่อยให้ browser เปิดแท็บใหม่ไปเจอหน้า error ของ Supabase เอง** — เซ็น**ก่อน**เปิดแท็บเสมอ ไม่ใช่เปิด
แท็บด้วย URL เปล่าแล้วค่อยรอ fetch (ถ้าเซ็นพัง ต้องไม่มีแท็บใหม่เปิดขึ้นมาเลย)

### 12.5 `accept` + ข้อความ error ตอนไฟล์ผิดชนิด

`accept="image/*,application/pdf"` เดิมกว้างกว่าที่ DB รับจริงแล้ว (`image/jpeg,png,webp,heic,gif` +
`application/pdf`) — เปลี่ยน `accept` ให้ตรงเป๊ะกันเลือกไฟล์ผิดชนิดจาก picker ได้บางส่วน **แต่ `accept`
เป็นแค่ตัวกรองใน UI ไม่ใช่ด่านจริง** (ลาก-วางไฟล์ หรือเลือก "ไฟล์ทั้งหมด" ในบาง OS ยังผ่านมาได้) —
ต้องอ่าน error จาก Storage ตอนอัปโหลดจริงแล้วแปลเป็นข้อความเจาะจง แทนข้อความรวมเดิม (`อัปโหลดไม่สำเร็จ
ลองใหม่อีกครั้ง`) ซึ่งไม่บอกว่าทำไม — เสนอ:
```
รองรับเฉพาะรูปภาพ (JPEG/PNG/WEBP/HEIC/GIF) หรือ PDF เท่านั้น
```
แสดงเฉพาะตอนตรวจได้ชัดว่าสาเหตุคือชนิดไฟล์ (จาก error code ที่ Storage ส่งกลับ) — ถ้าสาเหตุอื่น (เน็ตหลุด
ฯลฯ) ใช้ข้อความเดิม ไม่เดาเหตุผลผิดแล้วให้ข้อมูลที่ทำให้งงกว่าเดิม

### 12.6 ของที่ implement จริงตอน `E5` (ยังไม่ทำตอนนี้)
- ฟังก์ชันเซ็น signed URL ต้องมาจาก DAL/server (`lib/dal/` ตามที่ `architecture.md §3.2` วางไว้) ไม่ใช่ยิง
  `supabase.storage.createSignedUrl` ตรงจาก component เหมือนที่ `getPublicUrl` ทำอยู่วันนี้ — ผูกกับทิศทาง
  ทั่วไปที่ E3 วางไว้อยู่แล้ว (เขียน/อ่านผ่าน DAL ไม่ใช่ browser ยิงตรง) ไม่ใช่กติกาใหม่เฉพาะไฟล์นี้
- `PhotoLightbox` (component ที่ใช้ร่วมกับที่อื่นในเว็บ เช่นรูปสถานที่) ต้องรับ `src` แบบ lazy-signed ได้
  ไม่ใช่รับ URL ตรงๆ เหมือนวันนี้ — เช็คว่ามันถูกใช้ที่ไหนอีกก่อนแก้ signature (`PhotoGallery.tsx` เป็นต้น)
  กันผลกระทบไปหน้าอื่นที่ไม่เกี่ยวกับตั๋วเลย
- **timer ต่ออายุ thumbnail (12.2)** — ต้องเคลียร์ทั้งตอน modal `onClose` และตอน component unmount (สอง
  จังหวะไม่เหมือนกันเสมอไป เช่นปิดผ่าน `Esc`/คลิกนอกกล่อง) ไม่งั้น timer ค้างยิง `createSignedUrl` ต่อไป
  หลังโมดัลปิดไปแล้ว เสียโควตาและเป็นการเซ็น credential ที่ไม่มีใครใช้ต่อ

📌 **`D83`** (P1 ขอให้บันทึกมติ — เดิมเขียนผิดเป็น `D71` ซึ่งชนกับ `D71` จริงที่ใช้ไปแล้วในโปรเจกต์
ขอบคุณ P8 ที่หาเลขว่างให้): เซ็น signed URL ทุกเส้นทางด้วย **TTL สั้นเท่ากัน (60-120 วินาที)** ไม่มี
ข้อยกเว้นให้ thumbnail — ปุ่มกด "เปิดไฟล์"/lightbox เซ็นใหม่ทุกครั้งตอนคลิก · thumbnail เซ็นตอนเปิดโมดัล
แล้วต่ออายุเงียบๆ ด้วย timer ก่อนหมดอายุ ไม่ใช่เซ็น TTL ยาวเพื่อเลี่ยงการต่ออายุ — สอดคล้อง `mobile-arch.md
§11.15` (P7): signed URL คือ bearer credential ห้ามเก็บยาวกว่าที่ใช้จริง ไม่ว่าจะเก็บใน state/localStorage/
cache
