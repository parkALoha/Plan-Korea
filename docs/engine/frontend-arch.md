...
1. `updated_at timestamptz` ต่อแถวใน `catalog.places` (และ `catalog.place_names` ถ้าแยกตาราง — ข้อ 9.3)
   สำหรับ debug ว่าแถวไหนเปลี่ยนล่าสุด แต่ **ไม่ใช้เป็น cache key โดยตรง** เพราะ scan `max(updated_at)`
   ทั้งตารางแพงขึ้นเรื่อยๆ ตามจำนวนแถวและไม่ให้ tag ที่ invalidate ได้ตรงจุด
...
### 9.3 N+1 — ต้องเป็น query เดียวสำหรับ 1 วัน (schema เลือกได้ตอนนี้)

ความเสี่ยง N+1 จริงๆ ไม่ได้อยู่ที่ "แยกตารางกี่ตาราง" แต่อยู่ที่ **ยิง query กี่ครั้ง** — โค้ดปัจจุบันเสี่ยง
เพราะ resolve ทีละจุด (`resolvePlace.ts` ทำ lookup จาก object ในหน่วยความจำทีละ id) ถ้าย้ายไปฐานแล้วยังทำ
`SELECT ... WHERE id = ?` วนลูปต่อ stop จะกลายเป็น N query ต่อวันทันที ไม่เกี่ยวกับจำนวนตาราง

**แนะนำ:** resolve ทุก stop ของ 1 วันด้วย `WHERE id = ANY($ids)` เดียว (batch ids ก่อน แล้ว query ครั้งเดียว)
ส่วนเรื่องหลายภาษา (`place_names`) — ถ้าจะแยกตารางจริงเพื่อรองรับหลายประเทศ/หลายภาษาในอนาคต ให้ query ครั้ง
เดียวด้วย join + `json_agg`/`jsonb_object_agg` รวมชื่อทุกภาษาต่อ place มาในแถวเดียว ไม่ใช่ query แยกต่อภาษา —
หรือถ้าจำนวนภาษาต่อคลังไม่เยอะ (จริงๆ ตอนนี้มีแค่ th/en/local) เก็บเป็น `jsonb name_i18n` คอลัมน์เดียวใน
`catalog.places` เลยจะง่ายกว่า ไม่ต้อง join เลยแม้แต่ครั้งเดียว — **แนะนำทางนี้เว้นแต่ P1 มีเหตุผลอื่นให้แยก
ตาราง** เพราะภาระ query ที่ต้องแลกกับความยืดหยุ่นของตาราง `place_names` แยก ไม่คุ้มสำหรับจำนวนภาษาระดับนี้
