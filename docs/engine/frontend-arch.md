...
🔴 **สถานะปัจจุบัน (26 ส.ค. 2026, เย็น) — อ่านก่อนเชื่อว่า route ที่พูดถึงใน §11–§13/§16 ยังมีอยู่:**
`app/api/booking-file/[...path]/route.ts` ที่หัวข้อพวกนี้ออกแบบ/สร้าง/แก้กันไว้ **ถูกลบแล้ว** (`cb9be74`,
P1) เพราะไม่เคยมีใครเรียกจากฝั่ง UI เลยสักจุด (ดู §16) — งานจริงของ `E2-AC13` ③ จบที่การแก้ **ภายใน
`signStoredFile()`/`lib/engine/files.ts`** แทน (ไม่แตะ `sw.js`, ไม่มี route แล้ว) ตามที่บันทึกไว้ท้าย §16
**หัวข้อ §11–§13 ด้านล่างนี้เก็บไว้เป็นบันทึกการตัดสินใจ (เหตุผลหลายจุดยังถูกในบริบทของมันตอนนั้น) ไม่ใช่
สถานะปัจจุบัน** — ที่ไหนอ้างถึง route ว่า "มีอยู่"/"พร้อมใช้" ให้อ่านว่าหมดอายุแล้ว
...
**สรุปการตัดสินตอนนั้น:** เก็บ `storageKeyOf` ไว้ใช้ — ~~แต่ใช้ฝั่งเซิร์ฟเวอร์ในตัว route
`/api/booking-file/…` เอง~~ (route นั้นถูกลบแล้ว, §16) **ไม่ใช่เอามาคีย์แคชใน `sw.js` โดยตรง** ยังถูกอยู่ —
ข้อสรุปที่ยืนจริงจนถึงวันนี้: `storageKeyOf` ถูกใช้เป็นคีย์ Cache Storage **ใน `signStoredFile()` เอง**
(§16 ท้ายสุด) แทนที่จะอยู่ในชั้น route หรือชั้น `sw.js` — `sw.js` ไม่ต้องแก้อะไรเลยตามที่สรุปไว้ตอนนั้น
ก็ยังจริงอยู่ แค่คนละเหตุผล (ไม่ใช่เพราะ route คงที่ทำหน้าที่เป็นคีย์ แต่เพราะไม่มี fetch ข้าม origin ให้
`sw.js` เห็นเลยอีกต่อไป)
...
**ผิดสองชั้น ณ 26 ส.ค. 2026: ไม่เคยพร้อมใช้จริง (§16) และตอนนี้ไม่มีไฟล์นี้อยู่แล้ว (`cb9be74`) — networkFirst/
404-eviction ด้านล่างเป็นการตัดสินใจที่ใช้ไม่ได้กับ route ที่ไม่มีอยู่ เก็บไว้เป็นบันทึกเหตุผลเท่านั้น**)
...
เรียก `lib/engine/db.ts` เสมอ (แบบเดียวกับที่ `/api/booking-file/{tripId}/…` เคยทำ — ⚠️ ไฟล์นั้นถูกลบไป
แล้วเพราะไม่มีใครเรียก (§16, `cb9be74`) แต่ **pattern การวาง path ยังใช้จริงอยู่** ในทุก route ของ `E3`
ที่ลงจริง เช่น `app/api/engine/trips/[tripId]/custom-places/route.ts` — ใช้ pattern เดียวกันทั้งระบบ
ไม่ใช่คิดใหม่) ไม่ใช่ query param ซึ่งเผลอลืม validate ง่ายกว่า path segment ที่บังคับ
...
- ~~`/api/booking-file/[...path]` ที่มีอยู่แล้วจะกลายเป็นไม่มีใครเรียกจริง — ไม่ใช่ปัญหา เก็บไว้เป็น
  building block สำหรับอนาคตได้~~ **P1 ไม่เห็นด้วยกับข้อนี้และลบไฟล์ทิ้งจริง (`cb9be74`, 26 ส.ค. 2026):
  โค้ดที่ไม่มีใครเรียกคือโค้ดที่ไม่มีใครทดสอบ/ไม่มีใครดู แต่ยังรับ request จริงได้ — พื้นผิวโจมตีที่ไม่มี
  ประโยชน์แลกมา ถ้าวันหน้าต้องการ share link เขียนใหม่ได้ภายในครึ่งชั่วโมงด้วยข้อกำหนดจริงของวันนั้น
  เห็นด้วยกับเหตุผลนี้ ไม่มีเหตุผลค้าน**
...
### ✅ ลงมือแล้ว (`4445e19`) — P1 ยกให้แก้ตรง หลังปิด 3 ข้อที่ขอไว้ก่อนเขียนโค้ด

1. **CORS วัดจริงก่อนเขียนโค้ด ไม่ใช่สมมติฐาน** — probe จริงกับ signed URL ของ `booking-files` บน
   `engine-dev` ด้วย `Origin` header: ได้ `access-control-allow-origin: *` จริง ยืนยันว่า `fetch()`
   ที่เรียกเอง (ไม่ใช่ผ่าน `<img>`/`<a>`) จะได้ response แบบอ่านได้ ไม่ opaque
2. **`revokeObjectURL()`** — `forgetSignedFile()`/`forgetAllSignedFiles()` revoke ก่อนล้างแล้ว บันทึกไว้
   ในคอมเมนต์ว่าสัญญาของฟังก์ชันเปลี่ยนจาก "string ทิ้งได้เฉย ๆ" เป็น "ต้อง revoke" ตามที่ P1 ขอ
3. **โควตา Cache Storage** — LRU index ผ่าน `lib/localCache.ts` เดิม (ไม่เปิด IndexedDB ใหม่ ตามที่ตัดสิน
   ไว้ใน §13) เพดาน 40 ไฟล์ + evict ครึ่งหนึ่งแล้วลองใหม่ 1 ครั้งตอนเจอ `QuotaExceededError` ไม่สำเร็จก็
   ปล่อยผ่าน (ยังโชว์ไฟล์ตอนนี้ได้ แค่ไม่ persist รอบนี้) แทนที่จะพังการแสดงผลตอนออนไลน์
...
### แก้ (`bd2d64e`) — ใช้สัญญาณที่ `dayBridge.ts` คืนมาอยู่แล้ว ไม่เดาจากจำนวนแถว

P1/P7 ไล่ต่อพบว่า `buildDayBridge()` คืน `matched`/`unmatchedDb`/`unmatchedLegacy` มาให้อยู่แล้ว พร้อม
คอมเมนต์ของตัวเองที่เขียนไว้ก่อนบทสนทนานี้ว่า *"`0` กับ 'ไม่มีวันเลย' ต้องแยกออกจากกันที่ผู้เรียก"* —
`grep` ยืนยันว่าไม่มี hook ไหนอ่านค่าพวกนี้เลยสักตัว **ก่อนคอมมิตนี้**

**พิสูจน์กับของจริง ไม่ใช่แค่อ่านซอร์ส:** query `engine-dev` หาทริปที่สองของ P2 จริง ("P2 live-verify test
trip", `fe767e84-…`) → `trip_days` ในฐาน = **0 แถว** รัน `buildDayBridge` จริงกับข้อมูลนี้ได้
`matched === 0` และ `unmatchedLegacy.length === 11` (เท่าจำนวนวันทั้งหมด) — ตรงกับเงื่อนไขที่
`dayBridgeWarning()` เขียนไว้เอง ยืนยันการอ่านของ P1 ถูกต้องด้วยข้อมูลจริง

**เพิ่ม `reportDayBridgeWarningIfAny(bridge, totalLegacyDays)`** — ไม่ใช่ตัวแทน `reportDayBridgeDropIfAny`
เดิม แต่เป็นสัญญาณเสริม: ตัวใหม่เตือน**ก่อน**มีข้อมูลให้หาย (ทริปใหม่เปิดครั้งแรก 0 จุดแวะก็เตือนได้ทันที)
ตัวเดิมยังจำเป็นสำหรับสะพานที่ *บางส่วน* ใช้ได้ (`matched > 0` แต่ยังมีแถวหลุดทาง `unmatchedDb`) wiring
เข้าทั้ง 4 hook ที่ตอน construct bridge — `useStops`/`useBookings` ไม่เคยมีการเตือนเลยมาก่อน
`useDaySettings`/`useOvernightOverrides` มีแต่เป็น `console.warn` (ไม่มีใครเห็นนอก dev tools)

⚠️ **ระหว่างตรวจ พบเรื่องที่ยังไม่ได้พิสูจน์สด แต่ควรบันทึกไว้:** `buildDayBridge` จับคู่ด้วย**วันที่ปฏิทิน
ล้วน ไม่ผูกกับ tripId เลย** — ถ้าสองทริปมีวันที่ตรงกันโดยบังเอิญ (พบว่า trip fixture ของชุดทดสอบใช้ช่วงวันที่
...
## 20. P1 พลิกราก (`trip_days` ว่างเพราะ `create_trip` ไม่สร้างวัน ไม่ใช่สะพานจับคู่ไม่ได้)
### + ผมพบว่า §19's `reportDayBridgeWarningIfAny` เอง**ก็มีบั๊กเดียวกันที่เพิ่งแก้**

**บริบท:** P1 ขอให้ตรวจว่าแถบ (จาก §19) จะไม่ดังตลอดเวลาหลัง migration `create_trip_makes_days` ลง —
เพราะหลัง migration ทริปใหม่จะมี `trip_days` ครบ แต่วันเหล่านั้นจะ**ไม่มีวันไหนตรงกับปฏิทินของ `ITINERARY`
เลย** (`unmatchedDb` เต็ม) ซึ่ง `dayBridge.ts` เขียนไว้เองว่าเป็น **"ปกติสำหรับทริปที่สร้างบนแพลตฟอร์ม"**

**ตรวจแล้วพบว่าบั๊กเกิดไปแล้ว ไม่ใช่แค่จะเกิด:** `reportDayBridgeWarningIfAny` เดิมใช้
`bridge.unmatchedLegacy.length > 0` เป็นเงื่อนไข — และ `unmatchedLegacy` คือ**วันในไฟล์ `ITINERARY` (11 วัน)
ที่ไม่มีคู่ในฐาน** ไม่ใช่ `unmatchedDb` ค่านี้จะเป็น**เกือบ 11 ตลอดไปสำหรับทริปที่ไม่ใช่ทริปเกาหลี** เพราะ
ปฏิทินของมันไม่มีทางตรงกับ 11–21 ต.ค. 2026 ได้เลย ไม่ว่า `trip_days` ของทริปนั้นจะสมบูรณ์แค่ไหน
→ **แถบจะขึ้นตลอดไปสำหรับทริปแพลตฟอร์มทุกใบ ไม่ใช่แค่ตอนว่างเปล่า** — ผิดแบบเดียวกับ toast เดิมที่เพิ่งแก้
ใน §19 ทุกประการ เพียงแต่ย้ายที่ผิดจาก toast ไปอยู่ใน logic ของแถบแทน (`7a71671`)

**แก้:** เปลี่ยนสัญญาณให้รับ `dbDaysCount` (จำนวนแถวจริงของทริปนั้น) แทน `totalLegacyDays` (ค่าคงที่ 11)
```
looksBroken = dbDaysCount === 0 || (bridge.matched > 0 && bridge.unmatchedLegacy.length > 0)
```
- `dbDaysCount === 0` — ทริปไม่มีวันเลยสักวัน (กรณีที่ P2 วัดได้จริง)
- `matched > 0 && unmatchedLegacy.length > 0` — สะพาน**จับคู่ได้บางส่วนแล้ว** (แปลว่านี่คือทริปที่มี
  ความสัมพันธ์กับ `ITINERARY` จริง เช่นทริปเกาหลีหลัง `E7`) แต่ยังหลุดบางวัน — เก็บไว้ตามที่ P1 ต้องการ
- `matched === 0` เฉย ๆ **ไม่ใช่สัญญาณของความผิดปกติอีกต่อไป** — เป็นสภาพปกติของทริปแพลตฟอร์มทุกใบ

ยืนยัน: `eslint`/`tsc --noEmit` สะอาดทั้งคู่ (0 error รอบนี้ — `engineCrossUser.test.ts` error เดิมหายไปแล้ว
ด้วย ไม่ใช่ของผม) + ชุดทดสอบเต็ม 846 ผ่านทั้งหมด (60 ไฟล์ — เพิ่มจากรอบก่อนเพราะเซสชันอื่นเพิ่มไฟล์ทดสอบ)

**เพิ่มพร้อมกัน (③ ต่อ):** wiring toast เข้า 3 จุดที่เคยมีแค่ `console.error` เมื่อผู้ใช้พยายามเขียนวันที่ยัง
ไม่มีในฐาน (`useStops.insertAt`, `useDaySettings.writeRows`, `useOvernightOverrides.setOvernightCity`) —
ตรงกับบั๊กที่ P2 กดปุ่ม "+ เพิ่มลงวันนี้" จริงแล้วเจอ (ล้มเงียบ มีแค่ log ที่ผู้ใช้ไม่เห็น)

### ทิศของชั้น ② ที่ P1 ขอ (ยังไม่ใช่โค้ด — ตามที่ขอ)

P2 เปิดหน้าจอทริปวันเดียวจริงแล้วพบว่า 3 หน้าหลัก (`app/page.tsx`/`today`/`summary`) **ไม่ได้ fallback ไป
`ITINERARY` เพราะเขียนกันไว้แบบนั้น — มันไม่เคยอ่านจากทริปเลยสักบรรทัดตั้งแต่แรก** ทั้ง 3 หน้า import
`ITINERARY` ตรง ๆ ไม่มีเงื่อนไขอะไรทั้งนั้น (ตรงกับตัวเลข 67 จุด/13 ไฟล์ ที่ §0 นับไว้) — นี่คือของเดิมของเว็บ
เกาหลีที่ยังไม่เคยถูกแตะเลยตั้งแต่ `E5-AC1` ทำแค่ routing (`/trip/[tripId]`) กับ 9 hooks ที่อ่านจุดแวะ/booking/
overnight ผ่าน DB แล้ว — **โครงวัน (วันที่/เมือง/ป้าย) ของ 3 หน้านี้ไม่เคยย้ายเลย**

**ข้อเสนอ:** อย่าพยายามเดาสถานะจากสัญญาณ `dayBridgeIncomplete` เดิม (ตอนนี้มันแก้ให้ถูกต้องแล้วว่า "ทริป
แพลตฟอร์มที่มีวันจริง" ไม่ใช่ของผิดปกติ — เอาไปใช้กันแถบ content ทั้งหน้าจะขัดกับความหมายที่เพิ่งแก้)
**ต้องมีสัญญาณที่สองแยกกันเด็ดขาด** อาจเรียก `usePlatformTripUnsupported()` (ชื่อชั่วคราว) — `true` เมื่อ
`dbDaysCount > 0 && bridge.matched === 0` (ทริปมีวันจริงแต่ไม่ตรงกับ `ITINERARY` เลย = ทริปแพลตฟอร์มแท้ ๆ)
ให้ 3 หน้าหลักใช้ gate ทั้ง body แทนที่จะ render `ITINERARY` เงียบ ๆ เหมือนวันนี้ — โชว์ข้อความรอ `E5` แทน
ไม่ใช่ข้อมูลที่ผิดทั้งหมด (คนละอย่างกับแถบเตือนซึ่งจงใจไม่ขึ้นสำหรับกรณีนี้แล้ว)

**หลัง `E5` ลง** (แหล่งวันเปลี่ยนเป็น `trip_days.id` จริง): 3 หน้านี้อ่านวันจากฐานตรง ๆ ไม่ผ่าน `ITINERARY`
อีกต่อไป ปัญหาที่เหลือคือ "วันที่ไม่มี city/weather/ป้ายติดมาด้วย" ซึ่งเป็นของปกติสำหรับทุกทริปที่ไม่ใช่
ทริปเกาหลี ไม่ใช่บั๊ก — **แต่ "ควรมี metadata อะไรต่อวันของทริปแพลตฟอร์ม" เป็นคำถามที่กว้างกว่าโซนผม**
(น่าจะเป็นของ P5/P8 ตัดสินร่วมกับ P1 ว่า field ไหนของ `Day` type เป็น optional/ทริปเฉพาะ เทียบกับของที่
ทุกทริปต้องมี) — ผมแค่ชี้ทิศของ *เมื่อไหร่ต้องเปลี่ยนแหล่งข้อมูล* ไม่ได้ชี้ทิศของ *เนื้อหาแต่ละวันควรมีอะไร*

---

## 21. `usePlatformTripUnsupported` อนุมัติแล้ว → ทำจริงเป็น gate ชั้นบนสุด (`08c591c`) — กิ่งเดียวตามที่ P1 ขอ

**สิ่งที่ P1 ตัดสิน:**
- `Day` type — **ไม่ทำ optional** สองรูป + ตัวแปลงเดียว (แบบ `customPlaceShape.ts`/`toCustomPlace` +
  `assertJoined()`) ทริปเกาหลียังใช้ `Day` เดิมครบทุก field ตลอดไป · **ยังไม่เริ่ม** รอ migration ลง
- gate หน้า — **อนุมัติ ให้เริ่มได้เลย แต่กิ่งเดียว** (`dbDaysCount === 0`) ไม่ใส่กิ่ง `matched === 0` จนกว่า
  migration `create_trip_makes_days` ลง เพราะวันนี้ทริปทุกใบ `dbDaysCount === 0` เหมือนกันหมด — กิ่งที่สอง
  จะเขียวได้โดยไม่มีอะไรเดินผ่านจริง (รูปเดียวกับเคสไม่รันที่ทีมไล่ปิดกันทั้งวัน)
- ข้อความต้องไม่อ่านเหมือนบั๊ก: *"ทริปนี้ยังเปิดดูไม่ได้ ระบบกำลังรองรับทริปหลายใบอยู่"*

**สิ่งที่ทำจริง:**
- `hooks/useTripDaysGate.ts` (ใหม่) — fetch `/api/engine/trips/${tripId}/days` **ครั้งเดียว** คืน
  `"unknown" | "empty" | "has-days"` **fail-open โดยตั้งใจ**: ค่าเริ่มต้น/ตอน fetch ล้ม/เน็ตหลุด = `"unknown"`
  ไม่ใช่ `"empty"` — "ยังไม่รู้ว่ามีวันไหม" ≠ "รู้ว่าไม่มีวัน" (P1 เน้นย้ำ) กันไม่ให้ทริปเกาหลีจริงถูกบล็อกเพราะ
  fetch พลาดจังหวะเดียว
- `components/TripDaysEmptyNotice.tsx` (ใหม่) — คัดลอกโทน `TripStatusFallback.tsx` เป๊ะ
  (`flex min-h-full items-center justify-center p-8 text-center`)
- `components/TripDataProvider.tsx` — เรียก gate **ก่อน**มี provider ไหนเริ่ม mount เลย (ไม่ใช่แค่ซ่อน
  content หลัง fetch เสร็จ) ถ้า `gate === "empty"` คืน `TripDaysEmptyNotice` แทนทั้งต้นไม้ — ทริปที่บล็อกจะ
  ไม่ยิง request ของ `HotelsProvider`/`BookingsProvider`/`CustomPlacesProvider` เปล่า ๆ เลย
- **ครอบคลุมทั้ง 5 route ที่ใช้ `TripDataProvider`** โดยไม่ต้องแตะทีละหน้า (`page.tsx`/`today`/`summary` ที่
  bare + `/trip/[tripId]` ทั้ง 3) เพราะ mount ที่ provider เดียวตามที่ P1 ชี้ไว้

**ไม่แตะ:** เนื้อหาแต่ละวัน (`ITINERARY`) ยังเป็นแหล่งเดิมทุกบรรทัด — นี่คือ gate ชั้นบนสุด ไม่ใช่จุดเริ่ม
เปลี่ยนแหล่งข้อมูล ไม่ได้ลากไปถึงการเปลี่ยนตัวตนของวัน (`E5`) เลยแม้แต่จุดเดียว

**ยืนยัน:** `eslint` เจอ 1 จุดที่ต้องแก้ระหว่างเขียน (`react-hooks/set-state-in-effect` — ย้าย
`setState("unknown")` เข้าไปในบรรทัดแรกของ async callback แทนที่จะเรียกตรง ๆ ในตัว effect) แก้แล้วสะอาด ·
`tsc --noEmit` 0 error · test suite เต็ม 863/863 ผ่าน (62 ไฟล์)

⚠️ **ยังไม่ได้ยืนยันด้วยเบราว์เซอร์จริง** — เซสชันอื่นถือ dev server พอร์ตเดียวกันของ worktree นี้อยู่
...
## 23. `hotelLegs` แก้แล้ว (`2812dab`) — โชว์ที่พักจริงแบบแบน ไม่ซ่อนทั้งก้อน

P1 ตัดสินหลักการ: **อย่าซ่อนข้อมูลที่ถูก ซ่อนเฉพาะส่วนที่คำนวณจากแหล่งที่ผิด** — `hotels` (แถวจริงจากฐาน)
ถูกต้องเสมอไม่ว่าทริปไหน (ยืนยันแล้วใน §21/§22 ว่า `useHotels` คีย์ด้วย `check_in`/`check_out` ของแถวเอง
ไม่แตะ `ITINERARY`/`trip_days` เลย) — ที่ผิดคือ **การจัดกลุ่มเป็น "leg"** (`useHotelSchedule(itinerary,
hotels)` → `deriveHotelLegs(itinerary)`) ซึ่งคำนวณช่วงวันจาก `ITINERARY` ล้วน ๆ

**พบเพิ่มระหว่างแก้ (ไม่ใช่แค่ leg display ผิด — นับผิดด้วย):** `TripPrepPanel`'s `hotelsSetCount`/
`hotelsTotal` มาจาก `hotelLegs.filter(...).length`/`hotelLegs.length` — ทริปแพลตฟอร์มจะได้ **"ที่พัก 0/11
⚠️" ตลอดกาลไม่ว่าจะบันทึกที่พักไปกี่ที่แล้ว** เพราะ leg (จาก 11 วันเกาหลี) ไม่มีทางตรงกับคีย์ของที่พักจริง
เลย — คนละอาการกับที่ P1 ถามแต่รากเดียวกัน (leg ผูกกับ `ITINERARY`)

**ทำ:** `components/HotelsFlatList.tsx` (ใหม่) — list ที่พักจริงตรง ๆ ด้วย `city`/`check_in`/`check_out`
ของแถวเอง ไม่จัดกลุ่มเป็น leg wiring เข้า `page.tsx`'s `TripPrepPanel` (สลับกับ `HotelLegsPanel` ตาม
`dayPlanReady`) และ `summary/page.tsx`'s hotels-all section เดียวกัน พร้อมแก้ `hotelsSetCount`/
`hotelsTotal` ให้ใช้ `Object.keys(hotels).length` ตอน `!dayPlanReady` (ไม่มี "ครบ/ไม่ครบ" ให้เทียบ จึงไม่มี
`⚠️` ผิด ๆ อีกต่อไป)

🔴 **อ่านอย่างเดียว — ยังไม่มีปุ่มเพิ่ม/แก้ที่พักสำหรับทริปแพลตฟอร์ม** เพราะ `HotelEditModal` (เปิดจาก
`HotelLegsPanel`) ต้องมี `legId`/`city`/ช่วงวันจาก leg เสมอ — ทริปแพลตฟอร์มไม่มี leg ที่ถูกต้องให้ใช้เลย
การสร้างทางป้อนที่พักใหม่แบบไม่ผูก leg เป็นงานแยกที่ยังไม่เริ่ม (ไม่ใช่ขอบเขตของ gate รอบนี้) — ไม่กระทบ
วันนี้เพราะยังไม่มีทริปแพลตฟอร์มใบไหนมีที่พักจริงเลย (P1 ยืนยันจาก `engine-dev`) รายการนี้จึงว่างเปล่าในทาง
ปฏิบัติ แต่พร้อมโชว์ทันทีที่มีข้อมูลจริง

ยืนยัน: eslint/tsc สะอาดทั้งคู่ + test suite 885/885 ผ่าน (64 ไฟล์)

---

## 24. P2 ยิงครบ 6 จุด — `/summary` ดูเหมือนบล็อกทั้งหน้า แต่วัดแล้วไม่ใช่ (`90b1217`)

P2 ยิงจริงกับ `/summary`/`/trip/{id}/summary` พบว่าไม่เห็นหัวข้อที่พัก/ตั๋ว/checklist เลย ดูเหมือน gate
กว้างเกิน (ซ้ำรูปเดิมที่เจอกับ `TripDataProvider`) — P1 ขอให้เช็คก่อนแคบ gate เพราะมีคำอธิบายที่สองที่ให้
ผลจอเหมือนกันเป๊ะ: ทริปทดสอบไม่มีที่พัก/ตั๋ว/checklist จริงอยู่แล้ว section พวกนั้นก็ว่างเปล่าไม่ว่ามี gate
หรือไม่

**ตรวจสองทาง ไม่รอ P2 แยกเพิ่ม:**
1. **อ่านโค้ด** — `bookings.length > 0 &&`/`checklistItems.length > 0 &&` (§ hotels-all/bookings-all/
   checklist ใน `summary/page.tsx`) **ไม่ถูกแตะเลยในทุกรอบที่ผ่านมา** เป็นอิสระจาก `dayPlanGate`
   ทั้งหมด · `HotelsFlatList` เองก็ `return null` ถ้า `Object.values(hotels).length === 0`
2. **วัดจริงกับฐาน** — query `engine-dev` ตรง ๆ ด้วย service-role script (ทริป `e52b7d72-2a29-4bb5-
   9dc9-abf6a92c27e6`): `trip_hotels`/`bookings`/`checklist_items`/`trip_days` **ทั้ง 4 ตาราง = 0 แถว**

**สรุป: เป็น ② ไม่ใช่ ①** — gate ไม่ได้กว้างเกิน ทริปทดสอบใบนี้ไม่มีข้อมูลให้ section พวกนั้นแสดงจริง ๆ
ไม่ต้องแคบ gate ตามที่ P1 เตือนไว้ล่วงหน้า

**แก้ที่ต้องทำแน่นอนไม่ว่าคำตอบไหน — เสร็จแล้ว:** `DayPlanUnavailableNotice`'s บรรทัดที่สองเคยเขียน
"ที่พัก/ตั๋วของคุณยังอยู่ครบตามปกติ" ซึ่งอ้างสถานะข้อมูลของผู้ใช้จากหน้าที่บล็อกอยู่ — ตรวจสอบไม่ได้ และผิด
ทันทีที่ทริปนั้นไม่มีที่พัก/ตั๋วจริง (เหมือนทริปทดสอบข้างบนเป๊ะ) เปลี่ยนเป็น "ส่วนที่ไม่เกี่ยวกับแผนรายวัน
(ที่พัก/ตั๋ว/ของที่ต้องเตรียม) ยังใช้งานได้ตามปกติ" — อธิบายกฎ ไม่อ้างข้อมูลของผู้ใช้ ตระกูลเดียวกับ `D71`

ยืนยัน: eslint/tsc สะอาดทั้งคู่ + test suite 892/892 ผ่าน (65 ไฟล์)

---

## 25. P2 วัดเพิ่ม — จุดชี้ขาดจริงคือ `HotelsFlatList` ไม่ตรงรูปแบบแอป ไม่ใช่ gate กว้างเกิน (`28198f0`)

P2 เทียบทริปเดียวกัน (`e52b7d72`) ระหว่าง `/` (โชว์หัวข้อ + empty-state ครบทั้ง 3 ส่วน) กับ `/summary`
(ไม่มีร่องรอย 3 ส่วนนี้เลย) — P1 อ่านผลนี้ว่าเป็น **① gate กว้างเกิน** และขอให้แคบ

**ตรวจก่อนแก้ตามคำขอ — พบว่าการวินิจฉัยคลาดเคลื่อน 1 จุด สำคัญพอที่จะแก้คนละที่:**
- `git log -S"bookings.length > 0" -- app/summary/page.tsx` / เดียวกันกับ `checklistItems.length > 0`
  → ทั้งคู่มาจาก **`c235d1d` (7 ส.ค. 2026)** สามสัปดาห์ก่อนงานวันนี้ **ไม่เคยถูก `dayPlanGate` แตะเลย**
  การซ่อน section ทั้งก้อนเมื่อไม่มีข้อมูลบน `/summary` เป็นพฤติกรรมเดิมของหน้านี้เอง **ไม่ใช่ผลจาก gate**
- ส่วนที่ผิดจริงคือ `HotelsFlatList` (ที่ผมเพิ่งทำใน `2812dab`) — `return null` ตอนไม่มีที่พัก ต่างจาก
  `BookingsPanel`/`HotelLegsPanel` ที่โชว์หัวข้อ + ข้อความ empty-state เสมอ (`bookings.length === 0 ?
  "ยังไม่มีตั๋ว/booking เก็บไว้" : ...` ใน `BookingsPanel.tsx`) — **นี่คือของที่ P2 เห็นจริง ๆ ว่าหายไป**

**แก้:** `HotelsFlatList` เปลี่ยนจาก `if (rows.length === 0) return null` เป็นโชว์ `<section>` +
`<h2>🏨 ที่พักของทริป</h2>` เสมอ พร้อมข้อความ "ยังไม่มีที่พักบันทึกไว้" ตอนว่าง — ตรงกับรูปแบบเดียวกับ
`HotelLegsPanel`/`BookingsPanel` แล้ว

⚠️ **ยังไม่แก้ (ไม่ใช่ของที่ gate นี้ทำให้เกิด):** `/summary`'s bookings-all/checklist ยังซ่อน section ทั้ง
ก้อนเมื่อว่าง (พฤติกรรมเดิมจาก `c235d1d`) ต่างจาก `/` — เป็น UX inconsistency ที่มีมาก่อนงานวันนี้ ไม่เกี่ยว
กับ `trip_days`/gate เลย **P1 ยืนยันแล้วว่าไม่ใช่ ① และธรรมเนียม `length > 0 &&` ของ `/summary` น่าจะตั้งใจ
(เป็นหน้าดู/พิมพ์) ไม่ขอให้แก้** — จดไว้เผื่อมีคนรายงานซ้ำในอนาคต

ยืนยัน: eslint/tsc สะอาดทั้งคู่ + test suite 892/892 ผ่าน

📌 **ข้อสังเกตที่ P1 ฝากไว้ (ไม่ใช่บั๊ก แต่ควรรู้):** ทริปที่ **ทั้งไม่มีวันและไม่มีข้อมูลอื่นเลย** (ไม่มีที่พัก/
ตั๋ว/checklist สักอย่าง — ตรงกับทริปทดสอบ `e52b7d72`) จะทำให้ `/summary` เหลือแค่ `DayPlanUnavailableNotice`
อย่างเดียวทั้งหน้า เพราะ `HotelsFlatList` (โชว์หัวข้อเสมอ) เจอ `bookings-all`/`checklist` (ซ่อนทั้งก้อนเมื่อ
ว่าง) พร้อมกัน — ผู้ใช้อาจอ่านว่า "หน้าพัง" ทั้งที่เป็นผลลัพธ์ถูกต้องของสองเงื่อนไขที่บังเอิญเกิดพร้อมกัน ถ้ามี
รายงาน "summary ว่างเปล่า" ในอนาคต นี่คือคำอธิบายก่อนไล่หาบั๊กใหม่

---

## 26. ตอบคำถามตรง ๆ ของ P1 — `matched === 0` แต่มีวันจริง ควรขึ้นแถบไหม (`c46eeaf`)

P1 ชี้ว่า `reportDayBridgeWarningIfAny` (แก้เช้านี้ §19) กับ `reportDayBridgeDropIfAny` ไม่ตรงกันเรื่อง
`matched === 0`: ตัวแรกบอกว่า "ไม่ใช่สัญญาณอะไรทั้งนั้น" ส่วนตัวที่สองยังตั้งแถบเสมอเมื่อ `rows.length > 0
&& bridge.matched === 0` (เพราะ `reportDayBridgeDropIfAny(rows.length, bridge.matched)` ใน
`useOvernightOverrides.ts` — `dropped = rawCount > 0 && mappedCount < rawCount` เป็นจริงเสมอเมื่อ
`matched = 0` แต่ `rows.length > 0`) — ถามตรง ๆ ว่าทริปที่มีวันจริงแต่ `matched === 0` ควรขึ้นแถบไหม
ห้ามตอบ "แล้วแต่"

**ตอบ: ควร** — เหตุผล: มติเช้านี้ตั้งอยู่บนสมมติฐานว่า "หน้าเว็บจะจัดการทริปที่ `matched === 0` แต่มีวันจริง
ถูกทาง" ซึ่ง**ยังไม่จริง** — `useTripDaysGate` (§21/§22) เช็คแค่ `dbDaysCount === 0` ไม่ได้เช็ค `matched`
เลย ทริปแพลตฟอร์มที่มีวันจริง (ก่อน `E5` ลง) จะยัง render โครงวันจาก `ITINERARY` ทับอยู่ดี
(`itinerary.map` ใน `page.tsx`/`summary/page.tsx` ไม่เช็ค `matched`) — **หน้าจอผิดจริงในสภาพโค้ดวันนี้**
ไม่ใช่แค่ "ไม่มีอะไรให้เตือน" อย่างที่มติเช้าคิด

**พบว่าเงื่อนไขยุบเหลือเส้นเดียว:** `ITINERARY` มี 11 วันคงที่เสมอ → `matched === 0` แปลว่าทุกวันในไฟล์ตกไป
`unmatchedLegacy` ตามนิยามของ `buildDayBridge` (ดู `lib/engine/dayBridge.ts`) เงื่อนไขทั้งสองกรณีจึงยุบเหลือ
แค่ `bridge.unmatchedLegacy.length > 0` เส้นเดียว — **เหมือนโค้ดก่อนแก้เมื่อเช้าทุกประการ** ไม่ใช่เพราะเช้านี้
คิดผิด แต่เพราะสถานการณ์ที่จะทำให้ผลต่างกัน (gate ของหน้าเข้าใจ `matched`) ยังไม่มีอยู่จริงวันนี้ — วันที่มี
ค่อยแยกเงื่อนไขใหม่พร้อมกับตอนนั้น

**ทำ:** ลบพารามิเตอร์ `dbDaysCount` ออกจาก `reportDayBridgeWarningIfAny` (ไม่จำเป็นอีกต่อไป) แก้ 4 จุดเรียก
ใน `useStops`/`useDaySettings`/`useOvernightOverrides`/`useBookings` ให้ไม่ส่งค่านั้น — แก้
`lib/__tests__/dayBridgeIncomplete.test.ts`'s เคส `matched === 0` ทั้งที่มีวันจริง จาก `expect(...).toBe(false)`
เป็น `toBe(true)` ตามที่ P1 เขียนกำกับไว้ในไฟล์เองว่า "ถ้าตัดสินตรงข้าม เคสนี้จะแดง และนั่นคือหน้าที่ของมัน"

⚠️ **จุดที่ต้องกลับมาทบทวนพร้อมกัน:** ถ้าวันหนึ่ง gate ของหน้าเข้าใจ `matched` เอง (เพิ่มกิ่งที่สองตามที่
...
## 28. บั๊กจริงบนเว็บที่ผู้ใช้ใช้อยู่ — `409 ambiguous` ทุกครั้งที่โหลดหน้า (`171cbad`)

P1 เปิดเว็บจริงแล้วเจอ `GET /api/engine/plans` คืน `409 {"error":"ambiguous"}` ทุกครั้งที่โหลดหน้า → toast
แดง *"ลองรีเฟรชอีกครั้ง"* ที่รีเฟรชกี่ครั้งก็ไม่หาย — เพราะผู้ใช้มีทริปเกิน 1 ใบแล้วจริง ๆ (สภาพที่ 409 ตั้งใจ
ให้เกิด ไม่ใช่บั๊กของ `409` เอง)

**ราก:** `usePlans()` เขียนก่อน `E5-AC1` มี `/trip/[tripId]` เลยไม่เคยรับ `tripId` เข้ามาเลย — เรียก
`GET /api/engine/plans` (account-scoped) ซึ่งฝั่งเซิร์ฟเวอร์ต้องเดาทริปเองผ่าน `soleTrip()` ทั้งที่หน้านั้นรู้
ทริปที่ถูกต้องอยู่แล้ว (ผ่าน `useActiveTripId()`, ส่งเป็น prop ให้ 9 hook ที่เหลือใช้กันหมดแล้ว) —
`usePlans` เป็น hook เดียวที่ยังให้เซิร์ฟเวอร์เดาซ้ำ **สองกลไกตัดสินใจ "ทริปไหน" ในหน้าเดียวกัน คนละที่ คนละ
คำตอบ**

🎯 **เมื่อคืน `fetchReadJson` (§27) เปิด toast ให้ทุกจุดอ่านที่เคยเงียบ — งานทั้งสองฝั่งถูกคนละเรื่อง รวมกันได้
ผลที่ไม่มีใครตั้งใจ** `409 ambiguous` เป็นสถานะที่ต้องมีหน้าเลือกทริป ไม่ใช่ "อ่านไม่สำเร็จ" การจับมันเป็น read
failure คือจัดหมวดผิดตั้งแต่ต้น

**แก้:** `usePlans(tripId)` — รับ `tripId` เป็น prop บังคับ (เหมือนอีก 9 hook) ส่ง `?tripId=` เป็น query
param ทุกคำขอ (GET/POST/PATCH/DELETE) `app/api/engine/plans/route.ts`'s `ctx()` ใช้ `tripId` จาก query
ตรง ๆ ถ้ามี **ข้าม `soleTrip()` ทั้งเส้น** ปลอดภัยเท่ากับทุก route ใต้ `trips/[tripId]/` เพราะ
`plansOfTrip`/`insertPlan`/ฯลฯ ทุกตัวผ่าน `db` ที่เป็น client ผูก RLS ของผู้ใช้เอง — ทริปที่ผู้ใช้ไม่ได้เป็น
สมาชิกจะได้แถวว่าง/ถูกปฏิเสธเหมือนเดิมไม่ว่า `tripId` จะมาจากไหน ไม่ใช่ช่องโหว่ใหม่ `soleTrip()` ยังอยู่เป็น
fallback สำหรับผู้เรียกที่ไม่ส่ง `tripId` มา

🔴 **ไม่ได้แก้ด้วยการปิดเสียง toast** ตามที่ P1 ขอชัดเจน — ตัดเงื่อนไขที่ทำให้เกิด `ambiguous` ออกทั้งเส้นแทน
เคส ambiguous จริง (resolve ทริปไม่ได้เลยจริง ๆ) ยังเข้าถึง route นี้ไม่ได้เหมือนเดิม เพราะ `useActiveTripId()`
จะไม่ส่ง `tripId` ที่ไม่ resolve มาให้ตั้งแต่แรก

ยืนยัน: eslint/tsc สะอาดทั้งคู่ + test suite ที่เกี่ยวข้อง (`engineAttackSurface`/`realtimeNoteCoverage`/
`schemaPins`) ผ่านหมด — ชุดเต็มมีเคสล้มจากการชนกันของ fixture ข้ามเซสชัน (`trip_days_unique_date`
duplicate key ซ้ำ ๆ, ยืนยันด้วยการรันไฟล์เดี่ยว) ไม่เกี่ยวกับงานนี้
