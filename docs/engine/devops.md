...
## 15. Runbook — job `readonly-storage` (`E3`) · เขียน 27 ส.ค. 2026 โดย P6

**เจ้าของ job: P6** · **เจ้าของเทสต์: P4** (`lib/__tests__/readOnlyStorage.test.ts`)

### 15.1 🔴 ทำไมมันไม่รันเอง และจะไม่มีวันรันเอง

เทสต์นี้ต้อง **เข้าโหมด read-only ระดับ global และ commit ธง** — storage เป็นคนละ connection
จึงเลี่ยง rollback-trick ของ `E3-AC7` ไม่ได้
→ **ทุก write ของทุกเซสชันถูกบล็อกตลอดหน้าต่างที่รัน** · engine-dev เป็นฐานใบเดียวที่ 8 เซสชันใช้ร่วมกัน

🔴 **ห้ามผูกกับ `push` / `pull_request` / `schedule` เด็ดขาด** — job อัตโนมัติที่บล็อกทุกคน
คือ job ที่จะบล็อกใครสักคนตอนที่ไม่มีใครรู้ว่าเขากำลังใช้ฐานอยู่
· บังคับด้วย **2 ชั้น**: `on: workflow_dispatch` **และ** `if: github.event_name == 'workflow_dispatch'`
  ⚠️ **trigger อย่างเดียวไม่พอ** — job จะติดไปกับ `push`/`PR` ที่ workflow เดียวกันมีอยู่แล้ว

### 15.2 ⚠️ กับดักที่เสียเวลาไปแล้วหนึ่งรอบ — **ปุ่มไม่โผล่จนกว่าจะ push**

**GitHub แสดง "Run workflow" เฉพาะ workflow ที่อยู่บน branch *บน remote* แล้วเท่านั้น**
· 27 ส.ค. 2026: job อยู่ในทรีแล้ว เทสต์อยู่ในทรีแล้ว **แต่ 110 commit ยังไม่ขึ้น remote**
  → หน้า Actions ไม่รู้จัก job นี้เลย · P4 standby รอกดปุ่มที่ไม่มีอยู่
· 🎯 **`workflow_dispatch` ไม่ย้อนหลัง** — เขียนไฟล์เสร็จ ≠ กดได้

### 15.3 📋 ลำดับที่ต้องเกิด — ข้ามขั้นไม่ได้

| | ขั้น | ใครทำได้ |
|---|---|---|
| 1️⃣ | **push** ให้ workflow ขึ้น remote | ผู้ใช้ของเซสชันที่ push อนุมัติเท่านั้น (`D72`) |
| 2️⃣ | dispatch `phase=reproduce` → คาดว่า **ช่องยังเปิด** (ลบไฟล์ตั๋วได้) | คนที่รู้ว่าไม่มีใครใช้ engine-dev |
| 3️⃣ | ลง migration ที่จอดใน `pending-review/` — **นอก CI** | คนที่มี credential |
| 4️⃣ | dispatch `phase=verify` → คาดว่า **ลบไม่ได้แล้ว** | ต้องผ่าน 3️⃣ ก่อน |

🔴 **CI ลง migration เองไม่ได้** — secret ที่ workflow มีคือ `DEV_SUPABASE_URL` · `DEV_SUPABASE_ANON_KEY`
· `DEV_SERVICE_ROLE_KEY` · `DEV_PROJECT_REF` เท่านั้น · **ไม่มี `SUPABASE_ACCESS_TOKEN` ไม่มี db password**
· ✅ **และการแยกเป็น 2 dispatch ดีกว่าหน้าต่างเดียวอยู่แล้ว** — หน้าต่างสั้นลงครึ่งหนึ่ง และ
  **ฐานไม่ถูกล็อกระหว่างที่มีคนกำลังรัน migration** ซึ่งเป็นช่วงที่เปราะที่สุด

### 15.4 🛡️ สามชั้นที่กันคนละเรื่อง — เจตนาคือ**ไม่ให้ทับกัน**

| ชั้น | เจ้าของ | กันอะไร |
|---|---|---|
| `unsafe_state_reason` (`D65`) | P4 (ในเทสต์) | ปฏิเสธก่อนพลิกธง ถ้าสถานะฐานไม่ปลอดภัย |
| **pre-flight** `check-readonly-mode.py` | P6 (ใน job) | ปฏิเสธที่จะ*เริ่ม* ถ้าโหมดเปิดค้างอยู่ — มีคนอื่นอยู่ในหน้าต่าง หรือหน้าต่างก่อนค้าง |
| **expiry 2 นาที (ไม่ใช่ `null`)** | P4 (ฝั่ง DB) | 🔴 **ตาข่ายจริง** — ถ้า runner ตาย `if: always()` ก็ไม่รัน |
| `concurrency: engine-dev-live` | P6 | กันทับกับ job `rls` ที่**เขียนจริง** |

🎯 **`concurrency` คือชั้นที่สำคัญที่สุดและมองไม่เห็นที่สุด:** ถ้า `readonly-storage` กับ `rls` คนละ group
เมทริกซ์ RLS จะแดงเพราะ **เขียนไม่ได้ ไม่ใช่เพราะ policy ผิด** — **และมันดูเหมือนบั๊กความปลอดภัยทุกประการ**

### 15.5 🚨 ถ้าฐานค้างในโหมด read-only

**อาการ:** ทุกเซสชันเขียนอะไรไม่ได้เลย โดยไม่มีข้อความอธิบาย
· ขั้น **post-flight `if: always()`** ของ job ตรวจข้อนี้และ **ทำให้มันดัง** (`::error::`) — **แต่มันไม่ได้ปิดให้**
· 🎯 เงียบ = คนถัดไปเจอ write ล้มโดยไม่รู้สาเหตุ · ดัง = มีคนไปปิดได้ทันที

```bash
.github/check-readonly-mode.py     # แดง = โหมดยังเปิดอยู่
```
· ปิดด้วย `set_system_mode` — 🔴 **ใช้ named params เสมอ** (พารามิเตอร์ boolean สองตัวติดกัน สลับกันแบบเงียบได้)
· รอ 2 นาทีก็หายเองตาม expiry — **แต่ยืนยันด้วยด่านข้างบน อย่าเดา**

---

## 16. Node version — dev กับ CI ต้องจับคู่กันไว้ในเอกสาร (27 ส.ค. 2026 · P6 · P4 ชี้)

**สถานะวันนี้: `dev = 20.12.2` · `CI = 20.12.2` — ตรงกันเป๊ะ**
```
.github/workflows/ci.yml  → node-version: '20.12.2'  (3 จุด: guards/verify/rls)
เครื่อง dev นี้            → v20.12.2
package.json               → ไม่มี "engines" field กำกับไว้
```

### 🔴 ทำไมต้องจดทั้งที่วันนี้ไม่มีปัญหา

P4 ชี้ระหว่างตรึง `jsdom@25` (`7404c48`): **ถ้าวันหนึ่งมีคน bump `node-version` ใน `ci.yml` เป็น 22+
โดยไม่รู้ว่ามันผูกกับ `jsdom` เวอร์ชันบนเครื่อง dev** —

· `jsdom@26+` จะ **ทำงานได้บน CI (Node 22+) แต่พังบนเครื่อง dev (Node 20)**
  (`html-encoding-sniffer` ดึง `@exodus/bytes` ที่เป็น ESM ล้วน → `require()` ชน `ERR_REQUIRE_ESM`)
· **CI จะเขียว** เพราะมันไม่เจอปัญหานี้เลย — 🎯 **"CI เขียว" จึงไม่ใช่หลักฐานว่าปลอดภัยสำหรับเคสนี้โดยเฉพาะ**
· และอาการฝั่ง dev ที่พังคือ **`no tests` ไม่ใช่ `FAIL`** — ตัวเลขรวมลดลงเฉย ๆ โดยไม่มีอะไรแดง
  🔴 **เซตว่างอีกรูปหนึ่ง** (รูปที่ทีมนี้โดนมาแล้วหลายรอบวันนี้ — `git ls-files` กับ `[tripId]`,
  ทรีจำลองที่ไม่มี `supabase-platform/`) ต่างกันแค่ *ที่ที่มันโผล่*: รอบนี้โผล่ที่เครื่อง dev ไม่ใช่ที่ด่าน

### 📌 กติกา

**Node version ของ dev กับ CI ต้อง**เท่ากันเสมอ** จนกว่าจะมีคน**ตัดสินใจ**ขยับพร้อมกันทั้งคู่**
· ห้ามขยับ `node-version` ใน `ci.yml` ฝ่ายเดียว โดยไม่เช็คก่อนว่าเครื่อง dev ทุกเครื่องขึ้นตามด้วย
· `jsdom` ปลดตรึงจาก `25` ได้ **เมื่อทั้งสองฝั่งอยู่ Node 22+ แล้วเท่านั้น** (เงื่อนไขเดียวกับที่ P4 เขียนไว้ใน
  `package.json` คีย์ `"//jsdom"`)
· ⚠️ **`package.json` ไม่มี `"engines"` กำกับ** — ไม่มีอะไรเตือนตอน `npm install` ถ้า Node เครื่องไม่ตรง
  ยังไม่ใส่ตอนนี้เพราะจะกระทบทุกเครื่อง dev ทันที (นอกโซนตัดสินเดี่ยว) — จดไว้เป็นตัวเลือกที่ยังไม่ทำ

---

## 17. `E7-AC8` — `pg_dump` แทน `supabase db dump` · โควตา org เต็ม (28 ส.ค. 2026 · P6)

### 17.1 🔴 `supabase db dump` ใช้ไม่ได้บนเครื่องที่ไม่มี Docker
```
$ supabase db dump
failed to run docker. Docker Desktop is a prerequisite
```
CLI คำสั่งนี้เรียก Docker เสมอแม้จะแค่ต่อไปยังฐานคลาวด์ — **ไม่ได้รันอะไรใน container จริง แต่ยังเช็ค Docker ก่อน**
· ✅ **ทางที่ใช้ได้จริงและใช้แล้ว**: `brew install libpq && brew link --force libpq` แล้วเรียก `pg_dump` ตรง ๆ
  ต่อ connection string ของโปรเจกต์ (`pooler-url`/`project-ref` จาก `.temp/`) — ไม่ต้องมี Docker เลย
  (ไม่กี่สิบ MB แทนที่จะติดตั้ง Docker Desktop หลาย GB)
· 🔴 **ถ้าไฟล์ runbook ไหนในอนาคตเขียน `supabase db dump` ไว้ ให้เปลี่ยนเป็น `pg_dump` ทันที** —
  จะพังกับเครื่องที่ไม่มี Docker เสมอ ไม่ใช่ error ที่ควรไล่ debug

### 17.2 🔴 org เต็มโควตา — ช่องที่สองที่แผนต้องใช้ถูกของค้างยึดอยู่
`§9`/`§10` (มติ `D21`) วางแผนไว้ว่า org ใหม่ = **`engine-dev` (staging) + สำเนาแช่แข็ง = 2 พอดี** ตามเพดาน free tier
```
org osjepwxnnlvhubylqoyg (free = 2 projects)
  engine-dev                          ACTIVE
  "muphoneseller-netizen's Project"   INACTIVE · สร้าง 17 ส.ค. · ref bjlinogorqpjpksavdjl
```
ช่องที่สองถูกโปรเจกต์ที่**ดูเหมือนเป็นของ MU Phone** (โปรเจกต์คนละใบของผู้ใช้ ที่ `~/mu-phone`) ยึดอยู่

**ตัดสิน: ไม่ลบโปรเจกต์นั้น — สร้าง org ใหม่แทน**
· 🔴 **เหตุผล: ลบโปรเจกต์คือการทำลายข้อมูลที่กู้คืนไม่ได้ และไม่มีใครในทีมยืนยันได้ 100% ว่ามันตายแล้วจริง**
  แค่ *"ดูเหมือน"* ไม่ใช่ของเรา ไม่พอสำหรับการลบ — ชื่อบอกที่มา ไม่ได้บอกสถานะ
· ✅ สร้าง org ใหม่ **ย้อนกลับได้เต็มที่** — ถ้าผิดก็ลบ org เปล่านั้นทิ้งได้โดยไม่กระทบใคร ไม่มีข้อมูลจริงอยู่ในนั้น
· 🎯 **หลักการเดียวกับที่ใช้ทั้งวันกับโค้ด: เลือกทางที่ย้อนกลับได้ เมื่อทางที่ย้อนกลับไม่ได้ยังไม่จำเป็น**
· ⚠️ **การสร้าง org ใหม่เป็นการเปลี่ยนแปลงระดับบัญชี Supabase ของผู้ใช้ — ต้องขอผู้ใช้ก่อนเสมอ**
  ไม่ใช่การตัดสินใจที่ทำแทนได้ แม้จะย้อนกลับได้ก็ตาม

### 17.3 📌 `LAST BACKUP: No backups` — ยืนยันจาก dashboard จริงแล้ว
free tier ไม่มี auto-backup เลยแม้แต่ชิ้นเดียว ตรงกับที่เอกสารเตือนไว้ก่อนหน้า
· **ก่อนวันนี้ ฐานทริปจริงไม่มีสำเนาสำรองอยู่เลย** — ตอนนี้มี `pg_dump` แล้ว (`trip-schema.sql` 29K ·
  `trip-data.sql` 1.3M · เก็บนอกรีโปที่ `~/`) **แต่ยังไม่นับว่าเป็น backup จนกว่าจะ restore สำเร็จ**
  (มีไฟล์ ≠ กู้คืนได้ — ยังไม่มีใครพิสูจน์ว่าไฟล์นั้นใช้งานได้จริง)
· ข้อ (ข) ใน `§10` (อาจต้องจ่าย Pro ชั่วคราวเพื่อ PITR ตอน cutover) **ยังใช้ได้ทุกตัวอักษร** —
  สำเนาแช่แข็งเป็น snapshot ณ เวลาหนึ่ง ไม่ใช่การกู้กลับไปยังวินาทีก่อนคำสั่งที่พลาด

### 17.4 🔴 `db push` บนโปรเจกต์ใหม่ (ฐานเปล่า) จะไม่ผ่าน — ต้อง seed ก่อน (P1 พบใน `E7` local rehearsal · 29 ส.ค. 2026)

**ต่างจาก `§12.5`** ซึ่งเป็นขั้นตอน `db push` ของ `engine-dev` ที่โตมาพร้อมข้อมูลอยู่แล้ว — ข้อนี้คือสิ่งที่จะเกิด
**เฉพาะตอน push สายพาน 89 migration ลงโปรเจกต์ที่เพิ่งสร้างใหม่จาก org ใหม่** (ดู `§17.2`) ซึ่งเป็นฐานเปล่าสนิท

**สาเหตุ:** `20260826172600_e2_rank_shape_real_negative_test.sql` เป็น migration ที่ **ทดสอบด่านของตัวเองตอนรัน** —
หยิบแถวจริงจาก `trip_stops` มายิงค่าผิด (ต้องโดน `check_violation`) แล้วยิงค่าถูก (ทิศบวก) ถ้าตารางว่าง มันปฏิเสธด้วย:
```
trip_stops ว่าง — ทดสอบด่านไม่ได้ และ "ไม่มีอะไรให้ทดสอบ" ไม่ใช่ "ผ่าน"
```
🎯 **`engine-dev` ไม่เคยเจอเพราะมันโตมาพร้อมข้อมูลอยู่แล้ว — ไม่มีใครเคยลงสายพานนี้บนฐานเปล่ามาก่อนเลย**
คนที่เจอครั้งแรกจะอ่านว่า *migration พัง* ทั้งที่มันทำงานถูกตามที่ออกแบบ (ด่านปฏิเสธไม่ให้ตัวเองผ่านฟรีบนฐานว่าง)

**✅ ท่าที่ใช้ได้ (P1 ยืนยันแล้วใน `scripts/e7-local-rehearsal.sh`):** seed แถวเดียว (trip → plan → day → stop) **ก่อน**
ถึงใบ `e2_rank_shape_real_negative_test.sql` โดยใช้ **country code สงวน `zz`** เพื่อให้ reap-safe (แยกออกจากทริปจริงได้ชัดเจน แล้วเก็บกวาดทิ้งได้ตามปกติ)

⚠️ **อีกเงื่อนไขที่ควรรู้ไว้ ไม่น่ากัดแต่ให้จำ:** `20260826210000_e3_read_only_deny_by_user_context.sql` ล้มถ้ารันด้วย **superuser**
(ด่านจะดูเหมือนไม่ทำงานเพราะ superuser เปลี่ยนพฤติกรรม) — บน Supabase จริงรันด้วย role `postgres` ซึ่งไม่ใช่ superuser อยู่แล้ว จึงไม่น่าเจอ
แต่ถ้าใครซ้อมบนเครื่องด้วย role อื่น ให้ใช้ role แบบ `nosuperuser nobypassrls` เหมือนที่ `e7-local-rehearsal.sh` ทำ

**ผลต่อลำดับโควตาที่รอผู้ใช้ (`§17.2`):** ต้องเติมขั้น **"seed ก่อน push"** ต่อท้ายขั้นตอนสร้างโปรเจกต์ใหม่
ไม่งั้นวันที่ผู้ใช้กดสร้างโปรเจกต์สำเร็จ จะไปเจอ `db push` แดงทันทีและคิดว่ามีอะไรพัง ทั้งที่เป็นพฤติกรรมที่ถูกออกแบบไว้แล้ว
