...
2. ห้ามใส่ `SUPABASE_ACCESS_TOKEN` ไว้ใน env ของเครื่อง — ไม่มี token ก็ `link`/`push` ไม่ได้เลย
   เป็นด่านสุดท้ายที่ทำงานแม้คนพิมพ์ผิด
...
คำสั่งที่พิมพ์เผลอๆ ที่ root จะเจอ `supabase/` ที่ **ไม่มี `config.toml`** แล้ว CLI จะ error ทันที
ไม่ใช่เผลอไปรันใส่ของทริป
...
ใส่ใน `package.json` scripts ตอนระยะ 2 — **วันนี้ยังไม่แก้ `package.json` เพราะเป็นไฟล์นอกโซนผม**:
