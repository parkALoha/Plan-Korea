...
-- ⚠️ `P-01`…`P-12` เป็นเลขภายในไฟล์ของ P4 · อ้างที่นี่ได้เพราะระบุชื่อไฟล์กำกับเสมอ
--    แต่ห้ามใช้เป็น ID ลอยๆ — ระบบเลขที่อ้างข้ามไฟล์ได้มีชุดเดียวคือ `D<n>` ใน README
-- 🔴 `P-01` · `P-07` · `P-08` · `P-09` = **`D38`** — P4 ชี้ว่าทั้งสี่เป็นเหตุผลผิดตัวเดียวกัน
--    คนละหน้าเท่านั้น (*ที่ตั้ง ≠ สิทธิ์*) · อ้างข้ามไฟล์ให้ใช้ `D38` ไม่ใช่เลข P
...
create table public.copilot_proposals (
  ...
  --    การเดาให้คำตอบที่ถูกต้องทุกประการ — สำหรับแผนที่ผู้ใช้ไม่ได้เปิดอยู่ (ดู D36)
...
-- ---------------------------------------------------------------------------
-- 🔴 P-01 (= **D38**) — จุดที่ร่างแรกของผมพัง และเป็นความเข้าใจผิดที่ต้องเขียนไว้ให้ชัด
-- ---------------------------------------------------------------------------
...
--
--    🔴 นี่คือ D38 กลับมาในหน้าใหม่ และเป็นสัญชาตญาณเดียวกันเป๊ะ:
...
--
-- ⚠️ คำสั่งนี้ก็ติด D38 เหมือนกัน — เป็น UPDATE ที่ไม่มี policy รองรับ
